  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_MA_ENVIOS_MEDIO" as
  ...
  begin
    ...
    pkg_ma_envios.prc_co_configuraciones(p_id_envio_mdio                  => p_id_envio_mdio,
    ...
    v_api_url := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                 p_prmtro => 'API_URL');

    v_usrnme := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                p_prmtro => 'USRNME');

    v_password := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                  p_prmtro => 'PASSWORD');

    v_indctvo := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                 p_prmtro => 'INDCTVO');
    ...
    select json_object(
                       ...
                       key 'to' is
                       json_arrayagg(v_indctvo ||
                                     v_rt_ma_g_envios_medio.dstno),
    ...
    apex_web_service.g_request_headers(2).value := 'Basic ' ||
                                                   pkg_gn_generalidades.fnc_ge_to_base64(v_usrnme || ':' ||
                                                                                         v_password);

    --Realizamos la peticion
    v_json_respuesta := apex_web_service.make_rest_request(p_url         => v_api_url,
                                                           p_http_method => 'POST',
                                                           p_body        => v_json_request);

    if (json_value(v_json_respuesta, '$.accepted' default 'false' on error) =
       'true') then
  ...
  begin
    ...
    --Obtenermos los parametros de configuracion
    v_server_port := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                     p_prmtro => 'SERVER_PORT');

    v_smtp_srver := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                    p_prmtro => 'SMTP_SRVER');

    v_smtp_usrnme := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                     p_prmtro => 'SMTP_USRNME');

    v_smtp_password := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                       p_prmtro => 'SMTP_PASSWORD');

    v_workspace_id := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                      p_prmtro => 'WORKSPACE_ID');
    ...
    if v_cdgo_prvdres_envio = 'APMAIL' then
      -- Si es APEX MAIL
    ...
    elsif v_cdgo_prvdres_envio = 'MJET' then
      -- Si es MailJet
      DBMS_OUTPUT.PUT_LINE('Entro a MJET');
      select id_prvdor
        into v_id_prvdor
        from ws_d_provedores_api a
       where a.cdgo_api = 'MJET';

      DBMS_OUTPUT.PUT_LINE('Encontro v_id_prvdor: ' || v_id_prvdor);

      v_mnsje := substr(v_rt_ma_g_envios_medio.txto_mnsje,
                        1,
                        length(v_rt_ma_g_envios_medio.txto_mnsje) - 2);
      ...
      DBMS_OUTPUT.PUT_LINE('Msj a mandar: ' || v_mnsje);
      ...
      v_body := '{
               "Messages":[
                ...
                v_rt_ma_g_envios_medio.dstno || '"
                     ...
                     "Subject":"' ||
                v_rt_ma_g_envios_medio.asnto || '",
                     "HTMLPart":"' || v_mnsje || '",
                     "TextPart":"Saludos desde Postman",' ||
                '"EventPayload": "cdgo_clnte: ' || v_cdgo_clnte || '"';

      DBMS_OUTPUT.PUT_LINE('Body a mandar: ' || v_body);
      ...
      if v_total > 0 then
        ...
        for c_adjuntos in (select file_blob,
          ...
          if c_adjuntos.file_blob is not null then

            v_body := v_body || '
							{
                            "ContentType":"' ||
                      c_adjuntos.file_mimetype || '",
							"Filename":"' || c_adjuntos.file_name || '",
                            "Base64Content":"' ||
                      pkg_gn_generalidades.fnc_cl_convertir_blob_a_base64(p_blob => c_adjuntos.file_blob) || ' "
							}';
          ...
          elsif c_adjuntos.file_bfile is not null then
            v_body := v_body || '
              {
                            "ContentType":"' ||
                      c_adjuntos.file_mimetype || '",
              "Filename":"' || c_adjuntos.file_name || '",
                            "Base64Content":"' ||
                      pkg_gn_generalidades.fnc_cl_convertir_bfile_a_base64(p_bfile => c_adjuntos.file_bfile) || ' "
              }';
  ...
  begin
    ...
    --Obtenermos los parametros de configuracion
    v_url_server := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_parametros,
                                                    p_prmtro => 'URL_SERVER');

    --Obtenemos las preferencias
    v_id_alrta_tpo := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_preferencias,
                                                      p_prmtro => 'TPO_ALRTA');

    v_url_alrta := pkg_ma_envios.fnc_co_valor_json(p_json   => v_json_preferencias,
                                                   p_prmtro => 'URL');
    ...
    --Definimos el cuerpo de la peticion
    select json_object(key 'title' is v_rt_ma_g_envios_medio.asnto,
    ...
    insert into gti_aux
      (col1, col2)
    values
      ('v_url_server: ', v_url_server);

    insert into gti_aux
      (col1, col2)
    values
      ('v_rt_ma_g_envios_medio.dstno: ', v_rt_ma_g_envios_medio.dstno);
    ...
    --Realizamos la peticion
    v_json_respuesta := apex_web_service.make_rest_request(p_url         => v_url_server || '/' ||
                                                                            v_rt_ma_g_envios_medio.dstno,
                                                           p_http_method => 'POST',
                                                           p_body        => v_json_request);
    ...
    insert into gti_aux
      (col1, col2)
    values
      ('v_json_respuesta: ', v_json_respuesta);
