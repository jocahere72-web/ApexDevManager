  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_RE_DOCUMENTOS" as
  ...
  function fnc_rg_documentos_responsable(p_id_sjto_impsto      number,
                                         p_id_dcmnto           number,
                                         p_tpo_gnrcion_dcmnto  varchar2 default 'G',
                                         p_cdgo_idntfccion_tpo varchar2 default null,
                                         p_idntfccion          varchar2 default null)
    return varchar2 is
    -- !! --------------------------------- !! --
    -- !! Funcion para generar un documento !! --
    -- !! -------------------------------- !! --
  ...
  begin
    ...
    begin
      ...
      for c_rspnsble in (select a.cdgo_idntfccion_tpo,
        ...
        if ((p_tpo_gnrcion_dcmnto = 'R' and
           c_rspnsble.cdgo_idntfccion_tpo = p_cdgo_idntfccion_tpo and
           c_rspnsble.idntfccion = p_idntfccion) or
           (p_tpo_gnrcion_dcmnto = 'G' and c_rspnsble.prncpal_s_n = 'S')) then
  ...
  function fnc_cl_descuento_x_vgncia_prdo(p_cdgo_clnte                  in number,
                                          ...
                                          p_cdgo_orgen                  in varchar2 default 'DNO' --Se agrega el parametro p_cdgo_orgen para definir si el calculo del concepto proviene de DOCUMENTOS  Ejemplo: DNO = DOCUMENTO ; DCL= DECLARACIONES
    ...
    for c_rglas in (select *
      ...
      if v_cntdad_cndcion > 0 then
        ...
        if v_indcdor_otrga_dscnto = 'S' then
          ...
          --Validar concepto de sancion OJO SOLO CUANDO HAY UN VALOR A PAGAR MINIMO DE SANCON
          -- 29/04/2025
          for r1 in (select 1
                       from df_i_conceptos a
                      where a.id_cncpto = p_id_cncpto
                        and a.sncion = 'S') loop
            if v_vlor_sldo < 299000 then
              v_dtos_dscntos.vlor_dscnto    := 0;
              v_dtos_dscntos.prcntje_dscnto := 0;
            elsif (v_vlor_sldo - v_dtos_dscntos.vlor_dscnto) < 299000 then
              v_dtos_dscntos.vlor_dscnto    := (v_vlor_sldo - 299000);
              v_dtos_dscntos.prcntje_dscnto := round(v_dtos_dscntos.vlor_dscnto /
                                                     v_vlor_sldo,
                                                     2);
            end if;
          end loop;
        ...
        v_mnsje_rspsta := '<*********>  v_dtos_dscntos.vlor_dscnto ' ||
                          ...
                          v_exprsion || ' p_id_cncpto ' || p_id_cncpto;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              'pkg_re_documentos.fnc_cl_descuento_x_vgncia_prdo',
                              v_nl,
      ...
      for c_dcmnto_rpt in (select a.id_dcmnto,
                             ...
                             from (with w_re_g_documentos_detalle as (select sum(vlor_dbe) vlor_dbe,
                                    ...
                                    select a.id_dcmnto,
                                           ...
                                           sum(nvl(g.vlor_hber, 0)) vlor_dscnto_ipu,
                                           a.dscrpcion_vgncia,
                                           a.orden_agrpcion
                                      from re_g_documentos_detalle a
                                      join v_re_g_documentos b
                                      ...
                                      left join w_re_g_documentos_detalle e -- Concepto Interes
                                       ...
                                       and c.id_mvmnto_dtlle =
                                           e.id_mvmnto_dtlle
                                      left join (select f.id_dcmnto,
                                                        f.id_mvmnto_dtlle,
                                                        f.cdgo_cncpto_tpo,
                                                        sum(f.vlor_hber) vlor_hber
                                                   from re_g_documentos_detalle f -- Concepto Descuento
                                                  where f.id_dcmnto =
                                                        p_id_dcmnto
                                                  group by f.id_dcmnto,
                                                           f.id_mvmnto_dtlle,
                                                           f.cdgo_cncpto_tpo) g
                                        on g.id_dcmnto = a.id_dcmnto
                                       and a.id_mvmnto_dtlle =
                                           g.id_mvmnto_dtlle
                                       and g.cdgo_cncpto_tpo = 'D'
                                       and g.id_mvmnto_dtlle =
                                           c.id_mvmnto_dtlle
                                     where a.id_dcmnto = p_id_dcmnto
                                     group by a.id_dcmnto,
                                              a.dscrpcion_vgncia,
  ...
  -- DOCUMENTOS PORCENTUALIDAD

  -- !! ------------------------------------------------------ !! --
  -- !! Funcion para consultar si el sujeto es proindiviso     !! --
  -- !! (responbles con porcentaje de participación)           !! --
  -- !! ------------------------------------------------------ !! --
  function fnc_co_sujeto_proindiviso(p_cdgo_clnte     number,
                                     p_id_sjto_impsto number) return varchar2 is
    v_nl                    number;
    v_nmbre_up              varchar2(100) := 'pkg_re_documentos.fnc_co_sujeto_proindiviso';
    v_prcntje_prtcpcion     number;
    v_sin_prcntje_prtcpcion number;
    v_pro_indvso            si_i_predios.pro_indvso%type;
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Entrando. Consulta sujeto: ' || p_id_sjto_impsto,
                          1);

    begin
      /*select  nvl(pro_indvso , 'N')
      into    v_pro_indvso
      from    si_i_predios
      where   id_sjto_impsto = p_id_sjto_impsto;  */

      -- La suma de todos los porcentajes debe estar entre 99 y 101
      select sum(nvl(prcntje_prtcpcion, 0))
        into v_prcntje_prtcpcion
        from si_i_sujetos_responsable
       where id_sjto_impsto = p_id_sjto_impsto;

      if (v_prcntje_prtcpcion >= 99 and v_prcntje_prtcpcion < 101) then

        -- Ningún porcentaje debe ser 0
        select count(1)
          into v_sin_prcntje_prtcpcion
          from si_i_sujetos_responsable
         where id_sjto_impsto = p_id_sjto_impsto
           and prcntje_prtcpcion = 0;

        if v_sin_prcntje_prtcpcion = 0 then
          v_pro_indvso := 'S';
        else
          v_pro_indvso := 'N';
        end if;
  ...
  end fnc_co_sujeto_proindiviso;

  -- !! ------------------------------------------------------ !! --
  -- !! Funcion para consultar porcentaje de participación     !! --
  -- !! del responsable seleccionado                           !! --
  -- !! ------------------------------------------------------ !! --
  function fnc_co_responsable_porcentaje(p_cdgo_clnte       number,
                                         p_id_sjto_impsto   number,
                                         p_id_sjto_rspnsble number)
    return number is
    v_nl                number;
    v_nmbre_up          varchar2(100) := 'pkg_re_documentos.fnc_co_responsable_porcentaje';
    v_prcntje_prtcpcion number;
    v_pro_indvso        si_i_predios.pro_indvso%type;
  begin
    ...
    begin
      select nvl(prcntje_prtcpcion, 0)
        into v_prcntje_prtcpcion
        from si_i_sujetos_responsable
    ...
    return v_prcntje_prtcpcion;

  end fnc_co_responsable_porcentaje;

  -- !! ------------------------------------------------------ !! --
  -- !! Funcion para consultar porcentaje de participación     !! --
  -- !! por origen del responsable seleccionado                !! --
  -- !! ------------------------------------------------------ !! --
  function fnc_co_responsable_porcentaje_real(p_cdgo_clnte       number,
                                              p_id_sjto_impsto   number,
                                              p_id_orgen         number,
                                              p_prcntje_rspnsble number /*,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           p_cdgo_idntfccion_tpo  varchar2,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           p_idntfccion           varchar2*/)
    return number is
    v_nl             number;
    v_nmbre_up       varchar2(100) := 'pkg_re_documentos.fnc_co_responsable_porcentaje_real';
    v_prcntje_ttal   number := 100;
    v_exste_pgos     number := 0;
    v_prcntje_pgdo   number := 0;
    v_prcntje_dcmnto number := 0;
    v_prcntje_real   number := 0;
    v_pro_indvso     si_i_predios.pro_indvso%type;
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Sujeto: ' || p_id_sjto_impsto || ' - Origen: ' ||
                          p_id_orgen || ' - %:' || p_prcntje_rspnsble,
                          1);

    if p_prcntje_rspnsble > 0 then
      -- Consulta si el origen(liquidación) tienes pagos anteriores
      begin
        for c_pagos in (select distinct id_orgen_dtlle --, cdgo_idntfccion_tpo ,  idntfccion
                          from v_gf_g_movimientos_detalle
                         where cdgo_mvmnto_orgn = 'LQ'
                           and id_orgen = p_id_orgen
                           and cdgo_mvmnto_orgn_dtlle = 'RE') loop
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
                                'Recaudo: ' || c_pagos.id_orgen_dtlle,
                                1);
          -----
          /*if ( c_pagos.cdgo_idntfccion_tpo||c_pagos.idntfccion = p_cdgo_idntfccion_tpo||p_idntfccion  ) then
              pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'DOCUMENTO PAGADO', 1);
              return 0;
          end if;*/
          -----
          --begin
          for c_prcntje in (select distinct a.prcntje_prtcpcion prcntje_dcmnto
                            --into    v_prcntje_dcmnto
                              from re_g_recaudos b
                              join re_g_documentos a
                                on a.id_dcmnto = b.id_orgen
                              join re_g_documentos_detalle d
                                on a.id_dcmnto = d.id_dcmnto
                             where b.id_rcdo = c_pagos.id_orgen_dtlle
                               and a.tpo_gnrcion_dcmnto = 'R'
                               and d.prcntje_prtcpcion_apl >= 0)
          --order by d.id_dcmnto_dtlle desc fetch first 1 row only ;
           loop
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nl,
                                  'prcntje_dcmnto: ' ||
                                  c_prcntje.prcntje_dcmnto,
                                  1);
            v_prcntje_pgdo := v_prcntje_pgdo +
                              nvl(c_prcntje.prcntje_dcmnto, 0);
          end loop;
          ...
          v_prcntje_pgdo := v_prcntje_pgdo + nvl(v_prcntje_dcmnto,0);*/
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
      ...
      exception
        when others then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nl,
                                'Error al consultar pagos sujeto: ' ||
                                p_id_sjto_impsto || ' - ' || sqlerrm,
                                1);
          v_prcntje_pgdo := 0;
      end;

      if v_prcntje_pgdo > 0 then
        if v_prcntje_ttal = v_prcntje_pgdo then
          v_prcntje_real := 0;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
                                'v_prcntje_real_1: ' || v_prcntje_real,
                                1);
        else
          v_prcntje_real := round((p_prcntje_rspnsble /
                                  (v_prcntje_ttal - v_prcntje_pgdo)) *
                                  v_prcntje_ttal,
                                  2);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
                                'v_prcntje_real_2: ' || v_prcntje_real,
                                1);
        end if;
      else
        v_prcntje_real := p_prcntje_rspnsble;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              'v_prcntje_real_3: ' || v_prcntje_real,
                              1);
      end if;
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'v_prcntje_real_4: ' || v_prcntje_real,
                            1);
    end if;

    if v_prcntje_real > 99 and v_prcntje_real < 100 then
      v_prcntje_real := 100;
    elsif v_prcntje_real > 49 and v_prcntje_real < 50 then
      v_prcntje_real := 50;
    end if;
    ------------------------------------------------------------
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Porcentaje real: ' || v_prcntje_real,
                          1);
    ...
    return v_prcntje_real;

  end fnc_co_responsable_porcentaje_real;

  function fnc_gn_documento_porcentaje(p_cdgo_clnte                  number,
                                       p_id_impsto                   number,
                                       p_id_impsto_sbmpsto           number,
                                       p_cdna_vgncia_prdo            varchar2,
                                       p_cdna_vgncia_prdo_ps         varchar2,
                                       ...
                                       p_cdgo_idntfccion_tpo         varchar2 default null,
                                       p_idntfccion                  varchar2 default null,
                                       p_prcntje_prtcpcion           number default 0,
                                       p_prcntje_prtcpcion_apl       number default 0)
    return varchar2 is
    -- !! --------------------------------- !! --
    -- !! Funcion para generar un documento !! --
    -- !! -------------------------------- !! --
    ...
    v_nl                 number;
    v_nmbre_up           varchar2(100) := 'pkg_re_documentos.fnc_gn_documento_porcentaje';
    v_cdgo_rspsta        number;
    v_mnsje              varchar2(4000);
    v_crcter_lmtdor_cdna varchar2(1);
    v_nmro_dcmles        number;
  ...
  begin
      ...
      */
      ...
      if v_id_sjto_impsto > 0 then
        ...
        if v_nmro_dcmnto > 0 then
          ...
          v_mnsje := pkg_re_documentos.fnc_rg_documentos_responsable(p_id_sjto_impsto      => p_id_sjto_impsto,
                                                                     p_id_dcmnto           => v_id_dcmnto,
                                                                     p_tpo_gnrcion_dcmnto  => p_tpo_gnrcion_dcmnto,
                                                                     p_cdgo_idntfccion_tpo => p_cdgo_idntfccion_tpo,
                                                                     p_idntfccion          => p_idntfccion);

          -- 6. Se registra el detalle del documento
          begin
            -- 6.1 Se toma la cadena y se separan las vigencias y periodos
            /* pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'p_cdna_vgncia_prdo: ' || p_cdna_vgncia_prdo, 6);*/
            ...
            for c_vgncia_prdio in (select distinct vgncia, prdo, id_orgen
              ...
              for c_mvmnto_fncro in (select a.cdgo_mvmnto_orgn,
                                            ...
                                            b.id_cncpto,
                                            sum(b.vlor_dbe) -
                                            sum(b.vlor_hber) vlor_dbe,
                                            b.fcha_vncmnto,
                                            c.gnra_intres_mra,
                                            c.id_cncpto_intres_mra,
                                            /**round( ( (p_prcntje_prtcpcion / ( (sum(b.vlor_dbe) - sum(b.vlor_hber) ) /
                                            ( select  vlor_dbe
                                              from    gf_g_movimientos_detalle  d
                                              where   d.CDGO_MVMNTO_ORGN    = 'LQ'
                                              and     d.id_orgen            = b.id_orgen
                                              and     d.id_cncpto           = b.id_cncpto )
                                            ) ) / 100 ) , -2) prcntje_prtcpcion_apl**/
                                            pkg_re_documentos.fnc_co_responsable_porcentaje_real(p_cdgo_clnte       => p_cdgo_clnte,
                                                                                                 p_id_sjto_impsto   => p_id_sjto_impsto,
                                                                                                 p_id_orgen         => b.id_orgen,
                                                                                                 p_prcntje_rspnsble => p_prcntje_prtcpcion) prcntje_prtcpcion_apl
                                       from gf_g_movimientos_financiero a
                                       join v_gf_g_movimientos_detalle b
                                         on a.id_mvmnto_fncro =
                                            b.id_mvmnto_fncro
                ...
                v_mnsje := 'v_id_dcmnto => ' || v_id_dcmnto ||
                           ' c_mvmnto_fncro.id_mvmnto_dtlle => ' ||
                           c_mvmnto_fncro.id_mvmnto_dtlle ||
                           ' c_mvmnto_fncro.prcntje_prtcpcion_apl => ' ||
                           c_mvmnto_fncro.prcntje_prtcpcion_apl;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                      null,
                                      v_nmbre_up,
                ...
                if c_mvmnto_fncro.prcntje_prtcpcion_apl > 0 then
                  pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                        null,
                                        v_nmbre_up,
                                        v_nl,
                                        ...
                                        c_mvmnto_fncro.vlor_dbe,
                                        6);
                  if c_mvmnto_fncro.prcntje_prtcpcion_apl > 99 and
                     c_mvmnto_fncro.prcntje_prtcpcion_apl < 100 then
                    c_mvmnto_fncro.prcntje_prtcpcion_apl := 100;
                  else
                    ----c_mvmnto_fncro.vlor_dbe := round( c_mvmnto_fncro.vlor_dbe * (c_mvmnto_fncro.prcntje_prtcpcion_apl/100) , -2 ) ;
                    c_mvmnto_fncro.vlor_dbe := ceil((c_mvmnto_fncro.vlor_dbe *
                                                    (c_mvmnto_fncro.prcntje_prtcpcion_apl / 100)) / 100) * 100;
                  end if;
                  pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                        null,
                                        v_nmbre_up,
                                        ...
                                        6);
                  /***c_mvmnto_fncro.vlor_dbe := round( c_mvmnto_fncro.vlor_dbe * (c_mvmnto_fncro.prcntje_prtcpcion_apl/100) , -2 ) ;
                  --v_vlor_dbe_prcntje := round( c_mvmnto_fncro.vlor_dbe * (c_mvmnto_fncro.prcntje_prtcpcion_apl/100) , -2 ) ; -- 482600 482588
                  if ( v_vlor_dbe_prcntje - c_mvmnto_fncro.vlor_dbe ) > 100 then
                      c_mvmnto_fncro.vlor_dbe := v_vlor_dbe_prcntje;
                  end if;
                  ***/
                  pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                        null,
    ...
    exception
      when others then
        v_mnsje := 'Error al actualizar totales ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              v_mnsje,
                              2);
        v_mnsje := 'Totales ' || v_sum_vlor_dbe || ' - ' || v_sum_vlor_hber ||
                   ' - ' || v_sum_vlor_ttal || ' - ' || v_vlor_ttal_dcmnto;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
  ...
  end fnc_gn_documento_porcentaje;

  --Procedimiento que valida si el documento esta pagago
  procedure prc_vl_documento_pagado(p_cdgo_clnte          in df_s_clientes.cdgo_clnte%type,
                                    ...
                                    o_indcdor_dcmnto_pgdo out varchar2,
                                    o_vlor                out re_g_recaudos.vlor%type,
                                    o_fcha_rcdo           out re_g_recaudos.fcha_rcdo%type,
                                    o_cdgo_rspsta         out number,
                                    o_mnsje_rspsta        out varchar2) as
  ...
  begin
    ...
    --Respuesta Exitosa
    o_cdgo_rspsta         := 0;
    o_indcdor_dcmnto_pgdo := 'N';

    --Se Verifica si los Parametros de Entrada no esten Nulos
    if (p_nmro_dcmnto is null) then
      o_mnsje_rspsta := 'El número de documento se encuentra vacio.';
      o_cdgo_rspsta  := 1;
      return;
    end if;
    ...
    if (p_nmro_dcmnto like '120%') then
      ...
      exception
        when no_data_found then
          o_cdgo_rspsta  := 5;
          o_mnsje_rspsta := 'El documento ' || p_nmro_dcmnto ||
                            ' no existe.';
          return;
      ...
      --DBMS_OUTPUT.PUT_LINE('p_cdgo_clnte = ' || p_cdgo_clnte);
      --DBMS_OUTPUT.PUT_LINE('p_nmro_dcmnto = ' || p_nmro_dcmnto);
      ...
      begin
        ...
        --DBMS_OUTPUT.PUT_LINE('o_id_orgen = ' || o_id_orgen);
        --DBMS_OUTPUT.PUT_LINE('v_vlor = ' || v_vlor);

      exception
        when too_many_rows then
          o_cdgo_rspsta  := 10;
          o_mnsje_rspsta := 'No fue posible encontrar el documento de pago No. ' ||
                            p_nmro_dcmnto ||
                            ', ya que existe más de uno en el sistema.';
          return;
        when others then
          o_cdgo_rspsta  := 11;
          o_mnsje_rspsta := 'Error al consultar el documento de pago No. ' ||
                            p_nmro_dcmnto;
          return;
      end;
    end if;
    ...
    begin
      ...
      -- Se consultan los datos del recaudo
      select vlor, fcha_rcdo
        into o_vlor, o_fcha_rcdo
        from re_g_recaudos
       where cdgo_rcdo_orgn_tpo = o_cdgo_rcdo_orgn_tpo
         and id_orgen = o_id_orgen
         and cdgo_rcdo_estdo = 'AP';

      o_indcdor_dcmnto_pgdo := 'S';
      o_cdgo_rspsta         := 0;
      o_mnsje_rspsta        := 'El documento de pago No.' || p_nmro_dcmnto ||
                               ', se encuentra pagado.';
    exception
      when no_data_found then
        o_indcdor_dcmnto_pgdo := 'N';
        o_cdgo_rspsta         := 0;
        o_mnsje_rspsta        := 'El documento de pago No.' ||
                                 p_nmro_dcmnto ||
                                 ', NO se encuentra pagado.';
      when others then
        o_indcdor_dcmnto_pgdo := 'N';
        o_cdgo_rspsta         := 99;
