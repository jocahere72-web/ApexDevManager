  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GI_PREDIO_VALORIZACION" AS

  PROCEDURE PRC_CREA_PRDIO_VAL_PUNTUAL(p_cdgo_clnte      IN NUMBER,
  ...
  BEGIN
    ...
    BEGIN
      SELECT ID_IMPSTO
        INTO V_ID_IMPSTO_IPU
        FROM df_c_impuestos
       WHERE cdgo_impsto = 'IPU';

      SELECT ID_IMPSTO
        INTO V_ID_IMPSTO_VAL
        FROM df_c_impuestos
    ...
    BEGIN
      ...
      o_cdgo_rspsta  := 20;
      o_mnsje_rspsta := 'La referencia [' || P_RFRNCIA_CTSTRAL ||
                        '] ya tiene sujeto de Valorización';
      pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                            p_id_impsto  => null,
                            p_nmbre_up   => v_nmbre_up,
                            p_nvel_log   => v_nvel,
    ...
    BEGIN
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nvel,
                            'Sujeto predial: ' || V_ID_SJTO_IMPSTO_IPU,
                            3);
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        o_cdgo_rspsta  := 30;
        o_mnsje_rspsta := 'La referencia [' || P_RFRNCIA_CTSTRAL ||
                          '] no existe en el sistema para el impuesto predial.';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                              p_id_impsto  => null,
                              p_nmbre_up   => v_nmbre_up,
                              p_nvel_log   => v_nvel,
      ...
      WHEN OTHERS THEN
        o_cdgo_rspsta  := 40;
        o_mnsje_rspsta := 'Error al consultar impuesto predial para la referencia [' ||
                          P_RFRNCIA_CTSTRAL || ']. ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                              p_id_impsto  => null,
                              p_nmbre_up   => v_nmbre_up,
                              p_nvel_log   => v_nvel,
    ...
    IF V_ID_SJTO_IMPSTO_IPU IS NULL THEN
      o_cdgo_rspsta  := 50;
      o_mnsje_rspsta := 'No se encontró información del impuesto predial para la referencia: ' ||
                        P_RFRNCIA_CTSTRAL;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                            p_id_impsto  => null,
                            p_nmbre_up   => v_nmbre_up,
    ...
    BEGIN
      for c_sjto in (SELECT c.id_sjto,
                      ...
                      WHERE c.id_sjto_impsto = V_ID_SJTO_IMPSTO_IPU) loop
    ...
    BEGIN
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nvel,
                            'Responsables registrados para sujeto: ' ||
                            V_ID_SJTO_IMPSTO_VAL,
                            3);
    EXCEPTION
      WHEN OTHERS THEN
        o_cdgo_rspsta  := 60;
        o_mnsje_rspsta := 'Error al insertar sujeto responsable: ' ||
                          SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                              p_id_impsto  => null,
                              p_nmbre_up   => v_nmbre_up,
    ...
    BEGIN
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nvel,
                            'predio registrados para sujeto: ' ||
                            V_ID_SJTO_IMPSTO_VAL,
                            3);
    ...
    EXCEPTION
      WHEN OTHERS THEN
        o_cdgo_rspsta  := 70;
        o_mnsje_rspsta := 'Error al insertar traza para referencia: ' ||
                          p_rfrncia_ctstral || ' - ' || SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                              p_id_impsto  => null,
                              p_nmbre_up   => v_nmbre_up,
  ...
  EXCEPTION
    WHEN OTHERS THEN
      o_cdgo_rspsta  := 100;
      o_mnsje_rspsta := 'Error controlado referencia : ' ||
                        p_rfrncia_ctstral || ' - ' || SQLERRM;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                            p_id_impsto  => null,
                            p_nmbre_up   => v_nmbre_up,
  ...
  PROCEDURE PRC_ACTLZA_PRDIOS_VAL(p_cdgo_clnte      IN NUMBER,
                                  p_id_usuario      IN NUMBER,
                                  p_rfrncia_ctstral IN VARCHAR2,
                                  o_cdgo_rspsta     OUT NUMBER,
  ...
  BEGIN
    ...
    o_mnsje_rspsta := 'Inicio del procedimiento. Referencia: ' ||
                      p_rfrncia_ctstral;
    pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                          p_id_impsto  => null,
                          p_nmbre_up   => v_nmbre_up,
      ...
      EXCEPTION
        WHEN OTHERS THEN
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'Error en la actualización e inserción: ' ||
                                SQLERRM,
                                3);
          ROLLBACK;
      END;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nvel,
                          'Sujeto de Valorización modificado exitosamente',
                          3);

  EXCEPTION
    WHEN OTHERS THEN
      o_cdgo_rspsta  := 10;
      o_mnsje_rspsta := 'Error en la ejecución del procedimiento: ' ||
                        SQLERRM;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                            p_id_impsto  => null,
                            p_nmbre_up   => v_nmbre_up,
  ...
  PROCEDURE PRC_CREA_PRDIO_VAL(P_RSLCION_IGAC IN VARCHAR2,
  ...
  begin
    ...
    o_mnsje_rspsta := 'Inicio del procedimiento. resolución ' ||
                      P_RSLCION_IGAC;
    pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                          p_id_impsto  => null,
                          p_nmbre_up   => v_nmbre_up,
  ...
  begin
    ...
    for c_sujetos in (select c001 idntfccion_sjto
                       ...
                       where id_ssion = p_id_session
                         and c005 = 'VLD') loop
      begin
        PKG_GI_PREDIO_VALORIZACION.PRC_CREA_PRDIO_VAL_PUNTUAL(p_cdgo_clnte      => p_cdgo_clnte,
  ...
  exception
    when others then
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nvel,
                            'Error controlado: ' || sqlerrm,
                            1);

  end PRC_CREA_PRDIO_VAL_ARCHIVO;

  PROCEDURE PRC_ACTLZA_PRDIOS_VAL_MASIVO(p_cdgo_clnte      IN NUMBER,
                                         p_id_usuario      IN NUMBER,
                                         p_rfrncia_ctstral IN VARCHAR2
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        o_mnsje_rspsta := '10 - No se encontró información.';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                              p_id_impsto  => null,
                              p_nmbre_up   => v_nmbre_up,
                              p_nvel_log   => v_nvel,
    ...
    EXCEPTION
      WHEN OTHERS THEN
        o_mnsje_rspsta := '70 - Error al insertar traza para referencia: ' ||
                          p_rfrncia_ctstral || ' - ' || SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                              p_id_impsto  => null,
                              p_nmbre_up   => v_nmbre_up,
    ...
    IF p_rfrncia_ctstral is null then
      ...
      begin
        select a.vlor
          ...
          join ma_d_envios_medio_cnfgrcion b
            on a.id_envio_mdio_cnfgrcion = b.id_envio_mdio_cnfgrcion
      ...
      exception
        when no_data_found then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                '5 - No hay correo configurado para el envío ',
                                1);
      ...
      if v_correo is not null then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              '9 -No hay correo parametrizado para enviar Notificación del proceso ',
                              1);
      end if;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      o_mnsje_rspsta := '10 - Error en la ejecución del procedimiento: ' ||
                        SQLERRM;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                            p_id_impsto  => null,
                            p_nmbre_up   => v_nmbre_up,
