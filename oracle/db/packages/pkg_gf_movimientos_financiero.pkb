  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GF_MOVIMIENTOS_FINANCIERO" as
  ...
  function fnc_co_mvmnto_x_cncpto(p_cdgo_clnte        df_s_clientes.cdgo_clnte%type,
                                  ...
                                  p_id_rspnsble       number default 0)
    return g_mvmntos_x_cncpto
    pipelined is
    v_nmbre_up            varchar2(100) := 'pkg_re_documentos.fnc_co_mvmnto_x_cncpto';
    v_prcntje_real        number := 0;
    v_exprsion            df_i_definiciones_impuesto.vlor%type;
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          6,
                          'Inicia. p_id_rspnsble: ' || p_id_rspnsble,
                          1);
    ...
    exception
      when others then
        v_exprsion := 'round(:valor,-2)';
    end;

    if p_id_rspnsble > 0 then
      select prcntje_prtcpcion, cdgo_idntfccion_tpo, idntfccion
        into v_prcntje_prtcpcion, v_cdgo_idntfccion_tpo, v_idntfccion
        from si_i_sujetos_responsable
       where id_sjto_rspnsble = p_id_rspnsble;

      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            6,
                            'v_prcntje_prtcpcion fnc_co_mvmnto_x_cncpto: ' ||
                            v_prcntje_prtcpcion || ' - ' ||
                            v_cdgo_idntfccion_tpo || '-' || v_idntfccion,
                            1);
    ...
    for c_mvmntos in (select a.vgncia,
                             ...
                             a.id_mvmnto_fncro,
                             (a.vlor_sldo_cptal *
                             (v_prcntje_prtcpcion / 100)) vlor_sldo_cptal_prcntje,
                             (a.vlor_intres * (v_prcntje_prtcpcion / 100)) vlor_intres_prcntje,
                             /***case
                                when a.vlor_sldo_cptal = 0 then
                                    0
                                else
                                    round( (a.vlor_sldo_cptal * ( (v_prcntje_prtcpcion / ( a.vlor_sldo_cptal / nvl(a.deuda_inicial,1) ) ) / 100 )) , -2)
                             end vlor_sldo_cptal_prcntje,
                             case
                                when a.vlor_intres = 0 then
                                    0
                                else
                                    pkg_gn_generalidades.fnc_ca_expresion(p_vlor => (a.vlor_intres * ( (v_prcntje_prtcpcion / ( a.vlor_sldo_cptal / nvl(a.deuda_inicial,1) ) ) / 100 )),
                                                                          p_expresion => v_exprsion)
                             end vlor_intres_prcntje,***/

                             0                   vlor_ttal_prcntje,
                             v_prcntje_prtcpcion prcntje_real,
                             0                   indcdor_pgdo
                        from (select a.vgncia,
                                     ...
                                     (select vlor_dbe
                                       ...
                                       where d.CDGO_MVMNTO_ORGN = 'LQ'
                                         ...
                                         and d.id_cncpto = a.id_cncpto) deuda_inicial
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            6,
                            'id_orgen: ' || c_mvmntos.id_orgen || ' - ' ||
                            c_mvmntos.dscrpcion_cncpto,
                            1);
      -- Consulta porcentaje real
      v_prcntje_real := pkg_re_documentos.fnc_co_responsable_porcentaje_real(p_cdgo_clnte       => p_cdgo_clnte,
                                                                             p_id_sjto_impsto   => p_id_sjto_impsto,
                                                                             p_id_orgen         => c_mvmntos.id_orgen,
                                                                             p_prcntje_rspnsble => v_prcntje_prtcpcion);
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_prcntje_real,
                            1);
      if v_prcntje_real <> v_prcntje_prtcpcion then

        if v_prcntje_real > 99 and v_prcntje_real < 100 then
          v_prcntje_real                    := 100;
          c_mvmntos.vlor_sldo_cptal_prcntje := c_mvmntos.vlor_sldo_cptal;
          c_mvmntos.vlor_intres_prcntje     := c_mvmntos.vlor_intres;
        else
          ----c_mvmntos.vlor_sldo_cptal_prcntje   := round( (c_mvmntos.vlor_sldo_cptal * ( v_prcntje_real / 100 )),-2);
          c_mvmntos.vlor_sldo_cptal_prcntje := ceil((c_mvmntos.vlor_sldo_cptal *
                                                    (v_prcntje_real / 100)) / 100) * 100;
          c_mvmntos.vlor_intres_prcntje     := pkg_gn_generalidades.fnc_ca_expresion(p_vlor      => (c_mvmntos.vlor_intres *
                                                                                                    (v_prcntje_real / 100)),
                                                                                     p_expresion => v_exprsion);
        end if;

        /***c_mvmntos.vlor_sldo_cptal_prcntje   := (c_mvmntos.vlor_sldo_cptal * ( v_prcntje_real / 100 )) ;
        c_mvmntos.vlor_intres_prcntje       := (c_mvmntos.vlor_intres * ( v_prcntje_real / 100 )) ; ***/
        --c_mvmntos.vlor_ttal_prcntje         := ((c_mvmntos.vlor_sldo_cptal + c_mvmntos.vlor_intres) * ( v_prcntje_real / 100 )) ;
        c_mvmntos.prcntje_real := v_prcntje_real;
      else
        ----c_mvmntos.vlor_sldo_cptal_prcntje   := round(c_mvmntos.vlor_sldo_cptal_prcntje,-2);
        c_mvmntos.vlor_sldo_cptal_prcntje := ceil(c_mvmntos.vlor_sldo_cptal_prcntje / 100) * 100;
        c_mvmntos.vlor_intres_prcntje     := pkg_gn_generalidades.fnc_ca_expresion(p_vlor      => c_mvmntos.vlor_intres_prcntje,
                                                                                   p_expresion => v_exprsion);
      end if;
      ---------------------------------
      -- Se redondea
      /***c_mvmntos.vlor_sldo_cptal_prcntje   := round(c_mvmntos.vlor_sldo_cptal_prcntje,-2);
      c_mvmntos.vlor_intres_prcntje       := pkg_gn_generalidades.fnc_ca_expresion(p_vlor => c_mvmntos.vlor_intres_prcntje, p_expresion => v_exprsion);***/
      c_mvmntos.vlor_ttal_prcntje := c_mvmntos.vlor_sldo_cptal_prcntje +
                                     c_mvmntos.vlor_intres_prcntje;

      select count(1)
        into v_orgen_pgdo_rspnsble
        from v_gf_g_movimientos_detalle
       where cdgo_mvmnto_orgn = 'LQ'
         and id_orgen = c_mvmntos.id_orgen
         and cdgo_mvmnto_orgn_dtlle in ('RE', 'AJ')
         and cdgo_idntfccion_tpo = v_cdgo_idntfccion_tpo
         and idntfccion = v_idntfccion;

      if v_orgen_pgdo_rspnsble = 0 then
        c_mvmntos.indcdor_pgdo := 0; -- NO pagado
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              6,
                              'ORIGEN NOOOOOO PAGADO= ' || v_idntfccion,
                              1);
      else
        c_mvmntos.indcdor_pgdo := 1; -- pagado
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              6,
                              'ORIGEN PAGADO= ' || v_idntfccion,
                              1);
      end if;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, 6, 'Saliendo', 1);
  ...
  begin
    ...
    */
    ...
    for c_clnte in (select cdgo_clnte from df_s_clientes where actvo = 'S') loop
      ...
      for c_sjtos_impsto in (select a.id_sjto_impsto
                               from si_i_sujetos_impuesto a
                              where exists
                              (select 1
                                       from si_c_sujetos b
                                      where b.cdgo_clnte = c_clnte.cdgo_clnte
                                        and b.id_sjto = a.id_sjto)) loop
        begin
        ...
        if mod(v_count, 500) = 0 then
          pkg_sg_log.prc_rg_log(c_clnte.cdgo_clnte,
                                ...
                                6,
                                v_count || 'Sujetos Impuestos ',
                                1);
        end if;
      end loop;
    end loop;

    DBMS_STATS.GATHER_TABLE_STATS(ownname          => 'GENESYS',
                                  tabname          => 'GF_G_MVMNTOS_CNCPTO_CNSLDDO',
  ...
  begin
    ...
    begin
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nl,
                            'p_id_sjto_impsto ' || p_id_sjto_impsto ||
                            ', p_vgncia: ' || p_vgncia || ' estado ' ||
                            t_gf_g_movimientos_financiero.indcdor_mvmnto_blqdo,
                            1);
