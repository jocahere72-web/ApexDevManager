  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_PL_WORKFLOW_1_0" as
  ...
  function fnc_render(p_region              in apex_plugin.t_region,
    ...
    v_mstrar_btn_rchza varchar2(100) := p_region.attribute_04;
    ...
    v_mdo_rgrso_sin_prmso      varchar2(20) := nvl(p_region.attribute_06,
                                                   'PAGINA_ANTERIOR');
    v_aplccion_rgrso_sin_prmso integer := nvl(p_region.attribute_07,
                                              V('APP_ID'));
    v_pgna_rgrso_sin_prmso     integer := nvl(p_region.attribute_08, 1);
    v_id_usrio_espcfco         integer := nvl(p_region.attribute_05, -1);
    ...
    v_cntar_obsrvcion      number;
    v_exste_obsrvcion      varchar2(100);
    v_indcdor_rvsion_fljos varchar2(2);
    v_mstrar_btn_rchzar    varchar2(100);
    v_cdgo_estdo_rvsion    number;
    v_obsrvcion            varchar2(4000);
    v_vldar_estdo_fljo     number;
  ...
  begin
    ...
    if not v_encntro_prtcpte then
      --v_url := apex_util.prepare_url('f?p=' || v('app_id') || ':' || 1 || ':' ||
      v_url := apex_util.prepare_url('f?p=' || v_aplccion_rgrso_sin_prmso || ':' ||
                                     v_pgna_rgrso_sin_prmso || ':' ||
    ...
    begin
      ...
      if v_cntar_obsrvcion = 0 then
        v_exste_obsrvcion := 'display:none;';
      elsif v_cntar_obsrvcion > 0 then
        v_exste_obsrvcion := 'true';
      end if;
    ...
    --validar si la instancia flujo tiene el indicador de revision activa
    begin
      select indcdor_rvsion_fljos
        into v_indcdor_rvsion_fljos
        from wf_d_flujos_tarea a
       where a.id_fljo_trea = v_id_fljo_trea;

      if v_indcdor_rvsion_fljos = 'N' or v_indcdor_rvsion_fljos is null then
        v_mstrar_btn_rchzar := 'display:none;';

      elsif v_indcdor_rvsion_fljos = 'S' then

        select count(a.cdgo_estdo)
          into v_cdgo_estdo_rvsion
          from pq_g_rvsion_trea_fljo a
         where a.id_instncia_fljo = v_id_instncia_fljo
           and a.cdgo_estdo = 'R';

        if v_cdgo_estdo_rvsion > 0 then
          v_mstrar_btn_rchzar := 'display:none;';
        elsif v_cdgo_estdo_rvsion = 0 then
          select count(*)
            into v_vldar_estdo_fljo
            from v_wf_g_instancias_flujo
           where id_instncia_fljo = v_id_instncia_fljo
             and estdo_instncia = 'FINALIZADA';

          if v_vldar_estdo_fljo = 0 and v_mstrar_btn_rchza = 'true' then
            v_mstrar_btn_rchzar := 'true';
          elsif v_vldar_estdo_fljo > 0 or v_mstrar_btn_rchza = 'false' then
            v_mstrar_btn_rchzar := 'display:none;';
          end if;
        end if;
    ...
    begin
      select b.obsrvcion
       ...
       inner join pq_g_trea_fljo_obsrvcion b
          on a.id_rvsion_trea = b.id_rvsion_trea
       where cdgo_estdo = 'R'
         and b.id_instncia_fljo = v_id_instncia_fljo;
    exception
      when no_data_found then
        null;
      when others then
    ...
    --htp.p('</div></div>');
    --agregado por heider gomez
    htp.p('
                <div class="col col-12">
                    <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 10px; flex-wrap: wrap;">

            <button id="wf_btn_observacion" class="t-Button t-Button--warning t-Button--icon"
            type="button" title="Observaciones" aria-label="Observaciones"
            style="' || v_exste_obsrvcion ||
          'min-width: 90px; padding: 8px 16px; font-size: 14px;">
            <span class="t-Icon fa fa-comments-o" aria-hidden="true"></span>
            <span class="t-Button-label">Observaciones</span>
            </button>


                        <button id="wf_btn_rechazar" class="t-Button t-Button--danger t-Button--icon"
                        type="button" title="Rechazar" aria-label="Rechazar"
                        style="' || v_mstrar_btn_rchzar ||
          'min-width: 90px; padding: 8px 16px; font-size: 14px;">
                        <span class="t-Icon fa fa-times-circle" aria-hidden="true"></span>
                        <span class="t-Button-label">Rechazar</span>
                        </button>
            ...
            <div id="customModal" class="custom-modal" style="display:none;">
              <div class="custom-modal-content">
                ...
                <h2>Observación</h2>
                <textarea id="observacionText" rows="5" style="width: 100%;">' ||
          v_obsrvcion || '</textarea>

                    <div style="' || v_mstrar_btn_rchzar || '">
                    <label for="wf_file_upload">Adjuntar archivo: (Seleccionar documento)</label>
                    <input type="file" id="wf_file_upload"
                           accept=".pdf,.doc,.docx,.xls,.xlsx,image/*"
                           style="margin-top: 5px; margin-bottom: 15px;" />
                  </div>

                <br>
                <button id="wf_btn_guardar" type="button" class="t-Button t-Button--hot" style="' ||
          v_mstrar_btn_rchzar || '">
                  Guardar
                </button>
              </div>
  ...
  function fnc_ajax(p_region in apex_plugin.t_region,
    ...
    v_msg                  varchar2(4000);
    v_cdgo_error           number;
    v_id_usrio_espcfco     number := p_region.attribute_05;
    v_observacion          varchar2(500);
    v_id_qja_rclmo         number;
    v_indcdor_rvsion_fljos varchar2(2);
    v_id_usrio_obsrvcion   number;
    v_base64_dcmnto        clob;
    v_blob                 blob;
    v_file_name            varchar2(500);
    v_file_mimetype        varchar2(100);

  begin

    for i in 1 .. apex_application.g_f03.count loop
      v_json := v_json || apex_application.g_f03(i);
    end loop;
    ...
    exception
      when others then
        ...
        apex_json.write('msg',
                        'error en consulta de indicador de revision');
        apex_json.close_all();
    end;
    ...
    if v_accion_ajax = 'LIST' then
      ...
      begin
        ...
        if v_indcdor_rvsion_fljos = 'S' then

          pkg_pq_pqr.prc_ac_quejas_reclamo(p_id_instncia_fljo => v_id_instncia_fljo,
                                           p_id_fljo_trea     => v_id_fljo_trea);
        end if;
    ...
    elsif v_accion_ajax = 'RECHAZAR' then
      ...
      select jt.valor
        ...
        from json_table(v_json,
                        '$.param[*]'
                        columns(param_or varchar2(100) path '$.param_or',
                                valor varchar2(4000) path '$.valor')) jt
       where jt.param_or = 'observacion';

      begin
        select jt.valor
          into v_base64_dcmnto
          from json_table(v_json,
                          '$.param[*]'
                          columns(param_or varchar2(50) path '$.param_or',
                                  valor clob path '$.valor')) jt
         where jt.param_or = 'documento_blob';
      exception
        when no_data_found then
          v_base64_dcmnto := null;
        when others then
          v_base64_dcmnto := null;
      end;
      ...
      begin
        select jt.valor
          ...
          from json_table(v_json,
                          '$.param[*]'
                          columns(param_or varchar2(50) path '$.param_or',
                                  valor varchar2(200) path '$.valor')) jt
         where jt.param_or = 'documento_nombre';
      exception
        when no_data_found then
          v_file_name := null;
        when others then
      ...
      begin
        select jt.valor
          into v_file_mimetype
          from json_table(v_json,
                          '$.param[*]'
                          columns(param_or varchar2(50) path '$.param_or',
                                  valor varchar2(100) path '$.valor')) jt
         where jt.param_or = 'documento_tipo';
      exception
        when no_data_found then
          v_file_mimetype := null;
        when others then
          v_file_mimetype := null;
      end;

      -- Insertamos el rechazo en la base de datos

      select id_usrio
        into v_id_usrio_obsrvcion
        from wf_g_instancias_transicion a
       where id_instncia_fljo = v_id_instncia_fljo
         and id_fljo_trea_orgen = v_id_fljo_trea;

      if v_base64_dcmnto is not null then
        v_blob := pkg_gn_generalidades.fnc_decode_base64(v_base64_dcmnto);
      end if;

      begin

        prc_gn_rvsion_trea_fljo(p_cdgo_clnte       => v_cdgo_clnte,
                                p_id_qja_rclmo     => v_id_qja_rclmo,
                                p_id_fljo_trea     => v_id_fljo_trea,
                                p_id_instncia_fljo => v_id_instncia_fljo,
                                p_id_usrio         => v_id_usrio_obsrvcion,
                                p_cdgo_estdo       => 'R',
                                p_obsrvcion        => v_observacion,
                                p_dcmnto           => v_blob,
                                p_file_name        => v_file_name,
                                p_file_mimetype    => v_file_mimetype,
                                o_cdgo_rspsta      => v_cdgo_error,
                                o_mnsje_rspsta     => v_msg);
      ...
      exception
        when others then
          ...
          apex_json.write('type', 'ERROR');
          apex_json.write('msg',
                          'Error al registrar el rechazo: ' || sqlerrm);
      end;
        ...
        for c_v_wf_d_flujos_transicion in (select a.id_fljo_trea,
          ...
          if v_vldar then
            ...
            if v_cdgo_mtdo_asgncion = 'MAM' and v_id_usrio_trea is null then
              ...
              if v_cdgo_mtdo_asgncion = 'MAE' then
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          v_nl,
                          'p_id_fljo: ' || p_id_fljo || ' - p_id_usrio: ' ||
                          p_id_usrio || ' - p_id_prtcpte: ' || p_id_prtcpte,
                          1); -- ESCRIBIR EN EL LOG
    ...
    exception
      when others then
        ...
        o_id_fljo_trea     := null;
        o_cdgo_rspsta      := 20;
        o_mnsje            := 'Error al seleccionar detalles de transición: ' ||
                              sqlerrm;
        pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                              null,
                              'pkg_pl_workflow_1_0.prc_rg_instancias_flujo',
                              v_nl,
                              o_cdgo_rspsta || ' - ' || o_mnsje,
                              1); -- ESCRIBIR EN EL LOG
        return;
    end;
    ...
    exception
      when others then
        ...
        o_id_fljo_trea     := null;
        o_cdgo_rspsta      := 30;
        o_mnsje            := 'Error al determinar el participante: ' ||
                              sqlerrm;
        pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                              null,
                              'pkg_pl_workflow_1_0.prc_rg_instancias_flujo',
    ...
    exception
      when others then
        ...
        o_id_fljo_trea     := null;
        o_cdgo_rspsta      := 50;
        o_mnsje            := 'Error al insertar instancia de flujo: ' ||
                              sqlerrm;
        pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                              null,
                              'pkg_pl_workflow_1_0.prc_rg_instancias_flujo',
    ...
    exception
      when others then
        ...
        o_id_fljo_trea     := null;
        o_cdgo_rspsta      := 60;
        o_mnsje            := 'Error al insertar instancia de transición: ' ||
                              sqlerrm;
        pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                              null,
                              'pkg_pl_workflow_1_0.prc_rg_instancias_flujo',
    ...
    exception
      ...
      when others then
        ...
        o_id_fljo_trea     := null;
        o_cdgo_rspsta      := 70;
        o_mnsje            := 'Error al insertar estado de transición: ' ||
                              sqlerrm;
        pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                              null,
                              'pkg_pl_workflow_1_0.prc_rg_instancias_flujo',
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          'pkg_pl_workflow_1_0.prc_rg_instancias_flujo',
                          v_nl,
                          'Saliendo: ' || o_cdgo_rspsta || ' - ' || o_mnsje ||
                          ' - ' || v_id_instncia_fljo || ' - ' ||
                          v_id_fljo_trea,
                          1); -- ESCRIBIR EN EL LOG
  ...
  exception
    when others then
      ...
      o_id_fljo_trea     := null;
      o_cdgo_rspsta      := 999;
      o_mnsje            := 'Error al crear la instancia del flujo: ' ||
                            sqlerrm;
      return;
  ...
  begin
    ...
    exception
      when no_data_found then
        raise_application_error(-20999,
                                'Ocurrio un error al buscar el usuario. Verifique si el Usuario existe en el sistema ');
    end;

    begin
      --Si no lo encuentra, lo insertamos como participante
      insert into wf_d_flujos_tarea_prtcpnte
        (id_fljo_trea, tpo_prtcpnte, id_prtcpte, actvo)
      values
        (p_id_fljo_trea, 'USUARIO', p_id_usrio, 'S');
      return p_id_usrio;
    exception
      when others then
        raise_application_error(-20999,
                                'No se pudo insertar el usuario como participante de la tarea.');
    end;
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(23001,
                          null,
                          'pkg_pl_workflow_1_0.prc_rg_propiedad_evento',
                          6,
                          ' p_id_instncia_fljo: ' || p_id_instncia_fljo ||
  ...
  procedure prc_gn_rvsion_trea_fljo(p_cdgo_clnte       in number,
                                    p_id_rvsion_trea   in number default null,
                                    p_id_qja_rclmo     in number,
                                    p_id_fljo_trea     in number,
                                    p_id_instncia_fljo in number,
                                    p_id_usrio         in number,
                                    p_cdgo_estdo       in varchar2,
                                    p_obsrvcion        in varchar2,
                                    p_dcmnto           in blob default null,
                                    p_file_name        in varchar2 default null,
                                    p_file_mimetype    in varchar2 default null,
                                    o_cdgo_rspsta      out number,
                                    o_mnsje_rspsta     out varchar2) as
    v_nl             number;
    nmbre_up         varchar2(200) := 'pkg_pl_workflow_1_0.prc_gn_rvsion_trea_fljo';
    v_id_rvsion_trea number;
  begin
    o_cdgo_rspsta := 0;
    ...
    if p_cdgo_estdo = 'A' then

      begin
        update pq_g_rvsion_trea_fljo
           set cdgo_estdo = 'A'
         where cdgo_estdo = 'R'
           and id_instncia_fljo = p_id_instncia_fljo
           and id_rvsion_trea = p_id_rvsion_trea;
      exception
        when others then
          rollback;
          o_cdgo_rspsta  := 10;
          o_mnsje_rspsta := o_cdgo_rspsta ||
                            ' Problemas al actualizar registro de revision.' ||
                            sqlerrm;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                nmbre_up,
      ...
      begin
        ...
        update pq_g_trea_fljo_obsrvcion
           set gstion      = 'Observacion Gestionada',
               fcha_gstion = systimestamp
         where id_instncia_fljo = p_id_instncia_fljo
           and id_rvsion_trea = p_id_rvsion_trea;
      exception
        when others then
    ...
    elsif p_cdgo_estdo <> 'A' then

      -- Intentar insertar la nueva revisión
      begin
        insert into pq_g_rvsion_trea_fljo
          (id_qja_rclmo,
           id_fljo_trea,
           id_instncia_fljo,
           id_usrio,
        ...
        values
          (p_id_qja_rclmo,
           ...
           p_cdgo_estdo,
           systimestamp)
        returning id_rvsion_trea into v_id_rvsion_trea;
      ...
      exception
        when others then
          ...
          o_cdgo_rspsta  := 30;
          o_mnsje_rspsta := o_cdgo_rspsta ||
                            ' Problemas al insertar revisión.' || sqlerrm;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                nmbre_up,
                                v_nl,
      ...
      -- Llamar al procedimiento para registrar observación
      begin
        pkg_pl_workflow_1_0.prc_rg_rvsion_obsrvcion(p_cdgo_clnte       => p_cdgo_clnte,
                                                    p_id_qja_rclmo     => p_id_qja_rclmo,
                                                    p_id_fljo_trea     => p_id_fljo_trea,
                                                    p_id_instncia_fljo => p_id_instncia_fljo,
                                                    p_id_usrio         => p_id_usrio,
                                                    p_id_rvsion_trea   => v_id_rvsion_trea,
  ...
  end prc_gn_rvsion_trea_fljo;

  procedure prc_rg_rvsion_obsrvcion(p_cdgo_clnte       in number,
                                    p_id_qja_rclmo     in number,
                                    p_id_fljo_trea     in number,
                                    p_id_instncia_fljo in number,
                                    p_id_usrio         in number,
                                    p_id_rvsion_trea   in number,
                                    p_obsrvcion        in varchar2,
                                    p_dcmnto           in blob default null,
                                    p_file_name        in varchar2 default null,
                                    p_file_mimetype    in varchar2 default null,
                                    ...
                                    o_mnsje_rspsta     out varchar2) as
    v_nl     number;
    nmbre_up varchar2(200) := 'pkg_pl_workflow_1_0.prc_rg_rvsion_obsrvcion';
  ...
  begin
    ...
    begin
      ...
      if p_dcmnto is not null then

        insert into pq_g_trea_fljo_obsrvcion
          (id_rvsion_trea,
           id_instncia_fljo,
           obsrvcion,
           id_fljo_trea,
    ...
    exception
      when others then
        ...
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := o_cdgo_rspsta ||
                          ' Problemas al insertar observacion';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              nmbre_up,
                              v_nl,
