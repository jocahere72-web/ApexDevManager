  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_NT_NOTIFICACION" as

  procedure prc_co_min_orden(p_id_ntfccion in number,
  ...
  begin
    ...
    begin
      select count(*)
       ...
       inner join v_nt_g_notfccnes_gn_g_actos b
          on p_id_ntfccion = b.id_ntfccion
    ...
    begin
      select orden
        ...
        from (select a.orden
                from nt_d_acto_medio_orden a
               inner join nt_d_medio b
                  on a.id_mdio = b.id_mdio
               inner join v_nt_g_notfccnes_gn_g_actos c
                  on p_id_ntfccion = c.id_ntfccion
               where a.id_acto_tpo = c.id_acto_tpo
                 and ((o_cant_mdios > 1 and
                     a.id_mdio not in
                     (select id_mdio
                          from nt_g_notificaciones_detalle
                         where id_ntfccion = p_id_ntfccion)) or
                     (o_cant_mdios = 1))
               order by a.orden asc)
       where rownum = 1;
    exception
      when others then
        o_mnsje := 'Problemas al consultar el siguiente orden Id_ntfccion: ' ||
                   p_id_ntfccion || ' , ' || 'Cant:' || o_cant_mdios ||
                   ':::::' || SQLERRM;
        raise v_error;
    end;
  exception
  ...
  procedure prc_rg_notificacion_automatica(p_id_acto in number) is
  ...
  begin
    ...
    for c_actos in (select *
                     ...
                     where id_acto_rqrdo_ntfccion = p_id_acto) loop
      declare
      ...
      begin
        ...
        begin
          ...
          if (v_cant_responsables < 1) then
            v_mnsje := 'El acto No. ' || c_actos.nmro_acto ||
                       ' no tiene responsables por favor verifique..!!';
            raise v_error;
          end if;
        end;

        --//Consultamos y/o Insertamos en Notificaciones
        pkg_nt_notificacion.prc_rg_notificaciones(p_cdgo_clnte     => c_actos.cdgo_clnte,
  ...
  begin
    ...
    for c_detalle_notificacion in (select cdgo_clnte,
                                          id_acto,
                                          nmro_acto,
                                          id_ntfccion,
                                          id_ntfccion_dtlle,
                                    ...
                                    where cdgo_estdo in ('NEP', 'NGN')
                                      and trunc(fcha_fin_trmno) <
                                          trunc(sysdate)) loop
      declare
        v_error       exception;
      ...
      begin
        ...
        pkg_nt_notificacion.prc_co_min_orden(p_id_ntfccion => c_detalle_notificacion.id_ntfccion,
        ...
        -------------------------------------
        for c_medios in (select a.id_mdio,
                                a.indcdor_autmtco,
                                a.drcion,
                                a.undad_drcion,
                                a.dia_tpo
                          ...
                          inner join nt_d_medio b
                             on a.id_mdio = b.id_mdio
                          where a.id_acto_tpo =
                                c_detalle_notificacion.id_acto_tpo
                            and ((v_count_mdios > 1 and
                                a.id_mdio not in
                                (select id_mdio
                                     from nt_g_notificaciones_detalle
                                    where id_ntfccion =
                                          c_detalle_notificacion.id_ntfccion)) or
                                (v_count_mdios = 1))
                            and a.orden = v_min_orden
          ...
          begin
            if (c_medios.indcdor_autmtco = 'S') then
              pkg_nt_notificacion.prc_rg_notificacion_detalle(p_id_ntfccion_dtlle    => v_id_ntfccion_dtlle,
                                                              ...
                                                              p_fcha_fin_trmno       => pk_util_calendario.fnc_cl_fecha_final(p_cdgo_clnte    => c_detalle_notificacion.cdgo_clnte,
  ...
  procedure prc_ac_edicto(p_id_lte       in nt_g_lote.id_lte%type,
  ...
  begin
    ...
    begin
      select b.id_edcto, a.id_ntfcion_mdio_evdncia
        into v_id_edcto, v_id_ntfcion_mdio_evdncia
        from nt_g_lote a
       inner join nt_g_edicto b
          on a.id_ntfcion_mdio_evdncia = b.id_ntfcion_mdio_evdncia
    ...
    begin
      update nt_g_medio_entidad_evdncia
         set file_name     = substr(p_file_evdncia,
                                    instr(p_file_evdncia, '/') + 1),
             file_mimetype = v_file_mimetype,
             file_blob     = v_file_blob
       where id_ntfcion_mdio_evdncia = v_id_ntfcion_mdio_evdncia;
    exception
      when others then
        o_mnsje := 'Problemas al Actualizar Evidencia Asociada al Lote, ' ||
                   SQLERRM;
        raise v_error;
    end;
  ...
  procedure prc_rg_edicto(p_id_lte       in nt_g_lote.id_lte%type,
  ...
  begin
    ...
    begin
      select a.cdgo_clnte,
        ...
        from nt_g_lote a
       inner join v_nt_d_ntfccion_mdio_entdd b
          on a.id_entdad_clnte_mdio = b.id_entdad_clnte_mdio
       inner join nt_d_acto_medio_orden c
          on b.id_mdio = c.id_mdio
         and a.id_acto_tpo = c.id_acto_tpo
       where a.id_lte = p_id_lte;
    exception
      when others then
        o_mnsje := 'Problemas al consultar el medio asociado al acto, ' ||
                   SQLERRM;
        raise v_error;
    end;
    ...
    exception
      when others then
        o_mnsje := 'Problemas al insertar evidencia Id_Mdio: ' || v_id_mdio ||
                   ' , ' || SQLERRM;
        raise v_error;
    end;
    ...
    begin
      select * into v_csal from nt_d_causales where indcdor_ntfcdo = 'S';
    exception
      when others then
        o_mnsje := 'Problemas al consultar causal para notificar, ' ||
                   SQLERRM;
        raise v_error;
    end;

    --Cargamos la evidencia a los responsables del lote
    for c_responsables in (select id_ntfccion_rspnsble
                             from nt_d_lote_detalle
                            where id_lte = p_id_lte) loop
      begin
      ...
      exception
        when others then
          o_mnsje := 'Problemas al actualizar responsables asociados al lote, Id_Lote: ' ||
                     p_id_lte || ', ' || SQLERRM;
          raise v_error;
      end;
    end loop;
    ...
    for c_notificacion_dtlle in (select a.id_ntfccion_dtlle
                                   from v_nt_g_ntfccnes_rspnsble a
                                  inner join nt_d_lote_detalle b
                                     on a.id_ntfccion_rspnsble =
                                        b.id_ntfccion_rspnsble
                                  where b.id_lte = p_id_lte
                                  group by a.id_ntfccion_dtlle) loop
      begin
      ...
      exception
        when others then
          o_mnsje := 'Problemas al actualizar estado del intento de notificacion, ' ||
                     SQLERRM;
          raise v_error;
      end;
    end loop;
  ...
  procedure prc_rg_gaceta(p_id_lte        in nt_g_lote.id_lte%type,
  ...
  begin
    ...
    begin
      select a.cdgo_clnte, b.id_mdio, a.id_entdad_clnte_mdio
        into v_cdgo_clnte, v_id_mdio, v_id_entdad_clnte_mdio
        from nt_g_lote a
       inner join v_nt_d_ntfccion_mdio_entdd b
          on a.id_entdad_clnte_mdio = b.id_entdad_clnte_mdio
       inner join nt_d_acto_medio_orden c
    ...
    begin
      insert into nt_g_gaceta
        (id_ntfcion_mdio_evdncia, nmro_gceta, fcha_pblccion)
      values
        (v_id_ntfcion_mdio_evdncia, p_nmro_gceta, p_fcha_pblccion);
    exception
      when others then
  ...
  procedure prc_rg_prensa(p_id_lte       in nt_g_lote.id_lte%type,
                          p_ubccion      in varchar2,
  ...
  begin
    ...
    begin
      insert into nt_g_prensa
        (id_ntfcion_mdio_evdncia, ubccion, fcha_rgstro)
      values
        (v_id_ntfcion_mdio_evdncia, p_ubccion, p_fcha_rgstro);
    exception
      when others then
  ...
  procedure prc_rg_notificacion__puntual(p_cdgo_clnte          in number,
                                         p_id_acto             in number,
  ...
  begin
    ...
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al consultar el acto seleccionado Id_Acto: ' ||
                          p_id_acto;
        raise v_error;
    end;
    /*
    ...
    begin
      select case
               when count(a.id_acto_rspnsble) > 0 then
                'S'
               when count(a.id_acto_rspnsble) < 1 then
                'N'
             end
        into v_responsables
        from gn_g_actos_responsable a
       where a.id_acto = p_id_acto
         and a.indcdor_ntfccion = 'N';
      if (v_responsables = 'N') then
        o_cdgo_rspsta  := 2;
        o_mnsje_rspsta := 'El acto a notificar no tiene responsables pendientes por notificar por favor verifique, Id_Acto: ' ||
                          p_id_acto;
        raise v_error;
      end if;
    end;
    ...
    begin
      select id_entdad_clnte_mdio, id_mdio
       ...
       where cdgo_clnte = p_cdgo_clnte
         and cdgo_mdio = p_cdgo_mdio;
    exception
      when others then
        o_cdgo_rspsta  := 3;
        o_mnsje_rspsta := 'Problemas al consultar medio de notificacion puntual: ' ||
                          p_cdgo_mdio || ', ' || SQLERRM;
        raise v_error;
    end;
    ...
    exception
      when others then
        o_cdgo_rspsta  := 4;
        o_mnsje_rspsta := 'Problemas al consultar causal para notificar, ' ||
                          SQLERRM;
        raise v_error;
    end;
    ...
    begin
      select undad_drcion, dia_tpo, drcion
        into v_undad_drcion, v_dia_tpo, v_drcion
    ...
    exception
      ...
      when others then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := 'Problemas al consultar termino de tiempo para la notificacion puntual: ' ||
                          p_cdgo_mdio || ', id_acto_tpo: ' || v_id_acto_tpo ||
                          ' , ' || SQLERRM;
        raise v_error;
    end;
    ...
    exception
      ...
      when others then
        o_cdgo_rspsta  := 6;
        o_mnsje_rspsta := 'Problemas al consultar el intento de notificacion para el acto Id_Acto: ' ||
                          p_id_acto || ' Id_Medio: ' || v_id_mdio;
        raise v_error;
    end;
    ...
    if (v_id_ntfccion_dtlle is null) then
      ...
      pkg_nt_notificacion.prc_rg_evidencia(p_cdgo_clnte              => p_cdgo_clnte,
                                           ...
                                           p_file_blob               => v_file_blob,
                                           p_file_name               => substr(p_file_evdncia,
                                                                               instr(p_file_evdncia,
                                                                                     '/') + 1),
                                           p_file_mimetype           => v_file_mimetype,
      ...
      if (p_cdgo_mdio = 'PPN') then
        ...
        exception
          when others then
            o_cdgo_rspsta  := 10;
            o_mnsje_rspsta := 'Problemas al insertar evidencia en presentacion personal, ' ||
                              SQLERRM;
            raise v_error;
        end;
      elsif (p_cdgo_mdio = 'CCY') then
        begin
          insert into nt_g_conducta_concluyente
            (id_ntfcion_mdio_evdncia, fcha_prsntcion, fcha_rgstro)
          values
            (v_id_ntfcion_mdio_evdncia, p_fcha_prsntccion, sysdate);
        exception
          when others then
            o_cdgo_rspsta  := 11;
            o_mnsje_rspsta := 'Problemas al insertar evidencia en conducta concluyente, ' ||
                              SQLERRM;
            raise v_error;
        end;
      end if;
      ...
      if (p_cdgo_mdio = 'PPN') then
        ...
        exception
          when others then
            o_cdgo_rspsta := 13;
            o_cdgo_rspsta := 'Problemas al insertar evidencia en presentacion personal, ' ||
                             SQLERRM;
            raise v_error;
        end;
      elsif (p_cdgo_mdio = 'CCY') then
      ...
      exception
        when others then
          o_cdgo_rspsta  := 15;
          o_mnsje_rspsta := 'Problemas al actualizar estado de la notificacion Id_ntfccion: ' ||
                            p_id_ntfccion || ' , ' || SQLERRM;
          raise v_error;
      end;
      ...
      exception
        when others then
          o_cdgo_rspsta  := 16;
          o_mnsje_rspsta := 'Problemas al actualizar estado del intento de notificacion Id_ntfccion_dtlle:' ||
                            v_id_ntfccion_dtlle;
          raise v_error;
      end;
      ...
      for c_responsables in (select a.*
                              ...
                              inner join (select id_acto_rspnsble
                                           from json_table(p_json_rspnsbles,
                                                           '$[*]' columns
                                                           id_acto_rspnsble PATH
                                                           '$.ID_ACTO_RSPNSBLE')) b
                                 on a.id_acto_rspnsble = b.id_acto_rspnsble
                              where a.id_acto = p_id_acto
                                and a.indcdor_ntfccion = 'N') loop
        ...
        begin
          ...
          select id_ntfccion_rspnsble
           ...
           where id_ntfccion_dtlle = v_id_ntfccion_dtlle
             and id_acto_rspnsble = c_responsables.id_acto_rspnsble
          ...
          exception
            when others then
              o_cdgo_rspsta  := 17;
              o_mnsje_rspsta := 'Problemas al actualizar responsable del intento de notifiacion Id_Notificacion_Responsable: ' ||
                                v_id_ntfccion_rspnsble;
              raise v_error;
          end;
        exception
          when no_data_found then
  ...
  procedure prc_el_lote(p_id_lote   in number,
                        o_mnsje_tpo out varchar2,
                        o_mnsje     out varchar2) is
    v_error exception;
  ...
  begin
    ...
    begin
      delete from nt_d_lote_detalle where id_lte = p_id_lote;
    exception
      when others then
        o_mnsje := 'Problemas al eliminar detalle del Lote No. ' ||
                   p_id_lote;
        raise v_error;
    end;
    ...
    --Elimina el lote
    begin
      delete from nt_g_lote where id_lte = p_id_lote;
    exception
      when others then
  ...
  procedure prc_ac_lote(p_id_lote    in number,
  ...
  begin
      ...
      update nt_g_notificaciones_detalle a
       ...
       where id_ntfccion_dtlle in
             (select b.id_ntfccion_dtlle
               ...
               inner join v_nt_g_ntfccnes_rspnsble b
                  on a.id_acto_rspnsble = b.id_acto_rspnsble
    ...
    --Actualizamos el estado del lote
    begin
      update nt_g_lote set cdgo_estdo_lte = 'EPR' where id_lte = p_id_lote;
    exception
      when others then
  ...
  procedure prc_rg_detalle_lotes(p_id_lote                in number,
  ...
  begin
    insert into nt_d_lote_detalle
      ...
      select nvl(c.max_id_lte_dtlle, 0) + rownum,
             ...
             id_acto_rspnsble,
             id_ntfccion_rspnsble
        from (select a.id_acto, a.id_acto_rspnsble, a.id_ntfccion_rspnsble
                from v_nt_g_ntfccnes_rspnsble a
               inner join (Select ID_NTFCCION_DTLLE
                            From json_table(p_id_ntfccion_dtlle_json,
                                            '$[*]' columns ID_NTFCCION_DTLLE PATH
                                            '$.ID_NTFCCION_DTLLE')) b
                  on a.id_ntfccion_dtlle = b.id_ntfccion_dtlle
               order by 1, 2) a
        left join (select max(id_lte_dtlle) as max_id_lte_dtlle
                     from nt_d_lote_detalle) c
          on c.max_id_lte_dtlle = c.max_id_lte_dtlle;
  exception
    when v_error then
      if (o_mnsje is null) then
        o_mnsje := SQLERRM;
    ...
    if (p_id_acto is not null) then
      ...
      begin
        ...
        begin
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nivel,
                                'mirar si hay responsables' ||
                                v_cant_responsables || ' ' || systimestamp,
                                1);
          ...
          if (v_cant_responsables < 1) then
            ...
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  ...
                                  v_nmbre_up,
                                  v_nivel,
                                  'entro al if de v_cant_responsables ' || ' ' ||
                                  systimestamp,
                                  1);
          end if;
        ...
        if (o_mnsje_tpo = 'ERROR') then
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nivel,
                                'entra al if de error' || o_mnsje_tpo || ' ' ||
                                systimestamp,
                                1);
        end if;
      ...
      for c_actos in (select a.id_acto, a.nmro_acto
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nivel,
                              'entra al loop que recorre actos',
                              1);
  ...
  procedure prc_rg_notificaciones(p_id_ntfccion    out nt_g_notificaciones.id_ntfccion%type,
                                  p_id_acto        in number,
  ...
  begin
    begin
      ...
      select nmro_acto, id_ntfccion, id_acto, id_acto_tpo, cdgo_clnte
        into v_nmro_acto,
             p_id_ntfccion,
             v_id_acto,
             v_id_acto_tpo,
      ...
      exception
        when others then
          o_mnsje_tpo := 'ERROR';
          o_mnsje     := 'El tipo de acto, del acto No.' || v_nmro_acto ||
                         ',Id.' || p_id_acto ||
                         ', No se encuentra parametrizado en Secuencia Medios x Tipo de Acto';
          raise v_error;
      end;

      begin
        update nt_g_notificaciones
           set cdgo_estdo = 'NGN', indcdor_ntfcdo = 'N'
         where id_ntfccion = p_id_ntfccion;
      exception
        when others then
          o_mnsje_tpo := 'ERROR';
          o_mnsje     := 'Problemas al Actualizar notificacion del acto No.' ||
                         p_id_acto;
          raise v_error;
      end;
      ...
      -------------------------------------
      --//Obtenemos el Min orden
      pkg_nt_notificacion.prc_co_min_orden(p_id_ntfccion => p_id_ntfccion,
                                           o_min_orden   => v_min_orden,
                                           o_cant_mdios  => v_count_mdios,
                                           o_mnsje_tpo   => o_mnsje_tpo,
                                           o_mnsje       => o_mnsje);
    ...
    exception
      when no_data_found then
        ...
        --Consultamos el Acto
        begin
          select id_acto_tpo, nmro_acto
            into v_id_acto_tpo, v_nmro_acto
            from gn_g_actos
           where id_acto = p_id_acto;
        exception
          when others then
        ...
        begin
          insert into nt_g_notificaciones
            (id_acto, cdgo_estdo, indcdor_ntfcdo)
          values
            (p_id_acto, p_cdgo_estdo, p_indcdor_ntfcdo)
          returning id_ntfccion into p_id_ntfccion;
        exception
          when others then
            o_mnsje_tpo := 'ERROR';
            o_mnsje     := 'Problemas al registrar notificacion del acto No.' ||
                           p_id_acto || ' , ' || SQLERRM;
            raise v_error;
        end;
  ...
  procedure prc_rg_notificacion_detalle(p_id_ntfccion_dtlle    out nt_g_notificaciones_detalle.id_ntfccion_dtlle%type,
    ...
    --+---------------------------------+--
  begin
    insert into nt_g_notificaciones_detalle
      (id_ntfccion,
       id_mdio,
       id_entdad_clnte_mdio,
       fcha_gnrcion,
    ...
    values
      (p_id_ntfccion,
       p_id_mdio,
       p_id_entdad_clnte_mdio,
       p_fcha_gnrcion,
       p_fcha_fin_trmno,
       p_id_fncnrio_gnrcion)
    returning id_ntfccion_dtlle into p_id_ntfccion_dtlle;
  exception
    when others then
      o_mnsje_tpo := 'ERROR';
  ...
  procedure prc_rg_notificacion_respnsbles(p_id_ntfccion_dtlle       in nt_g_notificaciones_detalle.id_ntfccion_dtlle%type,
  ...
  begin
    ...
    if (p_json_rspnsbles is null) then
      for c_actos_responsables in (select id_acto_rspnsble,
                                          cdgo_idntfccion_tpo,
                                          idntfccion,
                                          prmer_nmbre,
                                          sgndo_nmbre,
                                    ...
                                    where a.id_acto = p_id_acto
                                      and a.indcdor_ntfccion = 'N'
                                      and a.id_acto_rspnsble =
                                          nvl(p_id_acto_rspnsble,
                                              a.id_acto_rspnsble)) loop
        --Valida Campos Obligatorios Notificaciones Responsables
        if (c_actos_responsables.cdgo_idntfccion_tpo is null or
           ...
           c_actos_responsables.id_mncpio_ntfccion is null) then
          o_mnsje := 'Campos Vacios en el responsable con identificacion ' ||
                     c_actos_responsables.idntfccion || ' del acto No. ' ||
                     v_rt_gn_g_actos.nmro_acto || ' , ';
        ...
        exception
          when others then
            o_mnsje := 'Problemas al registrar responsable Id: ' ||
                       c_actos_responsables.id_acto_rspnsble || ' , ' ||
                       SQLERRM;
            raise v_error;
        end;
      ...
      for c_actos_responsables in (select a.id_acto_rspnsble,
  ...
  procedure pr_co_archivo_evidencia(p_file_name     in varchar2,
  ...
  begin
    if p_file_name = 'no_data.pdf' then
      p_file_mimetype := 'application/pdf';
      p_file_blob     := EMPTY_BLOB();
    else
      begin
  ...
  procedure prc_rg_evidencia_puntual(p_cdgo_clnte           in number,
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nivel,
                          'Consultando archivo Subido, ' || systimestamp,
                          1);
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nivel,
                          'Registramos la evidencia,' || systimestamp,
                          1);
    --Registramos la evidencia
    begin
      insert into nt_g_medio_entidad_evdncia
    ...
    exception
      when others then
        o_mnsje := 'Problemas al insertar evidencia Id_Mdio: ' || p_id_mdio ||
                   ' , ' || SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    if (p_id_mdio = 1) then
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nivel,
                            'Inseramos en nt_g_correo_certificado, ' ||
                            systimestamp,
                            1);
      begin
        ...
        values
          (v_id_ntfcion_mdio_evdncia,
           pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                     p_nodo => 'nmro_orden'),
           pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                     p_nodo => 'nmro_guia'),
           pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                     p_nodo => 'cdgo_csal_entdad'),
           pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                     p_nodo => 'nmro_idntfccion'),
           pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                     p_nodo => 'nmbre_ntfccion'));
      exception
        when others then
          o_mnsje := 'Problemas al Registrar Evidencia Correo Certificado, ' ||
                     sqlerrm;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
      ...
      begin
        select cdgo_entdad
          ...
          from v_nt_d_ntfccion_mdio_entdd
         where cdgo_clnte = p_cdgo_clnte
           and id_entdad_clnte_mdio = p_id_entdad_clnte_mdio;
      exception
        when others then
      ...
      begin
        select cdgo_csal
         ...
         where cdgo_entdad = v_cdgo_entdad
           and cdgo_csal_entdad =
               pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                         p_nodo => 'cdgo_csal_entdad');
      exception
        when others then
      ...
      exception
        when others then
          o_mnsje := 'Problemas al consultar indicador si notifica al responsable, ' ||
                     sqlerrm;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
    ...
    elsif (p_id_mdio = 3) then
      --Edicto
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nivel,
                            'Insertamos en Edicto',
                            1);
      begin
        v_indcdor_ntfcdo := 'S';
        insert into nt_g_edicto
        ...
        values
          (v_id_ntfcion_mdio_evdncia,
           ...
           pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                     p_nodo => 'fcha_fin'),
           trunc(to_date(pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                                   p_nodo => 'fcha_fin'),
                         'DD-MON-YYYY')) -
           trunc(to_date(pkg_gn_generalidades.fnc_ca_extract_value(p_xml  => p_xml,
                                                                   p_nodo => 'fcha_incio'),
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nivel,
                            'Consultamos la causal que notifica',
                            1);
      begin
        select cdgo_csal
          into v_cdgo_csal
      ...
      exception
        when others then
          o_mnsje := 'Problemas al consultar causal que notifica, ' ||
                     sqlerrm;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nivel,
                          v_id_ntfcion_mdio_evdncia || ',' ||
                          v_indcdor_ntfcdo || ',' || p_id_fncnrio || ',' ||
                          v_cdgo_csal || ',' || p_id_ntfccion_rspnsble,
                          1);
    ...
    exception
      when others then
        o_mnsje := 'Problemas al actualizar responsable de la notificacion, ' ||
                   sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
  ...
  begin
    ...
    insert into muerto
      (n_001, v_001, v_002)
    values
      (0000, v_cdgo_rspsta, v_mnsje_rspsta);
    commit;
  ...
  begin
    ...
    begin
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'consulta proceso de carga v_exist:' || v_exist ||
                            systimestamp,
                            1);
    exception
      when no_data_found then
    ...
    for c_guia_notificacion in (select id_lte,
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'entrando al loop' || p_id_prcso_crga,
                            1);
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'Consulta medio responsable ' || v_id_mdio ||
                              systimestamp,
                              1);
      exception
        when no_data_found then
      ...
      if v_cdgo_mtdo_carga_guias = 'ATF' then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'guias cargadas AFT ' || o_mnsje_rspsta,
                              1);
        if o_cdgo_rspsta != 0 then
          o_cdgo_rspsta  := 40;
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nl,
                                o_mnsje_rspsta || systimestamp || 'AFT',
                                1);
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              o_mnsje_rspsta || systimestamp || 'DFS',
                              1);
        if o_cdgo_rspsta != 0 then
          o_cdgo_rspsta  := 50;
          o_mnsje_rspsta := o_cdgo_rspsta || ' - ' || o_mnsje_rspsta;
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'Se inserto en nt_g_medio_entidad_evdncia' ||
                              systimestamp,
                              1);
      exception
        when others then
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'inserto en nt_g_correo_certificado' ||
                              systimestamp,
                              1);
      exception
        when others then
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'Actualiza responsables' || systimestamp,
                              1);
      exception
        when others then
          o_cdgo_rspsta  := 80;
      ...
      if (v_cant_detalle_lote < 1) then
        ...
        begin
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nl,
                                'Actualiza estado del lote' || p_id_fncnrio,
                                1);
        exception
          when others then
            o_cdgo_rspsta  := 100;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Saliendo del loop' || systimestamp,
                          1);
    ...
    begin
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'consulta registros por procesar' || v_count,
                            1);
      ...
      if v_count = 0 then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'Actualiza proceso de carga a S' ||
                              p_id_prcso_crga,
                              1);
      end if;
  ...
  begin
    ...
    for c_guias in (select a.id_ntfcion_mdio_evdncia,
      ...
      begin
        ...
        update nt_g_ntfccnes_rspnsble
           set indcdor_ntfcdo = 'S', fcha_ntfccion = sysdate
         where id_ntfcion_mdio_evdncia = c_guias.id_ntfcion_mdio_evdncia;
        commit;
  ...
  procedure prc_rg_notfccion_gnrda_atmtca as
    v_mnsje_tpo    varchar2(20);
  ...
  begin
    ...
    for c_actos in (select *
                      from v_nt_g_notfccnes_gn_g_actos
                     where indcdr_ntfccion_atmtca = 'S') loop
      declare
        v_id_ntfccion       nt_g_notificaciones.id_ntfccion%type;
        v_id_ntfccion_dtlle nt_g_notificaciones_detalle.id_ntfccion_dtlle%type;
        v_error             exception;
      begin
        ...
        begin
          ...
          if (v_cant_responsables < 1) then
            v_mnsje_rspsta := v_mnsje_rspsta || ' El acto No. ' ||
                              c_actos.nmro_acto ||
                              ' no tiene responsables por favor verifique..!!';
            raise v_error;
          end if;
  ...
  begin
    ...
    for c_notificar in (select distinct b.id_acto,
                                        a.id_lte,
                                        e.id_sjto_impsto,
                                        f.id_envio_prgrmdo,
                                        f.idntfcdor
                          ...
                          join gn_g_actos_sujeto_impuesto d
                            on d.id_acto = a.id_acto
                          join si_i_sujetos_impuesto e
                            on e.id_sjto_impsto = d.id_sjto_impsto
                           and email is not null
                          join v_nt_d_acto_medio_orden f
                            on f.id_acto_tpo = a.id_acto_tpo
                           and upper(cdgo_mdio) = 'CEL'
                         where a.id_lte = p_id_lte) loop

      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
                            'c_notificar.idntfcdor: ' ||
                            c_notificar.idntfcdor,
                            1);
      ...
      if c_notificar.idntfcdor is not null then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'v_json_parametros: ' || v_json_parametros,
                              1);
        ...
        begin
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nl,
                                '1 msg respuesta prc_co_envio_programado: ' ||
                                o_mnsje_rspsta,
                                1);
        ...
        exception
          when others then
            ...
            o_mnsje_rspsta := 'o_cdgo_rspsta: ' || o_cdgo_rspsta ||
                              ': Error en los envios programados de notificaciones ' ||
                              sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nl,
                                  '2 msg respuesta prc_co_envio_programado: ' ||
                                  o_mnsje_rspsta,
                                  1);
            rollback;
            return;
        ...
        o_cdgo_rspsta  := 30;
        o_mnsje_rspsta := 'o_cdgo_rspsta: ' || o_cdgo_rspsta ||
                          ': No se encontro el identificador del envio programado ' ||
                          sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              '3 msg respuesta prc_co_envio_programado: ' ||
                              o_mnsje_rspsta,
                              1);
        rollback;
        return;
  ...
  exception
    when others then
      o_cdgo_rspsta  := 10;
      o_mnsje_rspsta := 'Problemas al enviar las notificaciones por mail para el id_lote, ' ||
                        p_id_lte || ' - ' || SQLERRM;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
                            '4 msg respuesta prc_co_envio_programado: ' ||
                            o_mnsje_rspsta,
                            1);
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Entrando Notificacion Puntual ' || systimestamp,
                          1);
    ...
    --Consultamos el acto a notificar
    begin
      select id_acto_tpo, file_blob, file_bfile
        into v_id_acto_tpo, v_file_blob, v_file_bfile
        from v_gn_g_actos
       where id_acto = p_id_acto;
    exception
      when others then
    ...
    if p_cdgo_mdio = 'CEL' and v_file_blob is null and v_file_bfile is null then
      o_cdgo_rspsta  := 5;
      o_mnsje_rspsta := 'No se puede realizar la notificación por Email por que el acto no tiene el Archivo Blob generado: ' ||
                        p_id_acto;
      raise v_error;
    end if;
    ...
    exception
      when others then
        o_cdgo_rspsta  := 3;
        o_mnsje_rspsta := 'Problemas al consultar medio de notificación puntual: ' ||
                          p_cdgo_mdio || ', ' || SQLERRM;
        raise v_error;
    end;
    ...
    exception
      ...
      when others then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := 'Problemas al consultar termino de tiempo para la notificación puntual: ' ||
                          p_cdgo_mdio || ', id_acto_tpo: ' || v_id_acto_tpo ||
                          ' , ' || SQLERRM;
        raise v_error;
    end;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'v_id_ntfccion_dtlle :' || v_id_ntfccion_dtlle,
                          1);
    ...
    if (v_id_ntfccion_dtlle is null) then
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'pr_co_archivo_evidencia v_id_ntfccion_dtlle is null v_file_mimetype :' ||
                            v_file_mimetype,
                            1);
      ...
      if (p_cdgo_mdio = 'PPN') then
        ...
        exception
          when others then
            o_cdgo_rspsta  := 11;
            o_mnsje_rspsta := 'Problemas al insertar evidencia en Pagina Web, ' ||
                              SQLERRM;
            raise v_error;
        end;
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'Insertando en nt_g_email_puntual',
                              1);
        ...
        begin
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nl,
                                'Insertando evidencia en nt_g_email_puntual',
                                1);
        ...
        exception
          when others then
            o_cdgo_rspsta  := 11;
            o_mnsje_rspsta := 'Problemas al insertar evidencia en email puntual, ' ||
                              SQLERRM;
            raise v_error;
        end;
      ...
      */
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'pr_co_archivo_evidencia v_id_ntfccion_dtlle is NOT null v_file_mimetype :' ||
                            v_file_mimetype,
                            1);
      ...
      if (p_cdgo_mdio = 'PPN') then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'Insertando en nt_g_email opcion 2',
                              1);
        ...
        begin
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nl,
                                'Insertando v_id_mdio: ' || v_id_mdio,
                                1);
        ...
        begin
          ...
          if p_indcdor_envia_email = 'S' then
            ...
            if (o_cdgo_rspsta != 0) then
              o_cdgo_rspsta  := 15;
              o_mnsje_rspsta := o_mnsje_rspsta ||
                                ' Error al intentar notificar el acto por email el id acto: ' ||
                                p_id_acto;
              pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                    null,
                                    v_nmbre_up,
                                    v_nl,
                                    '1 msg respuesta 1 email: ' ||
                                    o_mnsje_rspsta,
                                    1);
              raise v_error;
            end if;
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'va a actualizar nt_g_notificaciones cdgo_estdo = NEP',
                              1);
        update nt_g_notificaciones
           set cdgo_estdo = 'NEP'
         where id_ntfccion = p_id_ntfccion;
      exception
        when others then
          o_cdgo_rspsta  := 15;
          o_mnsje_rspsta := 'Problemas al actualizar estado de la notificación Id_ntfccion: ' ||
                            p_id_ntfccion || ' , ' || SQLERRM;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
                                '4 msg respuesta 1 email: ' ||
                                o_mnsje_rspsta,
                                1);
          raise v_error;
      end;
      ...
      exception
        when others then
          o_cdgo_rspsta  := 16;
          o_mnsje_rspsta := 'Problemas al actualizar estado del intento de notificación Id_ntfccion_dtlle:' ||
                            v_id_ntfccion_dtlle;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
                                '1 msg intentos noti: ' || o_mnsje_rspsta,
                                1);
          raise v_error;
      end;
    ...
    update nt_g_ntfccnes_rspnsble
     ...
     where id_acto_rspnsble in
           (select b.id_acto_rspnsble
              from gn_g_actos_responsable b
    ...
    when v_error then
      if (o_mnsje_rspsta is null or o_cdgo_rspsta is null) then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              '3 msg rta noti puntual: ' || o_cdgo_rspsta,
                              1);
      end if;
    when others then
      o_cdgo_rspsta  := 1;
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Entrando prc_rg_notificaciones_email_puntual ' ||
                          systimestamp,
                          1);
    ...
    exception
      when no_data_found then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              '1 msg respuesta v_id_entdad_clnte_mdio: ' ||
                              o_mnsje_rspsta,
                              1);
        return;
      when others then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'Problemas al consultar el id entidad cliente medio ' || ' ' ||
                          SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              '2 msg respuesta v_id_entdad_clnte_mdio: ' ||
                              o_mnsje_rspsta,
                              1);
        return;
    end;
    ...
    exception
      ...
      when others then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'Problemas al consultar el tipo de acto ' || ' ' ||
                          SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    exception
      when others then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              '1 msg respuesta insert nt g actos: ' ||
                              o_mnsje_rspsta,
                              1);
        rollback;
        return;
    ...
    begin
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'va a hacer la select de v_nt_g_notificaciones_detalle ',
                            1);
      select id_ntfccion_dtlle
        into v_id_ntfccion_dtlle
        from v_nt_g_notificaciones_detalle
    ...
    exception
      when no_data_found then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              '1 mnsje_rspsta v_id_ntfccion_dtlle: ' ||
                              o_mnsje_rspsta,
                              1);
        rollback;
        Return;
      when others then
        o_cdgo_rspsta  := 25;
        o_mnsje_rspsta := 'Problemas al consultar el id notificacion detalle ' || ' ' ||
                          SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              '2 mnsje_rspsta v_id_ntfccion_dtlle:' ||
                              o_mnsje_rspsta,
                              1);
        rollback;
        return;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'id_ntfccion_dtlle:' || v_id_ntfccion_dtlle,
                          1);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          v_nmbre_up,
    ...
    begin
      pkg_nt_notificacion.prc_rg_detalle_lotes(p_id_lote                => v_id_lte,
                                               p_id_ntfccion_dtlle_json => '[{"ID_NTFCCION_DTLLE":"' ||
                                                                           v_id_ntfccion_dtlle ||
                                                                           '"}]', --[{"ID_NTFCCION_DTLLE":"893024"}]
      ...
      if (v_mnsje_tpo = 'ERROR') then
        ...
        o_mnsje_rspsta := v_mnsje ||
                          ' No se pudo registrar el detalle del lote';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              '1 mnsje_rspsta prc_rg_detalle_lotes:' ||
                              o_mnsje_rspsta,
                              1);
        Rollback;
        Return;
    ...
    exception
      when others then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              '2 mnsje_rspsta prc_rg_detalle_lotes:' ||
                              o_mnsje_rspsta,
                              1);
        rollback;
        return;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'prc_rg_detalle_lotes v_mnsje_tpo:' ||
                          v_mnsje_tpo,
                          1);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          v_nmbre_up,
                          v_nl,
                          'prc_rg_detalle_lotes v_mnsje:' || v_mnsje,
                          1);
    ...
    Begin
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'prc_ac_lote:v_mnsje_tpo  ' || v_mnsje_tpo,
                            1);
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
                            'v_mnsje v_mnsje:' || v_mnsje,
                            1);
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
                            'v_id_lte:v_id_lte  ' || v_id_lte,
                            1);
    ...
    exception
      when others then
        o_cdgo_rspsta  := 40;
        o_mnsje_rspsta := 'No se pudo actualizar el lote ' || ' ' ||
                          SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    begin
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'valida si tiene email: ' || v_valida_email,
                            1);
    ...
    exception
      ...
      when others then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'Problemas al consultar el el correo electronico aspciado al sujeto impuesto - id acto ' || ' ' ||
                          SQLERRM;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              '2 msg respuesta valida email: ' ||
                              o_mnsje_rspsta,
                              1);
        rollback;
        return;
    ...
    if v_valida_email = 'S' then
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              '1 noti email msg codigo respuesta: ' ||
                              o_cdgo_rspsta,
                              1);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              '2 noti email msg mensaje respuesta: ' ||
                              o_mnsje_rspsta,
                              1);
        ...
        if o_cdgo_rspsta != 0 then
          o_cdgo_rspsta  := 45;
          o_mnsje_rspsta := o_mnsje_rspsta ||
                            'Error al  enviar el email de notificacion del id lote: ' ||
                            v_id_lte;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
                                '3 prc_rg_notificaciones email: ' ||
                                o_mnsje_rspsta,
                                1);
          rollback;
          return;
      ...
      exception
        when others then
          o_cdgo_rspsta  := 10;
          o_mnsje_rspsta := 'Problemas al enviar el correo ' || ' ' ||
                            SQLERRM;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
  ...
  exception
    when others then
      o_cdgo_rspsta  := 20;
      o_mnsje_rspsta := 'Problemas al enviar las notificaciones por mail para el id_acto, ' ||
                        p_id_acto || ' - ' || SQLERRM;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
                            '6 prc_rg_notificaciones email: ' ||
                            o_mnsje_rspsta,
                            1);
  ...
  begin
    ...
    select count(*)
      ...
      join si_i_sujetos_impuesto c
       ...
       and regexp_substr(c.email,
                         '[a-zA-Z0-9.%-]+@[a-zA-Z0-9.%-]+\.[a-zA-Z]{2,4}') is not null
     where id_lte = p_id_lte;
    ...
    if v_total_actos = v_total_sujetos_impsto_email then
      select count(*)
        into v_mostrar
        from nt_g_lote a
        join v_nt_d_ntfccion_mdio_entdd b
          on a.id_entdad_clnte_mdio = b.id_entdad_clnte_mdio
         and b.cdgo_mdio = 'CEL' -- Correo electronico
         and a.cdgo_estdo_lte = 'EPR' -- En proceso
        join nt_d_lote_detalle c
          on c.id_lte = a.id_lte
        join v_gn_g_actos d
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Entrando con id_nt_crtfcdo_json: ' ||
                          p_id_crtfcdo_json,
                          6);
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta || '. Id json' ||
                          p_id_crtfcdo_json ||
                          ' de los actos no encontrado con id. ';
    ...
    for c_actos in (select a.id_acto, a.nmro_acto
      ...
      exception
        when no_data_found then
          ...
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ...
                            c_actos.nmro_acto ||
                            ', o ya esta notificado. Verifique!!!.';
      ...
      if v_indcdor_ntfcdo = 'S' then
      ...
      elsif (v_indcdor_ntfcdo is null or v_indcdor_ntfcdo = 'N') then
        begin
          update gn_g_actos
        ...
        if (v_ntfcdo_pag_web = 'S' and v_indcdor_ntfcdo = 'S') then
          ...
          o_cdgo_rspsta  := 41;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta || ', El acto ' ||
                            c_actos.nmro_acto ||
                            ' ya esta publicado en pagina web ';
          raise v_error;
  ...
  procedure prc_gn_json_actos_pblcar(p_cdgo_clnte         in number,
                                     p_id_lte             in number,
                                     p_indcdor            in varchar2,
                                     p_fcha_pblccion      in date,
                                     o_nmro_actos         out number,
                                     o_id_nt_crtfcdo_json out number,
                                     o_cdgo_rspsta        out number,
                                     o_mnsje_rspsta       out varchar2) as

    v_nvl      NUMBER;
    v_nmbre_up sg_d_configuraciones_log.nmbre_up%TYPE := 'pkg_nt_notificacion.prc_gn_json_actos_pblcar';
    v_json     clob;
    v_error    exception;

  begin

    v_nvl := pkg_sg_log.fnc_ca_nivel_log(p_cdgo_clnte => p_cdgo_clnte,
                                         p_id_impsto  => NULL,
                                         p_nmbre_up   => v_nmbre_up);
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nvl,
                          'Entrando con lote ' || p_id_lte,
                          6);
    begin
      SELECT JSON_ARRAYAGG(JSON_OBJECT('ID_ACTO' VALUE id_acto) RETURNING CLOB)
        INTO v_json
        from (select distinct b.id_acto as id_acto
                from nt_d_lote_detalle a
                join gn_g_actos c
                  on a.id_acto = c.id_acto
                join v_nt_g_notfccnes_gn_g_actos b
               ...
               Where c.cdgo_clnte = p_cdgo_clnte
                 and a.id_lte = p_id_lte
                 and ((p_indcdor = 'S' and b.indcdor_ntfcdo = 'S') or
                     (p_indcdor = 'N' and b.indcdor_ntfcdo = 'N') or
                     (p_indcdor = 'W' and c.ntfcdo_pag_web = 'S') or
                     (p_indcdor = 'T' and c.ntfcdo_pag_web = 'N'))) -- web
       order by id_acto;
    exception
      when no_data_found then
        o_cdgo_rspsta  := 10;
    ...
    --Construimos el json con los datos del lote

    SELECT COUNT(*)
      INTO o_nmro_actos
      FROM dual,
           json_table(v_json, '$[*]' COLUMNS id_acto PATH '$.ID_ACTO');

    if (o_nmro_actos > 0) then
      ...
      exception
        when others then
          o_cdgo_rspsta  := 11;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ' Insertando en nt_g_certificados_json.';
      ...
      if (o_id_nt_crtfcdo_json is not null) then
        o_cdgo_rspsta  := 0;
        o_mnsje_rspsta := 'Se seleccionaron ' || o_nmro_actos || ' actos.';
      else
        o_cdgo_rspsta  := 12;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' actos no seleccionados.';
  ...
  procedure prc_rg_pag_web(p_cdgo_clnte      in number,
                           ...
                           p_id_crtfcdo_json in number,
                           p_json_actos      in clob default null,
                           p_indcdor_ntfca   in varchar2 default 'N',
                           o_cdgo_rspsta     out number,
                           o_mnsje_rspsta    out varchar2) as
  ...
  begin
    ...
    if (o_cdgo_rspsta = 0) then
      begin
        delete nt_g_certificados_json
         where id_nt_crtfcdo_json = p_id_crtfcdo_json
           and cdgo_rgstro_tpo = 'P';
      exception
        when others then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                6,
                                'No se pudo borra la información del json ' ||
                                sqlerrm,
                                6);
      end;
    end if;
  ...
  begin
    ...
    begin
      insert into nt_g_certificados_json
        (cdgo_clnte, cdgo_rgstro_tpo, json_crtfcdos)
      values
        (p_cdgo_clnte, p_cdgo_rgstro_tpo, p_json_actos)
      returning id_nt_crtfcdo_json into o_id_nt_crtfcdo_json;
    end;
    ...
    if (o_id_nt_crtfcdo_json is null) then
      ...
      SELECT COUNT(*)
        INTO o_nmro_actos
        FROM dual,
             json_table((select json_crtfcdos
                          from nt_g_certificados_json a
                         where a.id_nt_crtfcdo_json = o_id_nt_crtfcdo_json),
                        '$[*]' COLUMNS id_acto PATH '$.ID_ACTO');
  ...
  begin
    ...
    if (o_cdgo_rspsta = 0) then
      ...
      begin
        select json_object(key 'p_id_usuario' value p_id_usuario)
          into v_json_parametros
          from dual;
      ...
      o_cdgo_rspsta  := 10;
      o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                        ' al intentar publicar los actos';
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            'pkg_nt_notificacion.prc_rg_pag_web_job', --v_nmbre_up,
                            6,
  ...
  begin
    ...
    --Construimos el json con los datos del lote
    if (p_indcdor_ntfcdo = 'S') then
      begin
        SELECT JSON_ARRAYAGG(JSON_OBJECT('ID_ACTO' VALUE id_acto) RETURNING CLOB)
          INTO v_json
          FROM (SELECT DISTINCT b.id_acto
                  FROM nt_d_lote_detalle a
                  JOIN gn_g_actos c
                    ON a.id_acto = c.id_acto
                  JOIN v_nt_g_notfccnes_gn_g_actos b
                    ON a.id_acto = b.id_acto
                 WHERE a.id_lte = p_id_lte
                   AND b.indcdor_ntfcdo = p_indcdor_ntfcdo
                   and c.ntfcdo_pag_web = 'S'
                   and to_char(c.fcha_pblccion, 'yyyymmdd') =
                       to_char(p_fcha_pblccion, 'yyyymmdd'));
      exception
        when no_data_found then
          o_cdgo_rspsta  := 10;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
    ...
    if (o_nmro_actos > 0) then
      ...
      begin
        insert into nt_g_certificados_json
          (cdgo_clnte, cdgo_rgstro_tpo, json_crtfcdos)
        values
          (p_cdgo_clnte, p_cdgo_rgstro_tpo, v_json)
        RETURNING id_nt_crtfcdo_json into o_id_nt_crtfcdo_json;
      exception
        when others then
          o_cdgo_rspsta  := 11;
  ...
  procedure prc_gn_json_actos(p_cdgo_clnte      in number,
                              p_id_usrio        in number,
                              p_cdgo_acto_orgen in v_nt_g_notfccnes_gn_g_actos.cdgo_acto_orgen%type,
                              p_id_acto_tpo     in v_nt_g_notfccnes_gn_g_actos.id_acto_tpo%type,
                              p_fcha_incial     in date,
                              p_fcha_fnal       in date,
                              p_nmro_acto       in gn_g_actos.nmro_acto%type,
                              p_cdgo_rgstro_tpo in varchar2,
                              o_id_json         out number,
                              o_nmro_actos      out number,
                              o_cdgo_rspsta     out number,
                              o_mnsje_rspsta    out varchar2) as

    v_nl           NUMBER;
    v_nmbre_up     sg_d_configuraciones_log.nmbre_up%TYPE := 'pkg_nt_notificacion.prc_gn_json_actos';
    v_error        exception;
    v_json_actos   clob;
    v_nmro_actos   number;
    v_json_entrada clob;
  ...
  begin
    v_json_entrada := '{"p_cdgo_clnte":' || p_cdgo_clnte || ',
                        "p_id_usrio":' || p_id_usrio || ',
                        "p_cdgo_acto_orgen":' ||
                      p_cdgo_acto_orgen || ',
                        "p_id_acto_tpo":' ||
                      ...
                      p_fcha_incial || ',
                        ...
                        "p_nmro_acto":' || p_nmro_acto || '}';

    insert into muerto
      (n_001, v_001, c_001, t_001)
    values
      (28, 'parametros de entrada', v_json_entrada, systimestamp);
    commit;
      ...
      SELECT JSON_ARRAYAGG(JSON_OBJECT('ID_ACTO' VALUE id_acto) RETURNING CLOB)
        ...
        FROM (select a.id_acto
               ...
               where a.cdgo_clnte = p_cdgo_clnte
                 ...
                 and a.cdgo_acto_orgen =
                     nvl(P_CDGO_ACTO_ORGEN, a.cdgo_acto_orgen)
                 and a.id_acto_tpo = nvl(P_ID_ACTO_TPO, a.id_acto_tpo)
                 and ((trunc(a.fcha) between p_fcha_incial and p_fcha_fnal) or
                     (trunc(a.fcha) >= p_fcha_incial and
                     p_fcha_fnal is null) or
                     (trunc(a.fcha) <= p_fcha_fnal and p_fcha_fnal is null) or
                     (p_fcha_incial is null and p_fcha_fnal is null))
                 and not exists
               (select 1
                        from nt_d_lote_detalle n
                       where n.id_acto = a.id_acto)
                 and a.nmro_acto = NVL(P_NMRO_ACTO, a.nmro_acto));

    exception
      when no_data_found then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ', actos no encontrados con los parámetros ingresados';
    ...
    --verificamos que el json tenga registros
    SELECT COUNT(*)
      INTO o_nmro_actos
      FROM dual,
           json_table(v_json_actos, '$[*]' COLUMNS id_acto PATH '$.ID_ACTO');
    ...
    if (o_nmro_actos > 0) then
      ...
      begin
        insert into nt_g_certificados_json
          (cdgo_clnte, cdgo_rgstro_tpo, json_crtfcdos)
        values
          (p_cdgo_clnte, p_cdgo_rgstro_tpo, v_json_actos)
        RETURNING id_nt_crtfcdo_json into o_id_json;
      exception
        when others then
          o_cdgo_rspsta  := 13;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ', no fue posible generar el json de los actos.';
      ...
      o_cdgo_rspsta  := 15;
      o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                        ' error al intentar construir el json de los actos.';
  ...
  exception
    when v_error then
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            6);
      return;
  end prc_gn_json_actos;

  procedure prc_gn_json_actos_crtfcdo(p_cdgo_clnte         in number,
                                      p_id_usrio           in number,
                                      p_indcdor_ntfcdo     in varchar2,
                                      p_indcdor_ntfcdo_web in varchar2,
  ...
  begin
    ...
    begin
      SELECT JSON_ARRAYAGG(JSON_OBJECT('ID_ACTO' VALUE id_acto) RETURNING CLOB)
        ...
        FROM (select a.id_acto
                from v_gn_g_actos a
               where a.ntfcdo_pag_web = P_INDCDOR_NTFCDO_WEB --a.indcdor_ntfcdo = :P2024001_INDCDOR_NTFCDO
                 and a.id_acto_tpo = nvl(P_ID_ACTO_TPO, a.id_acto_tpo)
                 and a.nmro_acto = nvl(P_NMRO_ACTO, a.nmro_acto)
                 and (to_char(a.fcha, 'YYYYMMDD') >=
                     to_char(to_date(P_FCHA_INCIAL, 'DD/MM/YYYY'),
                              'YYYYMMDD') OR P_FCHA_INCIAL IS NULL)
                 and (to_char(a.fcha, 'YYYYMMDD') <=
                     to_char(to_date(P_FCHA_FNAL, 'DD/MM/YYYY'), 'YYYYMMDD') OR
                     P_FCHA_FNAL IS NULL)
                 and (to_char(a.fcha_pblccion, 'YYYYMMDD') =
                     to_char(to_date(P_FCHA_PBLCCION, 'DD/MM/YYYY'),
                              'YYYYMMDD') OR P_FCHA_PBLCCION IS NULL)
               order by a.id_acto
              /*where a.indcdor_ntfcdo = P_INDCDOR_NTFCDO
                and a.ntfcdo_pag_web = P_INDCDOR_NTFCDO_WEB
                and a.id_acto_tpo = nvl(P_ID_ACTO_TPO, a.id_acto_tpo)
                and a.nmro_acto = nvl(P_NMRO_ACTO, a.nmro_acto)
                and (to_char(a.fcha, 'YYYYMMDD') >=
                    to_char(to_date(P_FCHA_INCIAL, 'DD/MM/YYYY'),
    ...
    if (v_nmro_actos > 0) then
      ...
      if (o_id_json is not null) then
        o_cdgo_rspsta  := 0;
        o_mnsje_rspsta := 'Se seleccionaron ' || v_nmro_actos || ' actos.';
      else
        o_cdgo_rspsta  := 14;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' actos no seleccionados.';
  ...
  exception
    when v_error then
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            6);
      return;
  end prc_gn_json_actos_crtfcdo;

  function fnc_ca_sldo_crtra_acto(p_id_acto in gn_g_actos.id_acto%type)
    return number as
    ...
    v_intres            number;
    v_ttal              number;
    v_sldo_ttal         number := 0;
    v_intres_ttal       number := 0;
  ...
  begin
    begin
      ...
      select sum(c.vlor_sldo_cptal + c.vlor_intres)
        into v_sldo_ttal
        from v_gf_g_cartera_x_vigencia c
        join v_gn_g_actos_sujeto_impuesto a
          on a.id_sjto_impsto = c.id_sjto_impsto
        join gn_g_actos_vigencia b
          on b.id_acto = a.id_acto
    ...
    return v_sldo_ttal;
  end fnc_ca_sldo_crtra_acto;

  procedure prc_gn_json_actos_lte(p_cdgo_clnte     in number,
    ...
    v_nl         NUMBER;
    v_nmbre_up   sg_d_configuraciones_log.nmbre_up%TYPE := 'pkg_nt_notificacion.prc_gn_json_actos_lte';
    v_error      exception;
    v_json_actos clob;
    ...
    begin
      SELECT JSON_ARRAYAGG(JSON_OBJECT('ID_ACTO' VALUE id_acto) RETURNING CLOB)
        ...
        FROM (select distinct b.id_acto
                ...
                join v_nt_g_notfccnes_gn_g_actos b
                  on a.id_acto = b.id_acto
               Where a.id_lte = p_nmro_lte
                 and c.ntfcdo_pag_web = 'N'
                 and ((p_indcdor_ntfcdo = 'S' and b.indcdor_ntfcdo = 'S') or
                     (p_indcdor_ntfcdo = 'N' and b.indcdor_ntfcdo = 'N') or
                     ((p_indcdor_ntfcdo = 'T' or p_indcdor_ntfcdo is null) and
                     b.indcdor_ntfcdo in ('S', 'N')))
                 and b.nmro_acto = nvl(p_nmro_acto, b.nmro_acto));
    exception
      when no_data_found then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ', No se encontraron actos con los parámetros ingresados';
