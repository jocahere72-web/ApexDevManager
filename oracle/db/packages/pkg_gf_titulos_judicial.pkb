  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GF_TITULOS_JUDICIAL" as
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'v_xml: ' || apex_util.get_session_state('P2_XML'),
                          6);
  ...
  procedure prc_rg_anlisis_devolucion_ttlo(p_cdgo_clnte        in number,
    ...
    v_type_rspsta            varchar2(6);
    v_error_msg              number;
    v_exste_crtra            varchar2(6);
    v_exste_mdda_ctlar       varchar2(6);
    v_idntfccion_dmnddo      gf_g_titulos_judicial.idntfccion_dmnddo%type;
    v_nmro_ttlo_jdcial       gf_g_titulos_judicial.nmro_ttlo_jdcial%type;
    ...
    begin
      select a.id_fljo_trea
       ...
       where b.id_instncia_fljo = p_id_instncia_fljo
         and upper(a.nmbre_trea) = upper('Finalización');
    exception
      when others then
        o_cdgo_rspsta  := 3;
        o_mnsje_rspsta := 'Error - ' || o_cdgo_rspsta ||
                          ' No se encontró el id_fljo_trea_orgen. ' ||
                          sqlcode || ' ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    if o_id_acto is not null then
      ...
      if p_cdgo_ttlo_jdcial_estdo = 'CYD' then
        for c_ttlos_cyd in (select id_ttlo_jdcial, cdgo_ttlo_jdcial_estdo
          ...
          elsif c_ttlos_cyd.cdgo_ttlo_jdcial_estdo = 'CMP' then
                begin
                  update gf_g_titulos_judicial
                     set cdgo_ttlo_jdcial_estdo = 'CMP'
                   where id_ttlo_jdcial = c_ttlos_cyd.id_ttlo_jdcial;
          ...
          elsif c_ttlos_cyd.cdgo_ttlo_jdcial_estdo = 'CYC' then
            begin
              update gf_g_titulos_judicial
                 set cdgo_ttlo_jdcial_estdo = 'CYC'
               where id_ttlo_jdcial = c_ttlos_cyd.id_ttlo_jdcial;
  ...
  begin
    ...
    v_slct_vngcias := ' select c.id_sjto_impsto,
                  ...
                  d.id_prdo,
                                    ...
                                    0 as vlor_intres
                from gf_g_titulos_jdcial_impsto c
                join gf_g_titulos_jdcial_vgncia d on c.id_instncia_fljo = d.id_instncia_fljo
                where c.id_instncia_fljo = ' ||
                      v_id_instncia_fljo;
  ...
  procedure prc_rg_titulo_vigencias(p_cdgo_clnte    in number,
                                    ...
                                    p_json_compnsar in clob default null,
                                    p_id_usrio      in number,
        ...
        v_vlor_saf              number := 0;
        v_tpo_dcmnto            varchar2(3);

    begin

        pkg_gf_titulos_judicial.prc_rg_muerto(p_n_001 => 16,
                                              p_v_001 => 'p_json_ttlos',
                                              p_c_001 => p_json_ttlos);

        pkg_gf_titulos_judicial.prc_rg_muerto(p_n_001 => 16,
                                              p_v_001 => 'p_json_crtra',
                                              p_c_001 => p_json_crtra);

        o_cdgo_rspsta := 0;
       -- o_mnsje_rspsta := 'Registro de caretra exitoso.';
        ...
        if p_json_crtra is null then
            o_cdgo_rspsta := -20001;
            o_mnsje_rspsta := o_cdgo_rspsta || ', Error en la validación del json de la cartera. '||sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
            raise v_error;
        end if;
        ...
        exception when no_data_found then
            o_cdgo_rspsta := -20002;
            o_mnsje_rspsta := o_cdgo_rspsta || ', Error en la validación del json de los títulos. '||sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
            raise v_error;
        end;


        --Se busca el valor de los titulos a consignar
        begin
            select sum(vlor)
                into v_vlor_ttal_ttlo
            from gf_g_titulos_judicial
            where id_instncia_fljo = v_id_instncia_fljo
                and cdgo_ttlo_jdcial_estdo in ('ACN','CMP','CYC');
        exception when no_data_found then
            o_cdgo_rspsta  := -20003;
            o_mnsje_rspsta := o_cdgo_rspsta || ' - Problemas al consultar el valor del titulo ' || sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
            raise v_error;
        when others then
            o_cdgo_rspsta  := -20004;
        ...
        begin
            select id_ttlo_jdcial_impsto into v_id_ttlo_jdcial_impsto
            from gf_g_titulos_jdcial_impsto
            where id_instncia_fljo = v_id_instncia_fljo;
        exception
            when no_data_found then
                v_id_ttlo_jdcial_impsto := 0;
            when others then
                o_cdgo_rspsta  := -20014;
                o_mnsje_rspsta := o_cdgo_rspsta || ' - Problemas al consultar el id_ttlo_jdcial_impsto ' || sqlerrm;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
                raise v_error;
        end;

        if v_id_ttlo_jdcial_impsto = 0 then
            for impstos in (select id_impsto,
                               id_impsto_sbmpsto,
                               id_sjto_impsto
                         from json_table(p_json_crtra,
                                 ...
                                 --                       id_sjto_rspnsble number path '$.ID_SJTO_RSPNSBLE',
                                                        id_sjto_impsto number path '$.ID_SJTO_IMPSTO'
                                                        ))
                         group by id_impsto,
                                  id_impsto_sbmpsto,
                                  id_sjto_impsto
        ...
        loop
            begin
                select id_sjto_rspnsble
                    into v_id_sjto_rspnsble
                from si_i_sujetos_responsable a
                left join gf_g_titulos_judicial b on a.idntfccion = b.idntfccion_dmnddo
                where a.id_sjto_impsto = impstos.id_sjto_impsto
                fetch first row only;
            exception
                when others then
                    o_cdgo_rspsta  := -20014;
                    o_mnsje_rspsta := o_cdgo_rspsta || ' - Problemas al consultar la información del responsable ' || sqlerrm;
                    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
                    continue;
            end;
            ...
            begin
                ...
                values
                    (impstos.id_impsto,
                     impstos.id_impsto_sbmpsto,
                     impstos.id_sjto_impsto,
                     v_id_sjto_rspnsble,
                     v_id_instncia_fljo)
                     returning id_ttlo_jdcial_impsto into v_id_ttlo_jdcial_impsto;
            exception when others then
                o_cdgo_rspsta  := -20006;
                o_mnsje_rspsta := o_cdgo_rspsta || ' - Error al insertar impuesto de la cartera del título ' || sqlerrm;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
                continue;
            end;

            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'v_id_ttlo_jdcial_impsto: ' || v_id_ttlo_jdcial_impsto, 6);

        end loop;
        end if;

        for c_crtra_jdcial in (select a.id_impsto,
                                      a.id_impsto_sbmpsto,
                                      a.id_sjto_impsto,
                                      a.vgncia,
                                      a.prdo,
                                 ...
                                 from json_table(p_json_crtra,
                                                        ...
                                                        columns(id_impsto number path '$.ID_IMPSTO',
                                                                ...
                                                                id_sjto_impsto number path '$.ID_SJTO_IMPSTO',
                                                                vlor_cptal number path '$.VLOR_SLDO_CPTAL',
                                                                vlor_intres number path '$.VLOR_SLDO_INTRES',
                                                                vlor_dscto number path '$.VLOR_DSCNTO',
                                                                vlor_ttal number path '$.VLOR_TTAL',
                                                                vlor_ttal_aplcar number path '$.VLOR_TTAL_APLCAR',
                                                                vlor_ttal_aplccion number path '$.VLOR_TTAL_APLCCION',
                                                                vlor_dscnto_aplccion number path '$.VLOR_DSCNTO_APLCCION',
                                                                tpo_dcmnto varchar2 path '$.TPO_DCMNTO',
                                                                ...
                                                                cdgo_mvmnto_tpo  varchar2 path '$.CDGO_MVMNTO_TPO'))a
                                )
        loop
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Entrando al primer for ', 6);
            if c_crtra_jdcial.vlor_ttal_aplcar > 0 then
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Encontró saldo para aplicar ', 6);
                begin
                    insert into gf_g_titulos_jdcial_vgncia
                        (
                         vgncia,
                ...
                exception when others then
                    o_cdgo_rspsta  := -20007;
                    o_mnsje_rspsta := o_cdgo_rspsta || ' - problemas insertar las vigencias' || sqlerrm;
                    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
                    continue;
                end;
            else
                o_cdgo_rspsta  := -20008;
                o_mnsje_rspsta := o_cdgo_rspsta || ' - No hay cartera seleccionada';
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
                continue;
            end if;
        end loop;

        --AC 2026-02-18 registro de vigencias a compensar
        for compensar in (select a.id_impsto,
                                      a.id_impsto_sbmpsto,
                                      a.id_sjto_impsto,
                                      a.vgncia,
                                      a.prdo,
                                      ...
                                      a.id_orgen,
                                      a.cdgo_mvmnto_tpo
                                 from json_table(p_json_compnsar,
                                                        '$[*]'
                                                        columns(id_impsto number path '$.ID_IMPSTO.v',
                                                                id_impsto_sbmpsto number path '$.ID_IMPSTO_SBMPSTO.v',
                                                                id_sjto_rspnsble number path '$.ID_SJTO_RSPNSBLE',
                                                                vgncia varchar2 path '$.VGNCIA',
                                                                prdo varchar2 path '$.PRDO',
                                                                id_prdo number path '$.ID_PRDO',
        ...
        loop
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Entrando al segundo for ', 6);
            if compensar.vlor_ttal_aplcar > 0 then
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Encontró saldo para aplicar ', 6);
                begin
                    insert into gf_g_titulos_jdcial_vgncia
                        (
                    ...
                    values
                        (
                         ...
                         compensar.prdo,
                         compensar.id_prdo,
                         compensar.vlor_cptal,
                         compensar.vlor_intres,
                         compensar.vlor_dscto,
                         compensar.vlor_ttal,
                         v_vlor_ttal_ttlo,
                         compensar.vlor_ttal_aplcar,
                         compensar.vlor_ttal_aplccion,
                         compensar.vlor_dscnto_aplccion,
                         compensar.tpo_dcmnto,
                         compensar.cdgo_mvmnto_orgn,
                         compensar.id_orgen,
                         p_fcha_pryccion,
                         v_id_instncia_fljo,
                         compensar.id_impsto,
                         compensar.id_impsto_sbmpsto,
                         compensar.id_sjto_impsto,
                         'AC');
                exception when others then
                    o_cdgo_rspsta  := -20017;
                    o_mnsje_rspsta := o_cdgo_rspsta || ' - problemas insertar las vigencias' || sqlerrm;
        ...
        begin
            select
                (select nvl(sum(vlor_ttal_aplcar),0)
                 from json_table(p_json_crtra, '$[*]'
                       columns(vlor_ttal_aplcar number path '$.VLOR_TTAL_APLCAR'))
           ...
           /* select sum(vlor_ttal_aplcar)
            ...
            from json_table(p_json_crtra,
                                   '$[*]'
                                   columns(vlor_ttal_aplcar varchar2 path '$.VLOR_TTAL_APLCAR'))
            where vlor_ttal_aplcar > 0;*/
        exception
          when others then
            v_crtra_slccnda := 0;
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Valor seleccion: ' || v_crtra_slccnda, 6);
        --validamos si existe registro de saldo para vigencias futuras
        begin
            select nvl(sum(f.vlor_sldo_fvor),0)
                into v_vlor_saf
        ...
        if v_crtra_slccnda <> v_vlor_ttal_ttlo then
            o_cdgo_rspsta  := -20009;
            o_mnsje_rspsta := '' || o_cdgo_rspsta || ' El valor a consignar no es igual al valor del titulo.';
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
            raise v_error;
        end if;
    ...
    exception
        ...
        when others then
            rollback;
            o_cdgo_rspsta  := -20099;
            o_mnsje_rspsta := o_cdgo_rspsta || ' Error inesperado al procesar la cartera de los títulos. '||sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
            return;
  ...
  procedure prc_rg_titulo_vigencias_temp(p_cdgo_clnte    in number,
    ...
    v_nl number;
    v_error    exception;
    v_nmbre_up varchar2(100) := 'pkg_gf_titulos_judicial.prc_rg_titulo_vigencias_temp';
    v_id_ttlo_jdcial   number;
    v_id_instncia_fljo number;
    v_vlor_ttal_ttlo number := 0;
  begin
    ...
    if p_json_crtra is null then
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'JSON VACIO', 6);
    else
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'JSON LLENO', 6);
    end if;
    ...
    for c_ttlo_jdcial in (select id_ttlo_jdcial, id_instncia_fljo
                            from json_table(p_json_ttlos,
                                            ...
                                            columns(id_ttlo_jdcial varchar2 path
                                                    ...
                                                    '$.TTLO_JDCIAL_ESTDO'))
                           where TTLO_JDCIAL_ESTDO in ('ACN','CMP','CYC'))
    loop
  ...
  v_tabla := '  <div class="container" style="width:100%; margin-left:60px; align-items:center;" >
                <table border="1" style="border-collapse:collapse; width:90%; align:center">'||
               '<thead>' || '<tr>' ||
               '<th style="text-align: center; border:1px solid black; width:40%">' ||
               'Ref/ Nit' || '</th>' ||
               '<th style="text-align: center; border:1px solid black; width:10%">' ||
               'Vigencias' || '</th>' ||
    ...
    begin
      ...
      for cartera in (select c.idntfccion_sjto,
                             case
                                when a.tpo_dcmnto = 'DNO' then
                                    listagg(a.vgncia, ',') within group(order by a.vgncia)
                                when a.tpo_dcmnto = 'DAB' then
                                    listagg(a.vgncia, ',') within group(order by a.vgncia)
                             end vgncias,
                             to_char(a.vlor_ttal_aplcar,'FM$999G999G999G999G999G999G990') vlor_ttal_aplcar,
                             case
                                when a.tpo_dcmnto = 'DNO' then
                                    'Pago Total'
                                when a.tpo_dcmnto = 'DAB' then
                        ...
                        join gf_g_titulos_jdcial_impsto n on n.id_instncia_fljo = a.id_instncia_fljo
                        join v_si_i_sujetos_impuesto c on c.id_sjto_impsto = a.id_sjto_impsto
                        where a.id_instncia_fljo = p_id_instncia_fljo
                        group by c.idntfccion_sjto, a.tpo_dcmnto, a.vlor_ttal_aplcar
                        order by c.idntfccion_sjto, a.tpo_dcmnto, a.vlor_ttal_aplcar)
      loop
    ...
    v_tabla := v_tabla || '</tbody></table></div>';
  ...
  begin
        v_tabla := '<!DOCTYPE html>' ||
                   ...
                   '<thead>' ||
                   '<tr>' ||
                   '<th style="text-align: center; border: 1px solid black; width:30%"><b>Impuesto</b></th>' ||
                   '<th style="text-align: center; border: 1px solid black;  width:30%"><b>Sub Impuesto</b></th>' ||
                   '<th style="text-align: center; border: 1px solid black;  width:20%"><b>Valor Fraccion</b></th>' ||
                   '<th style="text-align: center; border: 1px solid black;  width:20%"><b>Observaciones</b></th>' ||
                   '</tr>' ||
                   '</thead>' ||
                   '<tbody>';

        for datos in (  select nvl(c.nmbre_impsto,'N/A')nmbre_impsto,
                               nvl(c.nmbre_impsto_sbmpsto,'N/A')nmbre_impsto_sbmpsto,
                               ltrim(rtrim(to_char(a.vlor, 'FM$999G999G999G999G999G999G990'))) vlor,
                               a.obsrvcn
                        from gf_g_titulos_jdcl_frccnmnto a
                        join gf_g_titulos_judicial b on b.id_instncia_fljo = a.id_instncia_fljo
                        left join v_df_i_impuestos_subimpuesto c on c.id_impsto = a.id_impsto and
                                                         c.id_impsto_sbmpsto = a.id_impsto_sbmpsto
                        where b.id_instncia_fljo = p_id_instncia_fljo
                        order by a.orden)
        loop
            v_tabla := v_tabla ||
                       '<tr>' ||
                       '<td style="text-align: center; border:1px solid black">' || datos.nmbre_impsto || '</td>' ||
                       '<td style="text-align: center; border:1px solid black">' || datos.nmbre_impsto_sbmpsto || '</td>' ||
                       '<td style="text-align: center; border:1px solid black">' || datos.vlor || '</td>' ||
                       '<td style="text-align: left; border:1px solid black">' || datos.obsrvcn || '</td>' ||
                       '</tr>';
        end loop;
  ...
  begin
    ...
    --Registra los docuemnto de cobro asociados al título judicial
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Entrando a ' || v_nmbre_up || ' con p_cdgo_clnte: ' || p_cdgo_clnte || ', p_id_ttlo_jdcial: ' || p_id_ttlo_jdcial || ', p_id_usrio: ' || p_id_usrio, 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'p_json_ttlos: ' || p_json_ttlos, 6);

    begin
        select id_instncia_fljo
            into v_id_instncia_fljo
        from gf_g_titulos_judicial where id_ttlo_jdcial = p_id_ttlo_jdcial;
    exception when no_data_found then
        o_cdgo_rspsta  := 89;
        o_mnsje_rspsta := o_cdgo_rspsta || ' - ' || 'Error validando los datos del título.';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta || ' , ' || sqlerrm, 6);
        raise v_error;
    end;

    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'v_id_instncia_fljo: '||v_id_instncia_fljo , 6);

    begin
      for c_ttlos_jdcial in (select id_ttlo_jdcial, ttlo_jdcial_estdo
                               from json_table(p_json_ttlos,
                                               '$[*]'
                                               columns(id_ttlo_jdcial number path '$.ID_TTLO_JDCIAL',
                                                       ttlo_jdcial_estdo varchar2 path '$.TTLO_JDCIAL_ESTDO'))
                              where ttlo_jdcial_estdo in ('ACN','CNS','CMP', 'CYC'))
      loop
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'despues de borrar docuemntos' , 6);

      --Se recorre la cartera para generar los documentos
      for c_cartera in (select a.id_impsto,
                               a.id_impsto_sbmpsto,
                               a.id_sjto_impsto,
                               sum(a.vlor_ttal_aplcar) vlor_ttla_aplcar,
                               a.tpo_dcmnto,
                               a.cdgo_mvmnto_orgn,
                               trunc(a.fcha_pryccion) fcha_pryccion,
                         ...
                         group by a.id_impsto,
                                  ...
                                  a.tpo_dcmnto,
                                  a.cdgo_mvmnto_orgn,
                                  trunc(a.fcha_pryccion)) loop

        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Antes de generar docuemnto' , 6);
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'v_id_dcmnto: ' ||v_id_dcmnto, 6);
        ...
        if v_id_dcmnto is null then
          ...
          for c_ttlos_jdcial in (select max(id_ttlo_jdcial) id_ttlo_jdcial,
                                        ...
                                        max(id_instncia_fljo) id_instncia_fljo
                                   from json_table(p_json_ttlos,
                                                   '$[*]'
                                                   columns(id_ttlo_jdcial varchar2 path '$.ID_TTLO_JDCIAL',
                                                           ttlo_jdcial_estdo varchar2 path '$.TTLO_JDCIAL_ESTDO',
                                                           id_instncia_fljo varchar2 path '$.ID_INSTNCIA_FLJP'))
                                  where ttlo_jdcial_estdo in ('ACN', 'CNS','CMP', 'CYC')) loop

            if c_ttlos_jdcial.ttlo_jdcial_estdo in ('ACN', 'CNS', 'CMP', 'CYC') then
              begin
                --Se Registran los documentos generados
                insert into gf_g_ttls_jdcl_impsto_dcmnt
                  ( id_ttlo_jdcial, id_dcmnto, id_instncia_fljo)
                values
                  (c_ttlos_jdcial.id_ttlo_jdcial,
                   v_id_dcmnto,
                   v_id_instncia_fljo);
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Saliendo de ' || v_nmbre_up, 6);
  ...
  begin
    ...
    o_mnsje_rspsta := 'Parámetros: ' || p_id_ttlo_jdcial || ',' ||
  ...
  begin
    ...
    o_cdgo_rspsta  := 0;
    o_mnsje_rspsta := 'Re - Asignación Exitosa.';
    ...
    exception
      when others then
        o_cdgo_rspsta  := 11;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' - Título judicial ' || v_nmro_ttlo_jdcial ||
    ...
    if ((v_id_ttlo_jdcial is not null) and
      ...
      exception
        when others then
          o_cdgo_rspsta  := 2;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ' - No fue posible re-asignar el título ' ||
      ...
      exception
        when others then
          ...
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ...
                            v_id_usurio ||
                            ' en wf_g_instancias_transicion para el título ' ||
                            v_nmro_ttlo_jdcial || '. ' || sqlerrm;
      ...
      exception
        when others then
          ...
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ' - No fue posible actualizar el usuario: ' ||
                            v_id_usurio ||
                            '  wf_g_instancias_participante, para el título ' ||
                            v_nmro_ttlo_jdcial || '. ' || sqlerrm;
      ...
      exception
        when others then
          ...
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ' - No fue posible actualizar el usuario: ' ||
                            v_id_usurio ||
                            ' wf_g_instancias_flujo para el título ' ||
                            v_nmro_ttlo_jdcial || '. ' || sqlerrm;
      ...
      exception
        when others then
          ...
          o_cdgo_rspsta  := 4;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ' - No fue posible registrar la re-asignación del título ' ||
                            v_nmro_ttlo_jdcial || '. ' || sqlerrm;
    ...
    o_cdgo_rspsta  := 0;
    o_mnsje_rspsta := 'Título No. ' || v_nmro_ttlo_jdcial ||
                      ' re-asignado con éxito';
...
procedure prc_rg_titulos_judiciales  (p_cdgo_clnte       in number,
    ...
    v_cdgo_bnco_cnsgnnte      varchar2(5);
    v_error                 exception;
    v_cdgo_bnco_rcddr         varchar2(5);
    v_exste                 number;
    ...
    for c_ttlos in (select nmro_ttlo,
                      ...
                      from json_table(p_json_ttlos,
                                      ...
                                      columns(nmro_ttlo varchar2 path
                                              ...
                                              nmbre_dmndo varchar2 path
                                              '$.NMBRE_DMNDO',
                                              cnsctvo_embrgo number path
                                              '$.CNSCTVO_EMBRGO',
                                              id_embrgo_rslcion number path
                                              '$.ID_EMBRGO_RSLCION',
                                              nmro_ttlo_pdre varchar2 path
                                              '$.NMRO_TTLO_PDRE',
                                              id_bnco_cnsgnnte number path
                                              '$.ID_BNCO_CNSGNNTE',
                                              id_bnco_rcddr number path
                                              '$.ID_BNCO_RCDDR',
      ...
      if c_ttlos.nmro_ttlo is null then
        ...
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' Json de los títulos incompleto, verifique!!!';
        raise v_error;
        ...
        begin
          select a.cdgo_bnco
            into v_cdgo_bnco_cnsgnnte
            from df_c_bancos a
           where a.id_bnco = c_ttlos.id_bnco_cnsgnnte
             and a.cdgo_clnte = p_cdgo_clnte;
        exception
          when no_data_found then
            v_cdgo_bnco_cnsgnnte := null;
          when others then
            o_cdgo_rspsta  := 3;
            o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta || ' - ' ||
                              'Error al consultar el banco consignante';
        ...
        begin
          select a.cdgo_bnco
            into v_cdgo_bnco_rcddr
            from df_c_bancos a
           where a.id_bnco = c_ttlos.id_bnco_rcddr
             and a.cdgo_clnte = p_cdgo_clnte;
        exception
          when no_data_found then
            v_cdgo_bnco_rcddr := null;
          when others then
            o_cdgo_rspsta  := 4;
            o_mnsje_rspsta := o_cdgo_rspsta || ' - ' ||
        ...
        if (v_exste > 0) then
          o_cdgo_rspsta  := 100;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta || 'El título ' ||
                            c_ttlos.nmro_ttlo ||
        ...
        if o_id_ttlo_jdcial is not null then
          pkg_gf_titulos_judicial.prc_rg_titulo_finaliza_traza(p_cdgo_clnte     => p_cdgo_clnte,
                                                               ...
                                                               p_cdgo_ttlo_nvo  => 'RGS',
                                                               p_obsrvcion      => 'Registro inicial de título judicial',
                                                               p_id_fljo_trea   => null,
                                                               o_cdgo_rspsta    => o_cdgo_rspsta,
                                                               o_mnsje_rspsta   => o_mnsje_rspsta);
          if o_cdgo_rspsta > 0 then
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Entrando a ' || v_nmbre_up || ', ' || systimestamp, 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'p_cdgo_clnte: ' || p_cdgo_clnte, 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'p_id_instncia_fljo: ' || p_id_instncia_fljo, 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'p_idntfccion_sjto: ' || p_idntfccion_sjto, 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'p_vlor_sldo_fvor: ' || p_vlor_sldo_fvor, 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'p_id_fncnrio: ' || p_id_fncnrio, 6);

    begin
        select id_fncnrio into v_id_fncnrio
        from v_sg_g_usuarios
        where id_usrio = p_id_fncnrio;
    exception
      when no_data_found then
        o_cdgo_rspsta  := 1111;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' - No se encontró la id_fncrio para usuario id: ' ||
                          p_id_fncnrio;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
        raise v_error;
    end;
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' - No se encontró la id_sjto_impsto para ' ||
                          p_idntfccion_sjto;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
        raise v_error;
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 2;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' - No se encontró el id_ttlo_jdcial para en esta instancia de flujo ' ||
                          p_id_instncia_fljo;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
        raise v_error;
  ...
  exception
    ...
    when others then
      rollback;
      o_cdgo_rspsta := -20099;
      o_mnsje_rspsta := o_cdgo_rspsta || ', Error inesperado al registrar el SAF. '||sqlerrm;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
      return;
  ...
  begin
    ...
    exception
      when others then
        o_cdgo_rspsta  := 01;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' Json de los titulos incompleto o vacío. Por favor verifique!!!. ' ||
                          sqlerrm;
        raise v_error;
    end;
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' No existe el título judicial. Por favor verifique!!!. ' ||
                          sqlerrm;
        raise v_error;
      ...
      when others then
        o_cdgo_rspsta  := 2;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' No es posible validar el título judicial. Por favor verifique!!!. ' ||
                          sqlerrm;
        raise v_error;
    end;
    ...
    exception
      when others then
        o_cdgo_rspsta  := 4;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' Al actualizar el título judicial. ' || sqlerrm;
        raise v_error;
    end;
  ...
  begin
    o_cdgo_rspsta  := 0;
    o_mnsje_rspsta := 'Proceso de reversión del título existoso.';
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 61;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' No se encontró información del título con id. ' ||
    ...
    if (v_cdgo_ttlo_jdcial_estdo in ('ASL', 'SDT', 'CDT', 'DVL')) then
      ...
      exception
        when others then
          o_cdgo_rspsta  := 500;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ' No se encontró el id_ttlo_jdcial_slctud. ' ||
                            sqlerrm;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
        ...
        if o_cdgo_rspsta = 0 then
          ...
          exception
            when others then
              ...
              o_cdgo_rspsta  := 509;
              o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                                ' No se puede reversar la devolución del título. ' ||
                                v_nmro_ttlo_jdcial || '. ' || sqlerrm;
              pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                    null,
                                    v_nmbre_up,
          ...
          o_mnsje_rspsta := o_mnsje_rspsta ||
                            ' Error al reversar la devolución del título. ' ||
                            v_nmro_ttlo_jdcial || '. ' || sqlerrm;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
      ...
      for frccnes in (select a.id_ttlo_jdcial_dcmnto_asccn,
        ...
        exception
          when others then
            ...
            o_cdgo_rspsta  := 86;
            o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                              ' No se puede reversar el fraccionamiento del título. ' ||
                              v_nmro_ttlo_jdcial || '. ' || sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
    ...
    if (v_cdgo_ttlo_jdcial_estdo in ('ACN', 'CNS', 'ATJ')) then
      ...
      if (v_rcdos > 0) then
        o_cdgo_rspsta  := 128;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' No se puede reversar el título. Ya existen recaudos aplicados. ' ||
                          sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
      ...
      for dcmntos in (select a.id_ttlo_jdcial_dcmnto_asccn,
        ...
        exception
          when others then
            ...
            o_cdgo_rspsta  := 300;
            o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                              ' Error en la reversion del título. ' ||
                              v_nmro_ttlo_jdcial || ' ' || sqlerrm || '. ' ||
                              sqlcode;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
    ...
    exception
      when others then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' No es posible consultar el id_instncia_fljo para el título ' ||
                          v_nmro_ttlo_jdcial || '. ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    if o_cdgo_rspsta = 0 then
      ...
      exception
        when others then
          o_cdgo_rspsta  := 148;
          o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                            ' Actualizando el estado del título. ' ||
                            sqlerrm;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
      ...
      o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                        ' Al reversar el título judicial. ' || sqlerrm;
  ...
  begin
    o_cdgo_rspsta  := 0;
    o_mnsje_rspsta := 'Flujo reversado con éxito.';
  ...
  begin
    o_cdgo_rspsta  := 0;
    o_mnsje_rspsta := 'Log registrado con éxito.';
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Parámetros de entrada: ' || p_cdgo_clnte || ',' ||
    ...
    exception
      when others then
        o_cdgo_rspsta  := 101;
        o_mnsje_rspsta := 'Error No. ' || o_cdgo_rspsta ||
                          ' Falló el registro del log. ' || sqlerrm;
  ...
  function fnc_gn_titulos_devolucion(p_id_instncia_fljo in number)
    return varchar2 is
  ...
  begin
    ...
    return v_titulos;
  end fnc_gn_titulos_devolucion;

function fnc_co_titulos_dvlcion_tabla(p_id_instncia_fljo      in number)
    return clob as
  ...
  begin
    ...
    v_tabla := ' <div class="container" style="width:100%; margin-left:60px; align-items:center;" >
               ...
               '<thead>' || '<tr>' ||
                '<th style="text-align: center; border:1px solid black">' ||'Número Del Título' || '</th>' ||
                '<th style="text-align: center; border:1px solid black">' ||'Valor Del Título' || '</th>' ||
                '</tr>' || '</thead>' ||
                '<tr>';

    for c_titulo in (select a.nmro_ttlo_jdcial,
                           LTRIM(RTRIM(TO_CHAR(a.vlor, 'FM$999G999G999G999G999G999G990'))) AS vlor
						  from gf_g_titulos_judicial a
						  left join v_mc_g_embargos_resolucion b
							on b.cnsctvo_embrgo = a.cnsctvo_embrgo
						  left join mc_g_embrgos_crt_prc_jrd c
						  ...
						  left join mc_g_embargos_cartera e
							on e.id_embrgos_crtra = b.id_embrgos_crtra
						 where a.id_instncia_fljo = p_id_instncia_fljo
						   and a.cdgo_ttlo_jdcial_estdo in ('ASL','SDT','FRC','CNS')
						 group by a.nmro_ttlo_jdcial, a.vlor   ) loop

      v_tabla := v_tabla ||
                  '<td style="text-align: center; border:1px solid black">' || c_titulo.nmro_ttlo_jdcial || '</td>' ||
                  '<td style="text-align: center; border:1px solid black">' ||c_titulo.vlor ||'</td>' ||
                  '</tr>';

    end loop;

    v_tabla := v_tabla || '</table><center>';

    return v_tabla;

end fnc_co_titulos_dvlcion_tabla;

function fnc_frmto_idntfccion_sjto(p_idntfccion_sjto in varchar2) return varchar2 as
  ...
  v_mnzna   varchar2(4);
  v_trrno   varchar2(4);
  v_cndcion varchar2(6);
  v_edfcio  varchar2(2);
  v_pso     varchar2(2);
  v_undad   varchar2(4);

  v_rfrncia_vsual varchar2(40);

begin

  -- eud: 22/08/2014
  -- objetivo: convertir el valor de la referencia catastral a un formato de visualizacion nuevo

  --  sitpr001('entrando a la funcion sitfn211', p_archvo);

  if p_idntfccion_sjto is not null then

    v_zna     := substr(p_idntfccion_sjto, 1, 2);
    v_sctor   := substr(p_idntfccion_sjto, 3, 2);
    v_cmna    := substr(p_idntfccion_sjto, 5, 2);
    v_brrio   := substr(p_idntfccion_sjto, 7, 2);
    v_mnzna   := substr(p_idntfccion_sjto, 9, 4);
    v_trrno   := substr(p_idntfccion_sjto, 13, 4);
    v_cndcion := substr(p_idntfccion_sjto, 17, 6);
    v_edfcio  := substr(p_idntfccion_sjto, 18, 2);
    v_pso     := substr(p_idntfccion_sjto, 20, 2);
    v_undad   := substr(p_idntfccion_sjto, 22, 4);

    v_rfrncia_vsual := v_zna || '-' || v_sctor || '-' || v_cmna || '-' ||
                       v_brrio || '-' || v_mnzna || '-' || v_trrno || '-' ||
                       v_cndcion || '-' || v_edfcio || '-' || v_pso || '-' ||
                       v_undad;

  else
    v_rfrncia_vsual := 'XX-XX-XX-XX-XXXX-XXXX-X-XX-XX-XXXX';
  end if;

  /*  sitpr001('saliendo de la funcion sitfn211_nva, referencia : ' ||
  v_rfrncia_vsual,
  p_archvo);*/

  return(v_rfrncia_vsual);

end fnc_frmto_idntfccion_sjto;

procedure prc_rg_titulos_judicial_fraccion(p_cdgo_clnte             in number,
                                           p_json_crtra             in clob,
                                           p_id_ttlo_jdcial 		in number,
                                           p_orden					in number,
                                           p_obsrvcn           	    in varchar2,
                                           p_indcdor_frccn_rgstrda  in varchar2 default 'N',
                                           p_id_ttlo_jdcial_rgstrdo in number default null,
                                           p_tpo_accion 			in varchar2,
                                           o_cdgo_rspsta            out number,
                                           o_mnsje_rspsta           out varchar2) as

    v_nl             number;
    v_nmbre_up       varchar2(100) := 'pkg_gf_titulos_judicial.prc_rg_titulos_judicial_fraccion';
    v_error          exception;
    ...
    v_vlor              number;
    v_id_ttlo_jdcial_frccn number;
    v_id_instncia_fljo     number;

    begin

        o_cdgo_rspsta := 0;
        o_mnsje_rspsta := 'Porceso exitoso.';

        v_nl := pkg_sg_log.fnc_ca_nivel_log(p_cdgo_clnte, null, v_nmbre_up);

        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Entrando a la up ' || v_nmbre_up || ', en ' || systimestamp, 6);
        ...
        begin
            ...
            from json_table(p_json_crtra,
                                ...
                                columns(id_impsto number path '$.ID_IMPSTO',
                                        id_impsto_sbmpsto number path '$.ID_IMPSTO_SBMPSTO',
                                        id_sjto_impsto number path '$.ID_SJTO_IMPSTO',
                                        idntfccion_sjto varchar2 path '$.IDNTFCCION_SJTO',
        ...
        exception when others then
            o_cdgo_rspsta := -20001;
            o_mnsje_rspsta := o_cdgo_rspsta || ', Error en el json de entrada. ' || sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
            raise v_error;
        end;
        ...
        exception
            when others then
                o_cdgo_rspsta := -20011;
                o_mnsje_rspsta := o_cdgo_rspsta || ', No se encontró el id del flujo instanciado. ' || sqlerrm;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
                raise v_error;
        end;

        begin
            insert into gf_g_titulos_jdcl_frccnmnto (ID_TTLO_JDCIAL,
                                                     ID_IMPSTO,
                                                     ID_IMPSTO_SBMPSTO,
                                                     ORDEN,
                                                     VLOR,
                                                     OBSRVCN,
                                                     INDCDOR_FRCCN_RGSTRDA,
                                                     ID_TTLO_JDCIAL_RGSTRDO,
                                                     ID_INSTNCIA_FLJO,
                                                     ID_SJTO_IMPSTO,
                                                     IDNTFCCION,
                                                     TPO_ACCION)
            values(p_id_ttlo_jdcial,
                   v_id_impsto,
                   v_id_impsto_sbmpsto,
                   p_orden,
                   v_vlor,
                   p_obsrvcn,
                   p_indcdor_frccn_rgstrda,
                   p_id_ttlo_jdcial_rgstrdo,
                   v_id_instncia_fljo,
                   v_id_sjto_impsto,
                   v_idntfccion_sjto,
                   p_tpo_accion)
                   returning id_ttlo_jdcial_frccn into v_id_ttlo_jdcial_frccn;

        exception when others then
            o_cdgo_rspsta := -20002;
            o_mnsje_rspsta := o_cdgo_rspsta || ', Erroe al insertar en gf_g_titulos_jdcl_frccnmnto. ' || sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
            raise v_error;
        end;

        for fracciones in (select  vgncia,
                                   id_prdo,
                                   vlor_ttal_aplcar
                            from json_table(p_json_crtra,
                                                '$[*]'
                            ...
                            order by vgncia, id_prdo)
        loop
            begin
                insert into gf_g_titulos_jdcl_frccnmnto_dtlle(id_ttlo_jdcial_frccn, vgncia, id_prdo, vlor)
                values(v_id_ttlo_jdcial_frccn, fracciones.vgncia, fracciones.id_prdo, fracciones.vlor_ttal_aplcar);
            exception when others then
                o_cdgo_rspsta := -20003;
                o_mnsje_rspsta := o_cdgo_rspsta || ', Erroe al insertar en gf_g_titulos_jdcl_frccnmnto_dtlle. ' || sqlerrm;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
                raise v_error;
            end;
        end loop;
    ...
    exception when v_error then
        rollback;
        return;
    end prc_rg_titulos_judicial_fraccion;

    procedure prc_rg_titulos_judicial_frccn_dvlcion(p_cdgo_clnte             in number,
                                                    p_id_ttlo_jdcial 		 in number,
                                                    p_orden					 in number,
                                                    p_obsrvcn           	 in varchar2,
                                                    p_indcdor_frccn_rgstrda  in varchar2 default 'N',
    ...
    v_nl             number;
    v_nmbre_up       varchar2(100) := 'pkg_gf_titulos_judicial.prc_rg_titulos_judicial_frccn_dvlcion';
    v_error          exception;
    ...
    v_vlor              number;
    v_id_ttlo_jdcial_frccn number;
    v_id_frccion        gf_g_titulos_jdcl_frccnmnto.id_ttlo_jdcial_frccn%type;
    ...
    begin
        ...
        begin
            select id_ttlo_jdcial_frccn
                into v_id_frccion
            from gf_g_titulos_jdcl_frccnmnto
            where id_ttlo_jdcial = p_id_ttlo_jdcial and tpo_accion = 'D';
        exception when no_data_found then
            v_id_frccion := 0;
        end;

        if v_id_frccion > 0 then
            o_cdgo_rspsta := -20003;
            o_mnsje_rspsta := o_cdgo_rspsta || ', Ya existe una fracción de devolución registrada para este título. Verifique!!';
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
            raise v_error;
        end if;
        ...
        begin
            insert into gf_g_titulos_jdcl_frccnmnto_dtlle (ID_TTLO_JDCIAL_FRCCN,
                                                           VLOR)
            values(v_id_ttlo_jdcial_frccn, p_vlor_frccion);
        exception
            when others then
                o_cdgo_rspsta := -20002;
                o_mnsje_rspsta := o_cdgo_rspsta || ', Error al insertar en gf_g_titulos_jdcl_frccnmnto_dtlle. ' || sqlerrm;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 6);
                raise v_error;
        end;
    ...
    exception when v_error then
        rollback;
        return;
    end prc_rg_titulos_judicial_frccn_dvlcion;

    procedure prc_rg_muerto(p_n_001 in number default null,
                        p_n_002 in number default null,
                        p_v_001 in varchar2 default null,
                        p_v_002 in varchar2 default null,
                        p_c_001 in clob default null,
                        p_b_001 in blob default null,
                        p_d_001 in date default null) as

    pragma AUTONOMOUS_TRANSACTION;

    begin
        insert into muerto(n_001, n_002, v_001, v_002, c_001, b_001, d_001, t_001)
        values(p_n_001, p_n_002, p_v_001, p_v_002, p_c_001, p_b_001, p_d_001, systimestamp);
        commit;
    end prc_rg_muerto;

     function fnc_co_titulos_tabla(p_id_instncia_fljo in number)
    return clob as
        v_tabla clob;
    begin
        ...
        v_tabla := ' <div class="container" style=width:100%; margin-left:60px; align-items:center; >
                   ...
                   'Número Del Título' || '</th>' ||
                   '<th style="text-align: center; border:1px solid black">' ||
                   'Valor Del Título' || '</th>' || '</tr>' || '</thead>' ||
                   '<tr>';
        ...
        for c_titulo in (select a.nmro_ttlo_jdcial,
                        ...
                        from gf_g_titulos_judicial           a
                        left join v_mc_g_embargos_resolucion b on b.cnsctvo_embrgo = a.cnsctvo_embrgo
                        left join mc_g_embrgos_crt_prc_jrd   c on c.id_embrgos_crtra = b.id_embrgos_crtra
                        left join cb_g_procesos_juridico     d on d.id_prcsos_jrdco = c.id_prcsos_jrdco
                        left join mc_g_embargos_cartera      e on e.id_embrgos_crtra = b.id_embrgos_crtra
                        where a.id_instncia_fljo = p_id_instncia_fljo
                        and  a.cdgo_ttlo_jdcial_estdo in ('ACN','CNS','CYC')
                        group by a.nmro_ttlo_jdcial, a.vlor, a.fcha_cnsttcion)
        loop

          v_tabla := v_tabla ||
                     '<td style="text-align: center; border:1px solid black">' ||
                     c_titulo.nmro_ttlo_jdcial || '</td>' ||
                     '<td style="text-align: center; border:1px solid black">' ||
                     c_titulo.vlor || '</td>' || '</tr>';
    ...
    end fnc_co_titulos_tabla;

    function fnc_co_titulos_cartera_consignar(p_id_instncia_fljo in number) return clob as
        v_tabla clob;
    begin

        v_tabla := ' <div class="container" style=width:100%; margin-left:60px; align-items:center; >
                    <table border="1" style="border-collapse:collapse; margin:0 auto; width:60%; align:center">' ||
                   '<thead>' || '<tr>' ||
                   '<th style="text-align: center; border:1px solid black">' ||
                   'Identificación' || '</th>' ||
                   '<th style="text-align: center; border:1px solid black">' ||
                   'Vigencia-Período' || '</th>' ||
                   '<th style="text-align: center; border:1px solid black">' ||
                   'Valor Liquidado' || '</th>'||'</tr>' || '</thead>' ||
                   '<tr>';

        for cartera in (select b.idntfccion_sjto,
                               a.vgncia ||'-'|| a.prdo as vgncias,
                               trim(to_char(sum(a.vlor_ttal_aplcar),'FM999G999G999G999G999G999G990')) as Valor_Liquidado
                        from gf_g_titulos_jdcial_vgncia a
                        join v_si_i_sujetos_impuesto b on a.id_sjto_impsto = b.id_sjto_impsto
                        where a.id_instncia_fljo = p_id_instncia_fljo
                        and a.cdgo_mvmnto_tpo = 'PC'
                        group by b.idntfccion_sjto, a.vgncia ||'-'|| a.prdo, a.id_orgen)
        loop
            v_tabla := v_tabla ||
                     '<td style="text-align: center; border:1px solid black">' ||
                     cartera.idntfccion_sjto || '</td>' ||
                     ...
                     cartera.vgncias || '</td>' ||
                     '<td style="text-align: center; border:1px solid black">' ||
                     cartera.Valor_Liquidado || '</td>' ||'</tr>';
        end loop;
        v_tabla := v_tabla || '</table><center>';
        return v_tabla;
    end fnc_co_titulos_cartera_consignar;

    function fnc_co_titulos_cartera_compensar(p_id_instncia_fljo in number) return clob as
        v_tabla clob;
    begin
        for crtra in (select    a.tpo_dcmnto,
                                listagg(distinct a.vgncia ||'-'||a.prdo)within group (order by a.vgncia ||'-'||a.prdo)vgncias,
                                sum(nvl(a.vlor_ttal,0))crtra,
                                pkg_gn_generalidades.fnc_number_to_text(sum(nvl(a.vlor_ttal,0)),'d')crtra_ltras,
                                b.idntfccion_sjto, a.vgncia ||'-'|| a.prdo as Vigencia_Período,
                                sum(a.vlor_ttal_aplcar)vlor_ttal_aplcar,
                                trim(to_char(sum(a.vlor_ttal_aplcar),'FM999G999G999G999G999G999G990'))as Valor_Liquidado,
                                pkg_gn_generalidades.fnc_number_to_text(sum(a.vlor_ttal_aplcar),'d')lqddo_ltras
                        from gf_g_titulos_jdcial_vgncia a
                        join v_si_i_sujetos_impuesto b on a.id_sjto_impsto = b.id_sjto_impsto
                        where a.id_instncia_fljo = p_id_instncia_fljo
                        and a.cdgo_mvmnto_tpo = 'AC'
                        group by b.idntfccion_sjto, a.vgncia ||'-'|| a.prdo, a.id_orgen, a.tpo_dcmnto)
        loop
            if v_tabla is null then
                if (crtra.tpo_dcmnto = 'DNO' and crtra.vlor_ttal_aplcar > 0) then
                    v_tabla := 'por valor de '||crtra.lqddo_ltras||' pesos mcte ($ '||crtra.Valor_Liquidado||') '||'sobre las vigencias '||crtra.Vigencia_Período;
                elsif (crtra.tpo_dcmnto = 'DAB' and crtra.vlor_ttal_aplcar > 0) then
                    v_tabla := v_tabla ||' y abónese el valor de '||crtra.Valor_Liquidado||
                              ' sobre las vigencias'||crtra.Vigencia_Período;
                end if;
            else
                if (crtra.tpo_dcmnto = 'DNO' and crtra.vlor_ttal_aplcar > 0) then
                    v_tabla := v_tabla || ', por valor de '||crtra.lqddo_ltras||' pesos mcte ($ '||crtra.Valor_Liquidado||') '||'sobre las vigencias '||crtra.Vigencia_Período;
                elsif (crtra.tpo_dcmnto = 'DAB' and crtra.vlor_ttal_aplcar > 0) then
                    v_tabla := v_tabla ||' y abónese el valor de '||crtra.lqddo_ltras||' pesos mcte ($ '||crtra.Valor_Liquidado||
                              ' sobre las vigencias'||crtra.Vigencia_Período;
                end if;
            end if;
        end loop;
        ...
        return v_tabla;
    end fnc_co_titulos_cartera_compensar;
