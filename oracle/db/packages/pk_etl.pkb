  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PK_ETL" as

  procedure prc_carga_intermedia_from_dir(p_cdgo_clnte    number,
                                          p_id_impsto     number default null,
                                          p_id_prcso_crga number) as
  ...
  begin
    ...
    v_nl := pkg_sg_log.fnc_ca_nivel_log(p_cdgo_clnte,
                                        p_id_impsto,
                                        'pk_etl.prc_carga_intermedia_from_dir');
    -- Traza
    ...
    if c1%found then
        ...
        begin
          ...
          if (apex_collection.collection_member_count(p_collection_name => v_collection_name) > 0) then
            ...
            for c_dtos in (select seq_id
              ...
              for c_clmnas in (select a.nmbre_clmna,
                                      b.vlor,
                                      (select m.oprdor
                                         from df_s_operadores_tipo m
                                        where m.id_oprdor_tpo =
                                              a.id_oprdor_tpo) as oprdor,
                                      a.vlor1,
                                      a.vlor2,
                                      a.indcdor_vlda,
                                      a.cdgo_dto_tpo,
                                      decode(a.cdgo_dto_tpo,
                                             'C',
                                             'varchar2',
                                             'N',
                                             'number',
                                 ...
                                 left join (select seq_id,
                                            ...
                                            where collection_name =
                                                  v_collection_name
                                              and seq_id = c_dtos.seq_id) unpivot(vlor
                                  for clmna in(c001,
                                              c002,
                                              c003,
                                              c004,
                                              ...
                                              c050)) b
                                   on 'C' ||
                                      lpad(replace(a.nmbre_clmna, 'CLUMNA'),
                                           3,
                                           0) = b.clmna
                                where a.id_crga = v_c1.id_crga
                                order by to_number(substr(a.nmbre_clmna, 7))) loop
                ...
                if (c_clmnas.indcdor_vlda = 'S') then
                  ...
                  v_cndcion := c_clmnas.vlor || ' ' || (case
                                 when c_clmnas.oprdor = 'LIKE' then
                                  'like ' || chr(39) || '%' || c_clmnas.vlor1 || '%' ||
                                  chr(39)
                                 ...
                                 when c_clmnas.oprdor = 'LIKE T' then
                                  ...
                                  c_clmnas.oprdor || (case
                                    when c_clmnas.oprdor in ('IN', 'NOT IN') then
                                     ' ( ' || c_clmnas.vlor1 || ')'
                                 ...
                                 when c_clmnas.oprdor = 'BETWEEN' then
                                  ' and ' ||
                                  pkg_gn_generalidades.fnc_co_formatted_type(p_tipo  => c_clmnas.tpo_clmna,
                  ...
                  --Valida la Condición
                  begin
                    execute immediate 'begin :result := ' || v_cndcion ||
                                      '; end;'
                    ...
                    when others then
                      ...
                      values
                        (p_id_prcso_crga,
                         'INTERMEDIA',
                         v_num_linea,
                         c_clmnas.nmbre_clmna,
                         v_cndcion);

                      v_respuesta := 'Linea # ' || v_num_linea ||
                                     '  Registro No Valido: ' || v_cndcion ||
                                     ' Error: ' || sqlerrm;
                      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                            null,
                                            v_nmbre_up,
              ...
              if (v_sw_datos_error = 0) then
                ...
                v_dml := v_dml || p_id_prcso_crga || ', ' || v_num_linea ||
                         ', ''SP'',' || v_sw_datos_error || v_datos || ')';
                ...
                exception
                  when others then
                    ...
                    v_respuesta := 'Linea # ' || v_num_linea || ' DML ' ||
                                   v_dml || ' Error: ' || sqlerrm;
            ...
            insert into et_g_procesos_carga_traza
              (id_prcso_crga,
               orgen,
               rgstros_prcsdos,
               rgstros_extsos,
      ...
      loop
        ...
        begin
          ...
          if v_c1.lneas_encbzdo < v_num_linea then
            ...
            if v_c1.cdgo_archvo_tpo = 'AF' then
              ...
              for i in (select a.*,
                               (select m.oprdor
                                  from df_s_operadores_tipo m
                                 where m.id_oprdor_tpo = a.id_oprdor_tpo) oprdor
                          from et_d_reglas_intermedia a
                ...
                v_dato     := replace(substr(v_line,
                                             i.crcter_incial,
                                             i.crcter_fnal - i.crcter_incial + 1),
                                      chr(39),
                                      chr(49844));
                ...
                if i.indcdor_vlda = 'S' then
                  ...
                  v_ddl_validacion := 'Select 1 a from dual where ' ||
                                      v_dato || ' ' || i.oprdor || ' ';
                  if i.oprdor in ('=', '>=', '<=', '>', '<', '<>') then
                  ...
                  v_respuesta := 'Linea # ' || v_num_linea ||
                                 '  v_ddl_validacion: ' || v_ddl_validacion;
                  ...
                  if dbms_sql.fetch_rows(v_cursor) = 0 then
                    ...
                    values
                      (p_id_prcso_crga,
                       ...
                       v_num_linea,
                       i.nmbre_clmna,
                       v_dato || ' ' || i.oprdor || v_validacion);

                    -- Generar LOG -Para critica-
                    v_respuesta := 'Linea # ' || v_num_linea ||
                                   '  Registro No Valido: ' ||
                                   i.nmbre_clmna || ': ' || v_dato || ' ' ||
                                   i.oprdor || ' ' || v_validacion;
                    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                          p_id_impsto,
                                          'pk_etl.prc_carga_intermedia_from_dir',
                                          v_nl,
              ...
              for i in (select *
                         ...
                         where id_crga = v_c1.id_crga
                         order by pscion) loop
                -- Recorremos las columnas de reglas
                v_columnas := v_columnas || i.nmbre_clmna || ', ';
                v_vector(i.pscion) := replace(v_vector(i.pscion),
                                              chr(39),
                                              chr(49844));
          ...
          v_respuesta := 'Linea # ' || v_num_linea || ' v_sw_datos_error: ' ||
                         v_sw_datos_error || ' Columnas: ' || v_columnas ||
                         ' Datos: ' || v_datos;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                p_id_impsto,
                                'pk_etl.prc_carga_intermedia_from_dir',
        ...
        exception
          when others then
            ...
            -- Registramos el Error
            v_respuesta      := 'Error en Linea # ' || v_num_linea ||
                                '. DML: ' || v_dml || '  Error: ' ||
                                sqlerrm;
            ...
            insert into et_g_procesos_carga_error
              (id_prcso_crga, orgen, nmero_lnea, vldcion_error)
            values
              (p_id_prcso_crga, 'INTERMEDIA', v_num_linea, v_respuesta);
        end;
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            'pk_etl.prc_carga_intermedia_from_dir',
                            v_nl,
                            'No se hallo del proceso de carga:' ||
                            p_id_prcso_crga,
                            1);
      null;
    end if;
  ...
  exception
    when no_data_found then
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            'pk_etl.prc_carga_intermedia_from_dir',
                            v_nl,
                            'Cierre del Archivo de Carga' ||
                            to_char(sysdate, 'DD/MON/YYYY HH24:MI:SS'),
                            5);

  end;

  procedure prc_carga_gestion(p_cdgo_clnte    number,
                              p_id_impsto     number default null,
                              p_id_prcso_crga number) as
  ...
  begin
    ...
    v_nl := pkg_sg_log.fnc_ca_nivel_log(p_cdgo_clnte,
                                        p_id_impsto,
                                        'pk_etl.prc_carga_gestion');
    -- Traza
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          p_id_impsto,
                          'pk_etl.prc_carga_gestion',
                          v_nl,
                          'Incia prc_carga_gestion. ' ||
                          to_char(sysdate, 'DD/MON/YYYY HH24:MI:SS'),
                          1);
    ...
    if c1%found then
      ...
      for a in (select /*+ RESULT_CACHE */
                 nmbre_tbla_dstno, min(nmro_orden) nmro_orden
        ...
        prc_get_columnas(a.nmbre_tbla_dstno,
                         v_c1.id_crga,
                         p_columnas_origen,
                         p_columnas_destino,
                         p_columnas_value);
        ...
        v_ddl := 'declare
              ...
              cursor c1 is
                          select /*+ RESULT_CACHE */ nmero_lnea, id_prcso_intrmdia, ' ||
                 p_columnas_origen || '
               ...
               begin
                for R1 in C1 loop
                  begin
                    insert into ' || a.nmbre_tbla_dstno ||
                 ' ( id_prcso_crga, id_prcso_intrmdia , nmero_lnea, ' ||
                 p_columnas_destino || ')
                       values ( ' || p_id_prcso_crga ||
                 ', r1.id_prcso_intrmdia , r1.nmero_lnea , ' ||
                 p_columnas_value || ');
                  ...
                  exception
                    when others then
                      ...
                      insert into et_g_procesos_carga_error (id_prcso_crga, orgen, nmero_lnea, vldcion_error)
                                                     values (' ||
                 p_id_prcso_crga || ', ''GESTION'', r1.nmero_lnea, v_respuesta);
                  end;
                end loop;
              end;';
        ...
        begin
          ...
          v_respuesta := 'Se insertaron: ' || pk_etl.v_crga_gstion ||
                         ' Registro(s), ' || v_error_crga_gstion ||
                         ' Registro(s) no Cargaron';
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                p_id_impsto,
                                'pk_etl.prc_carga_gestion',
                                v_nl,
          ...
          values
            (p_id_prcso_crga,
             'GESTION',
             v_crga_gstion + v_error_crga_gstion,
             v_crga_gstion,
             v_error_crga_gstion,
             v_fcha_incio_prcso,
             systimestamp);
        ...
        exception
          when others then
            ...
            v_respuesta  := 'Error DML: ' || v_ddl || '  Error: ' ||
                            sqlerrm;
      ...
      exception
        when others then
          raise_application_error(-20001,
                                  'No fue posible registrar la resolucion.' ||
                                  sqlerrm);
      end;
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(1,
                          1,
                          'pk_etl.prc_get_columnas',
                          v_nl,
                          'Incia prc_get_columnas. ' ||
                          to_char(sysdate, 'DD/MON/YYYY HH24:MI:SS'),
                          1);
    ...
    for a in (
              ...
              select clmna_orgen,
                      (case
                        when cdgo_dto_tpo = 'F' then
                         'to_timestamp( ' || clmna_orgen_vlue || ' , ' ||
                         chr(39) || frmto || chr(39) || ')'
      ...
      fetch c1
        into v_c1;
      if c1%found then
        p_columnas_origen := p_columnas_origen || 'case when ' ||
                             v_c1.clmna_orgen || ' ' ||
                             v_c1.dscrpcion_oprdor_tpo || ' then ' ||
                             v_c1.vlor_trnsfrmdo || 'end as ' ||
                             v_c1.clmna_orgen;
      else
    ...
    if length(p_columnas_origen) > 0 then
      -- Eliminamos el último carcater (, )
      p_columnas_origen  := substr(p_columnas_origen,
                                   1,
                                   length(p_columnas_origen) - 2);
      p_columnas_destino := substr(p_columnas_destino,
                                   1,
                                   length(p_columnas_destino) - 2);
      p_columnas_value   := substr(p_columnas_value,
                                   1,
                                   length(p_columnas_value) - 2);

      v_respuesta := 'p_columnas_origen --> ' || p_columnas_origen ||
                     ' p_columnas_destino ' || p_columnas_destino ||
                     ' p_columnas_value ' || p_columnas_value;
      pkg_sg_log.prc_rg_log(1,
                            1,
                            'pk_etl.prc_get_columnas',
                            v_nl,
    ...
    pkg_sg_log.prc_rg_log(1,
                          ...
                          'pk_etl.prc_get_columnas',
                          v_nl,
                          'Fin pk_etl.prc_get_columnas. ' ||
                          to_char(sysdate, 'DD/MON/YYYY HH24:MI:SS'),
                          1);

  end;

  procedure prc_carga_archivo_directorio(p_file_blob in blob,
  ...
  begin
    ...
    l_out_file := utl_file.fopen(l_drctrio,
                                 p_file_name,
    ...
    while l_blob_position <= l_blob_length loop
      ...
      dbms_lob.read(p_file_blob, l_chunk_size, l_blob_position, l_buffer);
  ...
  procedure prc_carga_intermedia_from_db(p_cdgo_clnte    number,
                                         p_id_impsto     number default null,
                                         p_id_prcso_crga number) as
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Incia prc_carga_intermedia. Optimizado ' ||
                          to_char(sysdate, 'DD/MON/YYYY HH24:MI:SS'),
                          1);
    ...
    if c1%found then
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nl,
                            'Tamaño Blob: ' ||
                            dbms_lob.getlength(v_c1.file_blob),
                            5);
      ...
      -- *************************************************************************
      -- INICIO DEL PROCESAMIENTO OPTIMIZADO
      -- *************************************************************************
      v_inicial := 1;

      LOOP
        -- 1. Buscamos el siguiente salto de línea usando INSTR (Mucho mas rápido)
        v_actual := dbms_lob.instr(v_clob, chr(10), v_inicial);

        IF v_actual = 0 THEN
          -- Si no hay mas enters, verificamos si queda un ultimo pedazo
        ...
        -- 2. Extraemos la línea
        v_linea := dbms_lob.substr(v_clob, v_actual - v_inicial, v_inicial);

        -- 3. Limpieza de caracteres de control (CR y LF)
        v_linea := replace(replace(v_linea, chr(13), ''), chr(10), '');

        -- Asignamos a la variable original
        v_line := v_linea;

        -- 4. Limpieza critica de memoria del vector (Evita fugas)
        if v_vector is not null then
          v_vector.delete;
        end if;

        -- 5. Lógica Interna Original
        begin
          v_num_linea := v_num_linea + 1;
          v_columnas  := '';
          v_datos     := '';

          -- LOG DE PROGRESO CADA 100 REGISTROS (Nuevo)
          if mod(v_num_linea, 100) = 0 then
            v_respuesta := 'Progreso: Linea #' || v_num_linea ||
                           ' - Exitos: ' || v_rgstros_extsos ||
                           ' - Errores: ' || v_rgstros_error;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  p_id_impsto,
                                  v_nmbre_up,
                                  6,
          ...
          -- Se valida si es encabezado
          if v_c1.lneas_encbzdo < v_num_linea then

            v_sw_datos_error := 0;

            -- LOGICA 'AF' (ANCHO FIJO) OPTIMIZADA
            if v_c1.cdgo_archvo_tpo = 'AF' then
              for i in (select a.*,
                               (select m.oprdor
                                  from df_s_operadores_tipo m
                ...
                v_columnas := v_columnas || i.nmbre_clmna || ', ';
                -- Protección por si la línea es más corta
                begin
                  v_dato := substr(v_line,
                                   i.crcter_incial,
                                   i.crcter_fnal - i.crcter_incial + 1);
                exception
                  when others then
              ...
              -- LOGICA 'DC' (DELIMITADO)
            elsif v_c1.cdgo_archvo_tpo = 'DC' then
              v_cont  := 0;
              pos_ini := 1;

              while (pos_ini < length(v_line) + 1) loop
                v_cont := v_cont + 1;
                v_vector.extend;
                ...
                if instr(v_line, v_c1.crcter_dlmtdo, pos_ini) > 0 then
                  ...
                  v_vector(v_cont) := substr(v_line, pos_ini);
                  pos_ini := length(v_line) + 2;
                end if;
              end loop;
              ...
              for i in (select *
                         ...
                         where id_crga = v_c1.id_crga
                         order by pscion) loop
                if i.pscion <= v_vector.count then
                  v_columnas := v_columnas || i.nmbre_clmna || ', ';
                  v_vector(i.pscion) := replace(v_vector(i.pscion),
                                                chr(39),
                                                chr(49844));
                  v_vector(i.pscion) := substr(v_vector(i.pscion),
                                               0,
                                               i.tmno);
                  v_datos := v_datos || '''' || v_vector(i.pscion) ||
                             ''', ';
            ...
            -- Eliminamos caracteres demás e Insertamos
            if length(v_columnas) > 0 then
              v_columnas := substr(v_columnas, 1, length(v_columnas) - 2);
              v_datos    := substr(v_datos, 1, length(v_datos) - 2);
            ...
            end if; -- Fin del insert

          end if; -- Fin del IF lneas_encbzdo
        ...
        exception
          when others then
            ...
            v_respuesta      := 'Error Linea # ' || v_num_linea || ' ' ||
                                sqlerrm;
            v_sw_datos_error := v_sw_datos_error + 1;
            v_rgstros_error  := v_rgstros_error + 1;
            insert into et_g_procesos_carga_error
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'TRAZA FINAL. Lineas:' || v_num_linea ||
                            ' Exitosos:' || v_rgstros_extsos || ' Errores:' ||
                            v_rgstros_error,
                            4);
    else
      -- No se hallo proceso de carga p_id_prcso_crga
