  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_MA_ENVIOS" as

  /*Procedimiento para la gestion de medio de envio programado*/
  procedure prc_cd_envios_programado_medio(p_cdgo_clnte            in number,
    ...
    case p_request
      when 'BTN_CREAR' then
        ...
        exception
          when others then
            o_cdgo_rspsta  := 1;
            o_mnsje_rspsta := 'Problemas al registrar medio de envio programado, ' ||
                              sqlerrm;
            raise v_error;
        end;
      when 'BTN_GUARDAR' then
        ...
        exception
          when others then
            o_cdgo_rspsta  := 2;
            o_mnsje_rspsta := 'Problemas al actualizar medio de envio programado, ' ||
                              sqlerrm;
            raise v_error;
        end;
      when 'BTN_ELIMINAR' then
        ...
        begin
          delete from ma_g_envios_programado_mdio
           where id_envio_prgrmdo_mdio = p_id_envio_prgrmdo_mdio;
        exception
          when others then
            o_cdgo_rspsta  := 2;
            o_mnsje_rspsta := 'Problemas al eliminar medio de envio programado, ' ||
                              sqlerrm;
            raise v_error;
        end;
      else
        o_cdgo_rspsta  := 2;
        o_mnsje_rspsta := 'Problemas al gestionar medio envio programado, request no encontrado' ||
                          sqlerrm;
        raise v_error;
    end case;
  exception
    when v_error then
      ...
      if (o_mnsje_rspsta is null) then
        o_mnsje_rspsta := 'Problemas al gestionar medio de envio programado';
      end if;
  end prc_cd_envios_programado_medio;

  /*Procedimiento para registrar un envio*/
  procedure prc_rg_envio(p_cdgo_clnte       in number,
                         p_id_envio_prgrmdo in ma_g_envios_programado_mdio.id_envio_prgrmdo%type,
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          nmbre_up,
                          v_nl,
                          'Despues del Insert - id_envio: ' || o_id_envio,
                          6);
  ...
  procedure prc_rg_envio_mdio(p_id_envio        in ma_g_envios_medio.id_envio%type,
  ...
  begin
    ...
    v_txto_mnsje := p_txto_mnsje;
    if p_cdgo_envio_mdio = 'EML' then
      --comprimir el html
      v_txto_mnsje := replace(replace(replace(v_txto_mnsje, chr(13), ''),
                                      chr(10),
                                      ''),
                              chr(9),
                              '');
      v_txto_mnsje := regexp_replace(v_txto_mnsje, ' {2,}', ' ');
    end if;

    insert into ma_g_envios_medio
      (id_envio, cdgo_envio_mdio, dstno, asnto, txto_mnsje)
    values
      (p_id_envio, p_cdgo_envio_mdio, p_dstno, p_asnto, v_txto_mnsje)
    returning id_envio_mdio into o_id_envio_mdio;
  exception
    when others then
      o_cdgo_rspsta  := 1;
      o_mnsje_rspsta := 'Problemas al registrar medio de envio, ' ||
                        sqlerrm;
  end prc_rg_envio_mdio;

  /*Procedimiento para registrar estado asociado a un envio*/
  procedure prc_rg_envio_estado(p_id_envio_mdio    in ma_g_envios_mdio_trza_estdo.id_envio_mdio%type,
  ...
  begin
    ...
    insert into ma_g_envios_mdio_trza_estdo
      (id_envio_mdio, cdgo_envio_estdo, obsrvcion, fcha_rgstro)
    values
      (p_id_envio_mdio, p_cdgo_envio_estdo, p_obsrvcion, systimestamp);
  ...
  procedure prc_rg_envio_adjntos(p_id_envio        in ma_g_envios_medio.id_envio%type,
                                 ...
                                 p_file_bfile      in ma_g_envios_adjntos.file_bfile%type default null,
  ...
  begin
    ...
    values
      (p_id_envio, p_file_blob, p_file_name, p_file_mimetype, p_file_bfile)
    returning id_envio_adjnto into o_id_envio_adjnto;
  exception
    when others then
      o_cdgo_rspsta  := 1;
      o_mnsje_rspsta := 'Problemas al registrar archivo adjunto, ' ||
                        sqlerrm;
  end prc_rg_envio_adjntos;

  /*Procedimiento para registrar un envio*/
  procedure prc_rg_envios(p_id_envio_prgrmdo in ma_g_envios_programado.id_envio_prgrmdo%type,
                          p_json_prmtros     in clob default null,
                          p_id_sjto_impsto   in number default null,
                          p_id_acto          in number default null) as
    --Manejo de Errores
    v_error exception;
    --Registro en Log
    v_nl        number;
  ...
  begin
    ...
    v_mnsje_log := 'Consultamos el envio programado - ' ||
                   rt_ma_g_envios_programado.dscrpcion;
    pkg_sg_log.prc_rg_log(8758, null, nmbre_up, v_nl, v_mnsje_log, 6);

    v_mnsje_log := 'Consultamos el id consulta principal - ' ||
                   rt_ma_g_envios_programado.id_envio_prgrmd_cnslta_prncpal;
    pkg_sg_log.prc_rg_log(8758, null, nmbre_up, v_nl, v_mnsje_log, 6);
    ...
    exception
      when others then
        v_mnsje_rspsta := 'Problemas al actualizar el estado del envio programado, ' ||
                          sqlerrm;
        raise v_error;
    end;
    ...
    begin
      ...
      select *
       ...
       where id_envio_prgrmdo_cnslta =
             rt_ma_g_envios_programado.id_envio_prgrmd_cnslta_prncpal;
    exception
      when others then
        v_mnsje_rspsta := 'Problemas al consultar la consulta principal asociada al envio programado, ' ||
                          sqlerrm;
        raise v_error;
    end;

    v_mnsje_log := 'Consultamos la consulta principal asociada al envio programado - ' ||
                   rt_ma_g_envios_prgrmdo_cnslta_p.cnslta ||
                   ' - rt_ma_g_envios_prgrmdo_cnslta_p.id_cnslta_mstro: ' ||
                   rt_ma_g_envios_prgrmdo_cnslta_p.id_cnslta_mstro;
    pkg_sg_log.prc_rg_log(8758, null, nmbre_up, v_nl, v_mnsje_log, 6);
    ...
    if (rt_ma_g_envios_prgrmdo_cnslta_p.id_cnslta_mstro is null) then
      ...
      pkg_sg_log.prc_rg_log(8758,
                            ...
                            nmbre_up,
                            v_nl,
                            'IF----is null-Validamos el tipo de consulta v_sql:=>' ||
                            v_sql,
                            6);
      ...
      pkg_sg_log.prc_rg_log(8758,
                            ...
                            nmbre_up,
                            v_nl,
                            'else----Validamos el tipo de consulta',
                            6);
      v_sql := pkg_cs_constructorsql.fnc_co_sql_dinamica(p_id_cnslta_mstro => rt_ma_g_envios_prgrmdo_cnslta_p.id_cnslta_mstro,
    ...
    pkg_sg_log.prc_rg_log(8758,
                          ...
                          nmbre_up,
                          v_nl,
                          'despues de v_c_columnas=>' || v_c_columnas,
                          6);
    ...
    pkg_sg_log.prc_rg_log(8758,
                          ...
                          nmbre_up,
                          v_nl,
                          ' despues de dbms_sql.describe_columns2',
                          6);
    ...
    pkg_sg_log.prc_rg_log(8758,
                          ...
                          nmbre_up,
                          v_nl,
                          'Entrando al cursor c_columnas',
                          6);
    for c_columnas in 1 .. v_nmro_clmna loop
      v_columnas_json := v_columnas_json || case
                           when v_columnas_json is null then
                            ''
                           else
                            ', '
                         end || 'key ''' || v_dscrpcion_tbla(c_columnas).col_name ||
                         ''' value "' || v_dscrpcion_tbla(c_columnas).col_name || '"';

      pkg_sg_log.prc_rg_log(8758,
                            null,
                            nmbre_up,
                            v_nl,
                            ' dentro del cursor' || v_columnas_json,
                            6);
    ...
    v_mnsje_log := 'Despues del for columnas - v_nmro_clmna: ' ||
                   v_nmro_clmna || ' - v_columnas_json: ' ||
                   v_columnas_json;
    pkg_sg_log.prc_rg_log(8758, null, nmbre_up, v_nl, v_mnsje_log, 6);
    ...
    begin
      ...
      select id_envio, to_char(fcha_rgstro, 'DD-Mon-RR HH24:MI:SS.FF')
        ...
        from (select ma_g_envios.id_envio, ma_g_envios.fcha_rgstro
                from ma_g_envios
               where id_envio_prgrmdo =
                     rt_ma_g_envios_programado.id_envio_prgrmdo
    ...
    v_mnsje_log := 'Consultamos el ultimo envio asociado al envio programado -  v_id_envio: ' ||
                   v_id_envio || ' - v_fcha_rgstro: ' || v_fcha_rgstro;
    pkg_sg_log.prc_rg_log(8758, null, nmbre_up, v_nl, v_mnsje_log, 6);

    --Remplazamos las variables en la SQL
    v_sql := replace(v_sql,
                     ':F_CDGO_CLNTE',
                     rt_ma_g_envios_programado.cdgo_clnte);
    v_sql := replace(v_sql,
                     ':USRIO_ENVIO_PRGRMDO',
                     rt_ma_g_envios_programado.id_usrio);
    v_sql := replace(v_sql,
                     ':FCHA_ENVIO',
                     'to_timestamp(''' || v_fcha_rgstro ||
    ...
    if (p_json_prmtros is not null) then
      pkg_sg_log.prc_rg_log(8758,
                            ...
                            nmbre_up,
                            v_nl,
                            'IF -Remplazamos las variables asociadas al JSON de parametros',
                            6);
      ...
      begin
        ...
        for i in 1 .. v_jsonKeys.count loop
          v_sql := replace(v_sql,
                           ':' || v_jsonKeys(i),
                           '''' || v_json_object.get_string(v_jsonKeys(i)) || '''');
        end loop;
      end;
    end if;

    v_sql_json := 'select json_object(' || v_columnas_json ||
                  ') as json_row from (' || v_sql || ')';
    ...
    pkg_sg_log.prc_rg_log(8758,
                          ...
                          nmbre_up,
                          v_nl,
                          'Recorremos los registros del cursor v_cursor',
                          6);
    pkg_sg_log.prc_rg_log(8758,
                          null,
                          nmbre_up,
    ...
    loop
      ...
      begin
        ...
        begin
          ...
          v_mnsje_log := 'Validamos el tipo de origen de los destinatarios, antes del case: ' ||
                         rt_ma_g_envios_programado.orgen_tpo_dstntrio;
          pkg_sg_log.prc_rg_log(8758, null, nmbre_up, v_nl, v_mnsje_log, 6);
              ...
              begin
                ...
                if (v_columnas_json is null) then
                  raise_application_error(-20001,
                                          'Problemas al consultar columnas requeridas');
                end if;

                v_mnsje_log := 'Consultamos la SQL origen: ' ||
                               rt_ma_g_envios_prgrmdo_cnslta_d.cnslta;
                pkg_sg_log.prc_rg_log(8758,
                                      null,
                                      nmbre_up,
                                      v_nl,
                ...
                begin
                  select *
                    ...
                    from ma_g_envios_prgrmdo_cnslta
                   where id_envio_prgrmdo_cnslta =
                         rt_ma_g_envios_programado.id_envio_prgrmd_cnslta_dstntr;
                exception
                  when others then
                    raise_application_error(-20001,
                                            'Problemas al consultar la consulta de destinatarios asociada al envio programado');

                end;

                v_mnsje_log := 'Antes del if: ' ||
                               rt_ma_g_envios_prgrmdo_cnslta_d.id_cnslta_mstro;
                pkg_sg_log.prc_rg_log(8758,
                                      null,
                                      nmbre_up,
                                      v_nl,
                ...
                if (rt_ma_g_envios_prgrmdo_cnslta_d.id_cnslta_mstro is null) then
                  ...
                  v_sql_origen_destinarios := pkg_cs_constructorsql.fnc_co_sql_dinamica(p_id_cnslta_mstro => rt_ma_g_envios_prgrmdo_cnslta_d.id_cnslta_mstro,
                                                                                        p_cdgo_clnte      => rt_ma_g_envios_programado.cdgo_clnte,
                                                                                        p_rturn_alias     => 'S');
                end if;
                ...
                begin
                  select id_envio, fcha_rgstro
                    into v_id_envio, v_fcha_rgstro
                    from (select ma_g_envios.id_envio,
                                 ma_g_envios.fcha_rgstro
                            from ma_g_envios
                           where id_envio_prgrmdo =
                                 rt_ma_g_envios_programado.id_envio_prgrmdo
                           order by fcha_rgstro desc)
                ...
                v_sql_destinatarios := replace(v_sql_destinatarios,
                                               ':F_CDGO_CLNTE',
                                               rt_ma_g_envios_programado.cdgo_clnte);
                v_sql_destinatarios := replace(v_sql_destinatarios,
                                               ':USRIO_ENVIO_PRGRMDO',
                ...
                begin
                  ...
                  v_jsonKeys    := v_json_object.get_keys;
                  for i in 1 .. v_jsonKeys.count loop
                    v_sql := replace(v_sql_destinatarios,
                                     ':' || v_jsonKeys(i),
                                     '''' ||
                                     v_json_object.get_string(v_jsonKeys(i)) || '''');
                  end loop;
                ...
                v_sql_destinatarios := 'select json_arrayagg(json_object(' ||
                                       v_columnas_json || '))from(' ||
                                       v_sql_origen_destinarios || ')';

                v_mnsje_log := 'Generamos la consulta para obtener el JSON -  v_sql_destinatarios: ' ||
                               v_sql_destinatarios;
                pkg_sg_log.prc_rg_log(8758,
                                      null,
                                      nmbre_up,
                ...
                execute immediate v_sql_destinatarios
                  into v_json_destinatarios;

                --Guardamos los destinatarios asociados al JSON
                select id_usrio, nmbre, nmro_cllar, email --, id_sjto_impsto  --##
                  bulk collect
                  into v_dstntrios
                  from json_table(v_json_destinatarios,
                                  '$' columns(id_usrio path '$.ID_USRIO',
                                          nmbre path '$.NMBRE',
            ...
            when 'LST' then
              ...
              select id_usrio, nmbre, nmro_cllar, email --, NULL  --##
               ...
               where id_envio_prgrmdo =
                 ...
                 and actvo = 'S';
            when 'DCP' then
              --> Definido en Consulta Principal
        ...
        exception
          when others then
            raise_application_error(-20001,
                                    'Problemas al cargar los destinatarios, ' ||
                                    sqlerrm);
        end;

        v_mnsje_log := 'Validamos si hay destinatarios: ' ||
                       v_dstntrios.count;
        pkg_sg_log.prc_rg_log(8758, null, nmbre_up, v_nl, v_mnsje_log, 6);
        ...
        if (v_dstntrios.count < 1) then
          raise_application_error(-20001,
                                  'No hay destinatarios cargados para realizar el envio.');
        end if;
        ...
        begin
          ...
          v_jsonKeys    := v_json_object.get_keys;
          for i in 1 .. v_jsonKeys.count loop
            v_json_prfrncias := replace(v_json_prfrncias,
                                        '#' || v_jsonKeys(i) || '#',
                                        v_json_object.get_string(v_jsonKeys(i)));
          end loop;
        end;

        v_mnsje_log := 'Remplazamos las variables asociadas al JSON de parametro: ' ||
                       to_char(v_json_prfrncias);
        pkg_sg_log.prc_rg_log(8758, null, nmbre_up, v_nl, v_mnsje_log, 6);
        ...
        v_mnsje_log := 'Llamamos al pkg_ma_envios.prc_rg_envio - o_cdgo_rspsta: ' ||
        ...
        for x in 1 .. v_dstntrios.count loop
          for c_medios_programados in (select *
            ...
            begin
              ...
              v_jsonKeys    := v_json_object.get_keys;
              for i in 1 .. v_jsonKeys.count loop
                v_txto_mnsje := replace(v_txto_mnsje,
                                        '#' || v_jsonKeys(i) || '#',
                                        v_json_object.get_string(v_jsonKeys(i)));
                v_asnto      := replace(v_asnto,
                                        '#' || v_jsonKeys(i) || '#',
                                        v_json_object.get_string(v_jsonKeys(i)));
              end loop;

              v_mnsje_log := 'Antes de pkg_ma_envios.prc_rg_envio_mdio - v_id_envio: ' ||
                             v_id_envio || ' - cdgo_envio_mdio: ' ||
              ...
              pkg_sg_log.prc_rg_log(8758,
                                    ...
                                    ' datos->     v_id_envio=>' ||
                                    v_id_envio || '-' ||
                                    c_medios_programados.cdgo_envio_mdio || '-' || v_dstntrios(x).email,
                                    6);
              ...
              v_mnsje_log := 'Despues de pkg_ma_envios.prc_rg_envio_mdio - o_cdgo_rspsta: ' ||
                             v_cdgo_rspsta || ' - o_mnsje_rspsta: ' ||
                             v_mnsje_rspsta;
              pkg_sg_log.prc_rg_log(8758,
                                    null,
              ...
              v_mnsje_log := 'Antes de prc_rg_envio_estado - v_id_envio_mdio: ' ||
                             v_id_envio_mdio;
              pkg_sg_log.prc_rg_log(8758,
                                    null,
                                    nmbre_up,
              ...
              v_mnsje_log := 'Despues de prc_rg_envio_estado - o_cdgo_rspsta: ' ||
                             v_cdgo_rspsta || ' - o_mnsje_rspsta: ' ||
                             v_mnsje_rspsta;
              pkg_sg_log.prc_rg_log(8758,
                                    null,
            ...
            when 'SQL' then
              begin
                ...
                select *
                  ...
                  from ma_g_envios_prgrmdo_cnslta
                 where id_envio_prgrmdo_cnslta =
                       rt_ma_g_envios_programado.id_envio_prgrmdo_cnslta_adjnto;

                if (rt_ma_g_envios_prgrmdo_cnslta_a.id_cnslta_mstro is null) then
                  v_sql_adjuntos := rt_ma_g_envios_prgrmdo_cnslta_a.cnslta;
                else
                  v_sql_adjuntos := pkg_cs_constructorsql.fnc_co_sql_dinamica(p_id_cnslta_mstro => rt_ma_g_envios_prgrmdo_cnslta_a.id_cnslta_mstro,
                                                                              p_cdgo_clnte      => rt_ma_g_envios_programado.cdgo_clnte,
                                                                              p_rturn_alias     => 'S');
                end if;
                ...
                if (p_json_prmtros is not null) then
                  ...
                  begin
                    ...
                    v_jsonKeys    := v_json_object.get_keys;
                    for i in 1 .. v_jsonKeys.count loop
                      v_sql_adjuntos := replace(v_sql_adjuntos,
                                                ':' || v_jsonKeys(i),
                                                '''' ||
                                                v_json_object.get_string(v_jsonKeys(i)) || '''');
                    end loop;
                ...
                execute immediate v_sql_adjuntos bulk collect
                  into v_archvos_adjntos;
              exception
                when others then
                  DBMS_OUTPUT.PUT_LINE(sqlerrm);
                  v_mnsje_log := 'Despues execute immediate v_sql_adjuntos : ' ||
                                 sqlerrm;
                  pkg_sg_log.prc_rg_log(8758,
                                        null,
                                        nmbre_up,
        ...
        for x in 1 .. v_archvos_adjntos.count loop
          ...
          begin
            pkg_ma_envios.prc_rg_envio_adjntos(p_id_envio        => v_id_envio,
            ...
            v_mnsje_log := 'Despues de pkg_ma_envios.prc_rg_envio_adjntos - o_cdgo_rspsta: ' ||
                           v_cdgo_rspsta || ' - o_mnsje_rspsta: ' ||
                           v_mnsje_rspsta;
            pkg_sg_log.prc_rg_log(8758,
                                  null,
  ...
  begin
    ...
    for c_envio_programado in (select a.*
                                ...
                                where a.actvo != 'S'
                                  and a.cnfrmcion_envio = 'S'
                                  and a.ejccion_tpo in ('INM', 'UNV', 'RPT')) loop
      declare
        v_cdgo_rspsta  number;
      ...
      begin
        ...
        if (not
            pkg_ma_envios.fnc_vl_registra_envio(p_id_envio_prgrmdo => c_envio_programado.id_envio_prgrmdo)) then
          DBMS_OUTPUT.put_line('No se cumplen las condiciones');
          continue;
        end if;

        -->Definimos el nombre para el JOB
        v_job_name := 'IT_MA_ENVIOS_PROGRAMADO_NUM_' ||
                      c_envio_programado.id_envio_prgrmdo;
        ...
        begin
          dbms_scheduler.create_job(job_name            => v_job_name,
                                    ...
                                    auto_drop           => true,
                                    comments            => 'JOB de mensajeria y alerta para la gestion del envio programado No. ' ||
                                                           c_envio_programado.id_envio_prgrmdo,
  ...
  function fnc_vl_registra_envio(p_id_envio_prgrmdo in ma_g_envios_programado.id_envio_prgrmdo%type)
    return boolean as
  ...
  begin
    ...
    case rt_ma_g_envios_programado.ejccion_tpo
      ...
      when 'RPT' then
        ...
        if (systimestamp between rt_ma_g_envios_programado.fcha_incio and
           rt_ma_g_envios_programado.fcha_fin) then
          declare
          ...
          begin
            select fcha_rgstro --, fcha_prgrmda
              ...
              from (select fcha_rgstro, fcha_prgrmda
                      from ma_g_envios
                     where id_envio_prgrmdo = p_id_envio_prgrmdo
                     order by fcha_rgstro desc)
             where rownum = 1;
            ...
            v_fecha_validacion := pk_util_calendario.fnc_cl_fecha_final(p_fecha_inicial => v_fcha_rgstro,
  ...
  function fnc_co_valor_json(p_json in clob, p_prmtro in varchar2)
    return varchar2 as
  ...
  procedure prc_co_envio_programado(p_cdgo_clnte     in number,
  ...
  begin
    ...
    for c_envios_programados in (select *
                                   from ma_g_envios_programado
                                  where cdgo_clnte = p_cdgo_clnte
                                    and idntfcdor = p_idntfcdor
                                    and ejccion_tpo = 'EVN'
                                    and cnfrmcion_envio = 'S') loop
      declare
      begin
        v_mensaje := 'For de envios programados - c_envios_programados.id_envio_prgrmdo: ' ||
                     c_envios_programados.id_envio_prgrmdo ||
        ...
        DBMS_OUTPUT.PUT_LINE('Envio:' ||
                             c_envios_programados.id_envio_prgrmdo);
        pkg_ma_envios.prc_rg_envios(p_id_envio_prgrmdo => c_envios_programados.id_envio_prgrmdo,
                                    p_json_prmtros     => p_json_prmtros,
                                    p_id_sjto_impsto   => p_id_sjto_impsto,
                                    p_id_acto          => p_id_acto);
      end;
  ...
  begin
    ...
    for c_envios in (select b.cdgo_clnte,
                            a.id_envio_mdio,
                            a.id_envio,
                            a.cdgo_envio_mdio
                       from ma_g_envios_medio a
                      inner join ma_g_envios b
                         on a.id_envio = b.id_envio
                      where a.cdgo_envio_estdo = 'ENC'
                      order by a.id_envio_mdio) loop

      --Consultamos la UP para realizar el envio
      pkg_ma_envios.prc_co_configuraciones(p_cdgo_clnte                     => c_envios.cdgo_clnte,
      ...
      v_bloque_pl := ' begin
                            PKG_MA_ENVIOS_MEDIO.' ||
                     v_rt_ma_d_envios_medio_cnfgrcion.undad_prgrma || '(
      ...
      begin
        execute immediate v_bloque_pl
          using in c_envios.id_envio_mdio, out v_cdgo_rspsta, out v_mnsje_rspsta;
      exception
        when others then
          v_cdgo_rspsta  := 1;
  ...
  procedure prc_co_configuraciones(p_id_envio_mdio                  in ma_g_envios_medio.id_envio_mdio%type,
    ...
    begin
      select *
       ...
       where cdgo_clnte = v_cdgo_clnte
         and cdgo_envio_mdio = v_cdgo_envio_mdio
         and actvo = 'S';
    exception
      when others then
        o_mnsje_rspsta := 'Problemas al consultar envio, ' || sqlerrm;
  ...
  procedure prc_co_configuraciones(p_cdgo_clnte                     in number,
                                   p_cdgo_envio_mdio                in ma_g_envios_medio.cdgo_envio_mdio%type,
                                   o_rt_ma_d_envios_medio_cnfgrcion out ma_d_envios_medio_cnfgrcion%rowtype,
                                   o_cdgo_rspsta                    out number,
                                   o_mnsje_rspsta                   out varchar2) as
  begin
    ...
    select *
      ...
      from ma_d_envios_medio_cnfgrcion
     where cdgo_clnte = p_cdgo_clnte
       and cdgo_envio_mdio = p_cdgo_envio_mdio
       and actvo = 'S';
  exception
    when others then
      o_cdgo_rspsta  := 1;
      o_mnsje_rspsta := 'Problemas al consultar configuraciones de envio, ' ||
                        sqlerrm;
  end prc_co_configuraciones;

  /*Funcion para consular columnas requeridas segun los medios de un envio programado*/
  function fnc_co_columnas_json_envios_programado(p_id_envio_prgrmdo in ma_g_envios_programado.id_envio_prgrmdo%type)
    return varchar2 as
    v_columnas_json varchar2(32000);
  begin
    --Consultamos las columnas requeridas
    select listagg('key ' || chr(39) || c.clmna_dstno || chr(39) ||
                   ' value ' || c.clmna_dstno,
                   ' ,') within group(order by clmna_dstno) as columns
      into v_columnas_json
      from ma_g_envios_programado_mdio a
     inner join ma_g_envios_programado b
        on a.id_envio_prgrmdo = b.id_envio_prgrmdo
     inner join ma_d_envios_medio_cnfgrcion c
        on a.cdgo_envio_mdio = c.cdgo_envio_mdio
       and c.cdgo_clnte = b.cdgo_clnte
       and c.actvo = 'S'
     where a.id_envio_prgrmdo = p_id_envio_prgrmdo
       and a.actvo = 'S';
  ...
  function fnc_co_json_medio_preferencias(p_id_envio_prgrmdo in ma_g_envios.id_envio%type)
    return clob as
    v_json clob;
  begin
    select json_arrayagg(json_object(key 'parametro' value d.prmtro,
                                     key 'valor' value c.vlor))
      into v_json
      from ma_g_envios_programado a
     inner join ma_g_envios_programado_mdio b
        on a.id_envio_prgrmdo = b.id_envio_prgrmdo
     inner join ma_g_envios_prgrmdo_mdio_pf c
        on b.id_envio_prgrmdo_mdio = c.id_envio_prgrmdo_mdio
     inner join ma_d_envios_mdio_cnfgrcn_pf d
        on c.id_envio_mdio_cnfgrcion_pf = d.id_envio_mdio_cnfgrcion_pf
     where a.id_envio_prgrmdo = p_id_envio_prgrmdo
       and b.actvo = 'S';
    return v_json;
  end fnc_co_json_medio_preferencias;

  function fnc_co_json_configuraciones(p_id_envio_mdio_cnfgrcion in number)
    return clob AS
    v_json_prmtros clob;
  begin
    --Generamos JSON de Configuraciones
    select json_arrayagg(json_object(key 'parametro' value prmtro,
                                     key 'valor' value vlor))
      into v_json_prmtros
      from ma_d_envios_mdio_cnfgrcn_pr
     where id_envio_mdio_cnfgrcion = p_id_envio_mdio_cnfgrcion
       and actvo = 'S';
  ...
  function fnc_vl_valida_confirmacion_envio_programado(p_id_envio_prgrmdo in ma_g_envios_programado.id_envio_prgrmdo%type)
    return varchar2 as
    rt_ma_g_envios_programado ma_g_envios_programado%rowtype;
  begin
    --Consultamos el envio programado
      ...
      when 'DCP' then
        ...
        begin
          ...
          if (rt_ma_g_envios_prgrmdo_cnslta.id_cnslta_mstro is null) then
            ...
            v_sql := pkg_cs_constructorsql.fnc_co_sql_dinamica(p_id_cnslta_mstro => rt_ma_g_envios_prgrmdo_cnslta.id_cnslta_mstro,
                                                               p_cdgo_clnte      => rt_ma_g_envios_programado.cdgo_clnte,
                                                               p_rturn_alias     => 'S');
          end if;
          ...
          for c_medios in (select *
                             from ma_g_envios_programado_mdio
                            where id_envio_prgrmdo =
                                  rt_ma_g_envios_prgrmdo_cnslta.id_envio_prgrmdo
                              and actvo = 'S') loop
            ...
            begin
              select clmna_dstno
                into v_clmna_dstno
                from ma_d_envios_medio_cnfgrcion
               where cdgo_clnte = rt_ma_g_envios_programado.cdgo_clnte
                 and cdgo_envio_mdio = c_medios.cdgo_envio_mdio
                 and actvo = 'S';

              --Recorremos las columnas de la consulta asociada
              for c_columnas in 1 .. v_nmro_clmna loop
                if (upper(v_dscrpcion_tbla(c_columnas).col_name) =
                   v_clmna_dstno) then
                  v_exste_clmna := 'S';
                  continue;
                end if;
