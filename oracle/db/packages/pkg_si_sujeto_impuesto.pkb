  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_SI_SUJETO_IMPUESTO" as
  ...
  procedure prc_rg_personas(p_json         in json_object_t,
    ...
    v_nl        number;
    v_mnsje_log varchar2(4000);
    v_nmbre_up constant varchar2(100) := 'pkg_si_sujeto_impuesto.prc_rg_personas';
    v_cdgo_clnte number;

    -- Variables para los datos del JSON
    ...
    v_sgndo_nmbre             si_i_personas.nmbre_rzon_scial%type;
    v_prmer_aplldo            si_i_personas.nmbre_rzon_scial%type;
    v_sgndo_aplldo            si_i_personas.nmbre_rzon_scial%type;
    ...
    -- Variable para construir el nombre completo
    v_nmbre_cmplto si_i_personas.nmbre_rzon_scial%type;

  begin

    o_cdgo_rspsta := 0; -- 0 = Éxito inicial

    -- No es necesario crear un nuevo objeto JSON, puedes usar el parámetro directamente.
    ...
    v_id_sjto_impsto          := p_json.get_string('id_sjto_impsto');
    v_cdgo_idntfccion_tpo     := p_json.get_string('cdgo_idntfccion_tpo');
    v_tpo_prsna               := p_json.get_string('tpo_prsna');
    v_prmer_nmbre             := p_json.get_string('prmer_nmbre');
    v_sgndo_nmbre             := p_json.get_string('sgndo_nmbre');
    v_prmer_aplldo            := p_json.get_string('prmer_aplldo');
    v_sgndo_aplldo            := p_json.get_string('sgndo_aplldo');
    v_nmbre_rzon_scial        := p_json.get_string('nmbre_rzon_scial');
    v_nmro_rgstro_cmra_cmrcio := p_json.get_string('nmro_rgstro_cmra_cmrcio');
    v_fcha_rgstro_cmra_cmrcio := p_json.get_string('fcha_rgstro_cmra_cmrcio');
    v_fcha_incio_actvddes     := p_json.get_string('fcha_incio_actvddes');
    v_nmro_scrsles            := p_json.get_string('nmro_scrsles');
    v_drccion_cmra_cmrcio     := p_json.get_string('drccion_cmra_cmrcio');
    v_id_sjto_tpo             := p_json.get_string('id_sjto_tpo');
    v_id_actvdad_ecnmca       := p_json.get_string('id_actvdad_ecnmca');

    -- MEJORA: Construir el nombre completo para persona natural una sola vez
    if v_tpo_prsna = 'N' then
      v_nmbre_cmplto := trim(v_prmer_nmbre || case
                               when v_sgndo_nmbre is not null then
                                ' ' || v_sgndo_nmbre
                             end || case
                               when v_prmer_aplldo is not null then
                                ' ' || v_prmer_aplldo
                             end || case
                               when v_sgndo_aplldo is not null then
                                ' ' || v_sgndo_aplldo
                             end);
    else
      v_nmbre_cmplto := v_nmbre_rzon_scial;
    end if;
    ...
    exception
      ...
      when too_many_rows then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := o_cdgo_rspsta || ' - ' ||
                          'Error: Existe más de una persona con el mismo id_sjto_impsto: ' ||
                          v_id_sjto_impsto;
        pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    if v_id_prsna is null then
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'Se registró la persona ' || o_id_prsna ||
                              ' correctamente.',
                              6);
      exception
        when others then
          o_cdgo_rspsta  := 2;
          o_mnsje_rspsta := o_cdgo_rspsta || ' - ' ||
                            'No se pudo registrar la persona. ' || sqlerrm;
          pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
      ...
      if v_tpo_prsna = 'N' then
        ...
        begin
          update si_i_personas
             set cdgo_idntfccion_tpo = v_cdgo_idntfccion_tpo,
                 nmbre_rzon_scial    = v_nmbre_cmplto
           where id_sjto_impsto = v_id_sjto_impsto;
        exception
          when others then
            o_cdgo_rspsta  := 3;
            o_mnsje_rspsta := o_cdgo_rspsta || ' - ' ||
                              'Error al actualizar la persona natural.';
            pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nl,
        ...
        -- Actualiza la persona de tipo jurídica
        begin
          update si_i_personas
             set cdgo_idntfccion_tpo     = v_cdgo_idntfccion_tpo,
                 nmbre_rzon_scial        = v_nmbre_rzon_scial,
                 nmro_rgstro_cmra_cmrcio = v_nmro_rgstro_cmra_cmrcio,
                 fcha_rgstro_cmra_cmrcio = v_fcha_rgstro_cmra_cmrcio,
                 fcha_incio_actvddes     = v_fcha_incio_actvddes,
                 nmro_scrsles            = v_nmro_scrsles,
                 drccion_cmra_cmrcio     = v_drccion_cmra_cmrcio,
                 id_sjto_tpo             = v_id_sjto_tpo,
                 id_actvdad_ecnmca       = v_id_actvdad_ecnmca
           where id_sjto_impsto = v_id_sjto_impsto;
        exception
          when others then
            o_cdgo_rspsta  := 4;
            o_mnsje_rspsta := o_cdgo_rspsta || ' - ' ||
                              'Error al actualizar la persona jurídica.';
            pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nl,
      ...
      pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'Se actualizó la persona ' || o_id_prsna ||
                            ' correctamente.',
                            6);
  ...
  procedure prc_ac_sujeto_responsable_portal(p_cdgo_clnte           in df_s_clientes.cdgo_clnte%type,
                                             p_id_sjto_rspnsble     in si_i_sujetos_responsable.id_sjto_rspnsble%type,
                                             p_id_sjto_impsto       in si_i_sujetos_responsable.id_sjto_impsto%type,
                                             p_cdgo_idntfccion_tpo  in si_i_sujetos_responsable.cdgo_idntfccion_tpo%type,
                                             p_idntfccion           in si_i_sujetos_responsable.idntfccion%type,
                                             p_prmer_nmbre          in si_i_sujetos_responsable.prmer_nmbre%type default null,
                                             p_sgndo_nmbre          in si_i_sujetos_responsable.sgndo_nmbre%type default null,
                                             p_prmer_aplldo         in si_i_sujetos_responsable.prmer_aplldo%type default null,
                                             p_sgndo_aplldo         in si_i_sujetos_responsable.sgndo_aplldo%type default null,
                                             p_prncpal_s_n          in si_i_sujetos_responsable.prncpal_s_n%type default null,
                                             p_cdgo_tpo_rspnsble    in si_i_sujetos_responsable.cdgo_tpo_rspnsble%type,
                                             p_prcntje_prtcpcion    in si_i_sujetos_responsable.prcntje_prtcpcion%type default null,
                                             p_orgen_dcmnto         in si_i_sujetos_responsable.orgen_dcmnto%type default null,
                                             p_id_pais_ntfccion     in si_i_sujetos_responsable.id_pais_ntfccion%type default null,
                                             p_id_dprtmnto_ntfccion in si_i_sujetos_responsable.id_dprtmnto_ntfccion%type default null,
                                             p_id_mncpio_ntfccion   in si_i_sujetos_responsable.id_mncpio_ntfccion%type default null,
                                             p_drccion_ntfccion     in si_i_sujetos_responsable.drccion_ntfccion%type default null,
                                             p_email                in si_i_sujetos_responsable.email%type,
                                             p_tlfno                in si_i_sujetos_responsable.tlfno%type,
                                             p_cllar                in si_i_sujetos_responsable.cllar%type,
                                             p_actvo                in si_i_sujetos_responsable.actvo%type default null,
                                             p_id_trcro             in si_i_sujetos_responsable.id_trcro%type default null,
                                             p_indcdor_mgrdo        in si_i_sujetos_responsable.indcdor_mgrdo%type default 'N',
                                             p_indcdor_cntrbynte    in si_c_terceros.indcdor_cntrbynte%type default 'N',
                                             p_indcdr_fncnrio       in si_c_terceros.indcdr_fncnrio%type default 'N',
    ...
    v_id_sjto_rspnsble    si_i_sujetos_responsable.id_sjto_rspnsble%type;
    v_orgen_dcmnto        varchar2(100) := '1';
    nmbre_up              varchar2(200) := 'pkg_si_sujeto_impuesto.prc_ac_sujeto_responsable_portal';
    v_nl                  number;
    v_cllar               number;
    v_tlfno               varchar2(20);
    v_email               varchar2(100);
    v_drccion_ntfccion    varchar2(100);
    v_cdgo_tpo_rspnsble   varchar2(3);
    v_prncpal_s_n         varchar2(1);
    v_id_nvdad_sjto       number;
    v_id_nvdad_dtlle      number;
    v_sjt_prncpal         number;
    v_id_sjto_rspnsble_pl number;
  ...
  begin
    ...
    if p_accion = 'I' then
      ...
      if v_sjt_prncpal > 0 and p_prncpal_s_n = 'S' then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := 'Error al actualizar registro, ya se encuentra un sujeto responsable como principal asociado al sujeto impuesto: ' ||
                          sqlerrm;
        rollback;
        return;
      ...
      begin
        ...
        insert into si_g_novedad_sjto_rspnsble
          (cdgo_nvdad,
           id_sjto_impsto,
           id_sjto_rspnsble_creado,
           id_usrio,
           fcha_rgstro)
        values
          (p_accion,
      ...
      exception
        when others then
          o_cdgo_rspsta  := 25;
          o_mnsje_rspsta := 'Error al insertar la novedad de insercciond e sujeto responsable: ' ||
                            sqlerrm;
          rollback;
          return;
      end;
      ...
      begin
        -- Buscar el responsable principal, si existe
        select id_sjto_rspnsble
          into v_id_sjto_rspnsble_pl
          from si_i_sujetos_responsable
         where id_sjto_impsto = p_id_sjto_impsto
           and prncpal_s_n = 'S'
           and rownum = 1;

        if p_prncpal_s_n = 'S' and
           p_id_sjto_rspnsble <> v_id_sjto_rspnsble_pl then
          o_cdgo_rspsta  := 28;
          o_mnsje_rspsta := 'Error al actualizar registro, ya se encuentra un sujeto responsable como principal asociado al sujeto impuesto.';
          rollback;
          return;
      ...
      exception
        when no_data_found then
          null; -- No hay principal registrado, se permite continuar
      end;
      ...
      select id_trcro
        ...
        from si_i_sujetos_responsable
       where id_sjto_impsto = p_id_sjto_impsto
         and id_sjto_rspnsble = p_id_sjto_rspnsble;
      ...
      begin
        ...
        values
          (p_accion,
           ...
           p_id_usrio,
           systimestamp)
        returning id_nvdad_sjto into v_id_nvdad_sjto;
      exception
        when others then
          o_cdgo_rspsta  := 30;
          o_mnsje_rspsta := 'Error al insertar la novedad de inserccion de sujeto responsable: ' ||
                            sqlerrm;
          rollback;
          return;
      end;
      ...
      begin
        ...
        insert into si_g_novedad_sjto_dtlle
          (id_nvdad_sjto, fcha_rgstro)
        values
          (v_id_nvdad_sjto, systimestamp)
        returning id_nvdad_dtlle into v_id_nvdad_dtlle;
      exception
        when others then
          o_cdgo_rspsta  := 40;
          o_mnsje_rspsta := 'Error al insertar la novedad de inserccion detalle de novedad: ' ||
                            sqlerrm;
          rollback;
          return;
      end;
      ...
      begin
        ...
        if v_id_trcro is not null then
          ...
          begin
            update si_c_terceros
               set drccion          = p_drccion_ntfccion,
                   drccion_ntfccion = p_drccion_ntfccion,
                   email            = p_email,
                   tlfno            = p_tlfno,
                   cllar            = p_cllar
        ...
        exception
          when others then
            o_cdgo_rspsta  := 60;
            o_mnsje_rspsta := 'Error al actualizar el responsable: ' ||
                              sqlerrm;
            rollback;
            return;
        end;
        ...
        begin
          update si_g_novedad_sjto_dtlle
             set prncpal_s_n_nvo       = p_prncpal_s_n,
                 prncpal_s_n_ant       = v_prncpal_s_n,
                 cdgo_tpo_rspnsble_nvo = p_cdgo_tpo_rspnsble,
                 cdgo_tpo_rspnsble_ant = v_cdgo_tpo_rspnsble,
                 drcc_nvo              = p_drccion_ntfccion,
                 drcc_ant              = v_drccion_ntfccion,
                 email_nvo             = p_email,
                 email_ant             = v_email,
                 tlfno_nvo             = p_tlfno,
                 tlfno_ant             = v_tlfno,
                 cllar_nvo             = p_cllar,
                 cllar_ant             = v_cllar
           where id_nvdad_dtlle = v_id_nvdad_dtlle;
        exception
          when others then
            o_cdgo_rspsta  := 70;
            o_mnsje_rspsta := 'Error al actualizar el responsable: ' ||
  ...
  exception
    when others then
      o_cdgo_rspsta  := 999;
      o_mnsje_rspsta := 'Error general en el procedimiento: ' || sqlerrm;
      rollback;

  end prc_ac_sujeto_responsable_portal;
