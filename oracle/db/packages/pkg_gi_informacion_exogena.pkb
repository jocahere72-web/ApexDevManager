  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GI_INFORMACION_EXOGENA" as
  ...
  begin
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Error al intentar obtener la informacion de archivo exogena.' ||
                          ' , ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              nmbre_up,
      ...
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al consultar la informacion de archivo exogena.' ||
                          ' , ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              nmbre_up,
    ...
    if v_gi_g_informacion_exogena.indcdor_prcsdo = 'S' then
      o_cdgo_rspsta  := 2;
      o_mnsje_rspsta := 'El archivo que intenta procesar ya se encuentra procesado ' ||
                        sqlerrm;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            nmbre_up,
    ...
    v_extnsion_archvo := substr(v_gi_g_informacion_exogena.file_name,
                                (instr(v_gi_g_informacion_exogena.file_name,
                                       '.',
                                       -3) + 1),
                                instr(v_gi_g_informacion_exogena.file_name,
                                      '.',
    ...
    --hallar nombre, encabezado y cantidad de hojas del tipo de archivo EXCEL para darle procesamiento a la informacion
    begin
    ...
    if v_extnsion_archvo = 'txt' then
      ...
      begin
          ...
          --hallar  encabezado y id_carga ETL del tipo de archivo para darle procesamiento a la informacion
          begin
                                                                                                                        ...
                                                                                                                        if( c_exgna.tpo_rgstro = 'C'  and v_prmtir_prcsar = 'S'  ) then
                                                                                                                            ...
                                                                                                                            exception
                                                                                                                                when no_data_found then
                                                                                                                                    ...
                                                                                                                                    o_cdgo_rspsta   := 6;
                                                                                                                                    o_mnsje_rspsta := 'Codigo identificacion tipo no homologado: '|| v_cdgo_idntfccion_tpo||', '||sqlerrm;
                                                                                                                                    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, nmbre_up,  v_nl, o_mnsje_rspsta, 6);
                                                                                                                                when others then
                                                                                                                                    v_cdgo_idntfccion_tpo := 'X';
                                                                                                                                    o_cdgo_rspsta := 6;
                                                                                                                            ...
                                                                                                                            exception
                                                                                                                                when others then
                                                                                                                                    rollback;
                                                                                                                                    o_cdgo_rspsta := 7;
                                                                                                                                    o_mnsje_rspsta := 'Error al ingresar informacion principal.'||sqlerrm;
                                                                                                                                    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, nmbre_up,  v_nl, o_mnsje_rspsta, 6);
                                                                                                                                    return;
                                                                                                                            end;
                                                                                                                        continue;
      ...
      begin
        ...
        for i in 1 .. v_cant_hojas loop
          ...
          for c_datos in (select line_number,
                                 col001,
                                 col002,
                                 col003,
                                 col004,
                                 ...
                                 col015
                            from gi_g_informacion_exogena a
                           cross join table(apex_data_parser.parse(p_content => a.file_blob, p_file_name => a.file_name, p_xlsx_sheet_name => v_hoja, p_skip_rows => v_lneas_encbzdo))
                           where a.id_infrmcion_exgna = p_id_infrmcion_exgna
                             and col001 is not null) loop
            if v_nmbre_archvo = 'Archivo 1' then
              --or (v_nmbre_archvo = 'Archivo 1' and v_hoja = 'sheet1.xml')then
              if c_datos.col001 is not null then
                insert into gi_g_exogena_retenciones
                ...
                values
                  ('RP',
                   ...
                   c_datos.col003,
                   c_datos.col007,
                   ROUND(TO_NUMBER(REPLACE(c_datos.col009, ',', '.'))),
                   TO_NUMBER(REPLACE(c_datos.col010, ',', '.')),
                   ROUND(TO_NUMBER(REPLACE(c_datos.col011, ',', '.'))),
                   c_datos.col012,
                   c_datos.col013,
                   p_id_infrmcion_exgna);
              else
            ...
            elsif v_nmbre_archvo = 'Archivo 2' then
              --or (v_nmbre_archvo = 'Archivo 1' and v_hoja = 'sheet2.xml') then
              if c_datos.col001 is not null then
                insert into gi_g_exogena_retenciones
                  (cdgo_exgna_tpo_rgstro,
                   idntfccion,
            ...
            elsif v_nmbre_archvo = 'Archivo 3' then
              --or (v_nmbre_archvo = 'Archivo 1' and v_hoja = 'sheet3.xml') then
              v_pnta_stio := 'N';
              if c_datos.col009 = 'SI' then
                v_pnta_stio := 'S';
              end if;
              if c_datos.col001 is not null then
                insert into gi_g_exogena_ingr_fra_muni
                  (dscrpcion_dprmnto,
                   dscrpcion_mncpio,
                   vlor,
                   plnta_sitio,
                ...
                values
                  (c_datos.col001,
                   c_datos.col003,
                   ROUND(TO_NUMBER(REPLACE(c_datos.col007, ',', '.'))),
                   v_pnta_stio,
                   p_id_infrmcion_exgna);
              else
                continue;
              end if;
            elsif v_nmbre_archvo = 'Archivo 4' then
              --or (v_nmbre_archvo = 'Archivo 1' and v_hoja = 'sheet4.xml') then
              if c_datos.col001 is not null then
      ...
      begin
        ...
        for i in 1 .. v_cant_hojas loop
          ...
          for c_datos in (select line_number,
                           ...
                           where a.id_infrmcion_exgna = p_id_infrmcion_exgna
                             and col001 is not null) loop
            if v_nmbre_archvo = 'Archivo ICA' and v_hoja = 'sheet1.xml' then
              if c_datos.col001 is not null then
                insert into gi_g_exogena_datos_princpal
                  (tpo_cntrbynte,
                   vgncia,
                   cdgo_idntfccion_tpo,
                   idntfccion,
                ...
                values
                  (c_datos.col001,
                   c_datos.col002,
                   case when c_datos.col003 = 'C.C.' then 'C' when
                   c_datos.col003 = 'NIT' then 'N' when
                   c_datos.col003 = 'C.E.' then 'E' when
                   c_datos.col003 = 'T.I' then 'T' when
                   c_datos.col003 = 'PASAPORTE' then 'P' end,
                   c_datos.col004,
                   c_datos.col005,
                   c_datos.col006,
                   c_datos.col007,
                   TRANSLATE(c_datos.col008,
                             '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/',
                             '0123456789'),
                   TRANSLATE(c_datos.col009,
                             '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/',
                             '0123456789'),
                   c_datos.col010,
                   REGEXP_REPLACE(c_datos.col011, '[^0-9]', ''),
                   nvl(c_datos.col012, 0),
                   p_id_infrmcion_exgna);
              else
                continue;
              end if;
            elsif v_nmbre_archvo = 'Archivo ICA' and v_hoja = 'sheet2.xml' then
              if c_datos.col001 is not null then
                insert into gi_g_exogena_retenciones
                  (cdgo_exgna_tpo_rgstro,
                   idntfccion,
                ...
                values
                  ('RP',
                   ...
                   c_datos.col002,
                   c_datos.col003,
                   ROUND(TO_NUMBER(REPLACE(c_datos.col004, ',', '.')))
                   --,c_datos.col005
                  ,
                   to_number(replace(c_datos.col005, ',', '.')),
                   ROUND(TO_NUMBER(REPLACE(c_datos.col006, ',', '.'))),
                   c_datos.col007,
                   c_datos.col008,
                   c_datos.col009,
                   p_id_infrmcion_exgna);
            ...
            elsif v_nmbre_archvo = 'Archivo ICA' and v_hoja = 'sheet3.xml' then
              if c_datos.col001 is not null then
                insert into gi_g_exogena_retenciones
                  (cdgo_exgna_tpo_rgstro,
                   idntfccion,
            ...
            elsif v_nmbre_archvo = 'Archivo ICA' and v_hoja = 'sheet4.xml' then
              v_pnta_stio := 'N';
              if c_datos.col004 = 'SI' then
                v_pnta_stio := 'S';
              end if;
              if c_datos.col001 is not null then
                insert into gi_g_exogena_ingr_dntr_muni
                  (dscrpcion_dprmnto,
                   dscrpcion_mncpio,
                   vlor,
                   plnta_sitio,
                ...
                values
                  (c_datos.col001,
                   c_datos.col002,
                   ROUND(TO_NUMBER(REPLACE(c_datos.col003, ',', '.'))),
                   v_pnta_stio,
                   c_datos.col006,
                   p_id_infrmcion_exgna);
              else
            ...
            elsif v_nmbre_archvo = 'Archivo ICA' and v_hoja = 'sheet5.xml' then
              v_pnta_stio := 'N';
              if c_datos.col004 = 'SI' then
                v_pnta_stio := 'S';
              end if;
    ...
    update gi_g_informacion_exogena
       set indcdor_prcsdo = 'S', mnsje_error = ''
     where id_infrmcion_exgna = p_id_infrmcion_exgna;

    commit;

    o_mnsje_rspsta := 'Actualizar indicador procesado para: ' ||
                      p_id_infrmcion_exgna;
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          nmbre_up,
  ...
  exception
    when others then
      ...
      o_mnsje_rspsta := 'Ha ocurrido un error al intentar registrar informacion exogena. ' ||
                        sqlerrm;
      update gi_g_informacion_exogena
         set indcdor_prcsdo = 'N', mnsje_error = o_mnsje_rspsta
       where id_infrmcion_exgna = p_id_infrmcion_exgna;
      commit;
      o_cdgo_rspsta := 99;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
  ...
  function fnc_vl_dato_numerico(v_vlor in varchar2) return varchar2 is
    v_number number;
  begin
    if REGEXP_LIKE(TRIM(v_vlor), '^[0-9]+$') then
      return 'S';
    else
      return 'N';
    end if;
  ...
  -- Funcion para validar datos numericos decimales
  function fnc_vl_dato_numerico_decimal(v_vlor in varchar2) return varchar2 is
    v_number number;
  begin
    if REGEXP_LIKE(TRIM(v_vlor), '^[0-9]+(\.[0-9]{1,2})?$') then
      return 'S';
    else
      return 'N';
    end if;
  exception
    when others then
      return 'N';
  end fnc_vl_dato_numerico_decimal;

  -- Funcion para validar datos alfabeticos
  function fnc_vl_dato_alfabetico(v_valor in varchar2) return varchar2 is
  begin
    if REGEXP_LIKE(TRIM(v_valor), '^[[:alpha:]ÁÉÍÓÚÜÑáéíóúüñ ]+$') then
      return 'S';
    else
      return 'N';
    end if;
  exception
    when others then
      return 'N';
  end fnc_vl_dato_alfabetico;

  function fnc_vl_dato_alfanumerico(v_valor in varchar2) return varchar2 is
  begin
    if REGEXP_LIKE(v_valor,
                   '^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9 \s \. \- \– \— \&]+$') then
      return 'S';
    else
      return 'N';
    end if;
  exception
    when others then
      return 'N';
  end fnc_vl_dato_alfanumerico;

  -- Funcion para validar estructura de email
  function fnc_vl_email(v_email in varchar2) return varchar2 is
  begin
  ...
  function fnc_vl_fecha(v_fecha in varchar2) return varchar2 is
    v_fcha date;
  begin
    if regexp_like(v_fecha,
                   '^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$') then
      begin
        v_fcha := to_date(v_fecha, 'YYYY-MM-DD');
        return 'S';
      exception
        when others then
          return 'N';
  ...
  procedure prc_rg_informacion_exogena_dtlle(p_sql in varchar2) as
    o_mnsje_rspsta varchar2(4000);
  begin
    begin
      insert into muerto (n_001, c_001) values (5, p_sql);
      EXECUTE IMMEDIATE p_sql;
    EXCEPTION
      WHEN OTHERS THEN
        insert into muerto (n_001, c_001) values (5, p_sql);
        o_mnsje_rspsta := 'Error' || ' , ' || sqlerrm;
        insert into muerto (v_001) values (' Error: ' || o_mnsje_rspsta);

    END;
  end prc_rg_informacion_exogena_dtlle;
