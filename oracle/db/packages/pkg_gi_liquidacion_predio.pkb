  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GI_LIQUIDACION_PREDIO" as
  ...
  */
  ...
  begin
    ...
    if v_cncptos = 0 then
      o_cdgo_rspsta  := 110;
      o_mnsje_rspsta := 'Liquidacion sin conceptos parametrizados para la vigencia[' ||
                        v_vgncia || ']';
      pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                            p_id_impsto  => null,
                            p_nmbre_up   => v_nmbre_up,
  ...
  */
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          ' p_id_impsto: ' || p_id_impsto ||
                          ' p_id_impsto_sbmpsto: ' || p_id_impsto_sbmpsto ||
                          ' p_vgncia: ' || p_vgncia || ' p_bse: ' || p_bse ||
                          ' p_area_trrno: ' || p_area_trrno ||
                          ' p_area_cnstrda: ' || p_area_cnstrda ||
                          ' p_cdgo_prdio_clsfccion: ' ||
                          p_cdgo_prdio_clsfccion || ' p_id_prdio_dstno: ' ||
                          p_id_prdio_dstno || ' p_id_prdio_uso_slo: ' ||
                          p_id_prdio_uso_slo || ' p_cdgo_estrto: ' ||
                          p_cdgo_estrto || ' p_id_obra: ' || p_id_obra,
                          1);

    if p_vgncia < v_vgncia then
      begin

        select id_dstno_antrior
      ...
      exception
        when others then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                6,
                                'Destino anterior para: ' ||
                                p_id_prdio_dstno || ' no encontrado.',
                                3);
      end;
    end if;
  ...
  */
  ...
  procedure prc_vl_limite_impuesto(p_cdgo_clnte           in df_s_clientes.cdgo_clnte%type,
                                   ...
                                   p_trfa                 in gi_g_liquidaciones_concepto.trfa%type,
    ...
    v_vgncia_actlzda_prdio        number; -- Req. 0026119
    v_id_prcso_crga_t6            number; -- Req. 0026119

    v_trfa_ant gi_g_liquidaciones_concepto.trfa%type;
  begin
    ...
    begin
      select /*+ RESULT_CACHE */
       b.vlor_lqddo, a.area_cnsctrda, b.id_lqdcion, a.area_trrno, b.trfa -- ley 1995
        into v_vlor_lqddo,
             v_area_cnstrda_ant,
             v_id_lqdcion,
             v_area_trrno_ant,
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            ' v_area_cnstrda_ant: ' || v_area_cnstrda_ant,
                            1);
    ...
    exception
      ...
      when others then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'No. ' || o_cdgo_rspsta ||
                          ' Excepcion controlada liq. anterior: ' ||
                          sqlerrm;
        return;
    end;
    ...
    exception
      when others then
        o_cdgo_rspsta  := 25;
        o_mnsje_rspsta := 'No ha indicador SMLMV parametrizado para la vigencia[' ||
                          p_vgncia || ']';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    -- Req. 0026119. Busca la vigencia en la que se actualizó catastralmente el predio
    begin
      select vgncia_actlzda, decode(vgncia_actlzda, null, 'N', 'S')
        into v_vgncia_actlzda_prdio, v_prdio_actlzdo
        from si_i_predios
       where id_sjto_impsto = p_id_sjto_impsto;
    ...
    exception
      when others then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'Error al consultar vigencia del predio: ' ||
                              p_idntfccion,
                              1);
        return;
    end;
    ...
    exception
      when others then
        select max(id_prcso_crga)
          into v_id_prcso_crga_t6
          from si_g_novedades_predial_t6;

        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              'v_id_prcso_crga_t6(max): ' ||
                              v_id_prcso_crga_t6,
                              1);
    end;

    v_json_cnddto := '{"P_BSE" :"' || p_bse || '"' || ',"P_VLOR_LQDDO" :"' ||
                     v_vlor_lqddo || '"' || ',"P_VLOR_CLCLDO" :"' ||
                     p_vlor_clcldo || '"' || ',"P_ID_PRDIO_DSTNO" :"' ||
                     p_id_prdio_dstno || '"' || ',"P_VGNCIA" :"' ||
                     p_vgncia || '"' || ',"P_CDGO_ESTRTO" :"' ||
                     p_cdgo_estrto || '"' || ',"P_CNTDAD_SLRIO" :"' ||
                     v_cntdad_slrio || '"' || ',"P_VLOR_SMMLV" :"' ||
                     v_vlor_SMMLV || '"' || ',"P_PRCNTJE" :"' || v_prcntje || '"' ||
                    ...
                    --',"P_LQDCION_PGDA" :"'        || v_lqdcion_pgda           || '"' ||
                     ...
                     ',"P_AREA_CNSTRDA_ANT" :"' || v_area_cnstrda_ant || '"' ||
                     ',"P_ID_SJTO_IMPSTO" :"' || p_id_sjto_impsto || '"' ||
                     ',"P_VGNCIA_PRDIO_ACT" :"' || v_vgncia_actlzda_prdio ||
                     '" }';
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'v_json_cnddto: ' || v_json_cnddto,
                          1);

    -- Req. 0024205. Busca la novedad de priorizacion de liquidacion
    -- Req. Valida novedad de acuerdo a la vignecia que se liquida y del predio actualizado
    if v_vgncia_actlzda_prdio is not null then
      begin
        select b.* --a.cdgo_nvdad
          into v_df_c_novdad_prrzcion_predio
          from si_g_novedades_predial_t6 a
          join df_c_novdad_prrzcion_predio b
            on a.cdgo_nvdad = b.cdgo_nvdad
         where rfrncia_igac = p_idntfccion
           and id_prcso_crga = v_id_prcso_crga_t6 -- select max(id_prcso_crga) from si_g_novedades_predial_t6 )
           and vgncia_lqda = p_vgncia
           and vgncia_prdio = v_vgncia_actlzda_prdio
           and rownum = 1;
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'v_cdgo_nvdad_prrzcion vigencias: ' ||
                              v_cdgo_nvdad_prrzcion,
                              1);
        dbms_output.put_line('v_cdgo_nvdad_prrzcion_1: ' ||
                             v_df_c_novdad_prrzcion_predio.PRCNTJE_LMTE_IMPSTO_2);
      exception
        /*when no_data_found then
        begin
            select  b.* --a.cdgo_nvdad
            ...
            where   rfrncia_igac = p_idntfccion
            and     id_prcso_crga = v_id_prcso_crga_t6--( select max(id_prcso_crga) from si_g_novedades_predial_t6 )
            and     vgncia_lqda is null
            and     vgncia_prdio is null
            and     rownum = 1 ;

            v_cdgo_nvdad_prrzcion := v_df_c_novdad_prrzcion_predio.cdgo_nvdad;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'v_cdgo_nvdad_prrzcion: '||v_cdgo_nvdad_prrzcion, 1);
            dbms_output.put_line('v_cdgo_nvdad_prrzcion_2: '||v_df_c_novdad_prrzcion_predio.PRCNTJE_LMTE_IMPSTO_2);
        exception
            when others then
                v_cdgo_nvdad_prrzcion := 99 ; -- No vino en la cinta, se valida el limite del impuesto igual que la vigencia 2023
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'v_cdgo_nvdad_prrzcion1(99): '||v_cdgo_nvdad_prrzcion, 1);
                dbms_output.put_line('v_cdgo_nvdad_prrzcion_3: '||v_cdgo_nvdad_prrzcion);
        end;*/
        when others then
          v_cdgo_nvdad_prrzcion := 24; -- Predio no actualizad
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
                                'v_cdgo_nvdad_prrzcion2(99): ' ||
                                v_cdgo_nvdad_prrzcion,
                                1);
          dbms_output.put_line('v_cdgo_nvdad_prrzcion_4: ' ||
                               v_cdgo_nvdad_prrzcion);
      end;

    else
      v_cdgo_nvdad_prrzcion := 24;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
                            'Predio NO actualizado',
                            1);
    end if;

    --if v_cdgo_nvdad_prrzcion != 99 then
    if (v_cdgo_nvdad_prrzcion not in (24, 99)) then
      --16/02/2024
      -- JSOn para validar novedades
      v_json_gnral := '{"P_CDGO_NVDAD" :"' ||
                      v_df_c_novdad_prrzcion_predio.cdgo_nvdad || '"' ||
                      ',"P_ID_SJTO_IMPSTO" :"' || p_id_sjto_impsto || '"' ||
                      ',"P_CDGO_CLNTE" : "' || p_cdgo_clnte || '"' ||
                      ',"P_ID_IMPSTO" : "' || p_id_impsto || '"' ||
                      ',"P_ID_IMPSTO_SBMPSTO" :"' || p_id_impsto_sbmpsto || '"' ||
                      ',"P_VGNCIA" :"' || p_vgncia || '"' || ',"P_BSE" :"' ||
                      p_bse || '"' || ',"P_CDGO_PRDIO_CLSFCCION" :"' ||
                      p_cdgo_prdio_clsfccion || '"' ||
                      ',"P_ID_PRDIO_DSTNO" :"' || p_id_prdio_dstno || '"' ||
                      ',"P_IDNTFCCION" :"' || p_idntfccion || '"' ||
                      ',"P_ID_PRDO" :"' || p_id_prdo || '"' ||
      ...
      if (v_lmte_impsto_clclar = 1) then
        --Ley
        v_fncion_ejectar := v_df_c_novdad_prrzcion_predio.lmte_impsto_1;
        v_json_gnral     := v_json_gnral || ',"P_PRCNTJE" :"' ||
                            v_df_c_novdad_prrzcion_predio.prcntje_lmte_impsto_1 ||
                            '" }';
      else
      ...
      ***/
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'Ingresó con prioridad 24 o no especificada a buscar los límites de ley. Para el predio: ' ||
                            p_idntfccion,
                            1);
      ...
      for c_lyes in (select *
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'entra al for: ' || c_lyes.id_rgla_exncion,
                              1);
        ...
        if v_indcdor_lmta_impsto = 'N' then
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nl,
                                'No esta exento. Validara candidato: ' ||
                                c_lyes.nmbre_rgla_cnddto,
                                1);
          ...
          --Ejecutamos la UP
          begin
            dbms_output.put_line('Ejecutando: ' || v_bloque_pl);
            execute immediate v_bloque_pl
  ...
  */
  ...
  begin
    ...
    -- Req.0028158. Actualiza los estratos a los predios con novedades de cambio de estrato de la
    -- vigencia anterior
    begin
      pkg_gi_liquidacion_predio.prc_ac_estratos_novedades(p_cdgo_clnte   => p_cdgo_clnte,
                                                          p_vgncia       => v_vgncia,
                                                          o_cdgo_rspsta  => v_cdgo_rspsta,
    ...
    exception
      when others then
        raise_application_error(-20001,
                                'Error al actualizar estratos de Novedades de Cambio de Estrato vigencia anterior');
    end;
    ...
    for c_prdio in (select /*+ RESULT_CACHE */
                    ...
                    --  and a.rfrncia_igac in ('0106000013090009000000000')
                    /*   and not exists
                    (select 1
                             from si_g_novedades_predial_t6 b
                            where b.rfrncia_igac = a.rfrncia_igac
                              and b.id_prcso_crga =
                                  (select max(id_prcso_crga)
                                     from si_g_novedades_predial_t6)
                              and b.cdgo_nvdad = 5) */ -- que no tenga en cuenta los que CANCELA
                    ) loop
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'EN EL FOR ' || c_prdio.RFRNCIA_IGAC,
                            1);
  ...
  */
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'INICIO BORRADO et_g_procesos_carga_error',
                          3);
    --Se Borra las Inconsistencia de la Preliquidacion
    delete et_g_procesos_carga_error
     where id_prcso_crga = p_id_prcso_crga
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'FIN BORRADO et_g_procesos_carga_error',
                          3);
    ...
    */
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'INICIO BORRADO gi_g_liquidaciones_concepto',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'FIN BORRADO gi_g_liquidaciones_concepto',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'INICIO BORRADO gi_g_liquidaciones_ad_predio',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'FIN BORRADO gi_g_liquidaciones_ad_predio',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'INICIA BORRADO gi_g_liquidaciones',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'FIN BORRADO gi_g_liquidaciones',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'INICA ACTUALZIACION gi_g_cinta_igac',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'FIN ACTUALZIACION gi_g_cinta_igac',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'INICIA BORRADO si_i_sujetos_rspnsble_hstrco',
                          3);
    ...
    pkg_sg_log.prc_rg_log(23001,
                          ...
                          v_nmbre_up,
                          6,
                          'FIN BORRADO si_i_sujetos_rspnsble_hstrco',
                          3);
    /*
  ...
  */
  ...
  begin
    ...
    if v_cdgo_nvdad = 0 then
      --Predio_Nuevo

      begin
        select decode(extract(year from FCHA_RGSTRO),
    ...
    elsif v_cdgo_nvdad = 19 then
      -- Destino_Economico_Lote_SinCambio
      ...
      select count(1)
        ...
        join gi_d_limites_destino b
         ...
         and a.id_prdio_dstno = b.id_prdio_dstno
         and to_date('01/01/' || json_value(p_xml, '$.P_VGNCIA'),
                     'DD/MM/RR') between trunc(fcha_incial) and
             trunc(fcha_fnal)
       where a.id_prdio_dstno = json_value(p_xml, '$.P_ID_PRDIO_DSTNO');

      if (v_count_dstno > 0 and (json_value(p_xml, '$.P_AREA_CNSTRDA') =
         nvl(v_area_cnstrda_ant, 0)) and
    ...
    elsif v_cdgo_nvdad = 20 then
      --Cambio_DestinoEconomico_AntesLote_AhoraNoLote
    ...
    elsif v_cdgo_nvdad = 21 then
      -- Cambio_DestinoEconomico_Area
      ...
      begin
        ...
        if ((v_id_prdio_dstno_ant !=
           json_value(p_xml, '$.P_ID_PRDIO_DSTNO')) or
    ...
    elsif v_cdgo_nvdad = 22 then
      --Predios_Rurales_Mayores100_Ha
    ...
    elsif v_cdgo_nvdad = 23 then
      -- Estrato1y2_AvaluoMenor_135SMLV
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nl,
                            'v_cntdad_slrio: ' || v_cntdad_slrio,
                            3);
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
                            'v_vlor_SMMLV: ' || v_vlor_SMMLV,
                            3);
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(23001,
                          null,
                          'pkg_gi_liquidacion_predio.prc_cl_limite_impuesto_pleno',
                          6,
                          'v_vlor_clcddo:' ||
                          json_value(p_xml, '$.P_VLOR_CLCLDO'),
                          3);
  ...
  begin
    ...
    v_cntdad_pntos := json_value(p_xml, '$.P_PRCNTJE'); --Req. 0026119
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'limita pntos_ipc - v_vlor_mxmo_incrmnto:' ||
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'limita porcentaje ipc - v_vlor_mxmo_incrmnto:' ||
                          v_vlor_mxmo_incrmnto || ' -v_vlor_clcddo: ' ||
                          v_vlor_clcddo,
                          3);
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'limita porcentaje - v_vlor_mxmo_incrmnto:' ||
                          v_vlor_mxmo_incrmnto || ' -v_vlor_clcddo: ' ||
                          v_vlor_clcddo,
                          3);
  ...
  -- req. 0026119
  procedure prc_cl_limite_impuesto_cantidad(p_xml         in clob,
                                            o_lmta_impsto out varchar2,
                                            o_vlor_lqddo  out number) is
    v_vlor_clcddo        number;
    v_prcntje            number;
  ...
  begin
    ...
    v_vlor_mxmo_incrmnto := json_value(p_xml, '$.P_VLOR_LQDDO') * v_prcntje;

    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          null,
                          v_nmbre_up,
                          v_nl,
                          'limita cantidad - v_vlor_mxmo_incrmnto:' ||
                          v_vlor_mxmo_incrmnto || ' -v_vlor_clcddo: ' ||
                          v_vlor_clcddo,
                          3);
  ...
  end prc_cl_limite_impuesto_cantidad;

  procedure prc_rv_preliquidacion_job as
  ...
  exception
    when others then
      pkg_sg_log.prc_rg_log(23001,
                            ...
                            v_nmbre_up,
                            6,
                            'Error prc_rv_preliquidacion_JOB: ' || sqlerrm,
                            3);
  ...
  -- Req.0028158. Actualiza los estratos a los predios con novedades de cambio de estrato de la
  -- vigencia anterior
  procedure prc_ac_estratos_novedades(p_cdgo_clnte   number,
                                      p_vgncia       number,
                                      o_cdgo_rspsta  out number,
                                      o_mnsje_rspsta out varchar2) as
    v_nmbre_up varchar2(100) := 'pkg_gi_liquidacion_predio.prc_ac_estratos_novedades';
    v_nvel     number;
    v_mnsje    clob;
  begin
    ...
    for c_estrtos in (select a.*, b.idntfccion_sjto
                        from si_g_novedades_prdio_estrto a
                        join v_si_i_sujetos_impuesto b
                          on a.id_sjto_impsto = b.id_sjto_impsto
                       where vgncia = p_vgncia
                         and indcdor_actlzdo = 'N'
                       order by a.fcha_rgstro) loop
      begin
        update si_i_predios
           set cdgo_estrto = c_estrtos.cdgo_estrto
         where id_sjto_impsto = c_estrtos.id_sjto_impsto;

        update si_g_novedades_prdio_estrto
           set indcdor_actlzdo = 'S'
         where id_nvd_prd_est = c_estrtos.id_nvd_prd_est;
      ...
      exception
        when others then
          rollback;
          v_mnsje := 'No se pudo actualizar el estrato - ' || sqlerrm;
          update si_g_novedades_prdio_estrto
             set obsrvcion_actlzdo = v_mnsje
           where id_nvd_prd_est = c_estrtos.id_nvd_prd_est;
          commit;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
  ...
  exception
    when others then
      rollback;
      o_cdgo_rspsta  := 99;
      o_mnsje_rspsta := 'Error al actualizar estratos de novedades vigencia[' ||
                        p_vgncia - 1 || ']: ' || sqlerrm;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
