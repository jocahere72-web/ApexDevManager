  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GN_GENERALIDADES" as
  ...
  begin
    ...
    v_acto_vlor_ttal := nvl(v_acto_vlor_ttal, 0);
  ...
  function fnc_cl_convertir_bfile_a_base64(p_bfile bfile) return clob is
    -- !! -------------------------------------------- !! --
    -- !! funcion combierte un bfile a cadena (base 64) !! --
    -- !! -------------------------------------------- !! --
    v_blob  blob;
    v_bfile bfile;
  begin
    ...
    v_bfile := p_bfile;
    -- cargar el bfile en un blob
    dbms_lob.createtemporary(v_blob, true); -- crear un blob temporal
    dbms_lob.open(v_bfile, dbms_lob.lob_readonly); -- abrir el bfile en solo lectura
    dbms_lob.loadfromfile(v_blob, v_bfile, dbms_lob.getlength(v_bfile)); -- cargar el bfile en el blob
    dbms_lob.close(v_bfile); -- cerrar el bfile

    return fnc_cl_convertir_blob_a_base64(v_blob);
  end fnc_cl_convertir_bfile_a_base64;
  ...
  begin
    ...
    if v_bncos_rcddres is not null then

      if (pkg_gn_generalidades.fnc_vl_pago_pse(p_cdgo_clnte          => p_cdgo_clnte,
                                               p_cdgo_impsto         => p_id_impsto,
                                               p_cdgo_impsto_sbmpsto => p_id_impsto_sbmpsto) = 'S') then
        if p_id_impsto = p_cdgo_clnte || 1 then
          v_bncos_rcddres := v_bncos_rcddres || ', Record, Almacenes Éxito';
        end if;
        v_bncos_rcddres := v_bncos_rcddres ||
                           '  y pago en línea PSE en la página web del Municipio';
      end if;
  ...
  */
  ...
  begin
    ...
    DBMS_OUTPUT.put_line(v_html_aux);
    l_comment := xmltype.createxml('<root><row>' || v_html_aux ||
                                   '</row></root>');
  ...
  begin
    ...
    while v_crcter_incial < v_crcter_fnal loop
      ...
      -- Valido que la porcion sea un HTML Valido
      if pkg_gn_generalidades.fnc_vl_html(v_html_aux) = true then
        v_nmro_prcion := v_nmro_prcion + 1;

        apex_collection.add_member(p_collection_name => 'DATOS',
        ...
        --DBMS_OUTPUT.put_line('10 HTML Valido. Inserto Porcion : ' || v_nmro_prcion );
        ...
        -- Hasta encontrar un HTML Valido o Alcanzar el Final del HTML
        ...
        for i in 1 .. v_hasta loop
          ...
          if pkg_gn_generalidades.fnc_vl_html(v_html_aux) = true then
            ...
            --DBMS_OUTPUT.put_line('10 HTML Valido. Inserto Porcion : ' || v_nmro_prcion || ' v_crcter_incial: ' || v_crcter_incial || ' p_tmno: ' || p_tmno  );
          ...
          if (v_crcter_incial + p_tmno) > v_crcter_fnal then
            --DBMS_OUTPUT.put_line('Salio del For Por alcanzar el fin sin HTM Valido');
            Exit;
          end if;
  ...
  end prc_html_dividir;*/

  /*
  ** Version mejorada de dividir HTML, paso de 2 minutos un html grande a 4 segundos
  ** JLUJAN 16/04/2025
  */
  ...
  procedure prc_html_dividir(p_html         in clob,
                             ...
                             o_mnsje_rspsta out varchar2) as
    v_html_aux    clob;
    v_pos_inicial number := 1;
    v_pos_final   number;
    v_longitud    number := dbms_lob.getlength(p_html);
    v_nmro_prcion number := 0;
    v_incremento  number;
    v_tag_abierto boolean;
    v_tag_inicio  number;
    v_pos_segura  number;
  begin
    -- validación inicial y configuración como en el original
    -- ...

    -- crear colección
    apex_collection.create_or_truncate_collection(p_collection_name => 'DATOS');

    while v_pos_inicial <= v_longitud loop
      -- calcular posición tentativa para el final del fragmento
      v_pos_final := least(v_pos_inicial + p_tmno - 1, v_longitud);

      -- buscar un punto seguro para cortar (después de un tag html completo)
      v_pos_segura := instr(p_html, '>', v_pos_final);
      if v_pos_segura = 0 then
        v_pos_segura := v_longitud;
      end if;

      -- extraer fragmento hasta el punto seguro
      v_html_aux := dbms_lob.substr(p_html,
                                    v_pos_segura - v_pos_inicial + 1,
                                    v_pos_inicial);

      -- validar fragmento con una sola llamada
      if pkg_gn_generalidades.fnc_vl_html(v_html_aux) = true then
        v_nmro_prcion := v_nmro_prcion + 1;
        ...
        v_pos_inicial := v_pos_segura + 1;
      else
        -- si no es válido, intentar cortar en un punto anterior
        v_pos_segura := dbms_lob.instr(p_html,
                                       '>',
                                       dbms_lob.instr(p_html,
                                                      '<',
                                                      v_pos_inicial));

        -- si encuentro un punto válido, procesar ese fragmento
        if v_pos_segura > v_pos_inicial then
          v_html_aux    := dbms_lob.substr(p_html,
                                           v_pos_segura - v_pos_inicial + 1,
                                           v_pos_inicial);
          v_nmro_prcion := v_nmro_prcion + 1;
          ...
          -- si todo falla, avanzar de manera segura
          v_pos_inicial := v_pos_inicial + p_tmno;
        end if;
      end if;
    end loop;

    o_cdgo_mnsje   := 0;
    o_mnsje_rspsta := 'Procesado correctamente. Fragmentos: ' ||
                      v_nmro_prcion;
    ...
    begin
      ...
      select 1
       ...
       where cdgo_clnte = p_cdgo_clnte
         ...
         and (id_impsto_sbmpsto = p_cdgo_impsto_sbmpsto or
             p_cdgo_impsto_sbmpsto is null)
         and actvo = 'S';
  ...
  begin
    ...
    -- Validar si el ID_CRGA   al modulo cautelar o al modulo de cobros
    -- GCB o MCA?
    ...
    if v_cdgo_prcso = 'GCB' then
      ...
      for c_sjtos_archvo in (select c.idntfccion,
        ...
        begin
          select a.id_sjto_impsto, a.id_impsto
            into v_id_sjto_impsto, v_id_impsto
        ...
        exception
          when no_data_found then
            v_mnsje := 'No se Encontraron Datos para el codigo de impuesto';
            raise_application_error(-20001, v_mnsje);
        end;
        --Alex
          ...
          begin
            select nvl(sum(vlor_sldo_cptal + vlor_intres), 0)
             ...
             (select 1
               ...
               and (pkg_cb_proceso_juridico.fnc_vl_determinacion_vigencia_prdo(p_id_sjto_impsto => a.id_sjto_impsto,
                                                                               p_vgncia         => a.vgncia,
                                                                               p_id_prdo        => a.id_prdo,
        ...
        if v_sldo_ttal_crtra > 0 then
          ...
          pkg_sg_log.prc_rg_log(23001,
                                ...
                                v_nmbre_up,
                                v_nl,
                                'Codigo de Impuesto  - v_cdgo_impsto: ' ||
                                v_cdgo_impsto,
                                1);
          ...
          if v_cdgo_impsto = 'IPU' then
            ...
            for c_rspnsbles in (select b.nmbre_rzon_scial,
              ...
              begin
                ...
                values
                  (v_id_prcsos_smu_sjto,
                   c_rspnsbles.cdgo_idntfccion_tpo,
                   c_rspnsbles.idntfccion_sjto,
                   c_rspnsbles.nmbre_rzon_scial,
                   '',
                   '.',
                   '',
                   'S',
          ...
          if v_id_sjto_impsto is null then
            ...
            values
              (p_cdgo_clnte,
               ...
               c_sjtos.vgncia_hsta,
               systimestamp,
               'No se pudo identificar el sujeto impuesto asociado al ID SUJETO #' ||
               c_sjtos.id_sjto);
            commit;
            continue;
          end if;
      ...
      /*elsif v_cdgo_prcso = 'FIS' then   --  Si el proceso es del modulo de FISCALIZACION (FIS)
                ...
                update fi_g_candidatos a
                 ...
                 where a.id_fsclzcion_lte = p_id_lte
                   ...
                   and not exists(select 1
                                    ...
                                    join si_i_sujetos_impuesto e on d.id_sjto = e.id_sjto
            ...
            for c_sjtos_archvo in (select e.id_sjto_impsto
                                     from v_gn_g_candidatos_carga c
                                     join si_c_sujetos d on d.idntfccion = c.idntfccion
                                     join si_i_sujetos_impuesto e on d.id_sjto = e.id_sjto and e.id_impsto= c.id_impsto
                                    where c.id_prcso_crga = p_id_prcso_crga
                                      and c.id_lte_prcso = p_id_lte
                                      and c.cdgo_prcso = v_cdgo_prcso
                                      and exists(select 1
                                                  from fi_g_candidatos j
                                                 where j.id_sjto_impsto = e.id_sjto_impsto
                                                   and j.id_fsclzcion_lte = c.id_lte_prcso
                                                   and j.actvo = 'N'
                                                   and j.indcdor_asgndo = 'N')
                                  )loop

                o_mnsje_rspsta := 'id_sjto_impsto: '||c_sjtos_archvo.id_sjto_impsto;
            ...
            for c_sjtos_archvo in (select c.idntfccion,
                                      ...
                                      from v_gn_g_candidatos_carga c
                                       join si_c_sujetos d on d.idntfccion = c.idntfccion
                                         join si_i_sujetos_impuesto e on e.id_sjto = d.id_sjto and e.id_impsto= c.id_impsto
                                      where c.id_prcso_crga = p_id_prcso_crga
                                        and c.id_lte_prcso = p_id_lte
                                        and c.cdgo_prcso = v_cdgo_prcso
                                        and not exists(select 1 from fi_g_candidatos a
        ...
        where id_fsclzcion_lte = p_id_lte;
                exception
                    when others then
                        o_cdgo_rspsta := 1;
                        o_mnsje_rspsta  := o_cdgo_rspsta||' - '||'No se pudo obtener el programa y subprograma del lote';
                        continue;
                end;

                o_mnsje_rspsta := 'v_id_prgrma: '||v_id_prgrma||', v_id_sbprgrma: '||v_id_sbprgrma;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 1);

                o_mnsje_rspsta := 'id_impsto: '||c_sjtos_archvo.id_impsto||', id_sjto: '||c_sjtos_archvo.id_sjto;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 1);
                ...
                o_mnsje_rspsta := 'periodo: '||c_sjtos_archvo.periodo||', cdgo_prdcdad: '||c_sjtos_archvo.cdgo_prdcdad||
                                  ', vgncia_dsde: '||c_sjtos_archvo.vgncia_dsde||', vgncia_hsta: '||c_sjtos_archvo.vgncia_hsta||
                                  ', id_impsto: '||c_sjtos_archvo.id_impsto||', id_impsto_sbmpsto: '||c_sjtos_archvo.id_impsto_sbmpsto;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 1);

                -- consultar el id periodo y vigencias a fiscalizar
                if (c_sjtos_archvo.periodo is not null and c_sjtos_archvo.cdgo_prdcdad is not null) then
                  begin
                    SELECT id_prdo
                    into   v_id_prdo
                    FROM df_i_periodos
              ...
              o_mnsje_rspsta := 'cdgo_prgrma: '||c_sjtos_archvo.cdgo_prgrma||' o cdgo_subprgrma: '||c_sjtos_archvo.cdgo_subprgrma;
              pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 1);
          ...
          begin
            ...
            when no_data_found then
              ...
              begin
                insert into fi_g_candidatos (id_impsto,                 id_impsto_sbmpsto,                 id_sjto_impsto,
                               id_fsclzcion_lte,          cdgo_cnddto_estdo,                 indcdor_asgndo,
                               id_prgrma,                 id_sbprgrma,                       cdgo_clnte)
                           values (c_sjtos_archvo.id_impsto, c_sjtos_archvo.id_impsto_sbmpsto, v_id_sjto_impsto,
                               p_id_lte,               'ACT',                               'N',
                               v_id_prgrma,               v_id_sbprgrma,                      p_cdgo_clnte)
                               returning id_cnddto into v_id_cnddto;
              exception
                when others then
                  o_cdgo_rspsta := 3;
                  o_mnsje_rspsta  := o_cdgo_rspsta||' - '||'No se pudo guardar el candidato con identificacion ' || '-' || c_sjtos_archvo.idntfccion;
                  rollback;
                  continue;
              end;
                  end;
        ...
        begin
                            ...
                            and   a.id_dclrcion_vgncia_frmlrio = c_canddto.id_dclrcion_vgncia_frmlrio;
                        exception
                            when no_data_found then
                                --Se inserta las vigencia periodo de los candidatos
                                begin
                                    insert into fi_g_candidatos_vigencia (id_cnddto,    vgncia,                 id_prdo,                    id_dclrcion_vgncia_frmlrio)
                                                                   values(v_id_cnddto,  c_canddto.vgncia, c_canddto.id_prdo,    c_canddto.id_dclrcion_vgncia_frmlrio
                                                                          );
                                exception
                                    when others then
                                        o_cdgo_rspsta := 4;
                                        o_mnsje_rspsta  := o_cdgo_rspsta||' - '||'No se pudo registrar las vigencia periodo del candidato con identificacion ' || '-' || c_sjtos_archvo.idntfccion || '-'||sqlerrm;
                                        rollback;
                                        continue;
                                end;
                        end;
            ...
            when c_sjtos_archvo.cdgo_prgrma = 'I' then
                    ...
                    begin
                            ...
                            exception
                                when no_data_found then
                                    --Se inserta las vigencia periodo de los candidatos
                                    begin
                                        insert into fi_g_candidatos_vigencia (id_cnddto,    vgncia,                 id_prdo,                    id_dclrcion_vgncia_frmlrio, id_dclrcion)
                                                                       values(v_id_cnddto,  c_canddto.vgncia, c_canddto.id_prdo,    c_canddto.id_dclrcion_vgncia_frmlrio, c_canddto.id_dclrcion
                                                                              );
                                    exception
                                        when others then
                                            o_cdgo_rspsta := 4;
                ...
                when c_sjtos_archvo.cdgo_prgrma = 'OLQ' THEN
                    o_mnsje_rspsta := 'cdgo_trbto_acto: '||c_sjtos_archvo.cdgo_trbto_acto||', fcha_expdcion: '||c_sjtos_archvo.fcha_expdcion||
                    ...
                    for c_canddto in ( select vgncia,
                                        ...
                                        --and id_prdo =  nvl(v_id_prdo, id_prdo)
                                        --and cdgo_prdcdad =  nvl(c_sjtos_archvo.cdgo_prdcdad, cdgo_prdcdad)
                                        and cdgo_impsto_acto = nvl(c_sjtos_archvo.cdgo_trbto_acto, cdgo_impsto_acto)
                                        --and to_char (fcha_rgstro, 'DD/MM/YY') = nvl(to_char (c_sjtos_archvo.fcha_expdcion, 'DD/MM/YY'), to_char (fcha_rgstro, 'DD/MM/YY'))
                                        and nmro_rnta = nvl(c_sjtos_archvo.nmro_rnta, nmro_rnta)
                                     )loop
            -- FIN <Validar cartera>
            -- 1. Incluir sujeto

            o_mnsje_rspsta := 'vgncia: '||c_canddto.vgncia||', id_prdo: '||c_canddto.id_prdo;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 1);
        ...
        begin
                ...
                exception
                    when no_data_found then
                        --Se inserta las vigencia periodo de los candidatos
                        begin
                           insert into fi_g_candidatos_vigencia (id_cnddto,    vgncia,           id_prdo, indcdor_fsclzcion_tpo,    fcha_expdcion, nmro_rnta)
                                                          values(v_id_cnddto,  c_canddto.vgncia, c_canddto.id_prdo, 'LQ', c_canddto.fcha_expdcion, c_canddto.nmro_rnta);
                        exception
                            when others then
                                o_cdgo_rspsta := 4;
                                o_mnsje_rspsta  := o_cdgo_rspsta||' - '||'No se pudo registrar las vigencia periodo del candidato con identificacion ' || '-' || c_sjtos_archvo.idntfccion || '-'||sqlerrm;
                ...
                o_mnsje_rspsta := 'v_id_cnddto_vgncia: '||v_id_cnddto_vgncia;
                pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 1);
  ...
  function fnc_cl_antrior_dia_habil(p_cdgo_clnte number, p_fecha in date)
    return date is

    /*
      Autor : BVILLEGAS
      Creado : 21/09/2023
      Descripcion: Funcion que devuelve el anterior dia habil a partir de una fecha
    */
    v_fecha_habil date;
  ...
  function fnc_gn_blob_documento(p_id_rprte    in number,
                                 p_prmtro_1    in number,
                                 p_prmtro_2    in number,
                                 p_nmbre_rprte in varchar,
                                 p_app_id      in number,
                                 p_page_id     in number) return blob is
    v_blob          blob;
  ...
  begin
    ...
    apex_util.set_session_state('P2_PRMTRO_1', p_prmtro_1);
    apex_util.set_session_state('P2_PRMTRO_2', p_prmtro_2);
    apex_util.set_session_state('P2_NMBRE_RPRTE', p_nmbre_rprte);
    ...
    -- DIRIGIMOS LA SESION A LA PAGINA QUE GENERA LA APROBACION
  ...
  exception
    when others then
      return null;
  end fnc_gn_blob_documento;

  --Función que devuelve el nombre del usuario
  --creacion 11/03/2025
  --autor William Mendoza
  function fnc_devuelve_usuario(p_cdgo_clnte number, p_id_usrio number)
    return varchar2 is
    v_nombre varchar2(1000);
  begin
    begin
      select replace(b.prmer_nmbre || ' ' || b.sgndo_nmbre || ' ' ||
                     b.prmer_aplldo || ' ' || b.sgndo_aplldo,
                     '.',
                     null) nombre
        into v_nombre
        from sg_g_usuarios a, si_c_terceros b
       where a.id_trcro = b.id_trcro
  ...
  function fnc_decode_base64(p_clob_in in clob)
