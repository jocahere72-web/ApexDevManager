  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_WS_RECAUDOS" is
    ...
    exception
      when no_data_found then
        ...
        exception
          when others then
            o_cdgo_rspsta  := 16;
            o_mnsje_rspsta := 'Numero de documento no valido.';
            return;
        end;
      when others then
        o_cdgo_rspsta  := 16;
  ...
  */
  ...
  begin
    ...
    if o_cdgo_rspsta = '0' then
      ...
      if p_cdgo_frma_pgo not in ('EF', 'CH', 'TR', 'MI', 'TC', 'DC', 'TD') then
        o_cdgo_rspsta  := 22;
  ...
  */
  ...
  procedure prc_ap_recaudo_webservice(p_cdgo_clnte   in number,
    ...
    v_nvel           number;
    v_nmbre_up       sg_d_configuraciones_log.nmbre_up%type := 'pkg_ws_recaudos.prc_ap_recaudo_webservice';
    v_nmro_pgos      number;
    ...
    if v_nmro_pgos > 0 then
      ...
      for control in (select a.id_bnco,
                       ...
                       (select 1
                               ...
                               where c.nmro_dcmnto = a.nmro_dcmnto
        ...
        if o_cdgo_rspsta = 0 and v_id_rcdo_cntrol > 0 then
          ...
          for recaudos in (select a.*, i.idntfccion_sjto
            ...
            pkg_re_recaudos.prc_rg_recaudo(p_cdgo_clnte         => p_cdgo_clnte,
                                           ...
                                           p_obsrvcion => 'Recaudo Web Service',
            ...
            if o_cdgo_rspsta = 0 and v_id_rcdo > 0 then
              ...
              if o_cdgo_rspsta = 0 then
                ...
                rollback; --Req0026276
                update ws_g_recaudos_webservice a
                   set a.obsrvcion_aplcion = 'Error aplicando el recaudo: ' ||
                                             o_mnsje_rspsta
                 where a.id_rcdo_wbsrvce = recaudos.id_rcdo_wbsrvce;
                commit; --Req0026276
                pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                      ...
                                      v_nmbre_up,
                                      v_nvel,
                                      'Error al aplicar el recaudo ' ||
                                      o_mnsje_rspsta,
                                      1);

              end if;
            else
              -- de o_cdgo_rspsta = 0 and v_id_rcdo_cntrol > 0 then

              rollback; --Req0026276
              update ws_g_recaudos_webservice a
                 set a.obsrvcion_aplcion = 'Error registrando el recaudo: ' ||
                                           o_mnsje_rspsta
               where a.id_rcdo_wbsrvce = recaudos.id_rcdo_wbsrvce;
              commit; --Req0026276
              pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                    ...
                                    v_nmbre_up,
                                    v_nvel,
                                    'Error al registar el recaudo ' ||
                                    o_mnsje_rspsta,
                                    1);

            end if;
          end loop; --for recaudos
        else
          --if o_cdgo_rspsta = 0 and v_id_rcdo_cntrol > 0 then
          ...
          rollback; --Req0026276
          update ws_g_recaudos_webservice a
             set a.obsrvcion_aplcion = 'Error registrando el control: ' ||
                                       o_mnsje_rspsta
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'Error al registar el recaudo control ' ||
                                o_mnsje_rspsta,
                                1);
        end if;

      end loop; --for control
    else
      o_cdgo_rspsta  := 3;
