  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_PL_FISCA_1_0" as
  ...
  begin
    ...
    if (nvl(p_label_colspan, 0) + nvl(p_value_colspan, 0) > p_colspan) then
      raise_application_error(-20001,
                              'El componente ' || p_id ||
                              ' no se pueden presentar debido a que la configuración no es válida.' ||
                              ' colspan: ' || p_colspan ||
  ...
  begin
    ...
    select listagg(a.vgncia_prdo, '-') as vigencia_periodo
      into v_vgncia_prdo
      from (select a.vgncia || '(' || listagg(a.prdo, ',') within group(order by a.vgncia, a.prdo) || ')' as vgncia_prdo
    ...
    v_elements := g_elements(t_element('p1_idntfccion_sjto_frmtda',
                             ...
                             t_element('p1_candidato',
                                       'Candidato:',
                                       v_candidato),
                             t_element('p1_nmbre_impsto',
                                       'Tributo:',
                                       v_nmbre_impsto),
                             t_element('p1_nmbre_impsto_sbmpsto',
                                       'SubTributo:',
                                       v_nmbre_impsto_sbmpsto),
                             t_element('p1_nmbre_prgrma',
                                       'Programa:',
                                       v_nmbre_prgrma),
                             t_element('p1_nmbre_sbprgrma',
                                       'SubPrograma:',
                                       v_nmbre_sbprgrma),
                             t_element('p1_fsclzcion_expdnte',
                                       'Expediente:',
                                       v_fsclzcion_expdnte),
                             t_element('p1_vgncia_prdo',
                                       'Vigencia Período:',
                                       v_vgncia_prdo));
    ...
    for i in 1 .. v_elements.count loop
      v_html := v_html ||
                fnc_create_display_only(p_id            => v_elements(i).id,
                                        p_colspan       => g_col_6,
    ...
    v_html := v_html ||
              fnc_create_region(p_colspan => g_col_12,
                                p_class   => 't-Region t-Region--hideShow t-Region--noPadding js-useLocalStorage t-Region--scrollBody t-Form--noPadding t-Form--stretchInputs margin-top-none margin-bottom-none margin-left-none margin-right-none a-Collapsible is-collapsed',
                                p_title   => 'Actos',
                                p_body    => fnc_create_region(p_colspan => g_col_12,
                                                               p_title   => '',
    ...
    begin
      ...
      open v_acts_gnrdos for
        ...
        --a.id_fsclzcion_expdnte_acto,
         ...
         decode(to_char(fcha_ntfccion, 'DD/MM/YYYY'),
                null,
                '-',
                to_char(fcha_ntfccion, 'DD/MM/YYYY')),
         '<a href="' ||
         APEX_UTIL.PREPARE_URL(p_url           => 'f?p=' || 50000 || ':' || 127 || ':' ||
                                                  v_session ||
                                                  '::NO:RP,127:P127_NOMBRE_TABLA,P127_COLUMNA_BLOB,P127_COLUMNA_FILENAME,P127_COLUMNA_MIME,P127_COLUMNA_CLAVE_PRIMARIA,P127_VALOR:v_gn_g_actos,FILE_BLOB,FILE_NAME,FILE_MIMETYPE,ID_ACTO,' ||
                                                  a.id_acto,
                               p_checksum_type => 'SESSION') ||
         '">Ver <span aria-hidden="true" class="fa fa-eye"></span></a>'
    ...
    begin
      ...
      open v_acts for
        ...
        select distinct /*+ RESULT_CACHE */ b.dscrpcion,
                        decode(a.indcdor_oblgtrio, 'S', 'Sí', 'No') indcdor_oblgtrio,
                        case
                          when (c.id_acto is null and
                               a.id_acto_tpo_rqrdo is null) or (c.id_acto is null and
                               a.id_acto_tpo_rqrdo is not null /*and d.id_acto is not null*/
                               ) then
                           '<a href="' ||
                           APEX_UTIL.PREPARE_URL(p_url           => 'f?p=' ||
                                                                    74000 || ':' || 17 || ':' ||
                                                                    v_session ||
                                                                    '::NO:RP,17:P17_ID_ACTO_TPO,P17_ID_FSCLZCION_EXPDNTE,P17_ID_FSCLZCION_EXPDNTE_ACTO,P17_IDNTFCCION,P17_ID_SJTO_IMPSTO,P17_PAGE_ID,P17_ID_CNDDTO:' ||
                                                                    a.id_acto_tpo || ',' ||
                                                                    e.id_fsclzcion_expdnte || ',' ||
                                                                    c.id_fsclzcion_expdnte_acto || ',' ||
                                                                    v_idntfccion_sjto_frmtda || ',' ||
                                                                    v(p_region.attribute_03) || ',' ||
                                                                    v('app_page_id') || ',' ||
                                                                    v_id_cnddto,
                                                 p_checksum_type => 'SESSION') || '">
                                                <center>
                                                    <button style="color:blue" type="button" class="a-Button a-Button--noLabel a-Button--iconTextButton" tabindex="0">' ||
                           nvl2(c.id_fsclzcion_expdnte_acto,
                                'Modificar ',
                                'Generar ') || '<span class="fa ' ||
                           nvl2(c.id_fsclzcion_expdnte_acto,
                                'fa-edit',
                                'fa-cog fa-spin') ||
                           '"></span>
                        ...
                        case
                          when (c.id_fsclzcion_expdnte_acto is not null and
                               c.id_acto is null and
                               a.id_acto_tpo_rqrdo is null and
                               ((pkg_fi_fiscalizacion.fnc_co_acto_revision(b.cdgo_clnte,
                                                                            v(p_region.attribute_04),
                                                                            b.id_acto_tpo) = 'S'))) or
                               (c.id_fsclzcion_expdnte_acto is not null and
                               c.id_acto is null and
                               a.id_acto_tpo_rqrdo is not null /*and d.id_acto is not null*/
                               and
                               ((pkg_fi_fiscalizacion.fnc_co_acto_revision(b.cdgo_clnte,
                                                                            v(p_region.attribute_04),
                                                                            b.id_acto_tpo) = 'S'))) then
                          --'<a onclick="apex.confirm(''Está seguro de confirmar el acto '||b.dscrpcion||''',{request:''GENERAR'', set:{''P'||v('app_page_id')||'_ID_FSCLZCION_EXPDNTE_ACTO'':'||c.id_fsclzcion_expdnte_acto||'}});">
                           --'<a href="#" onclick="apex.confirm(''¿Está seguro de confirmar el acto '||b.dscrpcion||'?'', {request: function(){generarActo(' || c.id_fsclzcion_expdnte_acto || ');}}); return false;">
                           '<a href="#" onclick="generarActo(' ||
                           c.id_fsclzcion_expdnte_acto ||
                           '); return false;">
                                                        <center>
          ...
          join fi_g_fiscalizacion_expdnte e
            on e.id_instncia_fljo = v(p_region.attribute_01)
          join fi_d_programas_acto g
            on b.id_acto_tpo = g.id_acto_tpo
          left join fi_g_fsclzcion_expdnte_acto c
            on c.id_acto_tpo = b.id_acto_tpo
           and c.id_fljo_trea = a.id_fljo_trea
           and c.id_fsclzcion_expdnte = e.id_fsclzcion_expdnte
          left join fi_g_fsclzcion_expdnte_acto d
            on d.id_acto_tpo = a.id_acto_tpo_rqrdo
           and d.id_fsclzcion_expdnte = e.id_fsclzcion_expdnte
           and d.id_acto is not null
          left join fi_d_actos_revision f
            on b.id_acto_tpo = f.id_acto_tpo
      ...
      open v_adjuntos for
        select *
          from (select /*+ RESULT_CACHE */
                 ...
                 '<a href="' ||
                 APEX_UTIL.PREPARE_URL(p_url           => 'f?p=' || 74000 || ':' || 91 || ':' ||
                                                          v_session ||
                                                          '::NO:RP,91:P91_ID_INSTNCIA_FLJO:' ||
                                                          a.id_orgen,
                                       p_checksum_type => 'SESSION') ||
                 '"> <span aria-hidden="true" class="fa fa-eye"></span></a>' as ir
                  from gn_g_adjuntos a
                  join fi_g_expedientes_analisis b
                    on a.id_orgen = b.id_instncia_fljo
                  join fi_g_fiscalizacion_expdnte c
                    on b.id_fsclzcion_expdnte = c.id_fsclzcion_expdnte
                  join pq_g_solicitudes g
                    on b.id_instncia_fljo_pdre = g.id_instncia_fljo
                  join sg_g_usuarios d
                    on a.id_usrio = d.id_usrio
                  join v_si_c_terceros e
                    on d.id_trcro = e.id_trcro
                  join v_wf_d_flujos_tarea f
                    on a.id_fljo_trea = f.id_fljo_trea
