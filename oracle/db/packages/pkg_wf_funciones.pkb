  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_WF_FUNCIONES" as

  /*Codigo de Error - Manejo de Errores de Workflow*/
  g_nmro_error number := -20999;

  /*Funcion Privada - Manejo de Errores de Workflow*/
  function fnc_wf_error(p_value in boolean, p_mensaje in varchar2)
    return varchar2 is
  begin
    if (not p_value) then
  ...
  function fnc_vl_acto(p_id_acto in gn_g_actos.id_acto%type) return varchar2 is
  ...
  */
  ...
  begin
    ...
    for acto_tipo in (select a.dscrpcion,
                       ...
                       inner join cb_g_procesos_jrdco_dcmnto c
      ...
      if acto_tipo.indcdor_ntfccion = 'S' and
         acto_tipo.fcha_ntfccion is not null then
        ...
        begin
          ...
          if v_fcha_fnal is not null then
            if trunc(systimestamp) >= trunc(v_fcha_fnal) then
              ...
              return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                                   p_mensaje => 'que el acto ' ||
                                                                lower(acto_tipo.dscrpcion) ||
                                                                ' cumpla el termino de ' ||
                                                                v_drcion || ' ' ||
                                                                lower(v_undad_drcion) || ',' ||
                                                                ' fecha del termino ' ||
                                                                to_char(trunc(v_fcha_fnal),
                                                                        'dd/mm/yyyy'));
            end if;
          else
      ...
      elsif acto_tipo.indcdor_ntfccion = 'S' and
            acto_tipo.fcha_ntfccion is null then

        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => 'notifique ' ||
                                                          lower(acto_tipo.dscrpcion));

      elsif acto_tipo.indcdor_ntfccion = 'N' then
        -- el acto no es notificable, sale OK 16/03/2022
        return 'S';
      end if;
  ...
  function fnc_vl_acto(p_id_instncia_fljo in number,
                       p_id_fljo_trea     in number) return varchar2 is
    v_id_acto gn_g_actos.id_acto%type;
  begin
    --return 'S';
    select c.id_acto
  ...
  function fnc_vl_prsc_incio(p_cdgo_clnte       in number,
                             p_id_instncia_fljo in number) return varchar2 is
  ...
  begin
    ...
    begin
      select a.id_prscrpcion, a.id_prscrpcion_tpo
        into v_id_prscrpcion, v_id_prscrpcion_tpo
    ...
    begin
      for c_inst in (select a.id_instncia_fljo,
                      ...
                      inner join v_si_c_sujetos b
                         on b.id_sjto = a.id_sjto
                      where a.cdgo_clnte = p_cdgo_clnte
                        and a.id_prscrpcion = v_id_prscrpcion) loop
        begin
          ...
          select count(*)
           ...
           where a.cdgo_clnte = p_cdgo_clnte
             and a.id_prscrpcion_sjto_impsto =
                 c_inst.id_prscrpcion_sjto_impsto;
          --Si hay un sujeto-impuesto sin vigencia entonces no es valido
          if (v_count_vig = 0) then
            return fnc_wf_error(p_value   => false,
                                p_mensaje => 'Por favor seleccione las vigencias a proyectar en esta prescripci?n para el sujeto-tributo No.' ||
                                             c_inst.idntfccion_sjto) || '.';
          end if;
        end;
      end loop;
    end;
  ...
  function fnc_vl_prsc_atrzcion(p_cdgo_clnte    in number,
                                p_id_prscrpcion in number) return varchar2 is
  ...
  begin
    ...
    begin
      select count(*)
        ...
        from gf_g_prescripciones a
       inner join gf_d_prescripciones_dcmnto b
          on b.id_prscrpcion_tpo = a.id_prscrpcion_tpo
       inner join gn_d_actos_tipo_tarea c
          on c.id_actos_tpo_trea = b.id_actos_tpo_trea
        left join gf_g_prscrpcns_dcmnto d
          on d.id_prscrpcion = a.id_prscrpcion
         and d.id_fljo_trea = c.id_fljo_trea
         and d.id_acto_tpo = c.id_acto_tpo
       where a.id_instncia_fljo = p_id_instncia_fljo
         and c.id_fljo_trea = p_id_fljo_trea
         and (b.cdgo_rspsta = a.cdgo_rspsta or b.cdgo_rspsta is null)
  ...
  begin
    ...
    begin
      select count(*)
       ...
       inner join gn_d_actos_tipo_tarea c
          on c.id_actos_tpo_trea = b.id_actos_tpo_trea
       inner join gf_g_prscrpcns_dcmnto d
          on d.id_prscrpcion = a.id_prscrpcion
         and d.id_fljo_trea = c.id_fljo_trea
         and d.id_acto_tpo = c.id_acto_tpo
       where a.id_instncia_fljo = p_id_instncia_fljo
    ...
    begin
      select count(*)
       ...
       inner join gf_g_prscrpcns_dcmnto d
         ...
         and d.id_fljo_trea = c.id_fljo_trea
         and d.id_acto_tpo = c.id_acto_tpo
       inner join v_gn_g_actos e
          on e.id_acto = d.id_acto
       where a.id_instncia_fljo = p_id_instncia_fljo
         and (b.cdgo_rspsta = a.cdgo_rspsta or b.cdgo_rspsta is null)
         and b.id_fljo_trea_cnfrmcion = p_id_fljo_trea
    ...
    begin
      select count(*)
       ...
       inner join v_nt_g_notfccnes_gn_g_actos b
          on b.id_acto = a.id_acto
       inner join gf_g_prescripciones c
          on c.id_prscrpcion = a.id_prscrpcion
  ...
  /*
	@Autor: JBadran
	@Creado: 19/03/2026
	@Descipción: Función que valida la cartera de la liquidación seleccionada este normalizada.
  */
  function fnc_vl_prescrpcns_crtera_estdo(p_id_prscrpcion 	  in number
									    , p_id_instncia_fljo  in number) return varchar2
  is
	v_cdgo_rspsta  number		  := 0;
    v_mnsje_rspsta varchar2(4000) := 'Señor(a) Usuario. la liquidación selccionada en la vigencia  - periodo: ';
  begin

	-- Iteramos todas las vigencias - periodos de la pescripción las cuales no cuentan con cartera normalizada.
	for c_vgncia_prdo in (select v.vgncia
							   , v.id_prdo
						  from gf_g_prescripciones                p
						  inner join gf_g_prscrpcnes_sjto_impsto  s on p.id_prscrpcion             = s.id_prscrpcion
						  inner join gf_g_prscrpcnes_vgncia       v on s.id_prscrpcion_sjto_impsto = v.id_prscrpcion_sjto_impsto
						  inner join gf_g_movimientos_financiero  m on v.id_lqdcion                = m.id_orgen
																   and v.vgncia                    = m.vgncia
																   and v.id_prdo                   = m.id_prdo
						  where p.id_prscrpcion         = p_id_prscrpcion
						    and p.id_instncia_fljo      = p_id_instncia_fljo
						    and m.cdgo_mvnt_fncro_estdo = 'AN'
						    and v.indcdor_mnja_orgen    = 'S'
						    and v.id_lqdcion is not null
	) loop
		v_cdgo_rspsta := 10;

		v_mnsje_rspsta := v_mnsje_rspsta || c_vgncia_prdo.vgncia || ' - ' || c_vgncia_prdo.id_prdo || ',';
	end loop;

	v_mnsje_rspsta := v_mnsje_rspsta  || ' debe estar activa, Por favor comuniquese con el area de soporte.';

	if v_cdgo_rspsta <> 0 then
		return fnc_wf_error(p_value   => false, p_mensaje => v_mnsje_rspsta);
	else
		return 'S';
	end if;
  end fnc_vl_prescrpcns_crtera_estdo;
  ...
  exception
    when others then
      return fnc_wf_error(p_value   => false,
                          p_mensaje => 'Error al validar registro del ajuste. ' ||
                                       sqlerrm);
  ...
  function fnc_vl_estado_acuerdo_slctdo(p_cdgo_clnte number,
                                        p_id_cnvnio  number) return varchar2 is
    -- !! ---------------------------------------------- !! --
  ...
  function fnc_vl_acuerdo_pagado(p_cdgo_clnte       number,
                                 p_id_instncia_fljo number) return varchar2 is
    ...
    begin
      select 'S'
        ...
        join (select id_cnvnio, count(id_cnvnio) cntdad
               ...
               group by id_cnvnio) c
          on c.id_cnvnio = b.id_cnvnio
         and c.cntdad = b.cntdad
        join gf_g_convenios_reversion d
    ...
    begin
      select 'S'
        ...
        join (select id_cnvnio,
                     count(decode(indcdor_cta_pgda, 'S', 1, null)) cntdad
                from gf_g_convenios_extracto
               group by id_cnvnio) c
          on c.id_cnvnio = b.id_cnvnio
         and b.cntdad >= c.cntdad
        left join gf_g_convenios_reversion d
          on d.id_cnvnio = a.id_cnvnio
       where a.cdgo_clnte = p_cdgo_clnte
  ...
  begin
    begin
      select b.cdgo_rcrso_tpo
        into v_cdgo_rcrso_tpo
        from gj_g_recursos a
        join gj_d_recursos_tipo_cliente b
          on a.id_rcrso_tipo_clnte = b.id_rcrso_tipo_clnte
       where a.id_instncia_fljo_hjo = p_id_instncia_fljo;
    end;
    if v_cdgo_rcrso_tpo <> 'RPS' then
      select case
               when count(a.id_rcrso_vgncia) > 0 then
                'S'
               when count(a.id_rcrso_vgncia) < 1 then
                'N'
             end
        into v_vgncias
        from gj_g_recursos_vigencia a
       inner join gj_g_recursos b
          on a.id_rcrso = b.id_rcrso
  ...
  function fnc_vl_gj_valida_air(p_id_instncia_fljo in number) return varchar2 is
    v_a_i_r varchar2(1);
  begin
    select a.a_i_r
      into v_a_i_r
  ...
  begin
    select case
             when count(a.id_actos_tpo_trea) > 0 then
              'S'
             when count(a.id_actos_tpo_trea) < 1 then
              'N'
           end
      into v_actos
      from gn_d_actos_tipo_tarea a
     inner join gj_g_recursos b
        on b.id_instncia_fljo_hjo = p_id_instncia_fljo
      left join gj_g_recursos_documento c
        on b.id_rcrso = c.id_rcrso
    ...
    select case
     ...
     inner join gj_g_recursos b
      ...
      left join gj_g_recursos_documento c
       ...
       and a.id_acto_tpo = c.id_acto_tpo
       and a.id_fljo_trea = c.id_fljo_trea
     inner join gj_d_actos_tipo_resolucion f
        on f.id_acto_tpo = a.id_acto_tpo
     where a.id_fljo_trea = p_id_fljo_trea
       and a.indcdor_oblgtrio = 'S'
       and c.id_rcrso_dcmnto is null
       and f.id_rcrso_tipo_clnte = b.id_rcrso_tipo_clnte;
    if (v_dcmntos = 'S') then
      return pkg_wf_funciones.fnc_wf_error(p_value   => false,
  ...
  begin
    select case
             when count(a.id_rcrso_dcmnto) > 0 then
              'N'
             when count(a.id_rcrso_dcmnto) < 1 then
              'S'
           end
      into v_actos
      from gj_g_recursos_documento a
  ...
  begin
    select case
             when count(a.id_rcrso_accion) > 0 then
              'S'
             when count(a.id_rcrso_accion) < 1 then
              'N'
           end
      into v_acciones
  ...
  begin
    ...
    if (v_actos = 'N') then
      return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                           p_mensaje => 'El acto de citación debe estar notificado, por favor verifique.');
    else
      return v_actos;
    end if;
  ...
  begin
    ...
    select case
             when count(a.indcdor_actlzar) > 0 then
              'S'
             when count(a.indcdor_actlzar) <= 0 then
              'N'
           end
      ...
      join v_pq_d_motivos_documento b
        on a.id_mtvo_dcmnto = b.id_mtvo_dcmnto
      join gj_g_recursos c
  ...
  function fnc_vl_acto_recurso(p_cdgo_clnte in number, p_id_acto in number)
    return varchar2 as
  ...
  begin
    --Se valida si el tipo de acto se encuentra en Gestión Jurídica
    begin
      select a.id_rcrso,
             a.a_i_r, --  A         ,   R
             a.cdgo_rspta, --(FVT,FVP,DFV) , (RCH)
             a.fcha_fin
        into v_id_rcrso, v_a_i_r, v_cdgo_rspta, v_fcha_fin
        from gj_g_recursos a
    ...
    --Se valida la recepción y respuestas de los Recursos para permitir a cada proceso avanzar o no.
    if v_id_rcrso is not null and v_a_i_r = 'R' and v_cdgo_rspta = 'RCH' and
       v_fcha_fin is not null then
      return 'N';
    elsif v_id_rcrso is not null and v_a_i_r = 'A' and v_cdgo_rspta = 'DFV' and
          v_fcha_fin is not null then
      return 'N';
    elsif v_id_rcrso is not null and v_a_i_r = 'A' and v_cdgo_rspta = 'FVP' and
          v_fcha_fin is not null then
      return 'N';
    elsif v_id_rcrso is not null and v_a_i_r = 'A' and v_cdgo_rspta = 'FVP' and
          v_fcha_fin is null then
      return 'S';
    elsif v_id_rcrso is not null and v_a_i_r = 'A' and v_cdgo_rspta = 'FVT' and
          v_fcha_fin is not null then
      return 'N';
    elsif v_id_rcrso is not null and v_a_i_r = 'A' and v_cdgo_rspta = 'FVT' and
          v_fcha_fin is null then
  ...
  function fnc_vl_gj_obsrvcion_etpa(p_id_instncia_fljo in number,
                                    p_id_fljo_trea     in number)
    return varchar2 as
    v_obsrvcn varchar2(1);
  begin
    select case
             when count(a.id_rcrso_dtlle) > 0 then
              'S'
             when count(a.id_rcrso_dtlle) < 1 then
              'N'
           end
      into v_obsrvcn
      from gj_g_recursos_detalle a
     inner join gj_g_recursos b
        on b.id_rcrso = a.id_rcrso
     where b.id_instncia_fljo_hjo = p_id_instncia_fljo
       and a.id_fljo_trea = p_id_fljo_trea
       and a.obsrvcion is not null
       and a.id_mtvo_clnte is null;

    if (v_obsrvcn = 'N') then
      return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                           p_mensaje => 'Señor usuario. Por favor, ingrese una observación de lo realizado en esta etapa para continuar.');
    else
      return v_obsrvcn;
    end if;
  end fnc_vl_gj_obsrvcion_etpa;

  function fnc_vl_gj_tpo_rcrso(p_id_instncia_fljo in number) return varchar2 as
    v_tpo_rcrso  varchar2(3);
    v_cdgo_rspta varchar2(3);
  begin
    begin
      select b.cdgo_rcrso_tpo, a.cdgo_rspta
        into v_tpo_rcrso, v_cdgo_rspta
        from gj_g_recursos a
        join gj_d_recursos_tipo_cliente b
          on a.id_rcrso_tipo_clnte = b.id_rcrso_tipo_clnte
       where a.id_instncia_fljo_hjo = p_id_instncia_fljo; --and b.cdgo_rcrso_tpo = 'RPS';
    end;
    if v_tpo_rcrso = 'RPS' then
      return 'S';
    elsif v_tpo_rcrso <> 'RPS' and v_cdgo_rspta in ('DFV', 'RCH') then
      return 'S';
    else
      return 'N';
    end if;
  end fnc_vl_gj_tpo_rcrso;

  function fnc_vl_gj_tp_rcrs_rpscn_indmsr(p_id_slctud        in number,
                                          p_id_instncia_fljo in number,
                                          p_id_acto          in number)
    return varchar2 as
    v_tpo_rcrso      varchar2(1);
    v_id_sjto_impsto number;
    v_cdgo_rspsta    varchar2(3) := 'N';
  begin
    begin
    ...
    exception
      when no_data_found then
        return v_cdgo_rspsta;
    end;
    begin
      /*select case when count(a.id_rcrso) > 0 then 'S'
                  when count(a.id_rcrso) < 1 then 'N'
             end
      into v_tpo_rcrso*/
      select case
               when a.cdgo_rspta is not null then
                'N'
               else
                'S'
             end
        ...
        join gj_d_recursos_tipo_cliente b
          on a.id_rcrso_tipo_clnte = b.id_rcrso_tipo_clnte
        join gj_g_recursos_documento c
          on c.id_acto = a.id_acto
        join gn_d_actos_tipo d
          on d.id_acto_tpo = c.id_acto_tpo
       where /*a.id_sjto_impsto = v_id_sjto_impsto
                                                                                                                                                                                                                                                                          and*/
       b.cdgo_rcrso_tpo = 'RPS'
      --and d.cdgo_acto_tpo = 'AUI'
       and a.id_acto = p_id_acto;
    exception
      when no_data_found then
        return v_cdgo_rspsta;
    end;
    if v_cdgo_rspsta = 'S' then
      return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                           p_mensaje => 'Señor usuario, esta identificación presentó un Recurso de Reposición contra el Auto Inadmisorio generado en esta instancia, verifique por favor.');
    else
      return v_cdgo_rspsta;
    end if;
  end fnc_vl_gj_tp_rcrs_rpscn_indmsr;

  /*<---------------------------------- Fin Funciones de Gestion Juridica---------------------------------->*/
      ...
      when no_data_found then
        begin
          select a.id_prcsos_jrdco
            into v_id_prcsos_jrdco
            from cb_g_procesos_juridico a
            join cb_g_prcsos_jrdc_acmld_dtll b
  ...
  begin
    ...
    begin
      select sum(a.vlor_sldo_cptal + a.vlor_intres)
        into v_sldo_crtera
        from v_gf_g_cartera_x_vigencia a
       where a.cdgo_mvnt_fncro_estdo in ('NO', 'CN')
         and exists
       (select 1
               ...
               where b.id_sjto_impsto = a.id_sjto_impsto
                 ...
                 and b.id_prcsos_jrdco = v_id_prcsos_jrdco);
    exception
      when others then
        v_sldo_crtera := 0;
    end;
  ...
  begin
    ...
    begin
      select ntfca_en_fljo
        into v_ntfca_en_fljo
    ...
    if v_ntfca_en_fljo = 'N' then
      ...
      for c_acto in (select distinct c.id_acto_tpo, indcdor_ntfccion
                       ...
                       join gn_d_actos_tipo_tarea c
                         on b.id_fljo_trea = c.id_fljo_trea
                       join gn_d_actos_tipo d
                         on c.id_acto_tpo = d.id_acto_tpo
                      where a.id_instncia_fljo = p_id_instncia_fljo
        ...
        begin
          ...
          elsif c_acto.indcdor_ntfccion = 'N' then
            -- el acto no es notificable, sale OK 16/03/2022
            return 'S';
          end if;
  ...
  begin
    begin
      ...
      select c.id_sldo_fvor_dvlcion, b.id_sld_fvr_cmpnscion
        into v_id_sldo_fvor_dvlcion, v_id_sld_fvr_cmpnscion
        from gf_g_saldos_favor_solicitud a
        left join gf_g_saldos_favor_cmpnscion b
          on a.id_sldo_fvor_slctud = b.id_sldo_fvor_slctud
        left join gf_g_saldos_favor_devlucion c
          on a.id_sldo_fvor_slctud = c.id_sldo_fvor_slctud
       where a.id_sldo_fvor_slctud = v_id_sldo_fvor_slctud;

      if v_id_sldo_fvor_dvlcion is not null and
         v_id_sld_fvr_cmpnscion is null then
        return 'S';
      elsif (v_id_sldo_fvor_dvlcion is not null and
            v_id_sld_fvr_cmpnscion is not null) or
            (v_id_sld_fvr_cmpnscion is not null) then
  ...
  begin
    begin
      select b.id_sldo_fvor_dvlcion
        ...
        from gf_g_saldos_favor_solicitud a
        join gf_g_saldos_favor_devlucion b
          on a.id_sldo_fvor_slctud = b.id_sldo_fvor_slctud
       where a.id_sldo_fvor_slctud = p_id_sldo_fvor_slctud;
  ...
  begin
    ...
    begin
      select b.id_sldo_fvor_dvlcion, c.id_sld_fvr_cmpnscion
        into v_id_sldo_fvor_dvlcion, v_id_sld_fvr_cmpnscion
        from gf_g_saldos_favor_solicitud a
        left join gf_g_saldos_favor_devlucion b
          on a.id_sldo_fvor_slctud = b.id_sldo_fvor_slctud
        left join gf_g_saldos_favor_cmpnscion c
          on a.id_sldo_fvor_slctud = c.id_sldo_fvor_slctud
       where a.id_sldo_fvor_slctud = p_id_sldo_fvor_slctud;
  ...
  begin
    ...
    select b.id_embrgos_rslcion,
           count(a.id_slctd_ofcio),
           count(c.id_dsmbrgo_ofcio)
      into v_id_embrgos_rslcion,
           v_entidades_embargo,
           v_entidades_desembargo
      ...
      left join mc_g_desembargos_oficio c
    ...
    if v_id_embrgos_crtra is not null then
      ...
      if v_cdgo_embrgos_tpo <> 'BIM' then
        ...
        if v_rsponsbles_id_cero = 0 then
          ...
          v_prmte_embrgar := 'N';
          v_mnsje         := v_mnsje ||
                             'No es posible enviar a embargo ya que hay responsables con cedula 0. ';
        end if;
        ...
        if v_prmte_embrgar = 'S' then
          ...
          if v_entdades_activas > 0 then
            ...
            v_prmte_embrgar := 'N';
            v_mnsje         := v_mnsje ||
                               'No es posible enviar a embargo ya que no hay entidades activas asociada a la cartera a embargar. ';
          end if;
        ...
        if v_rspnsbles_actvos > 0 then
          ...
          v_prmte_embrgar := 'N';
          v_mnsje         := v_mnsje ||
                             'No es posible enviar a embargo ya que no hay responsables activos para embargar. ';
        end if;
      ...
      if v_prmte_embrgar = 'S' then
        ...
        return fnc_wf_error(p_value => false, p_mensaje => v_mnsje);
      end if;
  ...
  begin
    ...
    select a.id_scstre_estdo
      ...
      join mc_g_secuestre_gestion b
        on b.id_scstre_gstion = a.id_scstre_gstion
  ...
  function fnc_vl_revision_titulo_ejctvo(p_id_ttlo_ejctvo in number)
    return varchar2 as
  ...
  function fnc_vl_aplicacion_titulo_ejctvo(p_id_ttlo_ejctvo in number)
    return varchar2 as
  ...
  function fnc_vl_concepto_titulo_ejctvo(p_id_ttlo_ejctvo in number)
    return varchar2 as
  ...
  function fnc_vl_revision_saldo_favor(p_id_sldo_fvor_slctud in gf_g_saldos_favor_solicitud.id_sldo_fvor_slctud%type)
    return varchar2 as
  ...
  function fnc_vl_aprobacion_saldo_favor(p_id_sldo_fvor_slctud in gf_g_saldos_favor_solicitud.id_sldo_fvor_slctud%type)
    return varchar2 as
  ...
  begin
    ...
    exception
      when others then
        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => '1. No se pudo obtener el sujeto impuesto del flujo ' ||
                                                          p_id_instncia_fljo);
    end;

    --Se obtiene el acto que se le va a validar el termino
    begin
      select a.id_acto_tpo, a.dscrpcion
        into v_id_acto_tpo, v_dscrpcion
        from gn_d_actos_tipo a
        join gn_d_actos_tipo_tarea b
          on a.id_acto_tpo = b.id_acto_tpo
       inner join fi_d_programas_acto c
          on a.id_acto_tpo = c.id_acto_tpo
       where a.cdgo_clnte = p_cdgo_clnte
    ...
    begin
      select id_acto, id_fsclzcion_expdnte_acto
        into v_id_acto, v_id_fsclzcion_expdnte_acto
  ...
  begin
    ...
    begin
      select b.id_cnddto, b.id_sjto_impsto
        into v_id_cnddto, v_id_sjto_impsto
        from fi_g_fiscalizacion_expdnte a
        join fi_g_candidatos b
          on a.id_cnddto = b.id_cnddto
       where a.id_instncia_fljo = p_id_instncia_fljo;
    exception
      when others then
        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => 'No se pudo obtener el sujeto impuesto del flujo ' ||
                                                          p_id_instncia_fljo);
    end;
           ...
           and a.id_dclrcion =
               (select max(g.id_dclrcion)
                 ...
                 where g.id_sjto_impsto = a.id_sjto_impsto
                   ...
                   and g.cdgo_dclrcion_estdo in ('PRS', 'APL'));
      exception
        when no_data_found then
          return 'N';
      end;
  ...
  begin
    ...
    --Se obtiene el candidato y el sujeto impuesto
    begin
      select b.id_cnddto, b.id_sjto_impsto, a.id_fsclzcion_expdnte
        into v_id_cnddto, v_id_sjto_impsto, v_id_fsclzcion_expdnte
        from fi_g_fiscalizacion_expdnte a
        join fi_g_candidatos b
          on a.id_cnddto = b.id_cnddto
       where a.id_instncia_fljo = p_id_instncia_fljo;
  ...
  begin
    ...
    begin
      select b.vlr_sncion
        into v_vlr_sncion
        from fi_g_candidatos a
        left join fi_d_programas_sancion b
          on a.id_prgrma = b.id_prgrma
       where b.cdgo_clnte = p_cdgo_clnte
         and a.id_cnddto = v_id_cnddto;
    exception
    ...
    for c_candidato in (select a.id_dclrcion, a.id_dclrcion_vgncia_frmlrio
                          from v_fi_g_fiscalizacion_expdnte_dtlle a
  ...
  begin
    ...
    begin
      ...
      for c_acto_tpo in (select b.id_acto,
                                ...
                                d.fcha_ntfccion
                           from fi_g_fiscalizacion_expdnte a
                          inner join fi_g_fsclzcion_expdnte_acto b
                             on a.id_fsclzcion_expdnte =
                                b.id_fsclzcion_expdnte
                          inner join gn_d_actos_tipo c
                             on b.id_acto_tpo = c.id_acto_tpo
                           left join gn_g_actos d
                             on b.id_acto = d.id_acto
                          where a.id_instncia_fljo = p_id_instncia_fljo) loop

        if c_acto_tpo.indcdor_ntfccion = 'S' and
           c_acto_tpo.fcha_ntfccion is null then
          return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                               p_mensaje => 'Genere, confirme y notifique el ' ||
                                                            lower(c_acto_tpo.dscrpcion));
        elsif c_acto_tpo.id_acto is null then
          return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                               p_mensaje => 'Genere y confirme el ' ||
                                                            lower(c_acto_tpo.dscrpcion));
        end if;
  ...
  function fnc_vl_expediente_padre(p_id_instncia_fljo in number)
    return varchar2 as
  ...
  begin
    ...
    begin
      select id_fsclzcion_expdnte_pdre
       ...
       where id_fsclzcion_expdnte_pdre =
  ...
  begin
    ...
    for c_acto in (select b.id_acto_tpo, b.dscrpcion, a.indcdor_oblgtrio
                     from gn_d_actos_tipo_tarea a
                     join gn_d_actos_tipo b
                       on a.id_acto_tpo = b.id_acto_tpo
                    where b.cdgo_clnte = p_cdgo_clnte
      ...
      if v_id_rcrso is not null and o_estdo_instncia <> 'FINALIZADA' then
        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => 'Gestion Juridica le de respuesta al recurso interpuesto por el contribuyente al acto ' ||
                                                          v_nmro_acto);
        ...
        for c_acto_vgncia in (select b.acptda_jrdca
                                from fi_g_fsclzcion_expdnte_acto a
                                join fi_g_fsclzcion_acto_vgncia b
                                  on a.id_fsclzcion_expdnte_acto =
                                     b.id_fsclzcion_expdnte_acto
                               where a.id_fsclzcion_expdnte_acto =
                                     v_id_fsclzcion_expdnte_acto) loop
  ...
  begin
    ...
    begin
      select b.id_acto_tpo, b.id_acto, c.dscrpcion
        into v_id_acto_tpo, v_id_acto, v_dscrpcion
        from fi_g_fiscalizacion_expdnte a
    ...
    begin
      ...
      if v_undad_drcion is null then
        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => 'Que parametrice el termino del acto ' ||
                                                          v_dscrpcion);
      end if;
  ...
  function fnc_vl_pago_pliego_cargo(p_id_instncia_fljo in number)
    return varchar2 as
  ...
  function fnc_vl_dfncion_emplzmnto_crrcn(p_cdgo_clnte in number)
    return varchar2 as
  ...
  begin
    ...
    begin
      select b.vlor
        ...
        join df_c_definiciones_cliente b
          on a.id_dfncion_clnte_ctgria = b.id_dfncion_clnte_ctgria
       where a.cdgo_clnte = p_cdgo_clnte
  ...
  begin
    ...
    for c_pliego in (select a.id_acto_tpo, a.dscrpcion
                       from gn_d_actos_tipo a
                       join gn_d_actos_tipo_tarea b
                         on a.id_acto_tpo = b.id_acto_tpo
                      where a.cdgo_clnte = p_cdgo_clnte
      ...
      --Se valida si el acto fue generado en el expediente de fiscalizacion
      begin
        select id_acto, id_acto_tpo
          into v_id_acto, v_id_acto_tpo
          from fi_g_fsclzcion_expdnte_acto
  ...
  begin
    ...
    begin
      select a.id_cnddto, a.id_sjto_impsto, c.cdgo_sbprgrma
        into v_id_cnddto, v_id_sjto_impsto, v_cdgo_sbprgrma
        from fi_g_candidatos a
        join fi_g_fiscalizacion_expdnte b
          on a.id_cnddto = b.id_cnddto
        join fi_d_subprogramas c
          on a.id_prgrma = c.id_prgrma
    ...
    case
      when v_cdgo_sbprgrma = 'SML' THEN
        for c_sncion in (select vgncia,
                                ...
                                cdgo_rspsta,
                                mnsje_rspsta
                           from json_table((select pkg_fi_fiscalizacion.fnc_co_sancion_mal_liquidada(v_id_cnddto,
                                                                                                    v_id_sjto_impsto)
                                             from dual),
                                           '$[*]'
      ...
      when v_cdgo_sbprgrma = 'EXT' THEN
        for c_sncion in (select vgncia,
                                ...
                                cdgo_rspsta,
                                mnsje_rspsta
                           from json_table((select pkg_fi_fiscalizacion.fnc_co_sancion_extemporanea(v_id_cnddto,
                                                                                                   v_id_sjto_impsto)
                                             from dual),
                                           '$[*]'
                                           columns(vgncia varchar2 path
      ...
      when v_cdgo_sbprgrma = 'NEI' THEN
        for c_sncion in (select vgncia,
                                ...
                                cdgo_rspsta,
                                mnsje_rspsta
                           from json_table((select pkg_fi_fiscalizacion.fnc_co_sancion_no_enviar_informacion(v_id_cnddto,
                                                                                                            v_id_sjto_impsto)
                                             from dual),
                                           '$[*]'
                                           columns(vgncia varchar2 path
  ...
  function fnc_vl_aplicacion_liquidacion(p_id_instncia_fljo in number)
    return varchar2 as
  ...
  function fnc_vl_dfncion_lqudcn_ofcl_afr(p_cdgo_clnte in number)
    return varchar2 as
  ...
  */
  ...
  begin
    ...
    exception
      when others then
        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => 'No se pudo obtener la informacion de la liquidacion del flujo : ' ||
                                                          p_id_instncia_fljo);
    end;
  ...
  */
  ...
  begin
    begin
      select b.id_cnddto,
        ...
        into v_id_cnddto,
             ...
             v_id_instncia_fljo
        from fi_g_fiscalizacion_expdnte a
        join fi_g_expedientes_analisis d
          on a.id_fsclzcion_expdnte = d.id_fsclzcion_expdnte
        join fi_g_fsclzc_expdn_cndd_vgnc e
          on a.id_fsclzcion_expdnte = e.id_fsclzcion_expdnte
    ...
    exception
      ...
      when others then
        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => 'No se pudo consultar el expediente del flujo ' ||
                                                          p_id_instncia_fljo);
    end;

    --Se consulta el indicador bloqueo
    begin
      select indcdor_blqdo, a.id_expdnte_anlsis
        into v_indcdor_blqdo, v_id_expdnte_anlsis
        from fi_g_expndnts_anlsis_dtlle a
        join fi_g_expedientes_analisis d
          on a.id_expdnte_anlsis = d.id_expdnte_anlsis
        join fi_g_fsclzc_expdn_cndd_vgnc b
          on a.id_fsclzc_expdn_cndd_vgnc = b.id_fsclzc_expdn_cndd_vgnc
        join fi_g_candidatos_vigencia c
    ...
    exception
      ...
      when others then
        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => 'Error al validar el indicador bloqueado del expediente ' ||
                                                          v_id_fsclzcion_expdnte);
    end;
    ...
    if v_indcdor_blqdo = 'S' then
      ...
      return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                           p_mensaje => 'Se encuentra activa en PQR una solicitud de análisis de expediente asociada al Radicado No. ' ||
                                                        v_nmro_rdcdo_dsplay ||
  ...
  /*
    Función que valida si existe un solicitud de análisis de expediente
    que este abierta, consultado el indicador bloqueado asociado a la tabla
    fi_g_fsclzc_expdn_cndd_vgnc, que hace referencia al id del expediente.
  ...
  begin
    ...
    begin
      select b.id_cnddto, b.id_sjto_impsto, a.id_fsclzcion_expdnte
        ...
        join fi_g_candidatos b
          on a.id_cnddto = b.id_cnddto
        join fi_g_expedientes_analisis c
          on a.id_fsclzcion_expdnte = c.id_fsclzcion_expdnte
       where c.id_expdnte_anlsis = p_id_expdnte_anlsis;
    exception
      when others then
        return pkg_wf_funciones.fnc_wf_error(p_value   => false,
                                             p_mensaje => 'No se pudo consultar el expediente analisis ' ||
                                                          p_id_expdnte_anlsis);
    end;
  ...
  */
  ...
  function fnc_vl_nvdd_rlqudcion(p_cdgo_nvdad_tpo in varchar2)
    return varchar2 is
 ...
 /*<-------------------------------------Funciones Titulo Judicial--------------------------------->*/
  ...
  function fnc_vl_consignacion_ttlo(p_id_instncia_fljo in number) return varchar2 as
  v_count       number;
...
begin
  begin
    ...
    from gf_g_titulos_judicial a
    where a.id_instncia_fljo = p_id_instncia_fljo
    and a.cdgo_ttlo_jdcial_estdo in ('ACN','CMP','CYC');
  exception
    when others then
      return 'N';
  end;
...
begin
  begin
    ...
    join gf_g_titulos_jdcial_impsto b on a.id_instncia_fljo = b.id_instncia_fljo
    join gf_g_titulos_judicial c on c.id_instncia_fljo = b.id_instncia_fljo
    where c.id_instncia_fljo = p_id_instncia_fljo
    and rownum = 1;
  exception
    when no_data_found then
        ...
        loop
            begin
                select c.id_acto
                    into v_id_acto
                from gf_g_titulos_judicial a
                inner join gf_g_ttls_jdcl_dcmnt_asccn b on b.id_ttlo_jdcial = a.id_ttlo_jdcial
                inner join gf_g_titulos_jdcial_dcmnto c on c.id_ttlo_jdcial_dcmnto = b.id_ttlo_jdcial_dcmnto
                where a.id_instncia_fljo = p_id_instncia_fljo
                and c.id_acto_tpo = v_acto.id_acto_tpo
...
return varchar2 as
  ...
  v_cdgo_clnte number;
  v_nmbre_up varchar2(100) := 'pkg_wf_funciones.fnc_vl_cnsgncion_ttlo_vlor';
  v_nl number;
...
begin
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte, null, v_nmbre_up, v_nl, 'Entrando a la funcion con parametros: ' || p_id_instncia_fljo, 6);

  --Obtenemos el valor de los titulos procesados
  begin
    select round(sum(a.vlor))
      into v_vlor_ttlos
    from gf_g_titulos_judicial a
    where a.id_instncia_fljo = p_id_instncia_fljo
    and a.cdgo_ttlo_jdcial_estdo in ('ACN','CMP','CYC');
  ...
  pkg_sg_log.prc_rg_log(v_cdgo_clnte, null, v_nmbre_up, v_nl, 'v_vlor_ttlos: ' || v_vlor_ttlos, 6);
  -- Buscamos el valor consignado con los titulos procesados
  begin
    select nvl(round(sum(t.vlor_ttal_aplcar)),0)
      into v_vlor_ttal_aplcar
    from gf_g_titulos_jdcial_vgncia t
    join gf_g_titulos_judicial c on c.id_instncia_fljo = t.id_instncia_fljo
    where c.cdgo_ttlo_jdcial_estdo in ('ACN','CMP','CYC')
    and t.id_instncia_fljo = p_id_instncia_fljo;

  exception
    when others then
      begin
        select nvl(round(sum(vlor_sldo_fvor)),0)
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte, null, v_nmbre_up, v_nl, 'v_vlor_ttal_aplcar: ' || v_vlor_ttal_aplcar, 6);
  begin
    select nvl(round(sum(vlor_sldo_fvor)),0)
      into v_vlor_sldo_fvor
    from gf_g_titulos_judicial_saldo_favor
  ...
  pkg_sg_log.prc_rg_log(v_cdgo_clnte, null, v_nmbre_up, v_nl, 'v_vlor_sldo_fvor: ' || v_vlor_sldo_fvor, 6);
  ...
  function fnc_vl_rvsion_obsrvcion(p_cdgo_clnte       in number,
                                   p_id_instncia_fljo in number)
    return varchar2 is
    v_count  number;
    v_result varchar2(1);
  ...
  end fnc_vl_rvsion_obsrvcion;


  function fnc_vl_prescrpcns_lqdcion_orgen(p_id_prscrpcion 	  in number
										 , p_id_instncia_fljo in number) return varchar2
  is
	v_cdgo_rspsta  number		  := 0;
    v_mnsje_rspsta varchar2(4000) := 'Señor(a) Usuario. la vigencia  - periodo: ';
  begin

	-- Iteramos todas las vigencias - periodos de lla pescripción las cuales no cumplan con la condición
	for c_vgncia_prdo in (select v.vgncia
							   , v.id_prdo
						    from gf_g_prescripciones              p
						  inner join gf_g_prscrpcnes_sjto_impsto  s on p.id_prscrpcion             = s.id_prscrpcion
	...
	v_mnsje_rspsta := v_mnsje_rspsta  || ' debe tener un origen asociado.';
  ...
  end fnc_vl_prescrpcns_lqdcion_orgen;
