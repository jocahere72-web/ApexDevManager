  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_SG_AUTORIZACION" is
      ...
      for c_prfles in (select id_prfil
        ...
        begin
          ...
          if v_id_prfil = 0 then
            ...
            -- Si lo encuentra, es porque si tiene permiso para el bot¿n
  ...
  begin
    ...
    DELETE FROM sg_g_perfiles_usuario
     WHERE ID_PRFIL = p_id_prfil
       AND ID_USRIO NOT IN
           (SELECT REGEXP_SUBSTR(p_usuarios, '[^:]+', 1, LEVEL)
              FROM DUAL
            CONNECT BY REGEXP_SUBSTR(p_usuarios, '[^:]+', 1, LEVEL) IS NOT NULL);
