  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GN_REGLAS_NEGOCIO" as
  ...
  -- Funciones para validar si el predio esta exento del limite del immpuesto
    ...
    begin
      ...
      */
      ...
      select id_prdio_dstno
       ...
       where b.id_sjto_impsto = json_value(p_xml, '$.P_ID_SJTO_IMPSTO')
  ...
  function fnc_vl_exncion_cambio_area_destino(p_xml clob) return varchar2 is
    -- !! -----------------------------------------------------------  !! --
    -- !! Funcion que valida que No hubo cambio de area o de destino   !! --
    -- !! -----------------------------------------------------------  !! --
    v_id_prdo_ant        number;
    v_id_prdio_dstno_ant number;
  ...
  begin
    ...
    v_area_trrno   := json_value(p_xml, '$.P_AREA_TRRNO');
    ...
    begin
      ...
      */
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nl,
                            'v_id_prdio_dstno: ' ||
                            json_value(p_xml, '$.P_ID_PRDIO_DSTNO'),
                            3);
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nl,
                              'v_dstno_hmlgdo: ' || v_dstno_hmlgdo,
                              3);

      exception
        when others then
          v_dstno_hmlgdo := json_value(p_xml, '$.P_ID_PRDIO_DSTNO');
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nl,
                                'Destino: ' ||
                                json_value(p_xml, '$.P_ID_PRDIO_DSTNO') ||
                                ' no homologado en df_c_destinos_homologados',
                                3);
      end;

      --> Cambio de area o de destino
      if (v_id_prdio_dstno_ant != v_dstno_hmlgdo) or
         (v_area_cnstrda <> nvl(v_area_cnstrda_ant, 0)) or
         (v_area_trrno <> nvl(v_area_trrno_ant, 0)) then
  ...
  function fnc_vl_exncion_area_rral(p_xml clob) return varchar2 is
    -- !! ---------------------------------------------------------------------  !! --
    -- !! Funcion que valida para rural si el area sea Menor a 100 hectareas     !! --
    -- !! ---------------------------------------------------------------------  !! --
  ...
  begin
    ...
    --> Area mayor a 100 hectareas para rural
    if (json_value(p_xml, '$.P_CDGO_PRDIO_CLSFCCION') = '01' and
       ((v_area_cnstrda > 1000000) or (v_area_trrno > 1000000))) then
      --pkg_sg_log.prc_rg_log( p_cdgo_clnte, null, v_nmbre_up,  v_nl, 'return N', 3);
      return 'N';
    else
      --pkg_sg_log.prc_rg_log( p_cdgo_clnte, null, v_nmbre_up,  v_nl, 'return S', 3);
  ...
  function fnc_vl_exncion_prdio_actlzdo(p_xml clob) return varchar2 is
    -- !! ---------------------------------------------------------------------  !! --
    -- !! Funcion que valida si el predio esta actualizado catastralmente        !! --
    -- !! ---------------------------------------------------------------------  !! --
    v_nmbre_up varchar2(500) := 'pkg_gn_reglas_negocio.fnc_vl_exncion_prdio_actlzdo';
    v_nl       number := 6;
  begin
    if (json_value(p_xml, '$.P_PRDIO_ACTLZDO') = 'S') then
      return 'N'; -- NO limita impuesto
    else
      return 'S'; -- Si limita impuesto
    end if;

  end fnc_vl_exncion_prdio_actlzdo;

  function fnc_vl_exncion_cmbio_trfa(p_xml clob) return varchar2 is
    -- !! ---------------------------------------------------------------------  !! --
    -- !! Funcion que valida si tuvo cambio de tarifas                           !! --
    -- !! ---------------------------------------------------------------------  !! --
    v_nmbre_up varchar2(500) := 'pkg_gn_reglas_negocio.fnc_vl_exncion_cmbio_trfa';
    v_nl       number := 6;

    v_trfa     number;
    v_trfa_ant number;
  begin
    v_trfa     := json_value(p_xml, '$.P_TRFA');
    v_trfa_ant := json_value(p_xml, '$.P_TRFA_ANT');

    if (v_trfa > nvl(v_trfa_ant, 0)) then
      return 'S'; -- Si limita impuesto
    else
      return 'N'; -- NO limita impuesto
    end if;

  end fnc_vl_exncion_cmbio_trfa;
  ...
  begin
    ...
    -- LIMITE 3
    /*Para las viviendas pertenecientes a los estratos 1 y 2 cuyo avaluo catastral sea hasta, 135 SMMLV,
    el incremento anual del Impuesto Predial,no podra sobrepasar el 100% del IPC. */
    if (v_count_estrto > 0 and to_number(json_value(p_xml, '$.P_BSE')) <=
       v_cntdad_slrio * v_vlor_SMMLV) then
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_vlor_clcddo,
                            3);
      if (v_vlor_mxmo_incrmnto >
         to_number(json_value(p_xml, '$.P_VLOR_CLCLDO'))) then
        o_lmta_impsto := 'N';
      ...
      -- LIMITE 1
      /*Independientemente del valor de catastro obtenido siguiendo los procedimientos del articulo anterior,
      para los predios que hayan sido objeto de actualizacion catastral y hayan pagado segun esa actualizacion,(EL PAGO NO SE TIENE EN CUENTA DE ACUERDO A JAVIER)
      sera del IPC+8 puntos porcentuales maximo del Impuesto Predial Unificado.*/
      if (v_prdio_actlzdo = 'S') then
        ...
        -- LIMITE 2
        /*Para el caso de los predios que no se hayan actualizado el limite sera de
        maximo 50% del monto liquidado por el mismo concepto el a?o inmediatamente anterior.*/
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          ' - v_vlor_lmte_impsto:' || v_vlor_lmte_impsto ||
                          ' - v_vlor_clcldo:' || v_vlor_clcldo,
                          3);
    --Verifica si el Area Construida Aumento
    if ((v_area_cnstrda > 0 and nvl(v_area_cnstrda_ant, 0) = 0) or
  ...
  procedure prc_vl_cnddto_lmt_impsto_acrdo_034_25(p_xml         clob,
                                                  o_lmta_impsto out varchar2,
                                                  o_vlor_lqddo  out number) is
    -- !! ------------------------------------------------------------  !! --
    -- !! Funcion que valida si es candidato para limitar impsto        !! --
    ...
    v_vlor_lqddo       number;
    v_vlor_clcldo      number;
    v_nmbre_up         varchar2(500) := 'pkg_gn_reglas_negocio.prc_vl_cnddto_lmt_impsto_acrdo_034_25';
    v_nl               number := 6;
    p_cdgo_clnte       number := 23001;
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          3);
    --Verifica si el Area Construida Aumento
    if (v_vlor_lqddo * 1.25 < v_vlor_clcldo) then
      o_lmta_impsto := 'S';
      o_vlor_lqddo  := v_vlor_lqddo * 1.25;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
  ...
  end prc_vl_cnddto_lmt_impsto_acrdo_034_25;
  ...
  begin
    ...
    if v_fcha_fnal is not null then
      if trunc(v_fcha_fnal) >=
         trunc(to_date(json_value(p_xml, '$.P_FCHA_PRYCCION'), 'DD/MM/YYYY')) then
        if v_cdgo_acto_tpo = 'PCN' then
  ...
  /*Valida los descuentos del acuerdo 034 ART 111 - Monter¿a*/
  function fnc_vl_descuento_formalizacion_ica(p_xml clob) return varchar2 is
    v_otrga_dscnto number;
  begin

    begin
      select count(*)
        into v_otrga_dscnto
        from genesys.v_si_i_sujetos_impuesto a
        join genesys.v_si_i_personas b
          on b.id_sjto_impsto = a.id_sjto_impsto
       where a.id_impsto = json_value(p_xml, '$.P_ID_IMPSTO')
         and to_char(a.fcha_rgstro, 'yyyy') = '2025'
         and to_char(b.fcha_incio_actvddes, 'yyyy') <= '2023'
         and a.id_sjto_impsto = json_value(p_xml, '$.P_ID_SJTO_IMPSTO');

      if v_otrga_dscnto > 0 then
        return 'S';
      else
        return 'N';
      end if;
  ...
  end fnc_vl_descuento_formalizacion_ica;
