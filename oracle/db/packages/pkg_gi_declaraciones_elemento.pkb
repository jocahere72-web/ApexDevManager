  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GI_DECLARACIONES_ELEMENTO" as

  /**/
  function fnc_co_vigencia(p_id_dclrcion in number) return varchar2 is
  ...
  /**/
  function fnc_co_destinatario(p_id_dclrcion in number) return clob is
    v_json       clob;
    v_cdgo_clnte number;
    v_dstntrio   clob;
  begin
    ...
    begin
      select json_arrayagg(json_object(key 'valor' value a.cdna) returning clob)
        into v_json
        from table(pkg_gn_generalidades.fnc_ca_split_table(p_cdna           => v_dstntrio,
                                                           p_crcter_dlmtdor => ',')) a;
    exception
      when others then
        v_json := '[]';
  ...
  /**/
  function fnc_co_tipo_sujeto(p_id_dclrcion in number) return clob is
    v_cdgo_clnte   number;
  ...
  begin
    begin
      ...
      select listagg('<span> ' || initcap(b.nmbre_sjto_tpo) || ' ' ||
                     decode(v_vlor,
                            b.id_sjto_tpo,
  ...
  /**/
  function fnc_co_uso(p_id_dclrcion in number) return clob is
    v_cdgo_clnte   number;
    v_vlor         varchar2(4000);
    v_cdgo_rspsta  number;
    v_mnsje_rspsta varchar2(4000);
  ...
  begin
    begin
      ...
      pkg_gi_declaraciones.prc_co_homologacion(p_cdgo_clnte    => v_cdgo_clnte,
                                               ...
                                               o_cdgo_rspsta   => v_cdgo_rspsta,
                                               o_mnsje_rspsta  => v_mnsje_rspsta);
      select listagg('<span> ' || b.nmbre_dclrcion_uso || ' ' ||
                     decode(v_vlor,
                            b.cdgo_dclrcion_uso,
                            '<span style="font-size: 18px;">&#x2612;</span>',
                            '<span style="font-size: 18px;">&#x2610;</span>') ||
                     ' </span>',
                     '') within group(order by b.id_dclrcion_uso desc)
        into v_uso
        from gi_d_declaraciones_uso b
       where b.cdgo_clnte = v_cdgo_clnte
         and b.cdgo_dclrcion_uso <> 'PAC';
    exception
      when others then
        v_uso := '';
    end;
  ...
  /**/
  function fnc_co_tipo_documento(p_id_dclrcion in number,
                                 p_cdgo_prpdad in varchar2) return clob is
    v_cdgo_clnte     number;
    v_cdgo_rspsta    number;
    v_mnsje_rspsta   varchar2(4000);
    v_vlor           varchar2(4000);
  ...
  begin
    begin
      ...
      select listagg('<span> ' || b.nmtcnco_idntfccion_tpo || ' ' ||
                     decode(v_vlor,
  ...
  /**/
  function fnc_co_numero_declaracion(p_id_dclrcion in number) return varchar2 is
  ...
  function fnc_co_tipo_sujeto_periodo(p_id_dclrcion in number) return clob is
    v_cdgo_clnte     number;
    v_cdgo_rspsta    number;
    v_mnsje_rspsta   varchar2(4000);
    v_vlor           varchar2(4000);
    ...
    begin
      select '<fieldset><table style="border: none !important;"><tr style="margin-left:10px !important; border: none !important;">' ||
             listagg('<td style="padding:5px !important; border: 1px dotted !important;"><span> ' ||
                     a.dscrpcion || '</span></td>',
                     '') within group(order by a.id_prdo) || '</tr><tr style="margin-left:10px !important; border: none !important;">' || listagg('<td style="padding:5px !important; border: 1px dotted !important;" ><center>' || decode(v_id_prdo, a.id_prdo, '<span style="font-size: 14px;">&#x2612;</span>', '<span style="font-size: 14px;">&#x2610;</span>') || '</center></td>', '') within group(order by a.id_prdo) || '</tr><legend style="font-size: 14px;">' || v_nmbre_sjto_tpo || '</legend></fieldset>'
        into v_prdo
        from (select case
                       when d.cdgo_prdcdad = 'BIM' then
                        substr(d.dscrpcion, 0, 3) ||
                        substr(d.dscrpcion, (instr(d.dscrpcion, ' - ')), 6)
                       when d.cdgo_prdcdad = 'MNS' then
                        substr(d.dscrpcion, 0, 3)
                       else
                        d.dscrpcion
                     end as dscrpcion,
               ...
               where exists (select c.cdgo_prdcdad
                        ...
                        join gi_g_declaraciones e
                          on e.id_dclrcion_vgncia_frmlrio =
  ...
  function fnc_co_tpo_rspnsble_atrzcion(p_id_dclrcion in number) return clob is
  ...
  begin
    begin
      ...
      select listagg('<span> ' || a.dscrpcion_rspnsble_tpo || ' ' ||
                     decode(v_vlor,
                            a.cdgo_rspnsble_tpo,
                            '<span style="font-size: 18px;">&#x2612;</span>',
                            '<span style="font-size: 18px;">&#x2610;</span>') ||
                     ' </span>',
                     '<br/>') within group(order by null)
