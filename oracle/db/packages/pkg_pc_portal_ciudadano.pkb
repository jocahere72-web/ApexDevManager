  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_PC_PORTAL_CIUDADANO" as
  ...
  begin
    ...
    begin
      ...
      begin
        select id_usrio, nmbre_trcro
          into o_id_usrio, o_nmbre_trcro
      ...
      o_tken := apex_jwt.encode(p_iss           => p_username,
                                ...
                                p_aud           => o_id_usrio,
                                p_exp_sec       => v_time,
                                p_other_claims  => '"cdgo_clnte": ' ||
                                                   apex_json.stringify(p_cdgo_clnte),
  ...
  begin
        ...
        begin
          ...
          begin
            update si_c_terceros
               set cdgo_idntfccion_tpo  = v_tp.cdgo_idntfccion_tpo,
                   idntfccion           = v_tp.idntfccion,
                   prmer_nmbre          = v_tp.prmer_nmbre,
                   sgndo_nmbre          = v_tp.sgndo_nmbre,
                   prmer_aplldo         = v_tp.prmer_aplldo,
                   sgndo_aplldo         = v_tp.sgndo_aplldo,
                   drccion              = v_tp.drccion,
                   id_pais              = v_tp.id_pais,
                   id_dprtmnto          = v_tp.id_dprtmnto,
                   id_mncpio            = v_tp.id_mncpio,
                   drccion_ntfccion     = v_tp.drccion_ntfccion,
                   id_pais_ntfccion     = v_tp.id_pais_ntfccion,
                   id_dprtmnto_ntfccion = v_tp.id_dprtmnto_ntfccion,
                   id_mncpio_ntfccion   = v_tp.id_mncpio_ntfccion,
                   email                = v_tp.email,
                   tlfno                = v_tp.tlfno,
                   gnro                 = v_tp.gnro,
                   ncnldad              = v_tp.ncnldad,
                   fcha_ncmnto          = v_tp.fcha_ncmnto,
                   id_pais_orgn         = v_tp.id_pais_orgn,
                   cllar                = v_tp.cllar
             where id_trcro = v_id_trcro;
          exception
            when others then
              o_cdgo_rspsta  := 6;
              o_mnsje_rspsta := 'No se pudo actualizar el tercero. ' ||
                                v_tp.idntfccion;
              return;
          end;
        exception
          when no_data_found then
            ...
            begin
              ...
              values
                (v_tp.cdgo_clnte,
                 ...
                 v_tp.id_dprtmnto,
                 v_tp.id_mncpio,
                 v_tp.drccion_ntfccion,
                 v_tp.id_pais_ntfccion,
                 v_tp.id_dprtmnto_ntfccion,
                 v_tp.id_mncpio_ntfccion,
                 v_tp.email,
                 v_tp.tlfno,
                 v_tp.gnro,
                 v_tp.ncnldad,
            ...
            exception
              when others then
                o_cdgo_rspsta  := 3;
                o_mnsje_rspsta := 'No se pudo registrar el tercero. ' ||
                                  v_tp.idntfccion;
                return;
            end;
        end;
    ...
    begin
      v_password := pkg_sg_autenticacion.fnc_sg_hash(lower(v_tp.idntfccion),
                                                     p_password);
      insert into sg_g_usuarios
        (user_name,
         id_trcro,
         actvo,
         fcha_crcion,
  ...
  begin
    ...
    begin
      v_password := pkg_sg_autenticacion.fnc_sg_hash(lower(v_idntfccion),
                                                     p_password);
  ...
  procedure prc_rg_usuario_portal(p_cdgo_clnte           in number,
  ...
  begin
    ...
    begin
      select lower(b.dscrpcion_idntfccion_tpo), a.email
        into v_idntfccion_tpo_i, v_email_i
        from si_c_terceros_portal a
      ...
      o_cdgo_rspsta  := 10;
      o_mnsje_rspsta := 'El usuario con ' || v_idntfccion_tpo_i || ' No. ' ||
                        p_idntfccion ||
                        ' ya se encuentra en proceso de registro con cuenta de correo electronico ' ||
                        v_email_i || '.';
      return;
    exception
      when no_data_found then
    ...
    begin
      ...
      exception
        when no_data_found then
          ...
          insert into si_c_terceros_portal
            (cdgo_clnte,
             cdgo_idntfccion_tpo,
             idntfccion,
             prmer_nmbre,
          ...
          values
            (p_cdgo_clnte,
             ...
             v_id_pais,
             p_cllar)
          returning id_trcro_prtal into v_id_trcro_prtal;
    ...
    pkg_ma_envios.prc_co_envio_programado(p_cdgo_clnte   => p_cdgo_clnte,
                                          ...
                                          p_json_prmtros => json_object(key
                                                                        'p_id_trcro_prtal'
                                                                        value
                                                                        v_id_trcro_prtal));
  ...
  exception
    when others then
      o_cdgo_rspsta  := 30;
      o_mnsje_rspsta := 'No se pudo realizar el registro, por favor intente nuevamente.' ||
                        sqlerrm;
  end prc_rg_usuario_portal;

  procedure prc_el_no_prcsdos(p_cdgo_clnte   in number,
                              o_cdgo_rspsta  out number,
                              o_mnsje_rspsta out varchar2) as
    v_cntdad_trcros   number;
    v_tmpo_trnscrrido number;
    v_cntar_rgstros   number := 0;
    v_tmpo_brrdo      number := 20; --tiempo en horas que debe transcurrir entre el registro y la hora actual
    v_json_trza       clob := '[]'; -- Inicializamos el JSON como un array vacío
    v_json_individual clob; -- Variable para cada JSON individual

    --log
    v_nl     number;
    nmbre_up varchar2(200) := 'pkg_pc_portal_ciudadano.prc_el_no_prcsdos';
  ...
  begin
    ...
    --Consulta que extrae la cantidad de registros a eliminar
    select count(id_trcro_prtal)
      into v_cntdad_trcros
      from si_c_terceros_portal
     where indcdor_prcsdo = 'N';
    -- fin Consulta que extrae la cantidad de registros a eliminar

    if v_cntdad_trcros > 0 then

      for terceros in (select id_trcro_prtal,
                              extract(minute
                                      from(systimestamp - fcha_rgstro)) +
                              60 *
                              extract(hour from(systimestamp - fcha_rgstro)) as dfrncia_mntos
                         from si_c_terceros_portal
                        where indcdor_prcsdo = 'N') loop
        --se valida que el tiempo transcurrido desde el registro sea mayor al parametro recibido
        if terceros.dfrncia_mntos > v_tmpo_brrdo then

          select json_object('id_trcro_prtal' VALUE ID_TRCRO_PRTAL,
                             ...
                             'id_dprtmnto' VALUE ID_DPRTMNTO,
                             'id_mncpio' VALUE ID_MNCPIO,
                             ...
                             'tlfno' VALUE TLFNO,
                             'gnro' VALUE GNRO,
                             'ncnldad' VALUE NCNLDAD,
                             'fcha_ncmnto' VALUE FCHA_NCMNTO,
                             'id_pais_orgn' VALUE ID_PAIS_ORGN,
                             'cllar' VALUE CLLAR,
                             'indcdor_prcsdo' VALUE INDCDOR_PRCSDO,
                             --'trmnos_cndcnes' VALUE TRMNOS_CNDCNES,
                             'fcha_rgstro' VALUE FCHA_RGSTRO) as json_individual
            into v_json_individual
            from si_c_terceros_portal
           where id_trcro_prtal = terceros.id_trcro_prtal;

          -- Agregar el JSON individual al JSON array acumulado
          if v_json_trza = '[]' then
            v_json_trza := '[' || v_json_individual || ']';
          else
            v_json_trza := rtrim(v_json_trza, ']') || ',' ||
                           v_json_individual || ']';
          end if;

          begin
            --Eliminar registro si_c_terceros_portal
            delete from si_c_terceros_portal
             where id_trcro_prtal = terceros.id_trcro_prtal;
          exception
            when others then
              o_cdgo_rspsta  := 10;
              ...
              pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                    ...
                                    6);
          end; --fin eliminar registro si_c_terceros_portal

          if o_cdgo_rspsta = 0 then
            v_cntar_rgstros := v_cntar_rgstros + 1;
          end if;

        end if; --fin validación tiempo transcurrido
      end loop;

      If v_json_individual is not null then
        begin
          insert into si_c_terceros_portal_traza
            (rgstros_elmndos, cntdad_rgstros_elmndos)
          values
            (v_json_trza, v_cntar_rgstros);
        exception
          when others then
            o_cdgo_rspsta  := 20;
            o_mnsje_rspsta := o_cdgo_rspsta ||
