  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GI_DETERMINACION" as
    ...
    begin
      ...
      begin
        ...
        if v_cdgo_rspsta_acto = 0 then
          ...
          rollback;
          o_cdgo_rspsta  := v_cdgo_rspsta_acto;
          o_mnsje_rspsta := 'No se logró generar el Acto: ' ||
                            o_mnsje_rspsta;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Entrando determinación: ' || p_id_dtrmncion,
                          1);
  ...
  procedure prc_gn_acto_blob_determinacion(p_cdgo_clnte       number,
                                           ...
                                           p_id_dtrmncion_lte number,
                                           p_id_plntlla       number,
                                           p_id_gnra_acto_tpo varchar2) as
    v_nl              number;
  ...
  begin
    ...
    for c_dtrmncion in (select a.id_dtrmncion,
                               a.id_sjto_impsto,
                               a.id_acto,
                               d.id_dcmnto
                          from gi_g_determinaciones a
                         ...
                         where id_dtrmncion_lte = p_id_dtrmncion_lte
                           ...
                           and (case
                                 when p_id_gnra_acto_tpo = 'T' then
                                  1
                                 when p_id_gnra_acto_tpo = 'E' and
                                      (d.file_blob is null or
                                      length(d.file_blob) < 500) then
                                  1
                                 else
                                  0
                               end) = 1) loop
      v_html := pkg_gn_generalidades.fnc_ge_dcmnto('{"ID_DTRMNCION":' ||
                                                   c_dtrmncion.id_dtrmncion || '}',
                                                   p_id_plntlla);
      begin
        update gi_g_determinaciones
           set dcmnto = v_html, id_plntlla = p_id_plntlla
      ...
      exception
        when others then
          ...
          -- TRAZA
          --v_cdgo_rspsta := 1;
          v_mnsje_rspsta := 'Error al actualizar dcmnto determinación Id: ' ||
                            c_dtrmncion.id_dtrmncion || sqlerrm;
      ...
      begin
        if c_dtrmncion.id_dcmnto is not null then
          update gn_g_actos
             set id_dcmnto = null
           where id_acto = c_dtrmncion.id_acto;
        end if;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          6,
                          'Saliendo Lote: ' || p_id_dtrmncion_lte,
                          1);
