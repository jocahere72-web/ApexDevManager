  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GI_DECLARACIONES_NVDAD" as
  ...
  begin
    ...
    if v_id_lqdcion is null then
        o_cdgo_rspsta  := 30;
        o_mnsje_rspsta := 'Error No ' ||o_cdgo_rspsta||
                          '. No existe liquidación para la declaracion - vigencia - período. Verifique!!! ';
    ...
    if v_id_sjto_impsto is null then
        o_cdgo_rspsta  := 31;
        o_mnsje_rspsta := 'Error No ' ||o_cdgo_rspsta||
                          '. No existe el id_sjto_impsto. Verifique!!!'||sqlerrm;
    ...
    if v_id_mvmnto_fncro is null then
        o_cdgo_rspsta  := 32;
        o_mnsje_rspsta := 'Error No ' ||o_cdgo_rspsta||
                          '. No existen movimientos financieros. Verifique!!!'||sqlerrm;
    ...
    begin
      ...
      select count(*)
       ...
       where a.id_sjto_impsto = v_id_sjto_impsto
         ...
         and a.id_mvmnto_fncro = v_id_mvmnto_fncro
         and a.fcha_mvmnto >
