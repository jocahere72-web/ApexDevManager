  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GF_CONVENIOS" as
  ...
  */
  ...
  begin
    ...
    for c_dtlle_crtra in (select a.cdgo_clnte,
      ...
      exception
        when others then
          p_mnsje := 'c_cartera.vlor_intres  ' || c_dtlle_crtra.vlor_intres;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                'pkg_gf_convenios.prc_rg_convenio',
                                v_nl,
  ...
  begin
    ...
    for c_slccion_acrdo_pgo in (select a.id_cnvnio
      ...
      if /* v_nmro_ctas_pgadas = 0 and */
       v_cdgo_cnvnio_estdo = 'APL' then
