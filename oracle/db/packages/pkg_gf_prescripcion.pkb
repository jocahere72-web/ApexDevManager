  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GF_PRESCRIPCION" as
  ...
  begin
    ...
    begin
      ...
      from gf_g_prescripciones 			  	 a
       inner join gf_g_prscrpcns_dcmnto 	 b on b.id_prscrpcion 		= a.id_prscrpcion
       inner join gn_g_actos 				 c on c.id_acto 	  		= b.id_acto
       inner join gn_d_actos_tipo_tarea 	 d on d.id_acto_tpo   		= b.id_acto_tpo 		and d.id_fljo_trea = b.id_fljo_trea
       inner join gf_d_prescripciones_dcmnto e on e.id_prscrpcion_tpo 	= a.id_prscrpcion_tpo 	and e.id_actos_tpo_trea = d.id_actos_tpo_trea
       where a.id_instncia_fljo 		= p_id_instncia_fljo
         and (e.cdgo_rspsta 			= a.cdgo_rspsta or e.cdgo_rspsta is null)
         and e.indcdor_rslve_prscrpcion = 'S'
         and b.id_acto is not null
    ...
    begin
      insert into gf_t_prscrpcnes_sjt_impst_vgnc
        select a.cdgo_clnte,
               ...
               b.vlor_intres
		from v_gf_g_prscrpcnes_vgncia         a
		inner join v_gf_g_cartera_x_concepto  b  on b.cdgo_clnte = a.cdgo_clnte and b.id_impsto          = a.id_impsto
																				and b.id_impsto_sbmpsto  = a.id_impsto_sbmpsto
																				and b.id_sjto_impsto     = a.id_sjto_impsto
																				and b.vgncia             = a.vgncia
																				and a.indcdor_mnja_orgen = 'N'
																				 or a.id_lqdcion		 = b.id_orgen
         where a.id_prscrpcion  = v_id_prscrpcion
           and a.indcdor_aprbdo = 'S';
    exception
      when others then
      ...
      for i in 1 .. v_tbla_dnmca.count loop
        ...
        begin
          select json_object('cdgo_clnte' 		 	 value v_cdgo_clnte,
                             ...
                             'obsrvcion' 			 value 'Ajuste que nace de la prescripcion no.' || v_id_prscrpcion,
                             ...
                             'detalle_ajuste' 		 value(select json_arrayagg(json_object('VGNCIA' 		  	value a.vgncia,
																							'ID_PRDO' 		  	value a.id_prdo,
																						    'ID_CNCPTO' 	  	value a.id_cncpto,
																						    'VLOR_SLDO_CPTAL' 	value a.vlor_sldo_cptal,
																						    'VLOR_AJSTE' 	    value a.vlor_sldo_cptal,
																						    'VLOR_INTRES' 	    value a.vlor_intres,
																						    'VLOR_INTRES_CPTAL' value a.vlor_intres,
																							'VLOR_AJSTE_INTRES' value a.vlor_intres,
																							 'ID_ORGEN' value (select x.id_lqdcion
																											    from v_gf_g_prscrpcnes_vgncia x
																												where x.id_prscrpcion_vgncia = a.id_prscrpcion_vgncia
																											    and x.indcdor_mnja_orgen = 'S'
																											   ))
														   returning clob)
  ...
  begin
    -- Determinamos el nivel del Log de la UPv
    v_nl := pkg_sg_log.fnc_ca_nivel_log(p_cdgo_clnte, null, 'pkg_gf_prescripcion.prc_co_prscrpcion_estdo_blqueo');
    ...
    v_sql := 'select b.id_impsto, ' || 'b.id_impsto_sbmpsto, ' 	  	||
             ...
             'b.idntfccion, ' 		|| 'c.vgncia, ' || 'c.id_prdo, '|| 'c.id_lqdcion ' ||
             'from        gf_g_prescripciones             a ' 		||
        ...
        for i in 1 .. v_pblcion.count loop
          ...
          begin
            pkg_gf_movimientos_financiero.prc_co_movimiento_bloqueada(p_cdgo_clnte           => p_cdgo_clnte,
                                                                      ...
                                                                      p_vgncia               => v_pblcion(i).vgncia,
                                                                      p_id_prdo              => v_pblcion(i).id_prdo,
																	  p_id_orgen			 => v_pblcion(i).id_lqdcion,
  ...
  begin
    -- Determinamos el nivel del Log de la UPv
    v_nl := pkg_sg_log.fnc_ca_nivel_log(p_cdgo_clnte, null, 'pkg_gf_prescripcion.prc_ac_prscrpcion_estdo_blqueo');
        ...
        for i in 1 .. v_pblcion.count loop
          ...
          begin
            pkg_gf_movimientos_financiero.prc_ac_indicador_mvmnto_blqdo(p_cdgo_clnte           => p_cdgo_clnte,
                                                                        ...
                                                                        p_vgncia               => v_pblcion(i).vgncia,
                                                                        p_id_prdo              => v_pblcion(i).id_prdo,
																		p_id_orgen_mvmnto	   => v_pblcion(i).id_lqdcion,
 ...
 /*
	@Autor: JBadran
	@Creado: 11/03/2026
	@Descipción: Función que valida si la vigencia periodo tiene multiples liquidaciones.
  */
  function fnc_vl_lqdcnes_vgncia_prdo(p_cdgo_clnte  	  in number
									, p_id_impsto		  in number
									, p_id_impsto_sbmpsto in number
									, p_id_sjto_impsto	  in number
									, p_vgncia			  in number
									, p_id_prdo			  in number) return varchar2
  as
    v_count number;
  begin
	begin
		-- Validamos si la vigencia periodo tiene multiples origenes (Liquidaciones)
		select count(*)
		  into v_count
		...
		where l.cdgo_clnte        = p_cdgo_clnte
		  ...
		  and l.vgncia            = p_vgncia
		  and l.id_prdo           = p_id_prdo;
	exception
		when others then
		  return 'N';
	end;
  ...
  end fnc_vl_lqdcnes_vgncia_prdo;



  -- PRSC_RV1 - Reversion de Aplicacion a Autorizacion
  procedure prc_rv_prscrpcion_aplicacion(p_cdgo_clnte       in  number
                                       , p_id_prscrpcion    in  number
                                       , p_id_instncia_fljo in  number
                                       , p_id_fljo_trea     in  number
                                       , p_id_usrio         in  number
                                       , p_dscrpcion        in  varchar2
                                       , o_cdgo_rspsta      out number
                                       , o_mnsje_rspsta     out varchar2) as
    v_nl                     number;
    v_nmbre_up               varchar2(200) := 'pkg_gf_prescripcion.prc_rv_prscrpcion_aplicacion';
    v_id_fljo_trea_aplccion  number;
    v_id_fljo_trea_atrzcion  number;
    v_id_fljo                number;
  begin
    o_cdgo_rspsta := 0;
    v_nl := pkg_sg_log.fnc_ca_nivel_log(p_cdgo_clnte, null, v_nmbre_up);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Iniciando reversion. Prescripcion: ' || p_id_prscrpcion, 1);

    -- 0. Obtener el flujo y las tareas de Aplicacion y Autorizacion dinamicamente
    begin
      select f.id_fljo into v_id_fljo
        from wf_g_instancias_flujo f
       where f.id_instncia_fljo = p_id_instncia_fljo;

      select ft.id_fljo_trea into v_id_fljo_trea_aplccion
        from wf_d_flujos_tarea ft
       where ft.id_fljo = v_id_fljo
         and ft.id_trea = 212;

      select ft.id_fljo_trea into v_id_fljo_trea_atrzcion
        from wf_d_flujos_tarea ft
       where ft.id_fljo = v_id_fljo
         and ft.id_trea = 140;
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Error al identificar las tareas del flujo: ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 3);
        return;
    end;

    -- 1. Validar que la prescripcion esta en etapa de Aplicacion
    declare
      v_cnt number;
    begin
      select count(*) into v_cnt
       ...
       where id_instncia_fljo = p_id_instncia_fljo
         and id_fljo_trea_orgen = v_id_fljo_trea_aplccion;
      if v_cnt = 0 then
        o_cdgo_rspsta  := 2;
        o_mnsje_rspsta := 'La prescripcion no se encuentra en etapa de Aplicacion.';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 2);
        return;
      end if;
    end;

    -- 2. Anular flujos hijos de ajuste generados
    begin
      for r in (select ig.id_instncia_fljo_gnrdo, ig.id_instncia_fljo_gnrdo_hjo
                  from wf_g_instancias_flujo_gnrdo ig
                 where ig.id_instncia_fljo = p_id_instncia_fljo
                   and ig.id_fljo_trea = v_id_fljo_trea_aplccion) loop
        update wf_g_instancias_flujo_gnrdo
           set indcdor_mnjdo = 'N'
         where id_instncia_fljo_gnrdo = r.id_instncia_fljo_gnrdo;
        update wf_g_instancias_flujo
           set estdo_instncia = 'ANULADA'
             , obsrvcion = 'Revertida por usuario - ' || p_dscrpcion
         where id_instncia_fljo = r.id_instncia_fljo_gnrdo_hjo;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Flujo hijo ' || r.id_instncia_fljo_gnrdo_hjo || ' anulado.', 1);
      end loop;
    exception
      when others then
        o_cdgo_rspsta  := 3;
        o_mnsje_rspsta := 'Error al anular flujo hijo de ajuste: ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 3);
        return;
    end;

    -- 3. Limpiar referencia del ajuste en las vigencias
    begin
      update gf_g_prscrpcnes_vgncia pv
         set pv.aplcdo   = null
           , pv.id_ajste = null
       where pv.id_prscrpcion_sjto_impsto in (
              select ps.id_prscrpcion_sjto_impsto
                from gf_g_prscrpcnes_sjto_impsto ps
               where ps.id_prscrpcion = p_id_prscrpcion);
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Vigencias limpiadas: ' || sql%rowcount, 1);
    exception
      when others then
        o_cdgo_rspsta  := 4;
        o_mnsje_rspsta := 'Error al limpiar vigencias: ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 3);
        return;
    end;

    -- 3.5 Finalizar la transicion de Aplicacion
    begin
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Transicion de Aplicacion finalizada.', 1);
    exception
      when others then
        o_cdgo_rspsta  := 7;
        o_mnsje_rspsta := 'Error al finalizar transicion de Aplicacion: ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 3);
        return;
    end;

    -- 4. Crear nueva instancia de transicion hacia Autorizacion
    begin
      insert into wf_g_instancias_transicion (
          id_instncia_trnscion, id_instncia_fljo, id_fljo_trea_orgen,
          fcha_incio, id_usrio, id_estdo_trnscion
      ) values (
          sq_wf_g_instancias_transicion.nextval, p_id_instncia_fljo, v_id_fljo_trea_atrzcion,
          systimestamp, p_id_usrio, 2
      );
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, 'Nueva transicion creada hacia Autorizacion.', 1);
    exception
      when others then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := 'Error al crear transicion: ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 3);
        return;
    end;
    ...
    begin
      insert into gf_g_prscrpcnes_obsrvcion (
          id_obsrvcion, id_prscrpcion, asnto_obsrvcion, obsrvcion,
          id_fljo_trea, id_usrio, fcha_obsrvcion, indcdor_cnfrmdo, cdgo_clnte
      ) values (
          sq_gf_g_prscrpcnes_obsrvcion.nextval, p_id_prscrpcion,
          'Reversion de Aplicacion a Autorizacion',
          p_dscrpcion, p_id_fljo_trea, p_id_usrio, systimestamp, 'S', p_cdgo_clnte
      );
    exception
      when others then
        o_cdgo_rspsta  := 6;
        o_mnsje_rspsta := 'Error al registrar observacion: ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 3);
        return;
    end;

    o_mnsje_rspsta := 'Reversion exitosa. La prescripcion ' || p_id_prscrpcion || ' fue devuelta a la etapa de Autorizacion.';
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 1);

  exception
    when others then
      o_cdgo_rspsta  := 99;
      o_mnsje_rspsta := 'Error general en reversion: ' || sqlerrm;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up, v_nl, o_mnsje_rspsta, 3);
  end prc_rv_prscrpcion_aplicacion;
