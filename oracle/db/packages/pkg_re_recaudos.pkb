  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_RE_RECAUDOS" as
  ...
  */
  ...
  procedure prc_rg_recaudo(p_cdgo_clnte         in re_g_recaudos_control.cdgo_clnte%type,
    ...
    v_id_adjnto_tpo gn_d_adjuntos_tipo.id_adjnto_tpo%type;
    v_directorio    varchar2(20) := 'TS_RECAUDOS';
    v_bfile         bfile;
    v_file_name     gn_g_adjuntos.file_name%type;
  begin
    ...
    -- REQ0029193. Valida si tiene adjuntos para guardar
    begin
      select id_adjnto_tpo
        into v_id_adjnto_tpo
        from gn_d_adjuntos_tipo
       where cdgo_adjnto_tpo = 'SRM'
         and actvo = 'S';
    exception
      when others then
        o_cdgo_rspsta  := 4;
        o_mnsje_rspsta := 'No se pudo obtener el tipo de adjunto';
        return;
    end;
    ...
    for c_adjntos in (select seq_id,
                       ...
                       where collection_name = 'ADJUNTOS_RECAUDO') loop
      if c_adjntos.blob is not null then

        v_file_name := 'REC_MAN_' || o_id_rcdo || '_' || c_adjntos.filename;

        -- Registramos el blob como archivo en disco
        pkg_gd_utilidades.prc_rg_dcmnto_dsco(p_blob         => c_adjntos.blob,
        ...
        if o_cdgo_rspsta = 0 then
          ...
          -- Se insertan los adjuntos en la solicitud vur
          begin
            insert into gn_g_adjuntos
              (cdgo_prcso,
               id_adjnto_tpo,
               cdgo_orgen_tpo,
               id_orgen,
          ...
          exception
            when others then
              ...
              pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                    ...
                                    1);
              return;
          end; -- Fin Se insertan los adjuntos en la solicitud vur

          apex_collection.delete_member(p_collection_name => 'ADJUNTOS_RECAUDO',
    ...
    end loop; -- Fin Regisro de adjuntos, se consultan los adjuntos de la collecci?n
    ---------------
  ...
  * @Creacion     : 01/08/2018
  * @Modificacion : 11/06/2019
  * @Modificacion2 : 22/07/2024 @Luis Ariza
    Se agrego el parametro p_cdgo_orgen para identificar cuando el origen del recaudo es del modulo de declaraciones.
    Permitiendo determinar la aplicación correcta del calculo de interes y descuentos de las declaraciones.
  */
  ...
  function prc_ap_recaudo_prprcnal(p_cdgo_clnte        in df_s_clientes.cdgo_clnte%type,
                                   ...
                                   p_cdgo_orgen        in varchar2 default null)
    return g_ap_rcdo
    pipelined is
  ...
  begin
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          6,
                          'Entrando prc_ap_rcdo_prprcnal p_json_crtra => ' ||
                          p_json_crtra,
                          6);
     ...
     (select *
        from json_table(nvl(p_json_crtra, '[]'),
                        ...
                        columns(id_mvmnto_fncro number path
                                ...
                                '$.indcdor_intres_bncrio', --ley 2155 descuentos 16/11/2021

                                vlor_intres_bncrio number path
                                '$.vlor_intres_bncrio')) --ley 2155 descuentos 16/11/2021
      )
    select a.*, a.vlor_sldo_cptal + a.vlor_intres as vlor_ttal
      ...
      from (select a.vgncia,
              ...
              from (select a.vgncia,
                           ...
                           --when (b.gnra_intres_mra = 'S' and trunc(a.fcha_vncmnto) < trunc(sysdate) and b.id_cncpto_intres_mra is not null) then
                           --Req0025065 Para las rentas, si se proyectaba el documento con intereses y se pagaba antes de que se venciera la liquidacion
                           --los intereses se van a saldo a favor,  ej: la deuda se vence el 24/05/2024 el documento se proyecta al 31/05/2024 con intereses
                           --y se paga el documento el 21/05/2024, para este ejemplo, se va el interes a saldo a favor ya que la a.fcha_vncmnto no es menor al sysdate
                             when (b.gnra_intres_mra = 'S' and
                                  v_indcdor_clcla_intres = 'S' and
                                  trunc(a.fcha_vncmnto) <
                                  trunc(p_fcha_vncmnto) and
                                  b.id_cncpto_intres_mra is not null) then --AGREGUE AND v_indcdor_clcla_intres
                              pkg_gf_movimientos_financiero.fnc_cl_interes_mora(p_cdgo_clnte        => p_cdgo_clnte,
                                                                                p_id_impsto         => p_id_impsto,
                                                                                p_id_impsto_sbmpsto => p_id_impsto_sbmpsto,
                                                                                p_vgncia            => a.vgncia,
                                                                                p_id_prdo           => a.id_prdo,
                                                                                p_id_cncpto         => a.id_cncpto,
                                                                                p_vlor_cptal        => --Se agrega case para determinar la aplicación del calculo de interes teniendo en cuenta
                                                                                --el valor final del concepto capital menos su descuento cuyo origen es de DECLARACIONES.
                                                                                 case
                                                                                   when p_cdgo_orgen = 'DCLL' then
                                                                                    (a.vlor_sldo_cptal - nvl(d.vlor_dscnto, 0))
                                                                                   else
                                                                                    a.vlor_sldo_cptal
                                                                                 end,
                                                                                p_indcdor_clclo     => 'CLD',
                      ...
                      from json_table(nvl(p_json_crtra, '[]'),
                                      ...
                                      columns(vgncia number path '$.vgncia',
                                              ...
                                              '$.id_orgen')) a
                      left join w_a d
                        on a.id_mvmnto_fncro = d.id_mvmnto_fncro
                       and a.id_impsto_acto_cncpto = d.id_impsto_acto_cncpto
                       and a.id_cncpto = d.id_cncpto_rlcnal
    ...
    begin
      ...
      for i in 1 .. v_crtra.count loop
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              'pkg_re_recaudos.prc_ap_rcdo_prprcnal',
                              6,
                              'v_crtra(i).vlor_sldo_cptal = ' || v_crtra(i).vlor_sldo_cptal,
                              2);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              'pkg_re_recaudos.prc_ap_rcdo_prprcnal',
                              6,
                              'v_crtra(i).vlor_intres = ' || v_crtra(i).vlor_intres,
                              2);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              'pkg_re_recaudos.prc_ap_rcdo_prprcnal',
                              6,
                              'v_crtra(i).vlor_dscnto_intres; = ' || v_crtra(i).vlor_dscnto_intres,
                              2);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              'pkg_re_recaudos.prc_ap_rcdo_prprcnal',
                              6,
                              'v_crtra(i).vlor_ttal; = ' || v_crtra(i).vlor_ttal,
                              2);
      ...
      a_ttal_intres := (case
                         when v_vlor_ttal <> 0 and v_indcdor_cnvnio = 'S' then
                          trunc(((v_vlor_rcdo + v_vlor_ttal_dcto +
                                v_vlor_ttal_dcto_in) * v_ttal_intres) /
                                v_vlor_ttal)
      ...
      if v_indcdor_cnvnio = 'S' then
        a_ttal_cptal := ((v_vlor_rcdo + v_vlor_ttal_dcto +
                        v_vlor_ttal_dcto_in) - a_ttal_intres);
      end if;
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            'pkg_re_recaudos.prc_ap_rcdo_prprcnal',
                            6,
                            'Proporcion v_ttal_cptal = ' || v_ttal_cptal,
      ...
      for i in 1 .. v_crtra.count loop
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              'pkg_re_recaudos.prc_ap_rcdo_prprcnal',
                              6,
                              'CONCEPTO v_crtra(i).id_cncpto =>' || v_crtra(i).id_cncpto,
                              2);
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              'pkg_re_recaudos.prc_ap_rcdo_prprcnal',
                              6,
                              'APLICACIÓN RECAUDO CONCEPTO  v_crtra(i).crtra_vlor_cptal =>' || v_crtra(i).crtra_vlor_cptal,
                              2);
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              'pkg_re_recaudos.prc_ap_rcdo_prprcnal',
                              6,
                              'APLICACIÓN RECAUDO CONCEPTO  v_crtra(i).crtra_vlor_intres =>' || v_crtra(i).crtra_vlor_intres,
                              2);
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              'pkg_re_recaudos.prc_ap_recaudo_prprcnal',
                              6,
                              'Antes if v_indcdor_cnvnio =  S v_ap_intres = ' ||
                              v_ap_intres ||
                              ' v_crtra(i).vlor_dscnto_intres ' || v_crtra(i).vlor_dscnto_intres,
                              2);

        /*if v_indcdor_cnvnio =  S v_ap_cptal = 5391 v_ap_intres -1 a_ttal_cptal : 1138420
        if v_indcdor_cnvnio =  S v_ap_intres = -1 v_crtra(i).vlor_dscnto_intres 4500
        Antes if v_indcdor_cnvnio =  S v_ap_intres = 4499 v_crtra(i).vlor_dscnto_intres 4500

        if v_indcdor_cnvnio =  S v_ap_cptal = 44913 v_ap_intres -1 a_ttal_cptal : 1138420
        if v_indcdor_cnvnio =  S v_ap_intres = -1 v_crtra(i).vlor_dscnto_intres 37350
        Antes if v_indcdor_cnvnio =  S v_ap_intres = 37349 v_crtra(i).vlor_dscnto_intres 37350*/

        if v_indcdor_cnvnio = 'S' then
          v_ap_cptal := v_ap_cptal - v_crtra(i).vlor_dscnto_cptal;

          --Si el descuento es mayor al interes calculado, el interes a aplicar es cero
          --Este caso se da para las carteras NO redondeadas (migracion) y descuento del 100%
          if v_crtra(i).vlor_dscnto_intres > v_ap_intres then
            v_ap_intres := 0;
          else
            v_ap_intres := v_ap_intres - v_crtra(i).vlor_dscnto_intres; --asi esta
          end if;
          /*
          if v_ap_intres between -100 and 100 then
             v_ap_intres := 0;
          end if;*/
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                'pkg_re_recaudos.prc_ap_recaudo_prprcnal',
                                6,
                                'if v_indcdor_cnvnio =  S v_ap_intres = ' ||
                                v_ap_intres ||
                                ' v_crtra(i).vlor_dscnto_intres ' || v_crtra(i).vlor_dscnto_intres,
                                2);
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                'pkg_re_recaudos.prc_ap_recaudo_prprcnal',
                                6,
                                'if v_indcdor_cnvnio =  S v_ap_cptal = ' ||
                                v_ap_cptal || ' v_ap_intres ' ||
                                v_ap_intres || ' a_ttal_cptal : ' ||
                                a_ttal_cptal,
                                2);
  ...
  */
  ...
  procedure prc_ap_declaracion(p_id_usrio       in sg_g_usuarios.id_usrio%type,
                               p_cdgo_clnte     in df_s_clientes.cdgo_clnte%type,
                               p_id_rcdo        in re_g_recaudos.id_rcdo%type,
                               p_vlor_sldo_fvor in number default 0,
                               o_id_sldo_fvor   out gf_g_saldos_favor.id_sldo_fvor%type,
                               o_cdgo_rspsta    out number,
                               o_mnsje_rspsta   out varchar2) as
    v_nvel               number;
    ...
    v_vlor_rcdo          number; --variable que contiene el valor del recaudo más el valor p_vlor_sldo_fvor.
  begin
    ...
    if (v_gi_g_declaraciones.id_dclrcion_crrccion is not null) then
      ...
      o_cdgo_rspsta  := 3;
      o_mnsje_rspsta := o_cdgo_rspsta || '. ' ||
                        'Se deja el json descuento vacio, para no aplicar dos veces descuentos cuando es una corrección';
      pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                            p_id_impsto  => null,
                            p_nmbre_up   => v_nmbre_up,
                            p_nvel_log   => v_nvel,
    ...
    v_vlor_rcdo := v_re_g_recaudos.vlor + p_vlor_sldo_fvor;
    for c_crtra in (select a.*
                    -- from table ( pkg_re_recaudos.prc_ap_recaudo_prprcnal( p_cdgo_clnte        => p_cdgo_clnte
                      from table(pkg_re_recaudos.prc_ap_recaudo_prprcnal_correccion(p_cdgo_clnte           => p_cdgo_clnte,
                                                                                    ...
                                                                                    p_vlor_rcdo            => v_vlor_rcdo --  v_re_g_recaudos.vlor
                                                                                   ,
                                                                                    p_json_crtra           => v_json,
                                                                                    p_id_dclrcion_crrccion => v_gi_g_declaraciones.id_dclrcion_crrccion)) a) loop
      ...
      begin
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'EN EL FOR  DE c_crtra pkg_re_recaudos.prc_ap_recaudo_prprcnal: ' ||
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'EN EL FOR  DE c_crtra.vlor_hber: ' ||
                              c_crtra.vlor_hber,
                              1);

        --Saldo a Favor, Pago Capital y Pago Interes
        if (c_crtra.cdgo_mvmnto_tpo in ('SF', 'PC', 'PI' /*, 'DC','DI'*/)) then
        ...
        begin
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'id_cncpto: ' || c_crtra.id_cncpto,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'c_crtra.vlor_dbe: ' || c_crtra.vlor_dbe ||
                                ' - c_crtra.vlor_hber: ' ||
                                c_crtra.vlor_hber,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
    ...
    if ((v_re_g_recaudos.vlor + p_vlor_sldo_fvor) <> v_vlor_aplcdo) then
      o_cdgo_rspsta  := 9;
      o_mnsje_rspsta := 'No fue posible aplicar el recaudo, ya que el valor aplicado ' ||
                        to_char(v_vlor_aplcdo,
                                'FM$999G999G999G999G999G999G990') ||
                        ' no corresponde al del recaudo AP_DE_1 ' ||
  ...
  */
  ...
  procedure prc_ap_documento_dno(p_id_usrio     in sg_g_usuarios.id_usrio%type,
    ...
    v_prcntje_prtcpcion_apl re_g_documentos_detalle.prcntje_prtcpcion_apl%type;
    v_cont                  number := 1;
  begin
    ...
    -- Si el documento es por Responsable(porcentualidad)
    if v_re_g_documentos.tpo_gnrcion_dcmnto = 'R' then

      v_json_crtra := '{"carteras":[';
      for c_dtos in (select a.*,
                            count(*) over (partition by null) cursor_length
                       from v_gf_g_cartera_x_concepto a
                      where a.cdgo_clnte = p_cdgo_clnte
                        and a.id_impsto = v_re_g_documentos.id_impsto
                        and a.id_impsto_sbmpsto =
                            v_re_g_documentos.id_impsto_sbmpsto
                        and a.id_sjto_impsto =
                            v_re_g_documentos.id_sjto_impsto
                        and a.id_mvmnto_fncro in
                            (select c.id_mvmnto_fncro
                              ...
                              group by c.id_mvmnto_fncro)
                        and a.vlor_sldo_cptal > 0) loop
        v_vlor_sldo_cptal       := v_vlor_sldo_cptal +
                                   c_dtos.vlor_sldo_cptal;
        v_prcntje_prtcpcion_apl := pkg_re_documentos.fnc_co_responsable_porcentaje_real(p_cdgo_clnte       => p_cdgo_clnte,
                                                                                        p_id_sjto_impsto   => v_re_g_documentos.id_sjto_impsto,
                                                                                        p_id_orgen         => c_dtos.id_orgen,
                                                                                        p_prcntje_rspnsble => v_re_g_documentos.prcntje_prtcpcion);
        ...
        v_json_crtra := v_json_crtra || '{
                        ...
                        c_dtos.cdgo_cncpto || '",
                                        "id_mvmnto_fncro"     : ' ||
                        c_dtos.id_mvmnto_fncro || ',
                                        "vlor_sldo_cptal"     : ' ||
                        (case
                          when v_prcntje_prtcpcion_apl = 100 then
                           c_dtos.vlor_sldo_cptal
                          else
                          ----round( ( c_dtos.vlor_sldo_cptal * ( v_prcntje_prtcpcion_apl / 100 ) )  , -2 )
                           ceil((c_dtos.vlor_sldo_cptal *
                                (v_prcntje_prtcpcion_apl / 100)) / 100) * 100
                        end) || ',
                                        "id_impsto_acto_cncpto" : ' ||
                        c_dtos.id_impsto_acto_cncpto || ',
                                        "fcha_vncmnto"      : "' ||
                        to_char(c_dtos.fcha_vncmnto, 'yyyy-mm-dd') || '",
                                        "cdgo_mvmnto_orgn"    : "' ||
                        c_dtos.cdgo_mvmnto_orgn || '",
                                        "id_orgen"        : ' ||
                        c_dtos.id_orgen || '}';

        if v_cont < c_dtos.cursor_length then
          v_json_crtra := v_json_crtra || ',';
          v_cont       := v_cont + 1;
        end if;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                          ...
                          p_nmbre_up   => v_nmbre_up,
                          p_nvel_log   => v_nvel,
                          p_txto_log   => 'v_json_dscnto: ' || v_json_dscnto,
                          p_nvel_txto  => 3);
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                          ...
                          p_nmbre_up   => v_nmbre_up,
                          p_nvel_log   => v_nvel,
                          p_txto_log   => 'FOTO EN re_g_recaudos_cartera',
                          p_nvel_txto  => 3);
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                          ...
                          p_nmbre_up   => v_nmbre_up,
                          p_nvel_log   => v_nvel,
                          p_txto_log   => 'ANTES DE prc_ap_recaudo_prprcnal',
                          p_nvel_txto  => 3);
        ...
        begin
          ...
          values
            (sq_gf_g_movimientos_detalle.nextval,
             ...
             c_crtra.fcha_vncmnto,
             c_crtra.id_impsto_acto_cncpto,
             v_re_g_documentos.cdgo_idntfccion_tpo,
             v_re_g_documentos.idntfccion /*
                                                                                                                                                                                                                                                                                                                                                                                         decode( v_re_g_documentos.tpo_gnrcion_dcmnto , 'R' , v_re_g_documentos.cdgo_idntfccion_tpo, null ),
                                                                                                                                                                                                                                                                                                                                                                                         decode( v_re_g_documentos.tpo_gnrcion_dcmnto , 'R' , v_re_g_documentos.idntfccion, null )*/);
        exception
          when others then
            o_cdgo_rspsta  := 8;
            o_mnsje_rspsta := o_cdgo_rspsta ||
        ...
        begin
          ...
          r_ap_rcdo.vlor_hber := (case
                                   when (v_dfrncia < 0) then
                                    c_crtera.vlor_sldo_cptal
                                   when (v_dfrncia >= 0 and
                                        c_crtera.vlor_dscnto_cptal > 0) then
                                    (v_vlor_dbe_dcmnto_dtlle -
                                    c_crtera.vlor_dscnto_cptal)
                                   else
                                    v_vlor_dbe_dcmnto_dtlle
  ...
  begin
    ...
    if (v_re_g_recaudos.vlor <> v_vlor_aplcdo) then
      ...
      o_mnsje_rspsta := 'No fue posible aplicar el recaudo, ya que el valor aplicado ' ||
                        to_char(v_vlor_aplcdo,
                                'FM$999G999G999G999G999G999G990') ||
                        ' no corresponde al del recaudo AP_DE_2 ' ||
                        to_char(v_re_g_recaudos.vlor,
                                'FM$999G999G999G999G999G999G990') || '.';
      return;
    end if;
  ...
  * @Creacion       : 01/08/2018
  * @Modificacion   : 01/08/2018
  * @Modificacion   : 07/11/2024 Luis Ariza
  */
  ...
  begin
    ...
    begin
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                            ...
                            p_nvel_log   => v_nvel,
                            p_txto_log   => 'SUJETO: ' ||
                                            v_re_g_recaudos.id_sjto_impsto,
                            p_nvel_txto  => 3);
    ...
    if (v_re_g_recaudos.cdgo_rcdo_orgn_tpo = 'DC') then
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nvel,
                            'INGRESA A DECLARACIONES UTL. VLOR RCDO: ' ||
                            v_re_g_recaudos.vlor,
                            3);
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nvel,
                            'VLOR RCDO:.' || v_re_g_recaudos.vlor,
                            3);

      --Indica si se Aplica el Recaudo de Declaracion
      v_vlor_ttal_rcdo := (v_re_g_recaudos.vlor + v_vlor_sldo_fvor); --Se suma el saldo a favor generado de la correción más el valor del recaudo
      if (v_vlor_ttal_rcdo > 0) then

        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nvel,
                              'INICA AP DECLARACIONES VLOR > ' ||
                              v_re_g_recaudos.vlor,
                              3);
        ...
        pkg_re_recaudos.prc_ap_declaracion(p_id_usrio       => p_id_usrio,
                                           p_cdgo_clnte     => p_cdgo_clnte,
                                           p_id_rcdo        => p_id_rcdo,
                                           p_vlor_sldo_fvor => v_vlor_sldo_fvor,
                                           o_id_sldo_fvor   => v_id_sldo_fvor,
                                           o_cdgo_rspsta    => o_cdgo_rspsta,
                                           o_mnsje_rspsta   => o_mnsje_rspsta);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'TERMINA AP DECLARACIONES VLOR > 0.',
                              3);
    ...
    --Req0028972
    if (v_re_g_recaudos.cdgo_rcdo_orgn_tpo = 'DC') then
      -- Se consulta si al documento pagado se encuentra en la tabla para realizar
      -- la accion de informar la aplicacion del pago
      begin
        select count(1)
          into v_exste
          from gn_g_acciones_dinamica
         where cdgo_prcso = 'RE'
           and id_impsto = v_re_g_documentos.id_impsto
           and id_impsto_sbmpsto = v_re_g_documentos.id_impsto_sbmpsto
           and nmbre_tbla = 'RE_G_DOCUMENTOS'
           and nmbre_cmpo = 'ID_DCMNTO'
           and orgen = v_re_g_documentos.id_dcmnto
           and estdo_prcsdo = 'PENDIENTE'
           and url_ntfccion is not null;

        if v_exste > 0 then
          -- Se consulta la up que debe ejecutar para realizar la accion parametrizada
          -- y se actualiza si el registro esta listo para procesar para que el job de co_transacciones solo
          -- procese a los que le entraron pago, pero por algun motivo no se pudo notificar
          begin
            ...
            pkg_gn_acciones_dinamicas.prc_gn_accion(p_cdgo_clnte        => p_cdgo_clnte,
                                                    p_id_impsto         => v_re_g_documentos.id_impsto,
                                                    p_id_impsto_sbmpsto => v_re_g_documentos.id_impsto_sbmpsto,
                                                    p_cdgo_prcso        => 'RE',
                                                    p_nmbre_tbla        => 'RE_G_DOCUMENTOS',
                                                    p_nmbre_cmpo        => 'ID_DCMNTO',
                                                    p_orgen             => v_re_g_documentos.id_dcmnto,
                                                    o_cdgo_rspsta       => v_cdgo_rspsta,
                                                    o_mnsje_rspsta      => v_mnsje_rspsta);
          exception
            when others then
  ...
  * @Modificacion : 11/06/2019
  * @Modificación : ?????   @Nivis
  * @Modificacion2 : 15/08/2024 @Luis Ariza
  Se agrego el parametro p_cdgo_orgen para identificar cuando el origen del recaudo es del modulo de declaraciones.
  Permitiendo determinar la aplicación correcta del calculo de interes y descuentos de las declaraciones.
  */
  function prc_ap_recaudo_prprcnal_correccion(p_cdgo_clnte           in df_s_clientes.cdgo_clnte%type,
                                              p_id_impsto            in df_c_impuestos.id_impsto%type,
                                              p_id_impsto_sbmpsto    in df_i_impuestos_subimpuesto.id_impsto_sbmpsto%type,
                                              p_fcha_vncmnto         in date,
                                              p_vlor_rcdo            in number,
                                              p_json_crtra           in clob,
                                              p_id_dclrcion_crrccion in number default null,
                                              p_id_dcmnto            in number default null)
    return g_ap_rcdo
    pipelined is
    type t_crtra is record(
      ...
      crtra_vlor_cptal        number,
      crtra_vlor_intres       number,
      clclo_dscnto_intres     number, --NUEVO
      vlor_sldo_cptal         number,
      vlor_intres             number,
      vlor_dscnto_cptal       number,
      id_cncpto_dscnto_cptal  df_i_conceptos.id_cncpto%type,
    ...
    v_fcha_cngla_intres    date; --Insolvencia Acuerdos de Pago
    v_nvel                 number := 6;
    v_nmbre_up             varchar2(100) := 'pkg_re_recaudos.prc_ap_recaudo_prprcnal_correccion';

    v_aplca_dscnto varchar2(10);
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nvel,
                          'v_indcdor_cnvnio: ' || v_indcdor_cnvnio,
                          p_nvel_txto => 1);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          v_nmbre_up,
                          v_nvel,
                          'v_indcdor_inslvncia: ' || v_indcdor_inslvncia,
                          p_nvel_txto => 1);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          v_nmbre_up,
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nvel,
                          'v_fcha_cngla_intres: ' || v_fcha_cngla_intres,
                          p_nvel_txto => 1);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          v_nmbre_up,
    ...
    select a.*,
      ...
      from (select a.vgncia,
                   ...
                   a.vlor_sldo_cptal       as crtra_vlor_cptal,
                   a.vlor_intres           as crtra_vlor_intres,
                   --Calculo Descuento Interes
                   (select z.vlor_dscnto
                      from table(pkg_re_documentos.fnc_cl_descuento_x_vgncia_prdo(p_cdgo_clnte           => p_cdgo_clnte,
                                                                                  ...
                                                                                  p_fcha_pryccion        => to_date(p_fcha_vncmnto,
                                                                                                                    'dd/mm/yyyy'),
                                                                                  p_vlor                 => a.vlor_intres,
                                                                                  p_cdna_vgncia_prdo_pgo => '{}',
                                                                                  p_cdgo_orgen           => 'DCL')) z
                     where z.id_dscnto_rgla is not null) as clclo_dscnto_intres, --Se agrega la linea "where   z.id_dscnto_rgla is not null" para solo traer los registros de descuentos otorgados
    ...
    declare
      ...
      v_vlor_ttal_dcto_in    number := 0;
      v_cncpto_ica           number;
      v_dfrncia_ttal_rcdo    number := 0;
      v_dfrncia_ttal_rcdo_sf number := 0;
    begin
      ...
      select id_cncpto
        ...
        from df_i_conceptos
       where id_impsto =
             (select id_impsto from df_c_impuestos where cdgo_impsto = 'ICA')
         and cdgo_cncpto = '001';
      ...
      for i in 1 .. v_crtra.count loop
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'id_cncpto_intres_mra: ' || v_crtra(i).id_cncpto_intres_mra,
                              p_nvel_txto => 1);
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'crtra_vlor_cptal: ' || v_crtra(i).crtra_vlor_cptal,
                              p_nvel_txto => 1);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nvel,
                              'crtra_vlor_intres: ' || v_crtra(i).crtra_vlor_intres,
                              p_nvel_txto => 1);
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'vlor_sldo_cptalXXXXXXXXX: ' || v_crtra(i).vlor_sldo_cptal,
                              p_nvel_txto => 1);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nvel,
                              'vlor_intres: ' || v_crtra(i).vlor_intres,
                              p_nvel_txto => 1);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nvel,
                              'clclo_dscnto_intres: ' || v_crtra(i).clclo_dscnto_intres,
                              p_nvel_txto => 1);
        ...
        begin
          select case
           ...
           WHERE a.id_dclrcion = p_id_dclrcion_crrccion
             and a.id_cncpto_rlcnal = v_crtra(i).id_cncpto_intres_mra;
        exception
          when no_data_found then
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nvel,
                                  'No encontró descuentos para el concepto interes : ' || v_crtra(i).id_cncpto_intres_mra,
                                  p_nvel_txto => 1);
          ...
          when others then
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  ...
                                  v_nmbre_up,
                                  v_nvel,
                                  'Error al consultar el descuentos para el concepto interes : ' || v_crtra(i).id_cncpto_intres_mra,
                                  p_nvel_txto => 1);
        ...
        if p_id_dclrcion_crrccion is not null then
          ...
          if v_crtra(i)
           .crtra_vlor_intres - nvl(v_crtra(i).clclo_dscnto_intres, 0) <= 0 and
              v_aplca_dscnto = 'S' then
            v_crtra(i).vlor_intres := 0;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nvel,
                ...
                .crtra_vlor_intres - nvl(v_crtra(i).clclo_dscnto_intres, 0)) > 0 and -- nvl(c.indcdor_intres_bncrio, 'M') = 'M' and
                v_indcdor_cnvnio = 'N' and v_aplca_dscnto = 'S' then
            v_crtra(i).vlor_intres := (v_crtra(i)
                                      .vlor_intres -
                                       nvl(v_crtra(i).clclo_dscnto_intres, 0));
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nvel,
                ...
                .crtra_vlor_intres - nvl(v_crtra(i).clclo_dscnto_intres, 0)) > 0 and
               --nvl(c.indcdor_intres_bncrio, 'M') = 'M' and
                v_indcdor_cnvnio = 'S' and v_aplca_dscnto = 'S' then
            v_crtra(i).vlor_intres := v_crtra(i).vlor_intres;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'vlor_intres nuevo: ' || v_crtra(i).vlor_intres,
                              p_nvel_txto => 1);
        ...
        if p_id_dclrcion_crrccion is not null then
          v_vlor_ttal := v_vlor_ttal + v_crtra(i).vlor_sldo_cptal + v_crtra(i).vlor_intres;
        else
          v_vlor_ttal := v_vlor_ttal + v_crtra(i).vlor_ttal;
        end if;
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                              ...
                              p_nmbre_up   => v_nmbre_up,
                              p_nvel_log   => v_nvel,
                              p_txto_log   => 'v_vlor_ttal: ' || v_vlor_ttal,
                              p_nvel_txto  => 1);
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                              ...
                              p_nmbre_up   => v_nmbre_up,
                              p_nvel_log   => v_nvel,
                              p_txto_log   => 'v_vlor_rcdo: ' || v_vlor_rcdo,
                              p_nvel_txto  => 1);
      ...
      v_dfrncia_ttal_rcdo := v_vlor_ttal - v_vlor_rcdo;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nvel,
                            'Diferencia: ' || v_dfrncia_ttal_rcdo,
                            1);

      if (v_dfrncia_ttal_rcdo > 0 and v_dfrncia_ttal_rcdo <= 2500) then
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nvel,
                              'Diferencia: ' || v_dfrncia_ttal_rcdo,
                              1);
        v_ttal_intres := v_ttal_intres - v_dfrncia_ttal_rcdo;
        v_vlor_ttal   := v_vlor_ttal - v_dfrncia_ttal_rcdo;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nvel,
                              'v_ttal_intres Diferencia: ' || v_ttal_intres,
                              1);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nvel,
                              'v_vlor_ttal Diferencia: ' || v_vlor_ttal,
                              1);
      end if;

      if v_dfrncia_ttal_rcdo < 0 then
        if (v_dfrncia_ttal_rcdo * (1 - 2)) > 0 and
           (v_dfrncia_ttal_rcdo * (1 - 2)) <= 2500 then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'Entro en else Diferencia: ' ||
                                v_dfrncia_ttal_rcdo,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                ...
                                'Diferencia: ' || v_dfrncia_ttal_rcdo,
                                1);
          if v_ttal_intres >= 0 then
            v_ttal_intres := v_ttal_intres +
                             (v_dfrncia_ttal_rcdo * (1 - 2));
            v_vlor_ttal   := v_vlor_ttal + (v_dfrncia_ttal_rcdo * (1 - 2));
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nvel,
                                  'v_ttal_intres Diferencia: ' ||
                                  v_ttal_intres,
                                  1);
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
            ...
            v_ttal_cptal := v_ttal_cptal + (v_dfrncia_ttal_rcdo * (1 - 2));
            v_vlor_ttal  := v_vlor_ttal + (v_dfrncia_ttal_rcdo * (1 - 2));
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'v_vlor_rcdo =>  ' || v_vlor_rcdo,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'v_vlor_ttal =>  ' || v_vlor_ttal,
                                1);
        end if;
      end if;

      /* v_dfrncia_ttal_rcdo_sf :=  v_vlor_rcdo - v_vlor_ttal;
      if (v_dfrncia_ttal_rcdo > 0 and v_dfrncia_ttal_rcdo <= 1000) then
        pkg_sg_log.prc_rg_log( p_cdgo_clnte , null , v_nmbre_up, v_nvel , 'v_dfrncia_ttal_rcdo_sf =>  '||v_dfrncia_ttal_rcdo_sf , 1 );

        v_dfrncia_ttal_rcdo_sf := v_vlor_rcdo - v_dfrncia_ttal_rcdo_sf ;

        pkg_sg_log.prc_rg_log( p_cdgo_clnte , null , v_nmbre_up, v_nvel , 'Valor final recaudo =>  '||v_dfrncia_ttal_rcdo_sf , 1 );
      ...
      for i in 1 .. v_crtra.count loop
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'v_crtra(i).id_impsto_acto_cncpto;=>  ' || v_crtra(i).id_impsto_acto_cncpto,
                              1);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nvel,
                              'v_crtra(i).id_cncpto =>  ' || v_crtra(i).id_cncpto,
                              1);
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
        ...
        if (v_dfrncia_ttal_rcdo > 0 and v_dfrncia_ttal_rcdo <= 2500 and v_crtra(i)
           .id_cncpto = v_cncpto_ica) then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 1. REGISTRO HABER  v_dfrncia_ttal_rcdo =>  ' ||
                                v_dfrncia_ttal_rcdo,
                                1);
          ...
          v_ap_intres := (case
                           when v_ttal_intres <> 0 then
                            trunc((a_ttal_intres *
                                  (v_crtra(i).vlor_intres - v_dfrncia_ttal_rcdo)) /
                                  v_ttal_intres)
                           else
                            0
                         end);
          ...
          v_ap_intres := (case
                           when v_ttal_intres <> 0 then
                            ...
                            0
                         end);*/ --habilitar  si no funciona
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'Descuento interes ICA =>  ' ||
                                (v_crtra(i)
                                .vlor_intres - v_dfrncia_ttal_rcdo),
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'última proporcion =>  ' || v_ttal_intres,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'última proporcion =>  ' || a_ttal_intres,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'última proporcion =>  ' || v_crtra(i).vlor_intres,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'última proporcion =>  ' || v_ap_intres,
                                1);

        end if;

        if v_dfrncia_ttal_rcdo < 0 and (v_dfrncia_ttal_rcdo * (1 - 2)) > 0 and
           (v_dfrncia_ttal_rcdo * (1 - 2)) <= 2500 and v_crtra(i)
          .id_cncpto = v_cncpto_ica then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 2. en diferencia negativa  v_crtra(i).vlor_intres =>  ' || v_crtra(i).vlor_intres,
                                1);

          v_ap_intres := (case
                           when v_ttal_intres <> 0 then
                            trunc((a_ttal_intres * (v_crtra(i).vlor_intres + (v_dfrncia_ttal_rcdo *
                                   (1 - 2)))) / v_ttal_intres)
                           else
                            0
                         end);
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                'última proporcion =>  ' || v_ap_intres,
                                1);
        elsif v_dfrncia_ttal_rcdo < 0 and
              (v_dfrncia_ttal_rcdo * (1 - 2)) > 0 and
              (v_dfrncia_ttal_rcdo * (1 - 2)) <= 2500 and v_crtra(i)
             .id_cncpto != v_cncpto_ica and i = 1 and v_crtra(i).vlor_intres > 0 then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 3. en el primer concepto con interes mayor a cero posición =>  ' || i,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
        ...
        elsif v_dfrncia_ttal_rcdo > 0 and v_dfrncia_ttal_rcdo <= 2500 and v_crtra(i)
             .id_cncpto != v_cncpto_ica and i = 1 and v_crtra(i).vlor_intres > 0 then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 4. NUEVA CONDICIÓN en v_crtra(i).vlor_intres=>  ' || v_crtra(i).vlor_intres,
                                1);
        ...
        elsif v_crtra(i).id_cncpto != v_cncpto_ica and v_crtra(i).vlor_intres > 0 and
               v_dfrncia_ttal_rcdo <= 2500 then
          --VALIDAR CONDICIÓN ERROR APLICAR PAGO

          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 5. en v_crtra(i).vlor_intres=>  ' || v_crtra(i).vlor_intres,
                                1);
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                '3. última proporcion =>  ' || v_crtra(i).vlor_intres,
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                '4. última proporcion =>  ' || v_ap_intres,
                                1);

        elsif v_crtra(i).id_cncpto != v_cncpto_ica and v_crtra(i).vlor_intres = 0 then

          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 6. en v_crtra(i).vlor_intres=>  ' || v_crtra(i).vlor_intres,
                                1);
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 7. en v_crtra(i).vlor_intres=>  ' || v_crtra(i).vlor_intres,
                                1);
        ...
        elsif v_dfrncia_ttal_rcdo = 0 and v_crtra(i)
             .id_cncpto = v_cncpto_ica then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 8. en v_crtra(i).vlor_intres=>  ' || v_crtra(i).vlor_intres,
                                1);
        ...
        elsif v_dfrncia_ttal_rcdo > 2500 and v_crtra(i)
             .id_cncpto != v_cncpto_ica then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 9. en v_crtra(i).vlor_intres=>  ' || v_crtra(i).vlor_intres,
                                1);
          v_ap_intres := (case
                           when v_ttal_intres <> 0 then
                            trunc((a_ttal_intres * v_crtra(i).vlor_intres) /
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'ENTRO 10. en v_crtra(i).vlor_intres=>  ' || v_crtra(i).vlor_intres,
                                1);
          v_ap_intres := (case
                           when v_ttal_intres <> 0 then
                            trunc((a_ttal_intres * v_crtra(i).vlor_intres) /
        ...
        if (i = v_crtra.count) then
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'Entro en if (i = v_crtra.count) v_sldo_na =>  ' ||
                                v_sldo_na,
                                1);
        end if;
        ...
        if (v_crtra(i).vlor_dscnto_cptal > 0) then
          ...
          v_ap_rcdo.id_cncpto_csdo   := v_crtra(i).id_cncpto_dscnto_cptal;
          v_ap_rcdo.id_cncpto_rlcnal := v_crtra(i).id_cncpto;
          v_ap_rcdo.vlor_hber        := round(v_crtra(i).vlor_dscnto_cptal,
                                              0); --,-3);
          --Guarda Fila 2 en Coleccion
          r_ap_rcdo.extend;
          r_ap_rcdo(r_ap_rcdo.count) := v_ap_rcdo;
        ...
        if (v_ap_cptal > 0) then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'Entro en if (v_ap_cptal > 0) v_ap_cptal =>  ' ||
                                v_ap_cptal,
                                1);
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'PRIMER RESULTADO  (v_ap_cptal + v_sldo_na) ' ||
                                (v_ap_cptal + v_sldo_na),
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'SEGUNDO RESULTADO  (v_ap_cptal - v_sldo_na)  ' ||
                                (v_ap_cptal - v_sldo_na),
                                1);
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                v_nmbre_up,
                                v_nvel,
                                'TERCER RESULTADO  v_ap_cptal ' ||
                                (v_ap_cptal),
                                1);
          if (v_sldo_na <= 2500 and v_sldo_na > 0 and i = v_crtra.count) then
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
                                  v_nvel,
                                  ...
                                  (v_ap_cptal + v_sldo_na),
                                  1);
            v_ap_rcdo.vlor_hber := round(v_ap_cptal + v_sldo_na, 0); --,-3);
            v_sldo_na           := 0;
            /*elsif (v_sldo_na < 0  and  (v_sldo_na * (1-2)) <= 2500  and i = v_crtra.count )then
            pkg_sg_log.prc_rg_log( p_cdgo_clnte , null , v_nmbre_up, v_nvel , 'NUEVO IF  2 v_sldo_na  '|| (v_ap_cptal - (v_sldo_na * (1-2))) , 1 );
            v_ap_rcdo.vlor_hber  := round(v_ap_cptal + v_sldo_na,0);--,-3);
            v_sldo_na := 0;*/
          else
            v_ap_rcdo.vlor_hber := round(v_ap_cptal, 0); --,-3);
            -- v_ap_rcdo.vlor_hber  := round(v_ap_cptal + (v_sldo_na * (1-2)) ,0);--,-3);

          end if;
          --v_ap_rcdo.vlor_hber       := round(v_ap_cptal,0);--,-3);  --HABILITAR PARA PROCESO DE APLICACIÓN ANTERIOR.
          --Guarda Fila 3 en Coleccion
          r_ap_rcdo.extend;
          r_ap_rcdo(r_ap_rcdo.count) := v_ap_rcdo;
        end if;
        ...
        begin
          ...
          if (v_crtra(i).vlor_dscnto_intres = 0 and v_ap_intres > 0) then
            ...
            --Ingreso Interes
            v_ap_rcdo.vlor_dbe := round(v_ap_intres, 0); --,-3);
            --Guarda Fila 4 en Coleccion
            r_ap_rcdo.extend;
            r_ap_rcdo(r_ap_rcdo.count) := v_ap_rcdo;
            ...
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  ...
                                  v_nmbre_up,
                                  v_nvel,
                                  'Ingreso v_crtra(i).vlor_dscnto_intres =>  ' || v_crtra(i).vlor_dscnto_intres,
                                  1);

            --Ingreso Interes
            v_ap_rcdo.vlor_dbe := round((v_ap_intres + v_crtra(i).vlor_dscnto_intres),
                                        0); --,-3);
            -- v_ap_rcdo.vlor_dbe := greatest(0, round((v_ap_intres + v_crtra(i).vlor_dscnto_intres), 0));
            ...
            v_ap_rcdo.id_cncpto_csdo   := v_crtra(i).id_cncpto_dscnto_intres;
            v_ap_rcdo.vlor_dbe         := 0;
            v_ap_rcdo.vlor_hber        := round(v_crtra(i).vlor_dscnto_intres,
                                                0); --,-3);
            v_ap_rcdo.id_cncpto_rlcnal := v_crtra(i).id_cncpto_csdo;
            --Guarda Fila 6 en Coleccion
            r_ap_rcdo.extend;
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'Verifica  si calculo I  v_ap_intres => ' ||
                              v_ap_intres,
                              1);
        ...
        if (v_ap_intres > 0) then
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'Entro en Verifica  si calculo I  v_ap_intres => ' ||
                                v_ap_intres,
                                1);
          ...
          v_ap_rcdo.cdgo_mvmnto_tpo  := 'PI';
          v_ap_rcdo.vlor_dbe         := 0;
          v_ap_rcdo.vlor_hber        := round(v_ap_intres, 0); --,-3);
          --Guarda Fila 7 en Coleccion
          r_ap_rcdo.extend;
          r_ap_rcdo(r_ap_rcdo.count) := v_ap_rcdo;
        end if;
      ...
      for i in 1 .. r_ap_rcdo.count loop
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              v_nmbre_up,
                              v_nvel,
                              'Entro en el nuevo for  => ' || r_ap_rcdo(i).vlor_hber,
                              1);
        ...
        begin
          ...
          if (r_ap_rcdo(v_index).cdgo_mvmnto_tpo = 'PC') then
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  ...
                                  v_nmbre_up,
                                  v_nvel,
                                  'Entro cdgo_mvmnto_tpo = PC  ' || v_index,
                                  1);
            ...
            if (v_index > 1 and r_ap_rcdo(v_index - 1).cdgo_mvmnto_tpo = 'DC') then
              ...
              pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                    ...
                                    v_nmbre_up,
                                    v_nvel,
                                    'VALOR CONCEPTO =>  ' || r_ap_rcdo(v_index).vlor_hber,
                                    1);
            ...
            if (v_sldo_na >= v_dfrncia) then
              ...
              pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                                    ...
                                    p_nmbre_up   => v_nmbre_up,
                                    p_nvel_log   => v_nvel,
                                    p_txto_log   => 'hay diferencia capital: ' ||
                                                    v_dfrncia,
                                    p_nvel_txto  => 1);

              --El Valor Aplicado en Capital se le Suma la Diferencia
              r_ap_rcdo(v_index).vlor_hber := round((r_ap_rcdo(v_index)
                                                    .vlor_hber + v_dfrncia),
                                                    -3);
              --Se Resta en el Saldo la Diferencia
              v_sldo_na := (v_sldo_na - v_dfrncia);
          ...
          if (r_ap_rcdo(v_index).cdgo_mvmnto_tpo = 'PI') then
            ...
            begin
              ...
              pkg_sg_log.prc_rg_log(p_cdgo_clnte => p_cdgo_clnte,
                                    ...
                                    p_nmbre_up   => v_nmbre_up,
                                    p_nvel_log   => v_nvel,
                                    p_txto_log   => 'hay diferencia interes: ' ||
                                                    v_dfrncia,
                                    p_nvel_txto  => 1);
              ...
              if (0 >= v_dfrncia) then
                ...
                r_ap_rcdo(v_index).vlor_hber := round((r_ap_rcdo(v_index)
                                                      .vlor_hber + v_dfrncia),
                                                      -3);
                r_ap_rcdo(v_ingrso_i).vlor_dbe := round((r_ap_rcdo(v_ingrso_i).vlor_dbe +
                                                         v_dfrncia),
                                                        -3);
                --Se Resta en el Saldo la Diferencia
                v_sldo_na := (v_sldo_na - v_dfrncia);
  ...
  end prc_ap_recaudo_prprcnal_correccion;
