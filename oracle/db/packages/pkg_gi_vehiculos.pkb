  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GI_VEHICULOS" as
  /*R.R:R.R*/
  ...
  procedure prc_co_grupo_adicional(p_id_sjto_impsto in number,
    ...
    cursor c_liquidacion(r_id_sjto_impsto number) is
      select *
        from gi_g_liquidaciones k
       where k.id_sjto_impsto = r_id_sjto_impsto
         and k.vgncia in
             (select max(i.vgncia)
                from gi_g_liquidaciones i
               where i.id_sjto_impsto = r_id_sjto_impsto)
         and k.cdgo_lqdcion_estdo = 'L';

    --36932347
    cursor c_liquidacion_ad(r_liquidacion number) is
      select *
        from gi_g_liquidaciones_ad_vehclo re
       where re.id_lqdcion = r_liquidacion;

    cursor c_grupo_ad(r_id_vhclo_grpo number) is
      select *
        from df_s_vehiculos_grupo pl
  ...
  begin
    ...
    if p_vgncia <= 2023 then
      ...
      exception
        when others then
          o_cdgo_rspsta  := 1;
          o_mnsje_rspsta := ' no se encontro indicador economico ' ||
                            sqlerrm || '-' || sqlcode;
          return;
      end;
    else
      /* calculo del valor minimo  de liquidacion */
      begin
        select trunc(ind.vlor * 3.68)
          into v_vlor_mnmo
          from df_s_indicadores_economico ind
         where p_vgncia between to_char(ind.fcha_dsde, 'yyyy') and
               to_char(ind.fcha_hsta, 'yyyy')
           and ind.cdgo_indcdor_tpo = 'UVT';
      exception
        when others then
          o_cdgo_rspsta  := 1;
          o_mnsje_rspsta := ' no se encontro indicador economico ' ||
  ...
  begin
    ...
    if v_datos_vhclo_msvo is not null then
      for i in v_datos_vhclo_msvo.first .. v_datos_vhclo_msvo.count loop
        ...
        delete from GI_I_LIQUIDACION_2026 a
         where a.id_sjto_impsto = v_datos_vhclo_msvo(i).id_sjto_impsto;
        ...
        sitpr001('Calculando: ' || v_datos_vhclo_msvo(i).id_sjto_impsto || '-' ||
                 ' placa: ' || v_idntfccion || 'marca: ' || v_datos_vhclo_msvo(i).cdgo_vhclo_mrca ||
                 ' Linea:' || v_datos_vhclo_msvo(i).id_vhclo_lnea ||
                 ' Cilindraje:' || v_datos_vhclo_msvo(i).clndrje ||
                 ' modelo:' || v_datos_vhclo_msvo(i).mdlo,
                 'validacion.txt');
        ...
        if v_grupo is null then
          sitpr001('No se encontro Grupo adicional en la placa ' ||
                   v_idntfccion,
                   'validacion.txt');
          v_valida := 'N';
        else
          sitpr001('Se encontro Grupo adicional en la placa ' ||
                   v_idntfccion || ' Grupo: ' || v_grupo,
                   'validacion.txt');
          v_valida := 'S';
        end if;
        ...
        if v_grupo is null then
          if v_datos_vhclo_msvo(i).id_vhclo_lnea = 21059 then
            ...
            sitpr001('No se encontro Grupo en la placa ' || v_idntfccion,
                     'validacion.txt');
            ...
            begin
              select j.id_vhclo_clse_ctgria
                ...
                from df_s_vehiculos_clase_ctgria j
               where j.cdgo_vhclo_clse in
                     (select ai.cdgo_vhclo_clse
                        from df_s_vehiculos_clase_ctgria ai
                       where ai.id_vhclo_clse_ctgria = v_id_clse_ctgria)
                 and j.vgncia = p_vgncia;
            exception
              when others then
                sitpr001('error ' || v_idntfccion || '-' || v_datos_vhclo_msvo(i).id_sjto_impsto,
                         'validacion.txt');
        ...
        if v_grupo is not null then
          ...
          if o_cdgo_rspsta <> 0 then
            ...
            o_mnsje_rspsta := 'Error al pasar la liquidacion No.' ||
                              v_o_id_lqdcion ||
                              ' a movimiento financiero. ' || o_cdgo_rspsta ||
                              ' - ' || o_mnsje_rspsta;
            return;
          end if;
        ...
        insert into gi_i_liquidacion_2026
          (cdgo_clnte,
           id_impsto,
           id_impsto_sbmpsto,
           vgncia,
