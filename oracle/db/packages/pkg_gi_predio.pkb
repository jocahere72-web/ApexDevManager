  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GI_PREDIO" as
  ...
  begin
    ...
    for c_cnta_igac in (select /*+ RESULT_CACHE */
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            v_nmbre_up,
                            v_nivel,
                            'nmero_lnea: ' || c_cnta_igac.nmero_lnea,
                            6);
  ...
  */
  ...
  procedure prc_cd_predio(p_id_usrio          in sg_g_usuarios.id_usrio%type,
                          ...
                          p_lttud             in si_i_predios.lttud%type default null,
                          p_lngtud            in si_i_predios.lngtud %type default null,
      ...
      begin
        update si_i_predios
           set avluo_ctstral        = p_avluo_ctstral,
               ...
               cdgo_prdio_tpo       = nvl(p_cdgo_prdio_tpo, cdgo_prdio_tpo),
               mtrcla_inmblria      = p_mtrcla_inmblria,
               pro_indvso           = nvl(p_pro_indvso, pro_indvso),
               avluo_trrno          = nvl(p_avluo_trrno, avluo_trrno),
               avluo_cnstrccion     = nvl(p_avluo_cnstrccion,
                                          avluo_cnstrccion),
               uso_cnstrccion       = nvl(v_uso_cnstrccion, uso_cnstrccion),
               lttud                = nvl(p_lttud, lttud),
               lngtud               = nvl(p_lngtud, lngtud)
         where id_sjto_impsto = o_id_sjto_impsto
        returning id_prdio into o_id_prdio;
      exception
        when others then
          o_mnsje := o_nmro_error ||
                     '. No fue posible actualizar los datos del predio. -p_avluo_ctstral:' ||
                     p_avluo_ctstral || ' -p_area_trrno:' || p_area_trrno ||
                     ...
                     ' -p_bse_grvble:' || p_bse_grvble || ' -lttud:' ||
                     p_lttud || ' -lngtud:' || p_lngtud ||
      ...
      exception
        when others then
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                p_dstno_ecnmco_igac || ' no encontrado.',
                                3);
      end;
  ...
  */
  ...
  procedure prc_rg_sjto_rspnsbles(p_id_prcso_crga          in et_g_procesos_carga.id_prcso_crga%type,
                                  ...
                                  p_indcdor_tpo_idntfccion in varchar2 default 'C') as
  begin
    ...
    for c_rspnsbles in (select /*+ RESULT_CACHE */
                         ...
                         nvl(ltrim(trim(a.nmro_dcmnto_igac), '0'), '0') as idntfccion,
                        ...
                        -- on trim(a.tpo_dcmnto_igac) = b.cdgo_idntfccion_tpo
                            on trim(a.tpo_dcmnto_igac) =
                               decode(p_indcdor_tpo_idntfccion,
                                      'C',
                                      b.cdgo_idntfccion_tpo,
                                      b.id_idntfccion_tpo) -- req.0024205
                         where a.id_prcso_crga = p_id_prcso_crga
  ...
  */
  ...
  procedure prc_rg_historico_predios(p_cdgo_clnte in df_s_clientes.cdgo_clnte%type,
    ...
    type t_prdio_hstrco is record(
      ...
      vgncia_actlzda       si_h_predios.vgncia_actlzda%type -- Limite Impuesto
      );
      ...
      --Registra Predio Historioco
        ...
        values
          (v_prdio_hstrco(i).id_prdio_hstrco,
           ...
           v_prdio_hstrco(i).vgncia_actlzda);
