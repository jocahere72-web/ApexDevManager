  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_SI_NOVEDADES_PERSONA" as
  /*
  ...
  */
  ...
  begin
    ...
    begin
      ...
      if p_cdgo_nvdad_tpo = 'ACT' then
        ...
        begin
          delete from gn_g_temporal where n001 = p_id_instncia_fljo;
        exception
          when others then
            o_cdgo_rspsta  := 6;
  ...
  procedure prc_rnc_novedad_persona(p_cdgo_clnte     in number,
                                    p_id_nvdad_prsna in number,
                                    p_id_usrio       in number,
                                    p_obsrvcion      in varchar2,
                                    o_cdgo_rspsta    out number,
    ...
    -- !! -------------------------------------------------------------- !! --
    -- !! Procedimiento para Reiniciar las novedades de personas !! --
    -- !! -------------------------------------------------------------- !! --
    ...
    -- Variables de Log
    v_nl       number;
    v_nmbre_up varchar2(1000) := 'pkg_si_novedades_persona.prc_rnc_novedad_persona';
  ...
  begin
    ...
    begin
      update si_g_novedades_persona
         set cdgo_nvdad_prsna_estdo = 'RNC'
       where id_nvdad_prsna = p_id_nvdad_prsna
         and cdgo_clnte = p_cdgo_clnte
         and id_instncia_fljo = v('F_ID_INSTNCIA_FLJO');
    exception
      when others then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := o_cdgo_rspsta || '. ' ||
                          'Error al actualizar la novedad. ' || sqlcode ||
                          ' -- ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    -- Se finaliza el flujo de la novedad
    begin
      pkg_pl_workflow_1_0.prc_rg_finalizar_instancia(p_id_instncia_fljo => v('F_ID_INSTNCIA_FLJO'),
                                                     p_id_fljo_trea     => v('F_ID_FLJO_TREA'),
                                                     p_id_usrio         => p_id_usrio,
                                                     o_error            => v_error,
                                                     o_msg              => o_mnsje_rspsta);
      ...
      if v_error = 'N' then
        o_cdgo_rspsta  := 20;
        o_mnsje_rspsta := o_cdgo_rspsta || '. ' ||
                          'Error al cerrar el flujo. ' || o_mnsje_rspsta;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
    ...
    exception
      when others then
        o_cdgo_rspsta  := 30;
        o_mnsje_rspsta := o_cdgo_rspsta || '. ' ||
                          'Error al cerrar el flujo' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
    ...
    --se extrae el usuario inicial al que se le asigno la novedad
    begin
      ...
      select a.id_usrio
        ...
        join wf_d_flujos_tarea b
         ...
         and indcdor_incio = 'S'
       where id_instncia_fljo = v('F_ID_INSTNCIA_FLJO');
    exception
      When others then
        o_cdgo_rspsta  := 40;
        o_mnsje_rspsta := o_cdgo_rspsta ||
                          ' - no se encontro usuario adignado para esta transición';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              ...
                              6);
        return;
    end; -- Fin extracción de los datos del participante

    --Buscamos el flujo de la solicitud de PQR
    begin
      select id_instncia_fljo, id_fljo_gnrdo
        into v_flujo_pqr, v_id_fljo
        from v_pq_g_solicitudes
       where ID_INSTNCIA_FLJO_GNRDO = v('F_ID_INSTNCIA_FLJO');
    exception
      when others then
        o_cdgo_rspsta  := 50;
        o_mnsje_rspsta := o_cdgo_rspsta ||
                          ' - no se encontro instancia flujo del PQR';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
                              ...
                              6);
        return;
    end; -- Fin busqueda de los datos del flujo PQR

    /*pkg_sg_log.prc_rg_log(p_cdgo_clnte, null,v_nmbre_up, v_nl, 'v(F_ID_INSTNCIA_FLJO): ' ||v('F_ID_INSTNCIA_FLJO') , 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null,v_nmbre_up, v_nl, 'v_id_fljo: ' ||v_id_fljo , 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null,v_nmbre_up, v_nl, 'v_id_prtcpte: ' ||v_id_prtcpte , 6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte, null,v_nmbre_up, v_nl, 'p_id_usrio: ' ||p_id_usrio , 6);*/

    --se crea la nueva instancia flujo
    begin
      pkg_pl_workflow_1_0.prc_rg_instancias_flujo(p_id_fljo          => v_id_fljo,
                                                  p_id_usrio         => null --p_id_usrio -- se comenta debido a que presentan errores al instanciar una nueva instancia de flujo
                                                 ,
                                                  p_id_prtcpte       => v_id_prtcpte,
                                                  o_id_instncia_fljo => v_id_instncia_fljo_hjo,
                                                  o_id_fljo_trea     => v_id_fljo_trea_hja,
                                                  o_mnsje            => v_mnsje);

      --se valida que la nueva instancia se haya creado
      ...
      --se acutaliza el flujo generado con elñ nuevo flujo
      Begin
        update wf_g_instancias_flujo_gnrdo
           set id_instncia_fljo_gnrdo_hjo = v_id_instncia_fljo_hjo
         where id_instncia_fljo_gnrdo_hjo = v('F_ID_INSTNCIA_FLJO')
           and id_instncia_fljo = v_flujo_pqr;
    ...
    exception
      when others then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              ...
                              6);
        return;
    end; --fin de la nueva instancia flujo

    --Se actualiza el usuario de la transición si este aun esta asignado a la tarea
    begin
      v_id_prtcpte := pkg_pl_workflow_1_0.fnc_co_instancias_prtcpnte(p_id_fljo  => v_id_fljo,
                                                                     p_id_usrio => v_id_prtcpte);
      if v_id_prtcpte <> 0 then
        update wf_g_instancias_transicion
           set id_usrio = v_id_prtcpte
    ...
    --Se inserta la observación del nuevo flujo
    begin
      insert into si_g_novedades_observacion
        (cdgo_clnte,
         id_nvdad,
         cdgo_nvdad_prsna_estdo,
         id_instncia_fljo,
         id_instncia_fljo_antror,
         id_fljo_trea,
         id_usrio,
         obsrvcion,
         fcha_obsrvcion)
    ...
    exception
      when others then
        o_cdgo_rspsta  := 90;
        o_mnsje_rspsta := o_cdgo_rspsta || '. ' ||
                          'Error al insertar la observación. ' || sqlcode ||
                          ' -- ' || sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
  ...
  Exception
    when others then
      o_cdgo_rspsta  := 100;
      o_mnsje_rspsta := o_cdgo_rspsta ||
                        ' - No se pudo reiniciar la Novedad persona';
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
                            v_nl,
