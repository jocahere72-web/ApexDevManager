  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GF_PAZ_Y_SALVO" AS
  ...
  begin
    ...
    exception
      when others then
        ...
        o_mnsje_rspsta := o_mnsje_rspsta ||
                          ' Excepcion al Registrar el Paz y salvo. ' ||
                          sqlerrm;
        o_cdgo_rspsta  := 1;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
    ...
    begin
      ...
      if (o_cdgo_rspsta != 0) or o_id_acto is null then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := 'No se genero el acto de paz y salvo. ' ||
                          o_mnsje_rspsta;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
    ...
    exception
      when no_data_found then
        o_mnsje_rspsta := o_mnsje_rspsta ||
                          ' No existe el id acto en la tabla actos. ' ||
                          sqlerrm;
        o_cdgo_rspsta  := 5;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
      ...
      when others then
        o_mnsje_rspsta := o_mnsje_rspsta ||
                          ' No se pudo consultar el numero del acto en la tabla actos. ' ||
                          sqlerrm;
        o_cdgo_rspsta  := 5;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
    ...
    exception
      when others then
        o_mnsje_rspsta := o_mnsje_rspsta ||
                          ' No se pudo actualizar el acto en la tabla de paz y salvo. ' ||
                          sqlerrm;
        o_cdgo_rspsta  := 5;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
  ...
  begin
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := 'No. ' || o_cdgo_rspsta ||
                          ': No se encontro el numero del acto ';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
      ...
      when others then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := 'No. ' || o_cdgo_rspsta ||
                          ': Problemas a consultar el numero del acto ';
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              v_nmbre_up,
                              v_nl,
    ...
    begin
      v_dcmnto := pkg_gn_generalidades.fnc_ge_dcmnto('<cdgo_clnte>' ||
                                                     p_cdgo_clnte ||
                                                     '</cdgo_clnte>
                          <id_impsto>' ||
                                                     p_id_impsto ||
      ...
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            ...
                            'p_id_impsto_sbmpsto :' || p_id_impsto_sbmpsto ||
                            'p_id_sjto_impsto: ' || p_id_sjto_impsto ||
                            'p_id_usrio: ' || p_id_usrio || 'p_cnsctvo: ' ||
                            v_nmro_acto || 'p_id_dcmnto: ' || p_id_dcmnto ||
    ...
    begin
      ...
      apex_util.set_session_state('P37_JSON',
                                  ...
                                  p_id_impsto_sbmpsto ||
                                  '","id_sjto_impsto":"' ||
                                  p_id_sjto_impsto || '","usrio":"' ||
                                  p_id_usrio || '","cnsctvo":"' ||
                                  v_nmro_acto || '","id_dcmnto":"' ||
                                  p_id_dcmnto || '","txto_ascda":"' ||
                                  p_txto_ascda || '","id_rprte":"' ||
                                  p_id_plntlla || '","id_pz_slvo":"' ||
                                  p_id_pz_slvo || '"}');
    ...
    if v_blob is not null then
      ...
      if (v_cdgo_dstno_dcmnto = 'BFILE') then
        ...
        exception
          when others then
            o_cdgo_rspsta  := 13;
            o_mnsje_rspsta := 'No. ' || o_cdgo_rspsta ||
                              ' : Error al actualizar el blob para el id_acto : ' ||
                              o_id_acto || '-' || sqlerrm;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  v_nmbre_up,
      ...
      o_cdgo_rspsta  := 14;
      o_mnsje_rspsta := 'No. ' || o_cdgo_rspsta ||
                        ': No se genero el blob para el id_acto : ' ||
                        o_id_acto || '-' || sqlerrm;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            v_nmbre_up,
  ...
  PROCEDURE PRC_GENERA_PAZ_SALVO_MASIVO(p_cdgo_clnte   IN NUMBER,
  ...
  BEGIN
    ...
    FOR CUR IN (SELECT N001 AS ID_IMPSTO,
                       N002 AS ID_SB_IMPSTO,
                       C006 AS ID_SJTO_IMPSTO
                  FROM GN_G_TEMPORAL
                 WHERE ID_SSION = p_id_session
                   AND C005 = 'Paz y Salvo') LOOP
      BEGIN
        pkg_gf_paz_y_salvo.prc_rg_paz_salvo(p_cdgo_clnte        => p_cdgo_clnte,
        ...
        IF o_cdgo_rspsta > 0 THEN
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'No se registra Paz y Salvo para: ' ||
                                CUR.ID_SJTO_IMPSTO || ' - ' ||
                                o_mnsje_rspsta,
                                1);
          CONTINUE;
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                ...
                                v_nmbre_up,
                                v_nvel,
                                'Error registro Paz y Salvo: ' ||
                                CUR.ID_SJTO_IMPSTO || ' - ' || SQLERRM,
                                1);
          CONTINUE;
      END;
    END LOOP;
  EXCEPTION
    WHEN OTHERS THEN
      ...
      raise_application_error(-20001,
                              'No se pudo realizar la ejecución: ' ||
                              SQLERRM);

  END PRC_GENERA_PAZ_SALVO_MASIVO;

  PROCEDURE PRC_GENERA_PAZ_SALVO_MASIVO_JOB(p_cdgo_clnte IN NUMBER,
                                            p_id_usuario IN NUMBER,
                                            p_id_session IN NUMBER,
                                            p_id_plntlla IN gn_d_plantillas.id_plntlla%TYPE) AS
    v_nvel            NUMBER;
  ...
  BEGIN
    ...
    BEGIN
      ...
      pkg_gf_paz_y_salvo.PRC_GENERA_PAZ_SALVO_MASIVO(p_cdgo_clnte   => p_cdgo_clnte,
      ...
      FOR c_paz_salvos IN (SELECT c.file_blob, c.file_name
                             FROM gn_g_temporal a
                             JOIN v_gn_g_actos b
                               ON a.n003 = b.id_acto
                             LEFT JOIN gd_g_documentos c
                               ON b.id_dcmnto = c.id_dcmnto
                            WHERE b.id_dcmnto IS NOT NULL
                              AND a.ID_SSION = p_id_session) LOOP
      ...
      BEGIN
        INSERT INTO gn_g_temporal
          (ID_SSION, C005, BLOB001, C007, C008, n004)
        VALUES
          (p_id_session,
           'ZIP_Paz_y_Salvo',
           v_zip_file,
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ROLLBACK;
          RAISE_APPLICATION_ERROR(-20001,
                                  'Error en el INSERT en gn_g_temporal: ' ||
                                  SQLERRM);
      END;
  ...
  -- Req.0027488
  function fnc_co_rspnsbles_cuota_parte(p_cdgo_clnte       number,
  ...
  begin
    ...
    v_select := '<table align="center" border="0px"  style="border-collapse: collapse; font-family: Arial">
                        <tr>
                            <th width="50%" style="text-align:center;"><FONT SIZE=4>Propietario:</th>
                            <th width="25%" style="text-align:center;"><FONT SIZE=4>Identificación</th>
                            <th width="25%" tyle="text-align:center;"><FONT SIZE=4>% de Participación</th>
                        </tr>';

    for c_rspnsbles in (select nmbre_rzon_scial,
                               idntfccion_rspnsble,
                               prcntje_prtcpcion
                          from v_si_i_sujetos_responsable
                         where id_sjto_rspnsble = p_id_sjto_rspnsble) loop
      v_select := v_select ||
                  '<tr><td style="text-align:left;"><FONT SIZE=3>' ||
                  c_rspnsbles.nmbre_rzon_scial ||
                  '</td>
                                       <td style="text-align:center;"><FONT SIZE=3>' ||
                  c_rspnsbles.idntfccion_rspnsble ||
                  '</td>
                                       <td style="text-align:right;"><FONT SIZE=3>' ||
                  to_char(c_rspnsbles.prcntje_prtcpcion, '99G999G990D00') ||
                  '</td>
                                   </tr>';
    end loop;
  ...
  end fnc_co_rspnsbles_cuota_parte;
