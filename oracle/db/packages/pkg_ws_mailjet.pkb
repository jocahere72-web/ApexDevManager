  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_WS_MAILJET" as
  ...
  begin
    ...
    if p_cdgo_api = 'STRT' then
      ...
      v_id_impsto_101 := pkg_ws_mailjet.fnc_ob_propiedad_provedor(p_cdgo_clnte,
                                                                  p_id_prvdor,
                                                                  'IIM'); --id impuesto 101

      select json_object('Referencia' value to_char(v_nmro_dcmnto),
                         ...
                         'Pagador' value
                         json_object('TipoDocumento' value
                                     decode(v_cdgo_idntfccion_tpo, 'C', 1, 2),
                                     'Identificacion' value v_idntfccion,
                                     'Nombre' value
                                     v_prmer_nmbre || ' ' || v_sgndo_nmbre ||
      ...
      v_username := fnc_ob_propiedad_provedor(p_cdgo_clnte,
                                              p_id_prvdor,
                                              'USR');
      v_password := fnc_ob_propiedad_provedor(p_cdgo_clnte,
                                              p_id_prvdor,
                                              'PWD');
  ...
  begin
    ...
    if v_status is not null then
      ...
      v_status      := 'ERROR';
      v_messagehref := json_value(v_resp, '$.ErrorMessage');
      v_messageid   := json_value(v_resp, '$.StatusCode');
    end if;
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'v_MessageID ' || v_messageid,
                          6);
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          v_nmbre_up,
                          v_nl,
                          'prc_ws_iniciar_transaccion -Saliendo- ',
                          1);
  ...
  begin
    ...
    begin
      insert into ma_g_envios_medio_rspst_tmp
        (cdgo_tpo_mvmnto,
         rspsta,
         status,
         messageid,
    ...
    exception
      when others then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := 'Error al insertar la respuesta del WEBHOOK.' ||
                          sqlerrm;
  ...
  exception
    when others then
      o_cdgo_rspsta  := 100;
      o_mnsje_rspsta := 'Error al intentar registrar respuesta del WEBHOOK.' ||
                        sqlerrm;
  ...
  procedure prc_co_transacciones(p_cdgo_clnte    in number,
                                 p_id_prvdor     in number,
                                 p_id_envio_mdio in number default null,
                                 o_cdgo_rspsta   out number,
                                 o_mnsje_rspsta  out varchar2) is
  ...
  begin
    ...
    for c_trnsccion in (select a.id_envio_mdio,
                               a.rspsta,
                               a.messagehref,
                               a.messageid
                          from ma_g_envios_medio a
                         where (p_id_envio_mdio is not null and
                               a.id_envio_mdio = p_id_envio_mdio)
                            or (p_id_envio_mdio is null and
                               a.cdgo_envio_mdio = 'EML' and
                               upper(a.status) in
                               ('SENT',
                                 'SUCCESS',
                                 'DEFERRED',
                                 'SOFTBOUNCED') and exists
                                (select 1
                                   from ma_g_envios_medio_respuesta b
                                  where a.id_envio_mdio = b.id_envio_mdio
                                    and b.cdgo_tpo_mvmnto = 'SONDEO'
                                  having count(*) <= 5 -- solo se preguntará por el email 3 veces
                                 ) and trunc(a.fcha_rspsta) >=
                                to_date('2024-09-01', 'YYYY-MM-DD') and
                                (trunc(a.fcha_rspsta) + 1) <= trunc(sysdate)) -- se comienza a preguntar por el envio al día siguiente de enviado
                         order by a.fcha_rspsta) loop
      ...
      exception
        when others then
          ...
          insert into muerto
            (v_001, c_001, t_001)
          values
            ('MAILjeT v_error_ws:', v_error_ws, systimestamp);
          commit;
      end;

      v_status      := upper(json_value(v_resp, '$.Data[0].Status'));
      v_fcha_rspsta := sysdate;
      ...
      pkg_ws_mailjet.prc_rg_respuesta(p_id_envio_mdio   => c_trnsccion.id_envio_mdio,
                                      ...
                                      p_fcha_rspsta     => nvl(v_fcha_rspsta,
                                                               sysdate) --sacarla de la respuesta ojo
                                     ,
                                      o_cdgo_rspsta     => o_cdgo_rspsta,
                                      o_mnsje_rspsta    => o_mnsje_rspsta);
  ...
  begin
    ...
    for i in (select c.cdgo_clnte,
                     b.id_envio_mdio,
                     a.id_envio_mdio_rspsta_tmp,
                     a.cdgo_tpo_mvmnto,
                     a.rspsta,
                     a.status,
                     b.messagehref,
                ...
                join ma_g_envios c
                  on c.id_envio = b.id_envio --ojoooo
      ...
      begin
        ...
        if sql%found then
          ...
          update ma_g_envios_medio_rspst_tmp
             set id_envio_mdio = i.id_envio_mdio, indcdor_prcsdo = 'S'
          ...
          if upper(i.status) in ('OPEN', 'CLICK') then
            ...
            update gn_g_actos_responsable
               set indcdor_ntfccion = 'S', fcha_ntfcion = v_fcha_ntfccion --i.fcha_rspsta
  ...
  begin
    ...
    for i in (select a.messageid,
                     b.cdgo_clnte,
                     b.id_acto,
                     a.status,
                     a.rspsta,
                     a.id_envio_mdio
                from ma_g_envios_medio_respuesta a
                join v_ma_g_envios_medio b
                  on b.id_envio_mdio = a.id_envio_mdio
                join gn_g_actos c
     ...
     loop
      begin
        ...
        -- se actualiza ma_g_envios_medio ?? indcdor_ ??? en 's' para no volverlo a actualizar?
        update ma_g_envios_medio
           set indcdor_ntfcdo = 'S', fcha_ntfcdo = v_fcha_ntfccion --i.fcha_rspsta
         where id_envio_mdio = i.id_envio_mdio
           and messageid = i.messageid
           and indcdor_ntfcdo = 'N';
      exception
  ...
  begin
    ...
    for c_respuestas in (select a.id_envio_mdio_rspsta_tmp,
                                a.cdgo_clnte,
                                b.url,
                                json_transform(a.rspsta,
                                               insert
                                               '$.fcha_rspsta' =
                                               to_char(a.fcha_rspsta,
                                                       'YYYY-MM-DD HH24:MI:SS')) as rspsta
                           from ma_g_envios_medio_rspst_tmp a
                           join ws_d_clientes_webhook b
                             on b.cdgo_clnte = a.cdgo_clnte
                          where a.ind_acse_rcbo = 'N'
                            and a.cdgo_clnte =
                                nvl(p_cdgo_clnte, a.cdgo_clnte)
      ...
      exception
        when others then
          ...
          v_sqlerrm := 'get_detailed_sqlerrm: ' || v_sqlerrm ||
                       ' sqlerrm: ' || sqlerrm;
      ...
      update ma_g_envios_medio_rspst_tmp
         set ind_acse_rcbo    = v_ind_acse_rcbo,
             rspsta_acse_rcbo = v_mnsje_rspsta
       where id_envio_mdio_rspsta_tmp =
             c_respuestas.id_envio_mdio_rspsta_tmp;
  ...
  function fnc_minify_html(p_html in clob) return clob is
  ...
  begin
    ...
    for v_linea in (select regexp_replace(trim(both ' ' from p_html),
                                          '\s+',
                                          ' ') as linea_contenido
                      from dual) loop
      dbms_lob.writeappend(v_minified_html,
                           length(v_linea.linea_contenido),
                           v_linea.linea_contenido);
    end loop;
