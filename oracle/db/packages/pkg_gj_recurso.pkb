  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PKG_GJ_RECURSO" as

  procedure prc_rg_recurso(p_cdgo_clnte           in number,
  ...
  begin
    ...
    begin
      select id_slctud, fcha_rdcdo, id_sjto_impsto
        into v_id_slctud, v_fcha_rdcdo, v_id_sjto_impsto
        from v_pq_g_solicitudes
       where id_instncia_fljo_gnrdo = p_id_instncia_fljo_hjo;
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al consultar datos de la solicitud';
    ...
    exception
      when others then
        o_cdgo_rspsta  := 2;
        o_mnsje_rspsta := 'Problemas al consultar flujo padre';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;
    ...
    exception
      when others then
        o_cdgo_rspsta  := 3;
        o_mnsje_rspsta := 'Problemas al consultar tipo de recurso';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;

    --Consultamos el tipo de acto que admite este tipo de recurso
    begin
      select c.id_acto
        into v_id_acto
        from gj_d_recursos_tipo_cliente a
        join gj_d_recurso_tipo_acto_tipo b
          on b.id_rcrso_tipo_clnte = a.id_rcrso_tipo_clnte
        join gn_g_actos c
          on c.id_acto_tpo = b.id_acto_tpo
       where a.id_rcrso_tipo_clnte = p_id_rcrso_tipo_clnte
         and c.id_acto = p_id_acto;
    exception
      when no_data_found then
        o_cdgo_rspsta  := 3;
        o_mnsje_rspsta := 'Por favor verifique que se encuentre Parametrizado como Acto que Admite este Recurso en la Parametrica "Tipos de Recursos por Cliente".';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
      when others then
        o_cdgo_rspsta  := 3;
        o_mnsje_rspsta := 'Problemas al consultar tipo de acto que admite este recurso';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;
    ...
    exception
      when others then
        o_cdgo_rspsta  := 4;
        o_mnsje_rspsta := 'Problemas al registrar recurso, ' || sqlerrm;
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;
    ...
    PKG_GJ_RECURSO.PRC_RG_RECURSOS_ITEM(p_cdgo_clnte       => p_cdgo_clnte,
    ...
    if (p_obsrvcion is not null) then
      PKG_GJ_RECURSO.PRC_RG_RECURSO_DETALLE(p_cdgo_clnte     => p_cdgo_clnte,
  ...
  exception
    when v_error then
      if (o_mnsje_rspsta is null or o_cdgo_rspsta is null) then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al registrar recurso';
      end if;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
                            'pkg_gj_recurso.prc_rg_recurso',
  ...
  procedure prc_rg_recrso_vgncias(p_cdgo_clnte           in number,
                                  p_id_instncia_fljo_hjo in gj_g_recursos.id_instncia_fljo_hjo%type,
  ...
  begin
    ...
    for c_vigencias in (select c.id_acto_vgncia,
                               a.id_rcrso,
                               c.vgncia,
                               c.id_prdo
                          from gj_g_recursos a
                         inner join v_pq_g_solicitudes b
                            on a.id_slctud = b.id_slctud
                         inner join gn_g_actos_vigencia c
                            on a.id_acto = c.id_acto
                           and b.id_sjto_impsto = c.id_sjto_impsto
                         inner join df_i_periodos d
                            on c.id_prdo = d.id_prdo
                         inner join (select id_acto_vgncia
                                      from json_table(p_json_vgncias,
                                                      '$[*]' columns
                                                      id_acto_vgncia PATH
                                                      '$.ID_ACTO_VGNCIA')) e
                            on c.id_acto_vgncia = e.id_acto_vgncia
                         where a.id_instncia_fljo_hjo =
                               p_id_instncia_fljo_hjo) loop
      begin
        insert into gj_g_recursos_vigencia
          (id_rcrso, id_acto_vgncia, vgncia, id_prdo)
        values
          (c_vigencias.id_rcrso,
           c_vigencias.id_acto_vgncia,
           c_vigencias.vgncia,
           c_vigencias.id_prdo);
      exception
        when others then
  ...
  procedure prc_el_recurso_vgncia(p_cdgo_clnte      in number,
  ...
  begin
    ...
    --
    begin
      delete from gj_g_recursos_vigencia
  ...
  procedure prc_ac_recurso_vgncia(p_cdgo_clnte               in number,
                                  p_id_instncia_fljo_hjo     in gj_g_recursos.id_instncia_fljo_hjo%type,
  ...
  procedure prc_ac_recurso_acciones(p_cdgo_clnte                in number,
                                    p_id_instncia_fljo_hjo      in gj_g_recursos.id_instncia_fljo_hjo%type,
  ...
  procedure prc_rg_recurso_detalle(p_cdgo_clnte     in number,
  ...
  begin
    ...
    v_nl := pkg_sg_log.fnc_ca_nivel_log(1,
                                        null,
                                        'pkg_gj_recurso.prc_rg_recurso_observacion');
    --
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          'pkg_gj_recurso.prc_rg_recurso_observacion',
                          ...
                          'Entrando:' || systimestamp,
                          1);
    if p_obsrvcion is not null or p_id_mtvo_clnte is not null then
      begin
      ...
      exception
        when others then
          o_cdgo_rspsta  := 2;
          o_mnsje_rspsta := 'Problemas al registrar detalle, ' || sqlerrm;
          v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
          v_nvl          := 1;
          raise v_error;
      end;
  ...
  exception
    when v_error then
      if (o_mnsje_rspsta is null or o_cdgo_rspsta is null) then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al registrar detalle recurso';
      end if;
      pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                            null,
  ...
  procedure prc_rg_gestion_plantilla(p_cdgo_clnte           in number,
                                     ...
                                     o_id_rcrso_dcmnto      out number) as
  ...
  begin
    ...
    exception
      when others then
        ...
        o_mnsje_rspsta := o_cdgo_rspsta ||
                          ' Problemas consultando el recurso asociado al flujo No. ' ||
                          p_id_instncia_fljo;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              nmbre_up,
    ...
    begin
      select a.id_fljo_trea_orgen
       ...
       where a.id_instncia_fljo =
             nvl(p_id_instncia_fljo_hjo, p_id_instncia_fljo)
    ...
    exception
      when others then
        o_cdgo_rspsta  := 2;
        o_mnsje_rspsta := o_cdgo_rspsta ||
                          ' Problemas consultando la etapa del flujo No.' ||
                          p_id_instncia_fljo;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              nmbre_up,
    ...
    begin
      select a.id_acto_tpo, b.id_acto_tpo_rqrdo
        into v_id_acto_tpo, v_id_acto_tpo_rqrdo
    ...
    exception
      when others then
        o_cdgo_rspsta  := 3;
        o_mnsje_rspsta := o_cdgo_rspsta ||
                          ' Problemas consultando plantilla de la etapa del flujo No.' ||
                          p_id_instncia_fljo;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              nmbre_up,
    ...
    if v_id_acto_tpo_rqrdo is not null then
      ...
      exception
        when no_data_found then
          o_cdgo_rspsta  := 4;
          o_mnsje_rspsta := o_cdgo_rspsta ||
                            ' No se encontró el acto requerido de la plantilla, por favor genere el Auto primero.';
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                nmbre_up,
                                v_nl,
        ...
        when others then
          o_cdgo_rspsta  := 4;
          o_mnsje_rspsta := o_cdgo_rspsta ||
                            ' Problemas consultando acto requerido de la plantilla No.' ||
                            p_id_plntlla;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                nmbre_up,
    ...
    begin
      select a.id_rcrso_dcmnto
       ...
       inner join gj_g_recursos b
          on b.id_rcrso = a.id_rcrso
       inner join v_wf_g_instancias_flujo c
          on c.id_instncia_fljo = b.id_instncia_fljo_hjo
    ...
    exception
      ...
      when others then
        o_cdgo_rspsta  := 5;
        o_mnsje_rspsta := o_cdgo_rspsta ||
                          ' Problemas consultando el documento del recurso No.' ||
                          v_id_rcrso;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              nmbre_up,
    ...
    if p_request = 'CREATE' and p_dcmnto is not null and
       v_id_rcrso_dcmnto is null then
      ...
      if pkg_gn_generalidades.fnc_vl_html(p_html => p_dcmnto) = true then
        ...
        begin
          ...
          insert into gj_g_recursos_documento
            (id_rcrso,
             id_fljo_trea,
             id_plntlla,
             id_acto_tpo,
          ...
          values
            (v_id_rcrso,
             ...
             v_dcmnto,
             p_id_usrio)
          returning id_rcrso_dcmnto into o_id_rcrso_dcmnto; --v_id_rcrso_dcmnto;
        exception
          when others then
            o_cdgo_rspsta  := 6;
            o_mnsje_rspsta := o_cdgo_rspsta ||
                              ' Problemas insertando el documento del recurso No.' ||
                              v_id_rcrso;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  nmbre_up,
        ...
        begin
          --pkg_sg_log.prc_rg_log( p_cdgo_clnte, null, nmbre_up,  v_nl, 'ANTES DE DIVIDIR HTML - aplccion: '||p_id_aplccion||', pgna: '||p_id_pgna||', usrio: '||p_id_usrio, 2);
          pkg_gn_generalidades.prc_html_dividir(p_html         => v_dcmnto,
                                                p_tmno         => 10000,
                                                p_id_aplccion  => p_id_aplccion,
                                                p_id_pgna      => p_id_pgna,
                                                p_id_username  => p_id_usrio,
    ...
    elsif p_request = 'SAVE' and p_dcmnto is not null and
          v_id_rcrso_dcmnto is not null then
      begin
        update gj_g_recursos_documento a
           set a.txto_dcmnto = p_dcmnto, a.id_usrio_gnrcion = p_id_usrio
         where a.id_rcrso_dcmnto = v_id_rcrso_dcmnto;
      exception
        when others then
          o_cdgo_rspsta  := 7;
          o_mnsje_rspsta := o_cdgo_rspsta ||
                            ' Problemas actualizando el documento del recurso No.' ||
                            v_id_rcrso;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                nmbre_up,
      ...
      o_id_rcrso_dcmnto := v_id_rcrso_dcmnto;
    elsif p_request = 'DELETE' and v_id_rcrso_dcmnto is not null then
      ...
      exception
        when others then
          o_cdgo_rspsta  := 8;
          o_mnsje_rspsta := o_cdgo_rspsta ||
                            ' Problemas eliminando el documento del recurso No.' ||
                            v_id_rcrso;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                nmbre_up,
  ...
  begin
    ...
    v_nl := pkg_sg_log.fnc_ca_nivel_log(1,
                                        null,
                                        'pkg_gj_recurso.prc_rg_etapa_documentos');
    ...
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                          o_cdgo_rspsta ||
                          ' Problemas al consultar el flujo No.' ||
    ...
    exception
      when others then
        ...
        o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                          o_cdgo_rspsta ||
                          ' Problemas al consultar valor del acto origen del recurso. ' ||
                          o_mnsje_rspsta;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              'pkg_gj_recurso.prc_rg_etapa_documentos',
    ...
    begin
      for c_documentos in (select a.id_rcrso_dcmnto,
                                  a.id_plntlla,
                                  d.id_rprte,
                                  a.id_acto_tpo,
                                  a.id_acto_rqrdo,
                                  ...
                                  a.id_fljo_trea
                             from gj_g_recursos_documento a
                            inner join (select b.cdna as id_rcrso_dcmnto
                                         from table(pkg_gn_generalidades.fnc_ca_split_table(p_cdna           => p_id_rcrso_dcmnto,
                                                                                            p_crcter_dlmtdor => ',')) b) c
                               on c.id_rcrso_dcmnto = a.id_rcrso_dcmnto
                            inner join gn_d_plantillas d
                               on d.id_plntlla = a.id_plntlla
                            where a.id_rcrso = v_id_rcrso
                              and a.id_fljo_trea =
                                  nvl(p_id_fljo_trea, a.id_fljo_trea)
                           --start with  a.id_acto_rqrdo     is      null
                           connect by nocycle
                            prior a.id_acto_rqrdo = prior a.id_acto) loop
        ...
        exception
          when others then
            ...
            o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                              o_cdgo_rspsta ||
                              ' Problemas al consultar si notifica automaticamente el tipo de acto No.' ||
                              c_documentos.id_acto_tpo || ' ' ||
                              o_mnsje_rspsta;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
        ...
        if (v_exist_vigencias = 'N') then
          v_slct_vgncias := 'select  a.id_sjto_impsto,
                                    ...
                                    where b.id_instncia_fljo_hjo = ' ||
                            p_id_instncia_fljo || 'and b.id_sjto_impsto = ' ||
                            v_id_sjto_impsto;
        ...
        exception
          when others then
            ...
            o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                              o_cdgo_rspsta ||
                              ' Problemas al registrar acto del documento No.' ||
                              c_documentos.id_rcrso_dcmnto || ' ' ||
                              o_mnsje_rspsta;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
        ...
        begin
          update gj_g_recursos_documento
             set id_acto = v_id_acto, id_usrio_autrza = p_id_usrio
        ...
        exception
          when others then
            ...
            o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                              o_cdgo_rspsta ||
                              ' Problemas al actualizar documento No.' ||
                              c_documentos.id_rcrso_dcmnto || ' ' ||
                              o_mnsje_rspsta;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
        ...
        exception
          when others then
            ...
            o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                              o_cdgo_rspsta ||
                              ' Problemas al consultar reporte No.' ||
                              c_documentos.id_rprte || ' ' ||
                              o_mnsje_rspsta;
            pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                  null,
                                  'pkg_gj_recurso.prc_rg_etapa_documentos',
        ...
        if v_blob is not null then
          ...
          exception
            when others then
              ...
              o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                                o_cdgo_rspsta ||
                                ' Problemas al actualizar acto No.' ||
                                v_id_acto || ' ' || o_mnsje_rspsta;
              pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                    null,
                                    'pkg_gj_recurso.prc_rg_etapa_documentos',
                                    v_nl,
          ...
          o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                            o_cdgo_rspsta ||
                            ' Problemas al generar documento del acto No.' ||
                            v_id_acto || ' ' || o_mnsje_rspsta;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
                                'pkg_gj_recurso.prc_rg_etapa_documentos',
    ...
    exception
      when others then
        ...
        o_mnsje_rspsta := '|Proceso prc_rg_etapa_documentos - Codigo: ' ||
                          o_cdgo_rspsta ||
                          ' Problemas al consultar variable de documentos ' ||
                          sqlerrm;
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              'pkg_gj_recurso.prc_rg_etapa_documentos',
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          null,
                          'pkg_gf_prescripcion.prc_rg_etapa_documentos',
                          v_nl,
                          'Terminado con exito.',
                          1);
  ...
  procedure prc_rg_mtvos_dcmntos(p_cdgo_clnte       in number,
                                 p_id_rcrso         in gj_g_recursos.id_rcrso%type,
                                 p_id_instncia_fljo in number,
                                 p_id_fljo_trea     in number,
                                 p_id_usrio         in number,
                                 p_json_dcmntos     in clob default null,
  ...
  begin
    ...
    --Consultamos los datos de la solicitud
    begin
      select id_slctud, fcha_rdcdo
        into v_id_slctud, v_fcha_rdcdo
        from v_pq_g_solicitudes
    ...
    if (p_json_dcmntos is not null) then
      PKG_PQ_PQR.PRC_AC_DOCUMENTOS(p_id_slctud => v_id_slctud,
                                   p_json      => p_json_dcmntos);
    end if;

    --Registramos los motivos en detalle
    for c_motivos in (Select a.id_mtvo_clnte,
                             a.id_mtvo,
                             a.a_i_r,
                             a.dscrpcion
                        From v_gj_d_motivos_cliente a
                       inner join (select id_mtvo_clnte
                                    from json_table(p_json_mtvos,
                                                    '$[*]' columns
                                                    id_mtvo_clnte PATH
                                                    '$.id_mtvo_clnte')) b
                          on a.id_mtvo_clnte = b.id_mtvo_clnte) loop
      PKG_GJ_RECURSO.PRC_RG_RECURSO_DETALLE(p_cdgo_clnte     => p_cdgo_clnte,
                                            p_id_rcrso       => p_id_rcrso,
                                            p_id_fljo_trea   => p_id_fljo_trea,
                                            p_id_mtvo_clnte  => c_motivos.id_mtvo_clnte,
                                            p_obsrvcion      => c_motivos.dscrpcion, --null, /*actualizado 23/10/2023*/
                                            p_id_usrio       => p_id_usrio,
                                            p_fcha           => systimestamp,
                                            o_id_rcrso_dtlle => v_id_rcrso_dtlle,
                                            o_cdgo_rspsta    => o_cdgo_rspsta,
  ...
  procedure prc_ac_air_recurso(p_cdgo_clnte   in number,
                               p_id_rcrso     in gj_g_recursos.id_rcrso%type,
  ...
  begin
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          1);
    begin
      update gj_g_recursos set a_i_r = p_a_i_r where id_rcrso = p_id_rcrso;
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al actualizar datos de la solicitud';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;
  ...
  procedure prc_ac_recurso(p_cdgo_clnte   in number,
                           p_id_rcrso     in gj_g_recursos.id_rcrso%type,
                           p_id_usrio     in number,
                           p_rspta        in varchar2,
  ...
  begin
    ...
    begin
      update gj_g_recursos
         set cdgo_rspta = p_rspta, fcha_fin = p_fcha_fin
  ...
  procedure prc_ac_fnlza_fljo(p_id_instncia_fljo in number,
  ...
  begin
    ...
    v_nl := pkg_sg_log.fnc_ca_nivel_log(v_cdgo_clnte,
                                        null,
                                        'pkg_gj_recurso.prc_ac_fnlza_fljo');
    --
    ...
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al consultar el motivo asociado a la instancia del flujo';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;
    ...
    PKG_GJ_RECURSO.PRC_CO_ACTO_RESOLUCION(p_cdgo_clnte       => v_cdgo_clnte,
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          'pkg_gj_recurso.prc_ac_fnlza_fljo',
                          v_nl,
                          'Ejecutamos la UP pkg_pq_pqr.prc_ac_solicitud',
                          1);
    --Finalizamos el flujo de PQR

    --Registramos las propiedades del evento
    --/*****************************/
    --ACT Acto de resolución
    --USR Usuario
    --FCH Fecha final
    --MTV Motivo
    --OBS Observación
    ...
    begin
      select b.cdgo_rspsta_pqr
        ...
        from gj_g_recursos a
       inner join gj_d_recursos_respuesta b
          on a.cdgo_rspta = b.cdgo_rspta
       where a.id_instncia_fljo_hjo = p_id_instncia_fljo;
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al consultar respuesta de PQR asociada a la instancia del flujo';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;
    ...
    pkg_pl_workflow_1_0.prc_rg_propiedad_evento(p_id_instncia_fljo => p_id_instncia_fljo,
                                                p_cdgo_prpdad      => 'OBS',
                                                p_vlor             => 'Flujo finalizado exitosamente.');
    pkg_pl_workflow_1_0.prc_rg_propiedad_evento(p_id_instncia_fljo => p_id_instncia_fljo,
                                                p_cdgo_prpdad      => 'RSP',
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          'pkg_gj_recurso.prc_ac_fnlza_fljo',
                          v_nl,
                          'Ejecuta UP prc_rg_finalizar_instancia: ' ||
                          p_id_instncia_fljo || ' , ' || p_id_fljo_trea ||
                          ' , ' || v_id_usrio || ' , ' || systimestamp,
                          1);
    begin
      ...
      pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                            ...
                            'pkg_gj_recurso.prc_ac_fnlza_fljo',
                            v_nl,
                            'prc_rg_finalizar_instancia MNSJE: ' ||
                            o_mnsje_rspsta,
                            1);
      if v_o_error = 'N' then
        o_cdgo_rspsta  := 1;
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          'pkg_gj_recurso.prc_ac_fnlza_fljo',
                          v_nl,
                          'Desmarcación de Cartera',
                          nvl(v_nvl, 1));
    begin
      pkg_gj_recurso.prc_ac_cartera(p_cdgo_clnte       => v_cdgo_clnte,
                                    p_id_usrio         => v_id_usrio,
                                    p_id_instncia_fljo => p_id_instncia_fljo,
                                    p_marcacion        => 'N',
    ...
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          ...
                          'pkg_gj_recurso.prc_ac_fnlza_fljo',
                          v_nl,
                          'Finaliza Desmarcación de Cartera COD: ' ||
                          o_cdgo_rspsta,
                          nvl(v_nvl, 1));
  ...
  procedure prc_gn_flujo_instancias(p_cdgo_clnte           in number,
                                    p_id_instncia_fljo     in number,
                                    p_id_instncia_fljo_hjo in number default null,
                                    p_id_fljo_trea         in number,
                                    p_id_usrio             in number,
  ...
  begin
    ...
    v_mnsje_log := 'p_cdgo_clnte: ' || p_cdgo_clnte || ' - ' ||
                   'p_id_instncia_fljo: ' || p_id_instncia_fljo || ' - ' ||
    ...
    begin
      for c_flujos in (select a.id_rcrso_accion,
                        ...
                        inner join gj_d_recursos_tipo_accion c
                           on a.id_rcrso_tpo_accion = c.id_rcrso_tpo_accion
                        inner join gj_d_acciones d
                           on c.id_accion = d.id_accion
                        where b.id_instncia_fljo_hjo = p_id_instncia_fljo
                          and a.actvo = 'S') loop
        begin
          --Generación del JSON
          ...
          for c_parametros in (select *
                                 from gj_d_parametros
                                where id_rcrso_tpo_accion =
                                      c_flujos.id_rcrso_tpo_accion
                                  and actvo = 'S'
            ...
            if (c_parametros.tpo_orgn = 'I') then
              --Item de Pagina
              begin
                select a.vlor
                  into v_vlor
                 ...
                 inner join gj_g_recursos b
                    on a.id_rcrso = b.id_rcrso
                 where b.id_instncia_fljo_hjo = p_id_instncia_fljo
                   and a.id_trea_item = c_parametros.id_trea_item;
              exception
                when others then
                  v_vlor := null;
              end;
              --v_mnsje_log := 'Paramtro de origen I: '||c_parametros.nmbre_prmtro||'VLOR:'||v_vlor;
              --pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up,  v_nl, v_mnsje_log, 6);
              v_json_object.put(c_parametros.nmbre_prmtro, v_vlor);
            elsif (c_parametros.tpo_orgn = 'E') then
              --Estatico
              v_vlor := c_parametros.orgen;
              --v_mnsje_log := 'Paramtro de origen E: '||c_parametros.nmbre_prmtro||'VLOR:'||v_vlor;
              --pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up,  v_nl, v_mnsje_log, 6);
              v_json_object.put(c_parametros.nmbre_prmtro, v_vlor);
            elsif (c_parametros.tpo_orgn = 'F') then
              --Funcion
              ...
              begin
                execute immediate 'select PKG_GJ_JSON_ACCIONES.' ||
                                  c_parametros.orgen || '(p_cdgo_clnte => ' ||
                                  p_cdgo_clnte || ',' || 'p_id_usrio => ' ||
                                  p_id_usrio || ',' ||
                                  'p_id_rcrso_accion => ' ||
                                  c_flujos.id_rcrso_accion || ',' ||
                                  'p_id_prmtro => ' ||
                                  c_parametros.id_prmtro || ',' ||
                                  'p_id_instncia_fljo => ' ||
                                  p_id_instncia_fljo || ')' || ' from dual'
                  into v_vlor_clob;
                v_json_element := JSON_Element_T.parse(v_json_object.stringify);
                --v_mnsje_log := 'Paramtro de origen F: '||c_parametros.nmbre_prmtro||'VLOR:'||v_vlor_clob;
                --pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up,  v_nl, v_mnsje_log, 6);
            ...
            elsif (c_parametros.tpo_orgn = 'A') then
              --Item de Aplicación
              v_vlor := v(c_parametros.orgen);
              --v_mnsje_log := 'Paramtro de origen A: '||c_parametros.nmbre_prmtro||'VLOR:'||v_vlor;
              --pkg_sg_log.prc_rg_log(p_cdgo_clnte, null, v_nmbre_up,  v_nl, v_mnsje_log, 6);
              v_json_object.put(c_parametros.nmbre_prmtro, v_vlor);
            end if;
        ...
        begin
          PKG_PL_WORKFLOW_1_0.PRC_RG_GENERAR_FLUJO(p_id_instncia_fljo => nvl(p_id_instncia_fljo_hjo,
                                                                             p_id_instncia_fljo),
                                                   p_id_fljo_trea     => p_id_fljo_trea,
                                                   p_id_usrio         => p_id_usrio,
          ...
          -- Verificamos los datos en el log
          v_mnsje_log := 'PKG_PL_WORKFLOW_1_0.PRC_RG_GENERAR_FLUJO: ' ||
                         'o_cdgo_rspsta: ' || o_cdgo_rspsta || ' - ' ||
                         'o_mnsje_rspsta: ' || o_mnsje_rspsta;
          pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                                null,
        ...
        exception
          when others then
            o_cdgo_rspsta  := 1;
            o_mnsje_rspsta := 'Problemas al generar flujo, ' ||
                              o_mnsje_rspsta;
            raise v_error;
        end;
        ...
        if (v_id_instncia_fljo is null) then
          o_cdgo_rspsta  := 1;
          o_mnsje_rspsta := 'La instancia del flujo generado de la acción No. ' ||
                            c_flujos.id_rcrso_accion || ' esta vacia';
          raise v_error;
        end if;
        ...
        exception
          when others then
            o_cdgo_rspsta  := 1;
            o_mnsje_rspsta := 'Problemas al consultar accion No. ' ||
                              c_flujos.id_rcrso_accion;
            raise v_error;
        end;
        ...
        exception
          when others then
            o_cdgo_rspsta  := 3;
            o_mnsje_rspsta := 'Problemas al actualizar acción, ' ||
                              o_mnsje_rspsta;
            raise v_error;
        end;
    ...
    exception
      when v_error then
        ...
        pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                              null,
                              'pkg_gj_recurso.prc_gn_flujo_instancias',
                              v_nl,
                              o_cdgo_rspsta || ' - ' || v_mnsje_log,
                              v_nvl);
      when others then
    ...
    pkg_sg_log.prc_rg_log(p_cdgo_clnte,
                          ...
                          v_nmbre_up,
                          v_nl,
                          'Procedimiento Terminado:' || systimestamp,
                          6);

  end prc_gn_flujo_instancias;

  procedure prc_co_acto_resolucion(p_cdgo_clnte       in number,
                                   p_id_instncia_fljo in number,
  ...
  begin
    ...
    for c_actos_resolucion in (select id_acto_tpo
                                ...
                                where id_rcrso_tipo_clnte =
                                      v_id_rcrso_tipo_clnte
                                  and actvo = 'S'
                                order by orden asc) loop
      begin
        /*  select a.id_acto,
        ...
        select a.id_acto, a.id_acto_tpo, c.fcha
          into o_id_acto, o_id_acto_tpo, o_fcha
          from gj_g_recursos_documento a
         ...
         where b.id_instncia_fljo_hjo = p_id_instncia_fljo
           ...
           and c.fcha =
               (select max(e.fcha)
                 ...
                 inner join gn_g_actos e
                    on d.id_acto = e.id_acto
                 inner join gj_g_recursos h
                    on d.id_rcrso = h.id_rcrso
    ...
    if (o_id_acto is null) then
      o_cdgo_rspsta  := 4;
      o_mnsje_rspsta := 'Problemas al consultar el acto que resuelve el recurso';
      v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
      v_nvl          := 1;
      raise v_error;
    end if;
  ...
  procedure prc_rg_recursos_item(p_cdgo_clnte       in number,
                                 p_id_instncia_fljo in number,
  ...
  begin
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'No se hallaron resultado al consultar la instancia del flujo en la tabla de recursos';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
      when others then
        o_cdgo_rspsta  := 2;
        o_mnsje_rspsta := 'Problemas al consultar el recurso asociado a la instancia del flujo';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;
    ...
    for c_item in (select a.*
                    ...
                    inner join wf_d_flujos_tarea b
      ...
      if (c_item.tpo_orgn = 'E') then
        ...
        --Ejecutamos la sql
        begin
          execute immediate c_item.orgen
            into v_vlor;
        exception
          when others then
            v_vlor := null;
        ...
        --Ejecutamos la funcion
        begin
          execute immediate 'select ' || c_item.orgen || ' from dual'
            into v_vlor;
        exception
          when others then
            v_vlor := null;
  ...
  procedure prc_rg_recursos_accion(p_cdgo_clnte       in number,
                                   p_id_instncia_fljo in number,
                                   p_id_usrio         in gj_g_recursos_item.id_usrio%type,
  ...
  begin
    ...
    for c_acciones in (select a.id_rcrso_tpo_accion
                         from gj_d_recursos_tipo_accion a
                        inner join (select id_rcrso_tpo_accion
                                     from json_table(p_json_acciones,
                                                     '$[*]' columns
                                                     id_rcrso_tpo_accion PATH
                                                     '$.ID_RCRSO_TPO_ACCION')) b
                           on a.id_rcrso_tpo_accion = b.id_rcrso_tpo_accion) loop
      begin
        insert into gj_g_recursos_accion
          (id_rcrso, id_rcrso_tpo_accion, id_ajste_mtvo, obsrvcion)
        values
          (v_id_rcrso, c_acciones.id_rcrso_tpo_accion, null, null);
      exception
        when others then
          o_cdgo_rspsta  := 1;
  ...
  procedure prc_el_recursos_accion(p_cdgo_clnte       in number,
                                   p_id_instncia_fljo in number,
                                   p_id_usrio         in gj_g_recursos_item.id_usrio%type,
  ...
  begin
    ...
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al eliminar las vigencias';
        v_mnsje_log    := o_mnsje_rspsta || ' , ' || sqlerrm;
        v_nvl          := 1;
        raise v_error;
    end;
  ...
  procedure prc_ac_recursos_accion(p_cdgo_clnte      in number,
                                   p_id_usrio        in gj_g_recursos_item.id_usrio%type,
                                   p_id_rcrso_accion in gj_g_recursos_accion.id_rcrso_accion%type,
  ...
  begin
    ...
    begin
      update gj_g_recursos_accion
         set id_ajste_mtvo = p_id_ajste_mtvo, obsrvcion = p_obsrvcion
       where id_rcrso_accion = p_id_rcrso_accion;
    exception
      when others then
        o_cdgo_rspsta  := 1;
  ...
  procedure prc_ac_recursos_accion_mnjdr(p_id_instncia_fljo     in number,
                                         p_id_fljo_trea         in number,
  ...
  begin
    ...
    exception
      when others then
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'Problemas al consultar instancia Id.' ||
    ...
    begin
      select 'S'
       ...
       where id_instncia_fljo = p_id_instncia_fljo
         ...
         and id_instncia_fljo_gnrdo_hjo = p_id_instncia_fljo_hjo
         and indcdor_mnjdo != 'S';
    exception
      when no_data_found then
        return;
    ...
    exception
      when others then
        o_cdgo_rspsta  := 15;
        o_mnsje_rspsta := 'Problemas al consultar usuario asociado al flujo,' ||
                          sqlerrm;
        raise v_error;
    end;

    --Consultamos la acción
    begin
    ...
    exception
      when others then
        o_cdgo_rspsta  := 16;
        o_mnsje_rspsta := 'Problemas al consultar tipo de accion,' ||
                          sqlerrm;
        raise v_error;
    end;

    v_indcdor_extso := pkg_gj_recurso.fnc_co_eventos_propiedad(p_id_instncia_fljo => p_id_instncia_fljo_hjo,
                                                               p_cdgo_prpdad      => 'EXT');
    v_obsrvcion     := pkg_gj_recurso.fnc_co_eventos_propiedad(p_id_instncia_fljo => p_id_instncia_fljo_hjo,
                                                               p_cdgo_prpdad      => 'OBS');

    --Validamos y ejecutamos acción dependiendo del indicador de exitoso
    if (v_indcdor_extso = 'S') then
      if (v_rt_gj_d_recursos_tipo_accion.tpo_accion_indcdor_extso_s = 'EUP') then
        null;
      elsif (v_rt_gj_d_recursos_tipo_accion.tpo_accion_indcdor_extso_s =
            'INF') then
        null;
      end if;
    elsif (v_indcdor_extso = 'N') then
      --generacion de flujo resolucion acalratoria por ajuste no aprobado no aplicado.
      if (v_rt_gj_d_recursos_tipo_accion.tpo_accion_indcdor_extso_n = 'EUP') then
        null;
      elsif (v_rt_gj_d_recursos_tipo_accion.tpo_accion_indcdor_extso_n =
            'INF') then
        --Instanciamos el flujo asociado a la acción
        begin
    ...
    exception
      when others then
        o_cdgo_rspsta  := 17;
        o_mnsje_rspsta := 'Problemas al actualizar acción,' || sqlerrm;
        raise v_error;
    end;

    if (v_indcdor_extso = 'S') then
      begin
        update gj_g_recursos_vigencia
           set indcdr_fvrble = 'S'
         where vgncia in
               (select a.vgncia
                  from gj_g_rcrsos_accn_vgnc_cncpt a
                  join gj_g_recursos_accion b
                    on a.id_rcrso_accion = b.id_rcrso_accion
                 where b.id_instncia_fljo_hjo = p_id_instncia_fljo_hjo);
      exception
        when others then
          o_cdgo_rspsta  := 18;
          o_mnsje_rspsta := 'Problemas al actualizar gj_g_recursos_vigencia,' ||
                            sqlerrm;
          raise v_error;
      end;
    end if;
  ...
  function fnc_co_eventos_propiedad(p_id_instncia_fljo in number,
  ...
  begin
    select a.vlor
     ...
     inner join wf_g_instancias_flujo_evnto b
        on a.id_instncia_fljo_evnto = b.id_instncia_fljo_evnto
     inner join gn_d_eventos_propiedad c
        on a.id_evnto_prpdad = c.id_evnto_prpdad
     where b.id_instncia_fljo = p_id_instncia_fljo
       and c.cdgo_prpdad = p_cdgo_prpdad;
    return v_valor;
  exception
    when others then
      return null;
  ...
  procedure prc_ac_cartera(p_cdgo_clnte       in number,
  ...
  begin
    ...
    if (p_marcacion = 'S') then
      --Valida si Marcamos o Desmarcamos la cartera
      ...
      if (v_indc_mtv = 'N') then
        --Validamos si marcamos solamente las vigencias asociadas al recurso
        --Validamos si las vigencias estan asociadas al recurso
        declare
          v_existe_vigencias varchar2(1);
        begin
          select case
                   when count(b.id_rcrso_vgncia) > 0 then
                    'S'
                   when count(b.id_rcrso_vgncia) < 1 then
                    'N'
                 end
            into v_existe_vigencias
            from gj_g_recursos a
           inner join gj_g_recursos_vigencia b
              on a.id_rcrso = b.id_rcrso
           where a.id_instncia_fljo_hjo = p_id_instncia_fljo;
        ...
        for c_vigencias in (select a.cdgo_clnte,
                             ...
                             inner join v_pq_g_solicitudes b
                                on a.id_slctud = b.id_slctud
                             inner join gj_g_recursos_vigencia c
                                on a.id_rcrso = c.id_rcrso
                             inner join gn_g_actos_vigencia d
                                on a.id_acto = d.id_acto
                               and c.id_acto_vgncia = d.id_acto_vgncia
                             where a.id_instncia_fljo_hjo =
                                   p_id_instncia_fljo) loop
          ...
          exception
            when others then
              o_cdgo_rspsta  := 1;
              o_mnsje_rspsta := 'Problemas al actualizar la cartera del recurso No. ' ||
                                c_vigencias.id_rcrso || ', cdgo. ' ||
                                o_cdgo_rspsta || ' ,' || sqlerrm;
              raise v_error;
          end;
          ...
          begin
            update gf_g_movimientos_financiero
               set cdgo_mvnt_fncro_estdo = 'RC' /*,
                                                                        estdo_blqdo             = 'S'*/
             where id_sjto_impsto = c_vigencias.id_sjto_impsto
          ...
          exception
            when others then
              o_cdgo_rspsta  := 1;
              o_mnsje_rspsta := 'Problemas al actualizar la cartera del recurso No. ' ||
                                c_vigencias.id_rcrso || ' Vigencia ' ||
                                c_vigencias.vgncia;
              raise v_error;
          end;

        end loop;
      elsif (v_indc_mtv = 'S') then
        -- Se marca toda la cartera asociada al Sujeto Impuesto
        for c_vigencias in (select a.cdgo_clnte,
                                   a.id_rcrso,
                                   c.id_acto_vgncia,
                                   c.id_acto,
          ...
          exception
            when others then
              o_cdgo_rspsta  := 5;
              o_mnsje_rspsta := 'Excepcion no fue posible contruir los movimientos consolidado.' ||
                                sqlerrm;
              raise v_error;
          end;
        end loop;

      elsif (v_indc_mtv = -1) then
        -- Si la definicion no existe
        o_cdgo_rspsta  := 1;
        o_mnsje_rspsta := 'La definición no se encuentra por favor verifique.';
        raise v_error;
      end if;
    elsif (p_marcacion = 'N') then
      --Desmarcamos la cartera que se encuentra en recurso
      for c_vigencias in (select a.cdgo_clnte,
                                 a.id_rcrso,
                                 c.id_acto_vgncia,
                                 c.id_acto,
                          ...
                          b.id_sjto_impsto   = c.id_sjto_impsto*/
                           inner join gj_g_recursos_vigencia d
                              on a.id_rcrso = d.id_rcrso
                           inner join gn_g_actos_vigencia c
                              on a.id_acto = c.id_acto
                             and d.id_acto_vgncia = c.id_acto_vgncia
  ...
  begin
    select case
     ...
     where cdgo_clnte = JSON_VALUE(p_xml, '$.P_CDGO_CLNTE') /*pkg_gn_generalidades.fnc_ca_extract_value( p_xml => p_xml , p_nodo => 'P_CDGO_CLNTE' )*/
       and id_sjto_impsto = JSON_VALUE(p_xml, '$.P_ID_SJTO_IMPSTO') /*pkg_gn_generalidades.fnc_ca_extract_value( p_xml => p_xml , p_nodo => 'P_ID_SJTO_IMPSTO' )*/
       and vgncia = JSON_VALUE(p_xml, '$.P_VGNCIA') /*pkg_gn_generalidades.fnc_ca_extract_value( p_xml => p_xml , p_nodo => 'P_VGNCIA' )*/
       and id_prdo = JSON_VALUE(p_xml, '$.P_ID_PRDO') /*pkg_gn_generalidades.fnc_ca_extract_value( p_xml => p_xml , p_nodo => 'P_ID_PRDO' )*/
       and cdgo_mvnt_fncro_estdo = 'RC';
  ...
  procedure prc_ac_fnlza_fljo_resolucion(p_id_instncia_fljo in number,
                                         p_id_fljo_trea     in number) as
    --Manejo de Errores
    v_error        exception;
    o_cdgo_rspsta  number;
  ...
  begin
    ...
    v_nl := pkg_sg_log.fnc_ca_nivel_log(v_cdgo_clnte,
                                        null,
                                        'pkg_gj_recurso.prc_ac_fnlza_fljo_resolucion');
    --
    pkg_sg_log.prc_rg_log(v_cdgo_clnte,
                          null,
                          'pkg_gj_recurso.prc_ac_fnlza_fljo_resolucion',
    ...
    if (p_id_rcrso_accion_vgnc_cncpto is not null) then
      if (nvl(p_vlor_ajste, 0) = 0) then
        begin
          delete from gj_g_rcrsos_accn_vgnc_cncpt
           where id_rcrso_accion = p_id_rcrso_accion
             and id_rcrso_accion_vgnc_cncpto =
                 p_id_rcrso_accion_vgnc_cncpto;
          --      raise    exception_vlor_ajste;
        exception
          when exception_vlor_ajste then
            raise_application_error(-20001,
                                    'Los valores de Ajuste no pueden cero, ');
          when others then
            raise_application_error(-20001,
                                    'Problemas al eliminar registro en vigencias por concepto, ' ||
                                    sqlerrm);
        end;
        ...
        exception
          when others then
            raise_application_error(-20001,
                                    'Problemas al actualizar registro en vigencias por concepto, ' ||
                                    sqlerrm);
        end;
      end if;
    else
      ...
      exception
        when others then
          raise_application_error(-20001,
                                  'Problemas al registrar en vigencias por concepto, ' ||
                                  sqlerrm);
      end;
    end if;
  ...
  begin
    ...
    begin
      ...
      select k.nmro_rdcdo_dsplay,
        ...
        join wf_g_instancias_transicion c
          on c.id_instncia_fljo = b.id_instncia_fljo
        join v_wf_d_flujos_tarea d
          on d.id_fljo_trea = c.id_fljo_trea_orgen
        join v_gj_g_recursos e
          on e.id_instncia_fljo_hjo = c.id_instncia_fljo
        left join v_pq_g_solicitudes k
          on k.id_slctud = e.id_slctud
       where d.cdgo_clnte = p_cdgo_clnte
         and e.id_acto = p_id_acto
         and c.id_instncia_trnscion =
             (select max(l.id_instncia_trnscion)
    ...
    exception
      when no_data_found then
        o_cdgo_rspsta  := 10;
        o_mnsje_rspsta := ' EL Acto  ' || p_id_acto ||
                          ' no tiene ningun tipo de recurso interpuesto ';
        return;
      when others then
        o_cdgo_rspsta  := 20;
        o_mnsje_rspsta := ' |GJ_01- Proceso No. 10 - Codigo: ' ||
                          o_cdgo_rspsta ||
                          'Se produjo error al consultar el recurso del acto  - ' ||
                          p_id_acto || ' - SQLERRM - ' || SQLERRM;
        return;
    end;
    ...
    exception
      when others then
        ...
        o_mnsje_rspsta := '|PRSC12-' || o_cdgo_rspsta ||
                          ' , no se puede construir el j_son ' ||
                          o_mnsje_rspsta;
        return;
    end;
