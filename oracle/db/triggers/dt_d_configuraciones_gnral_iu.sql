  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "DT_D_CONFIGURACIONES_GNRAL_IU" for
insert or update on dt_d_configuraciones_gnral
compound trigger
d number;
before each row is
begin
	if inserting then
		if ( :new.id_cnfgrcion_gnral is null ) then
			d := sq_dt_d_configuraciones_gnral.nextval;
...
/
ALTER TRIGGER "DT_D_CONFIGURACIONES_GNRAL_IU" ENABLE;
