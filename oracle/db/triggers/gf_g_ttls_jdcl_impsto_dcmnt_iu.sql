  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "GF_G_TTLS_JDCL_IMPSTO_DCMNT_IU"
	for insert or update on gf_g_ttls_jdcl_impsto_dcmnt
	compound  trigger

	before each row is
	begin
		if inserting then
			if :new.id_ttl_jdcl_impst_dcmnt is null then
				:new.id_ttl_jdcl_impst_dcmnt := sq_gf_g_ttls_jdcl_impsto_dcmnt.nextval;
			end if;
...
/
ALTER TRIGGER "GF_G_TTLS_JDCL_IMPSTO_DCMNT_IU" ENABLE;
