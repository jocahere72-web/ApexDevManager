  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "PRC_MIGRA_PAGOICA_P1" ( p_id_entdad         in  number
...
begin
  ...
  v_archvo_log   := 'PRC_MIGRA_PAGOICA_P1_LOG_'||v_fcha_hra||'.txt';
  v_archvo_cmmit := 'PRC_MIGRA_PAGOICA_P1_COMMIT_'||v_fcha_hra||'.txt';
  v_archvo_error := 'PRC_MIGRA_PAGOICA_P1_ERROR_'||v_fcha_hra||'.txt';
  v_cntdor       := 0;
  ...
  GENESYS.sitpr001('Iniciando proceso de revision de pagos ICA (PRC_MIGRA_PAGOICA_P1)', v_archvo_log);
...
exception
  when others then
    v_mnsje_rspsta := '. No fue posible realizar la migracion de recaudos ICA (PRC_MIGRA_PAGOICA_P1).' ||sqlcode||'-'||sqlerrm;
    GENESYS.sitpr001(v_mnsje_rspsta, v_archvo_error);
end PRC_MIGRA_PAGOICA_P1;
