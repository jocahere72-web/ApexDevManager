  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "PRC_MIGRA_DOCUICA_04_RI" ( p_id_entdad         in  number
...
v_cdgo_dcmnto_tpo   re_g_documentos.cdgo_dcmnto_tpo%type;
v_indcdor_entrno    re_g_documentos.indcdor_entrno%type;

v_id_cnvnio         gf_g_convenios.id_cnvnio%type;
v_nmro_cta          re_g_documentos.nmro_cta%type;
/*
v_cnvnio            boolean;
...
begin
   ...
   v_archvo_log   := 'PRC_MIGRA_DOCUICA_04_RI_LOG_'||v_fcha_hra||'.txt';
   v_archvo_cmmit := 'PRC_MIGRA_DOCUICA_04_RI_COMMIT_'||v_fcha_hra||'.txt';
   v_cntdor       := 0;
   ...
   -- escribir a log
   v_txto := 'Se inicia proceso de cargue de documentos ica PRC_MIGRA_DOCUICA_04_RI';

   GENESYS.sitpr001(v_txto, v_archvo_cmmit);

   for r1 in c1(p_cdgo_clnte, p_id_entdad) loop      --, p_fcha_dsde, p_fcha_hsta

     GENESYS.sitpr001('Procesando: '||r1.id_intrmdia||', Doc: '||r1.clmna1||'('||r1.nmro_lnea||'), ID: '||r1.clmna1||'-'||r1.clmna48, v_archvo_log);
     ...
     v_vlor_ttal_dbe  := nvl(r1.clmna9,0) + nvl(r1.clmna10,0) + nvl(r1.clmna15,0) + nvl(r1.clmna11,0);
     v_vlor_ttal_hber := nvl(r1.clmna14,0);

     v_indcdor_entrno := 'PBLCO';

     if r1.clmna3 = 'GDA' then       v_cdgo_dcmnto_tpo := 'DAB';
     elsif r1.clmna3 = 'GDD' then    v_cdgo_dcmnto_tpo := 'DNO';
     elsif r1.clmna3 = 'GDI' then    v_cdgo_dcmnto_tpo := 'DCO';
     elsif r1.clmna3 = 'GDW' then    v_cdgo_dcmnto_tpo := 'DNO';
                                     v_indcdor_entrno  := 'PRVDO';
     end if;

     v_id_cnvnio := null;
     v_nmro_cta  := null;

     if r1.clmna12 > 0 and  r1.clmna3 = 'GDI' then
       v_nmro_cta  := r1.clmna13;

       begin
         select t.id_cnvnio into v_id_cnvnio
           from GF_G_CONVENIOS t
          WHERE t.cdgo_clnte     = r1.cdgo_clnte and
                t.id_sjto_impsto = v_id_sjto_impsto and
                t.nmro_cnvnio    = r1.clmna12;
       exception
         when no_data_found then
           GENESYS.sitpr001('No encontro convenio: '||r1.clmna12||' de '||r1.clmna1||' ('||r1.clmna48||')', v_archvo_log);
           v_id_cnvnio := null;
         when too_many_rows then
           GENESYS.sitpr001('Convenio: '||r1.clmna12||' de '||r1.clmna1||' ('||r1.clmna48||'), duplicado', v_archvo_log);
           v_id_cnvnio := null;
       end;
     end if;
     ...
     begin
       ...
       values(r1.cdgo_clnte, v_id_impsto, v_id_impsto_sbmpsto, v_id_sjto_impsto,
              r1.clmna2, v_cdgo_dcmnto_tpo,
              v_id_cnvnio, v_nmro_cta,
              TO_DATE(r1.clmna5,'DD/MM/YYYY'), TO_DATE(r1.clmna6,'DD/MM/YYYY'),
              v_vlor_ttal_dbe, v_vlor_ttal_hber, r1.clmna8,
              r1.clmna7, r1.clmna8,
              null, v_indcdor_entrno, null, null)
       returning ID_DCMNTO
            into v_id_dcmnto;
     exception
       when others then
