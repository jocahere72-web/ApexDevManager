  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "JL_INICIAR_COBRO_PREDIAL" (p_cdgo_clnte        number,
                                                     p_id_fncnrio        number default 194,
  ...
  cursor c1 is
    ...
    -- where a.idntfccion_sjto = '0103000003090001000000000'
     group by a.id_sjto, a.id_sjto_impsto
    having sum(vlor_sldo_cptal + vlor_intres) >= p_mnto_mnmo
     order by a.id_sjto;
  ...
  cursor c2(r_id_sjto_impsto number) is
    select a.id_sjto_impsto,
     ...
     where a.cdgo_clnte = p_cdgo_clnte
       and a.id_impsto = p_cdgo_clnte || 1
  ...
  cursor c3(r_id_sjto_impsto number) is
    select d.*
      from temp_seleccion_iniciar_cobro d
     where d.id_sjto_impsto = r_id_sjto_impsto
     order by d.id_sjto_impsto, d.vgncia, d.id_cncpto;
