  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "TEMP_CREA_PRDIO_VAL_MSVO" (P_MNSJE OUT VARCHAR2) is

  --BUSCAMOS EL SUJETO
  CURSOR c1(r_idntfccion varchar2) IS
    SELECT c.id_sjto
      FROM si_c_sujetos c
     WHERE c.idntfccion = r_idntfccion;

  --SUJETO IMPUESTO PREDIAL
  ...
  CURSOR c4(r_idntfccion varchar2) IS
    SELECT i.id_sjto_impsto
      FROM v_si_i_sujetos_impuesto i
     WHERE i.id_impsto = 2300115
       AND i.idntfccion_sjto = r_idntfccion;

  V_ID_SJTO_IMPSTO_PRDAL NUMBER;

begin
  for c_datos in (select t.rslcion,
                         t.rdccion,
                         t.rfrncia_igac,
                         a.fcha_aplccion,
                         t.id_sjto_impsto
                    from si_g_resolucion_igac_t1 t
                    join si_g_resolucion_aplicada a
                      on a.id_rslcion_aplcda = t.id_rslcion_aplcda
                   where t.id_rslcion_aplcda is not null
                     and t.id_sjto_impsto is not null
                     and to_char(a.fcha_aplccion, 'yyyymmdd') =
                         to_char(sysdate, 'yyyymmdd')) loop

    begin
      BEGIN
        BEGIN
          p_mnsje := NULL;
          FOR r4 IN c4(c_datos.rfrncia_igac) LOOP
            p_mnsje := 'SUJETO IMPUESTO YA EXISTE EN VALORIZACION';
          END LOOP;
          IF p_mnsje IS NULL THEN
            FOR r1 IN c1(c_datos.rfrncia_igac) LOOP
