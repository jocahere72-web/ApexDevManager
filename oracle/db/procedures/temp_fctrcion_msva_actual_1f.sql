  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "TEMP_FCTRCION_MSVA_ACTUAL_1F" (p_lote number) is
  ...
  cursor c1 is
    ...
    -- and d.id_dcmnto=7319426 ----ojo quitar es para pruebas
...
begin
  ...
  delete from impresion_masiva_actual t where t.id_lote = p_lote;
  commit;
  ...
  for r1 in c1 loop
    ...
    v_ttal_pgar   := v_actual_ttal_ttal;
    ...
    insert into impresion_masiva_actual
      (id_lote,
       campo1,
       campo2,
       campo3,
    ...
    values
      (p_lote, --lote de impresion
       ...
       null, --AVALUO ANTERIOR
       null, --LIQUIDADO ANTERIOR
       r1.dscrpcion_prdo_dstno, --DESTINO
       r1.dscrpcion_estrto, --ESTRATO
       ltrim(to_char(r1.avluo_ctstral, '999,999,999,999')), --AVALUO ACTUAL
       v_trfa, --TARIFA
       r1.prmer_nmbre, --PROPIETARIO
       r1.cdgo_idntfccion_tpo, --TIPO IDENTIFICACION
       r1.id_rspnsble, --NUMERO DE IDENTIFICACION
       to_char(v_fcha_up, 'DD/MM/YYYY'), -- FECHA UP
       v_nmro_dcmnto_up, --NUMERO DOCUMENTO UP
       v_bnco_up, --BANCO UP
       ltrim(to_char(v_vlor_up, '999,999,999,999')), --VALOR UP
       ltrim(to_char(v_prdial_cptal_actual, '999,999,999,999')), --VALOR PREDIAL
       ltrim(to_char(v_bmbros_cptal_actual, '999,999,999,999')), --VALOR BOMBEROS
       ltrim(to_char(v_actual_ttal_cptal, '999,999,999,999')), --TOTAL CAPITAL
       ltrim(to_char(v_prdial_ttal_actual, '999,999,999,999')), --SUB TOTAL PREDIAL
       ltrim(to_char(v_bmbros_ttal_actual, '999,999,999,999')), --SUB TOTAL BOMBEROS
       ltrim(to_char(v_prdial_dscnto_actual, '999,999,999,999')), --DESCUENTO 1
       null, --DESCUENTO 2
       ltrim(to_char(v_ttal_pgar, '999,999,999,999')), --TOTAL A PAGAR 1
       NULL, --TOTAL A PAGAR 2
       v_cdgo_brras, --CODIGO EAN 1
       null, --CODIGO EAN 2
       to_char(r1.fcha_vncmnto, 'DD/MM/YYYY'), --FECHA VENCIMIENTO 1
       null, --FECHA VENCIMIENTO 2
       v_txto_cdgo_brras, --VALOR CODIGO BARRAS 1
       null,
       ltrim(to_char(v_dscntos_vgncia_antrior, '999,999,999,999')), -- DESCUENTO
       ltrim(to_char(v_actual_ttal_intres, '999,999,999,999')), -- INTERES
       v_vgncias_dda); --VGNCIAS CON DEUDA
  end loop;

  commit;

end temp_fctrcion_msva_actual_1f;
/
