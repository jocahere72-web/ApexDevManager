  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "PRC_MIGRA_PAGOS_P2" ( p_id_entdad         in  number
            ...
            from (
                  select m.clmna4, m.clmna5, m.clmna6,
                   ...
                   where m.CDGO_CLNTE        = r_cdgo_clnte and
                         ...
                         --and m.clmna4            = '00'
                         --and (m.clmna5           is null)
                         --and m.clmna6            = '19/02/2013 00:00:00'
...
begin
  ...
  v_archvo_log   := 'prc_migra_pagos_p2_LOG_'||v_fcha_hra||'.txt';
  v_archvo_cmmit := 'prc_migra_pagos_p2_COMMIT_'||v_fcha_hra||'.txt';
  v_archvo_error := 'prc_migra_pagos_p2_ERROR_'||v_fcha_hra||'.txt';
  v_cntdor       := 0;
...
exception
  when others then
    v_mnsje_rspsta := '. No fue posible realizar la migracion de recaudos (pagos p2).' ||sqlcode||'-'||sqlerrm;
    GENESYS.sitpr001(v_mnsje_rspsta, v_archvo_error);
end PRC_MIGRA_PAGOS_P2;
