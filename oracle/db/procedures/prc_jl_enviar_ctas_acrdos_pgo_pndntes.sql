  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "PRC_JL_ENVIAR_CTAS_ACRDOS_PGO_PNDNTES" (p_cdgo_clnte number,
                                                                  p_nmro_dias  number) is
  ...
  /*
  --Variables para generar el archivo en disco duro
...
begin
  ...
  for r1 in (select a.id_cnvnio,
                    ...
                    a.email, --'jlujan@infortributos.com' email, --
                    a.nmro_cnvnio,
                    c.idntfccion_sjto,
                    ...
                    max(b.fcha_vncmnto) fcha_vncmnto,
              ...
              group by a.id_cnvnio,
                       ...
                       to_char(a.fcha_slctud,
                               'dd "de" month"de" yyyy',
                               'nls_date_language=spanish')
             having trunc(sysdate) + p_nmro_dias = max(trunc(b.fcha_vncmnto))
    ...
    if v_id_dcmnto is null then
      ...
      select max(d.id_dcmnto)
       ...
       where d.cdgo_clnte = r1.cdgo_clnte
         and d.Id_Sjto_Impsto = r1.id_sjto_impsto
    ...
    if length(r1.cllar) = 10 and r1.cllar like '3%' then
      ...
      temp_enviar_sms(p_tlfno     => r1.cllar, --'3005020203',
                      p_mnsje     => 'La Alcaldia de Monteria le recuerda que el proximo ' ||
                                     to_char(r1.fcha_vncmnto, 'dd/mm/yyyy') ||
                                     ' vence el pago para la(s) cuota(s) ' ||
                                     replace(r1.nmro_cta, ':', ',') ||
                                     ' del acuerdo de pago ' ||
                                     r1.nmro_cnvnio,
                      p_tpo_mnsje => 'CUOTA CONVENIOS');
    end if;
  end loop;

end PRC_JL_ENVIAR_CTAS_ACRDOS_PGO_PNDNTES;
/
