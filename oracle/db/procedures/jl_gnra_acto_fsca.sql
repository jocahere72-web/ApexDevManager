  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "JL_GNRA_ACTO_FSCA" (p_cdgo_clnte   number,
                                              p_nmro_expdnte varchar2,
                                              p_id_acto_tpo  number,
                                              p_rgnra_html   varchar2 default 'N',
  ...
  v_id_fncnrio                fi_g_fiscalizacion_expdnte.id_fncnrio%type;
...
begin
  ...
  select a.id_fsclzcion_expdnte,
    ...
    join v_si_i_sujetos_impuesto g
      on g.id_sjto_impsto = c.id_sjto_impsto
  ...
  v_xml := '<data>
           ...
           '</cdgo_clnte><id_instncia_fljo>' || v_id_instncia_fljo ||
           '</id_instncia_fljo><id_cnddto>' || v_id_cnddto ||
           '</id_cnddto><id_usrio>' || v_id_usrio || '</id_usrio>
              </data>';
  ...
  if p_rgnra_html = 'S' then
    ...
    v_json_envia.put('IDNTFCCION', v_idntfccion);
    v_json_envia.put('ID_CNDDTO', v_id_cnddto);
