prompt --application/pages/page_00082
begin
wwv_flow_api.create_page(
 p_id=>82
,p_user_interface_id=>wwv_flow_api.id(36090105988528368)
,p_name=>unistr('Inscripci\00F3n Oficiosa')
,p_step_title=>unistr('Inscripci\00F3n Oficiosa')
,p_autocomplete_on_off=>'OFF'
,p_javascript_code=>wwv_flow_string.join(wwv_flow_t_varchar2(
unistr('var pageId = &APP_PAGE_ID.;  // Extrae el n\00FAmero de la pagina.'),
'async function generarActo(p_id_expdnte_acto){',
'    console.log(''pageId:''+pageId)',
'    let $waitPopup = apex.widget.waitPopup();',
'    try{',
'    console.log(''entrando al JS:''+pageId)',
'    await apex.server.process("generarActo", {',
'            pageItems: "#P"+pageId+"_ID_CNDDTO",',
'            x01: p_id_expdnte_acto}, {',
'            success: function(data) {',
'                    console.log(''DATA:''+JSON.stringify(data));',
'                    if (data.cdgo_rspsta > 0) {',
'                        console.log(''ERROR:''+data.mnsje_rspsta)',
'                        apex.message.clearErrors();',
'                        apex.message.showErrors([{',
'                            type: "error",',
'                            location: "page",',
'                            message: data.mnsje_rspsta,',
'                            unsafe: false',
'                        }]);',
'                    } else {',
'                        console.log(''OK:''+data.mnsje_rspsta)',
'                        apex.submit();',
'                    }',
'                }',
'            })',
'    }catch(e){',
'        console.error(e);',
'    }finally{',
'        $waitPopup.remove();',
'    }',
'};'))
,p_step_template=>wwv_flow_api.id(36470205860968633)
,p_page_template_options=>'#DEFAULT#'
,p_last_updated_by=>'BVILLEGAS'
,p_last_upd_yyyymmddhh24miss=>'20240712072131'
);
wwv_flow_api.create_page_plug(
 p_id=>wwv_flow_api.id(116143105068421340)
,p_plug_name=>'Flujo'
,p_region_template_options=>'#DEFAULT#:t-Region--removeHeader:t-Region--scrollBody'
,p_plug_template=>wwv_flow_api.id(36503027129968651)
,p_plug_display_sequence=>10
,p_include_in_reg_disp_sel_yn=>'Y'
,p_plug_display_point=>'BODY'
,p_plug_source_type=>'PLUGIN_COM.INFORTRIBUTOS.WORKFLOW'
,p_plug_query_options=>'DERIVED_REPORT_COLUMNS'
,p_attribute_01=>'&F_CDGO_CLNTE.'
,p_attribute_02=>'&F_ID_INSTNCIA_FLJO.'
,p_attribute_03=>'&F_ID_FLJO_TREA.'
,p_attribute_04=>'true'
);
wwv_flow_api.create_page_plug(
 p_id=>wwv_flow_api.id(116143194658421341)
,p_plug_name=>unistr('Proceso Fiscalizaci\00F3n')
,p_region_template_options=>'#DEFAULT#:t-Region--scrollBody'
,p_plug_template=>wwv_flow_api.id(36503027129968651)
,p_plug_display_sequence=>20
,p_include_in_reg_disp_sel_yn=>'Y'
,p_plug_display_point=>'BODY'
,p_plug_source_type=>'PLUGIN_COM.INFORTRIBUTOS.CANDIDATO'
,p_plug_query_options=>'DERIVED_REPORT_COLUMNS'
,p_attribute_01=>'F_ID_INSTNCIA_FLJO'
,p_attribute_02=>'F_ID_FLJO_TREA'
,p_attribute_03=>'P82_ID_SJTO_IMPSTO'
,p_attribute_04=>'F_ID_FNCNRIO'
,p_attribute_05=>'P82_ID_FSCLZCION_EXPDNTE'
);
wwv_flow_api.create_page_plug(
 p_id=>wwv_flow_api.id(368877530758125866)
,p_plug_name=>'Opciones'
,p_region_template_options=>'#DEFAULT#:t-Region--removeHeader:t-Region--scrollBody'
,p_plug_template=>wwv_flow_api.id(36503027129968651)
,p_plug_display_sequence=>30
,p_include_in_reg_disp_sel_yn=>'Y'
,p_plug_display_point=>'REGION_POSITION_03'
,p_plug_source=>wwv_flow_string.join(wwv_flow_t_varchar2(
'<br>',
'<h5><b>',
'        <i><center>Opciones <i class="fa fa-gears" aria-hidden="true"></i></center>',
'        </i> ',
'    </b>',
'</h5>'))
,p_plug_query_options=>'DERIVED_REPORT_COLUMNS'
,p_attribute_01=>'N'
,p_attribute_02=>'HTML'
);
wwv_flow_api.create_page_plug(
 p_id=>wwv_flow_api.id(386052407309276274)
,p_plug_name=>'Ayuda'
,p_region_template_options=>'#DEFAULT#:t-Region--removeHeader:t-Region--scrollBody'
,p_plug_template=>wwv_flow_api.id(36503027129968651)
,p_plug_display_sequence=>40
,p_include_in_reg_disp_sel_yn=>'Y'
,p_plug_display_point=>'REGION_POSITION_03'
,p_plug_source=>wwv_flow_string.join(wwv_flow_t_varchar2(
'<br>',
'<h5>',
'    <b>',
'        <center>Ayuda <i class="fa fa-question-circle" aria-hidden="true"></i></center>',
'    </b>',
'</h5>',
'',
'<p align="justify">',
unistr('<i>Funcionalidad que permite generar, modificar, visualizar y confirmar los actos parametrizados en la etapa Resoluci\00F3n por no declarar que no requieren confirmaci\00F3n por el supervisor.</i>'),
'</p>',
'',
'',
'<p align="justify">',
unistr('<i>Si desea conocer m\00E1s informaci\00F3n referente a la funcionalidad dir\00EDjase al Manual de Usuario.</i>'),
'</p>'))
,p_plug_query_options=>'DERIVED_REPORT_COLUMNS'
,p_attribute_01=>'N'
,p_attribute_02=>'HTML'
);
wwv_flow_api.create_page_button(
 p_id=>wwv_flow_api.id(9007619214767686)
,p_button_sequence=>10
,p_button_plug_id=>wwv_flow_api.id(368877530758125866)
,p_button_name=>'BTN_RGRSAR'
,p_button_action=>'REDIRECT_PAGE'
,p_button_template_options=>'#DEFAULT#:t-Button--warning:t-Button--iconLeft:t-Button--stretch'
,p_button_template_id=>wwv_flow_api.id(36555682372968674)
,p_button_image_alt=>'Regresar'
,p_button_position=>'BELOW_BOX'
,p_button_redirect_url=>'f?p=&APP_ID.:11:&SESSION.::&DEBUG.:RP::'
,p_icon_css_classes=>'fa-angle-left'
,p_grid_new_grid=>false
);
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(9005369602767678)
,p_name=>'P82_ID_CNDDTO'
,p_item_sequence=>10
,p_item_plug_id=>wwv_flow_api.id(116143194658421341)
,p_display_as=>'NATIVE_HIDDEN'
,p_attribute_01=>'Y'
);
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(9005726145767682)
,p_name=>'P82_ID_SJTO_IMPSTO'
,p_item_sequence=>20
,p_item_plug_id=>wwv_flow_api.id(116143194658421341)
,p_display_as=>'NATIVE_HIDDEN'
,p_attribute_01=>'Y'
);
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(9006114577767682)
,p_name=>'P82_ID_FSCLZCION_EXPDNTE'
,p_item_sequence=>30
,p_item_plug_id=>wwv_flow_api.id(116143194658421341)
,p_display_as=>'NATIVE_HIDDEN'
,p_attribute_01=>'Y'
);
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(9006551498767682)
,p_name=>'P82_ID_IMPSTO'
,p_item_sequence=>40
,p_item_plug_id=>wwv_flow_api.id(116143194658421341)
,p_display_as=>'NATIVE_HIDDEN'
,p_attribute_01=>'Y'
);
wwv_flow_api.create_page_item(
 p_id=>wwv_flow_api.id(9006972535767682)
,p_name=>'P82_ID_FSCLZCION_EXPDNTE_ACTO'
,p_item_sequence=>50
,p_item_plug_id=>wwv_flow_api.id(116143194658421341)
,p_display_as=>'NATIVE_HIDDEN'
,p_attribute_01=>'N'
);
wwv_flow_api.create_page_process(
 p_id=>wwv_flow_api.id(9008323840767689)
,p_process_sequence=>10
,p_process_point=>'AFTER_HEADER'
,p_process_type=>'NATIVE_PLSQL'
,p_process_name=>unistr('Carga informaci\00F3n')
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'begin',
'',
'    select a.id_cnddto,',
'           a.id_impsto,',
'           a.id_sjto_impsto,',
'           c.id_fsclzcion_expdnte',
'    into   :P82_ID_CNDDTO,',
'           :P82_ID_IMPSTO,',
'           :P82_ID_SJTO_IMPSTO,',
'           :P82_ID_FSCLZCION_EXPDNTE',
'    from v_fi_g_candidatos          a',
'    join fi_g_fiscalizacion_expdnte c on a.id_cnddto        = c.id_cnddto',
'    join v_si_i_sujetos_impuesto    d on a.id_sjto_impsto   = d.id_sjto_impsto',
'    where c.id_instncia_fljo = :F_ID_INSTNCIA_FLJO;',
'    ',
'',
'exception',
'    when others then',
'        :P82_ID_CNDDTO    := null;',
'        :P82_ID_SJTO_IMPSTO := null;',
'        :P82_ID_FSCLZCION_EXPDNTE := null;',
'end;'))
,p_error_display_location=>'INLINE_IN_NOTIFICATION'
);
wwv_flow_api.create_page_process(
 p_id=>wwv_flow_api.id(9008712804767691)
,p_process_sequence=>10
,p_process_point=>'AFTER_SUBMIT'
,p_process_type=>'NATIVE_PLSQL'
,p_process_name=>'Generar Acto'
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'declare',
'    v_cdgo_rspsta     number;',
'    v_mnsje_rspsta    varchar(1000);',
'    v_xml             clob;',
'    v_id_fsclzcion_expdnte_acto   number;',
'    v_id              number;',
'    v_cdgo_acto_tipo  varchar2(3);',
'    ',
'    v_nl                        number;',
'    ',
'    --variables para registrar la novedad ',
'    v_id_instncia_fljo    number;',
'    v_id_fljo_trea        number;',
'    v_mnsje               varchar2(4000);',
'    v_obsrvcion           varchar2(4000);',
'    v_id_fljo             number;',
'    o_id_session          number;',
'    o_id_fljo             number;',
'    o_cdgo_rspsta         varchar2(4000);',
'    o_mnsje_rspsta        varchar2(4000);',
'    ',
'begin',
'       v_nl := pkg_sg_log.fnc_ca_nivel_log(23001, null, ''pkg_fi_fiscalizacion.prc_gn_novedades'');',
'',
'        pkg_sg_log.prc_rg_log(23001, null, ''pkg_fi_fiscalizacion.prc_gn_novedades'',  v_nl, ''Entrando:'' || systimestamp, 1);',
'',
'    ',
'',
'',
'    pkg_fi_fiscalizacion.prc_rg_acto(p_cdgo_clnte                  => :F_CDGO_CLNTE,',
'                                     p_id_usrio                    => :F_ID_USRIO,  ',
'                                     p_id_fsclzcion_expdnte_acto   => :P82_ID_FSCLZCION_EXPDNTE_ACTO,',
'                                     p_id_cnddto                   => :P82_ID_CNDDTO,',
'                                     o_cdgo_rspsta                 => v_cdgo_rspsta,',
'                                     o_mnsje_rspsta                => v_mnsje_rspsta);',
'                                     ',
'',
'    if v_cdgo_rspsta > 0 then',
'        raise_application_error(-20001, v_mnsje_rspsta);',
'    end if;',
'    ',
'    ',
'    begin',
'      select ',
'             a.id_fljo',
'       into  v_id_fljo',
'       from v_wf_d_flujos a',
'      join wf_d_flujos_usuarios b',
'        on b.id_fljo    = a.id_fljo',
'     where a.cdgo_clnte = :F_CDGO_CLNTE',
'       and b.id_usrio   = :F_ID_USRIO   ',
'       and a.id_prcso   = nvl(86, a.id_prcso)',
'       and a.indcdor_incia_usrio_fnal = ''S''',
'       and b.actvo = ''S''',
'       and a.cdgo_fljo = ''NPR'';',
'      exception',
'            when others then',
'               o_mnsje_rspsta := o_cdgo_rspsta || ''error en la select  :'' || v_id_fljo  ;',
'               pkg_sg_log.prc_rg_log(23001, null, ''pkg_fi_fiscalizacion.prc_gn_novedades'',  v_nl, o_mnsje_rspsta, 6); ',
'        end;',
'     ',
'     o_mnsje_rspsta := o_cdgo_rspsta || ''fisca antes de prc_gn_novedades  :'' || v_id_fljo ||''-''||',
'                                              v_id_fljo ||''-''||:P82_ID_IMPSTO ||''-''||:F_ID_USRIO  ;',
'     pkg_sg_log.prc_rg_log(23001, null, ''pkg_fi_fiscalizacion.prc_gn_novedades'',  v_nl, o_mnsje_rspsta, 6);   ',
'        ',
'    ',
unistr('    v_obsrvcion := ''ACTIVACI\00D3N OFICIOSA POR FISCALIZACI\00D3N OMISOS DESCONOCIDOS'';'),
'    /*pkg_wf_flujo.prc_rg_instancias_flujo(p_id_fljo => :P17_FLUJOS, ',
'                                         p_id_usrio => :F_ID_USRIO,',
'                                         p_id_prtcpte => null);*/',
'   ',
'  ',
'   ',
'   pkg_fi_fiscalizacion.prc_gn_novedades(p_cdgo_clnte 		   => :F_CDGO_CLNTE',
'                                       , p_id_impsto         => :P82_ID_IMPSTO ',
'                                       , p_id_impsto_sbmpsto => 2300122',
'                                       , p_id_usrio          => :F_ID_USRIO',
'                                       , p_id_prtcpte        => :F_ID_USRIO',
'                                       , p_id_sjto_impsto    => :P82_ID_SJTO_IMPSTO          ',
'                                       , p_obsrvcion         => v_obsrvcion',
'                                       , p_id_fljo           => v_id_fljo',
'                                       , o_id_session        => o_id_session',
'                                       , o_id_instncia_fljo  => o_id_fljo',
'                                       , o_cdgo_rspsta       => o_cdgo_rspsta',
'                                       , o_mnsje_rspsta      => o_mnsje_rspsta);',
'                                       ',
'    o_mnsje_rspsta := o_cdgo_rspsta || ''-''|| o_mnsje_rspsta  ;',
'     pkg_sg_log.prc_rg_log(23001, null, ''pkg_fi_fiscalizacion.prc_gn_novedades'',  v_nl, o_mnsje_rspsta, 6);       ',
'     ',
'    if v_id_instncia_fljo is null then   ',
'    insert into muerto(v_001,  t_001) values(''fisca error ''|| v_id_fljo , systimestamp); commit;',
'        raise_application_error( -20001 , v_mnsje );            ',
'    end if;',
'   ',
'end;'))
,p_error_display_location=>'INLINE_IN_NOTIFICATION'
,p_process_when=>'GENERAR'
,p_process_when_type=>'REQUEST_EQUALS_CONDITION'
);
wwv_flow_api.create_page_process(
 p_id=>wwv_flow_api.id(31686474694237307)
,p_process_sequence=>20
,p_process_point=>'ON_DEMAND'
,p_process_type=>'NATIVE_PLSQL'
,p_process_name=>'generarActo'
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'declare',
'    v_cdgo_rspsta    number         := 0;',
'    v_mnsje_rspsta   varchar2(4000) := ''OK'';',
'    v_id_expdnte_acto number        := apex_application.g_x01;',
'    ',
'    v_id_fljo             number;',
'    v_id_session          number;',
'    v_id_instncia_fljo    number;',
'begin',
'    :P82_ID_FSCLZCION_EXPDNTE_ACTO := v_id_expdnte_acto;    ',
'    begin',
'        pkg_fi_fiscalizacion.prc_rg_acto(p_cdgo_clnte                  => :F_CDGO_CLNTE,',
'                                         p_id_usrio                    => :F_ID_USRIO,  ',
'                                         p_id_fsclzcion_expdnte_acto   => :P82_ID_FSCLZCION_EXPDNTE_ACTO,',
'                                         p_id_cnddto                   => :P82_ID_CNDDTO,',
'                                         o_cdgo_rspsta                 => v_cdgo_rspsta,',
'                                         o_mnsje_rspsta                => v_mnsje_rspsta);',
'    end;',
'    /*begin',
'       select a.id_fljo',
'         into v_id_fljo',
'         from wf_d_flujos a',
'         --join wf_d_flujos_usuarios b on b.id_fljo    = a.id_fljo',
'        where a.cdgo_clnte = :F_CDGO_CLNTE',
'          and a.cdgo_fljo = ''NPR''',
'          and a.actvo = ''S''',
'          --and b.id_usrio   = :F_ID_USRIO   ',
'          --and a.id_prcso   = nvl(86, a.id_prcso)',
'          --and b.actvo = ''S'';',
'          --and a.indcdor_incia_usrio_fnal = ''S''',
'          ;',
'    exception',
'        when others then',
'            v_cdgo_rspsta  := 1;',
'            v_mnsje_rspsta := ''Problemas al consultar flujo'';',
'    end;',
'    begin',
'        pkg_fi_fiscalizacion.prc_gn_novedades(p_cdgo_clnte 		 => :F_CDGO_CLNTE',
'                                           , p_id_impsto         => :P82_ID_IMPSTO ',
'                                           , p_id_impsto_sbmpsto => 2300122',
'                                           , p_id_usrio          => :F_ID_USRIO',
'                                           , p_id_prtcpte        => :F_ID_USRIO',
'                                           , p_id_sjto_impsto    => :P82_ID_SJTO_IMPSTO          ',
unistr('                                           , p_obsrvcion         => ''ACTIVACI\00D3N OFICIOSA POR FISCALIZACI\00D3N OMISOS DESCONOCIDOS'''),
'                                           , p_id_fljo           => v_id_fljo',
'                                           , o_id_session        => v_id_session',
'                                           , o_id_instncia_fljo  => v_id_instncia_fljo',
'                                           , o_cdgo_rspsta       => v_cdgo_rspsta',
'                                           , o_mnsje_rspsta      => v_mnsje_rspsta);',
'    end;*/',
'    apex_json.open_object;',
'    apex_json.write(''cdgo_rspsta'', v_cdgo_rspsta);',
'    apex_json.write(''mnsje_rspsta'', v_mnsje_rspsta);',
'    apex_json.close_object;',
'exception ',
'    when others then',
'        apex_json.open_object;',
'        apex_json.write(''cdgo_rspsta'', v_cdgo_rspsta);',
'        apex_json.write(''mnsje_rspsta'', v_mnsje_rspsta);',
'        apex_json.close_object;',
'        rollback;',
'end;'))
,p_error_display_location=>'INLINE_IN_NOTIFICATION'
);
end;
/
