prompt --application/shared_components/user_interface/lovs/lv_tipo_dia
begin
wwv_flow_api.create_list_of_values(
 p_id=>wwv_flow_api.id(100297956748923823)
,p_lov_name=>'LV_TIPO_DIA'
,p_lov_query=>'.'||wwv_flow_api.id(100297956748923823)||'.'
);
wwv_flow_api.create_static_lov_data(
 p_id=>wwv_flow_api.id(100298211215923834)
,p_lov_disp_sequence=>1
,p_lov_disp_value=>unistr('H\00E1bil')
,p_lov_return_value=>'H'
);
wwv_flow_api.create_static_lov_data(
 p_id=>wwv_flow_api.id(100298681378923843)
,p_lov_disp_sequence=>2
,p_lov_disp_value=>'Calendario'
,p_lov_return_value=>'C'
);
end;
/
